...
*/
    ...
    input[type=file ] {
        background-color: transparent;
        box-shadow: none;
    <?php if (empty($conf->global->THEME_SHOW_BORDER_ON_INPUT)) { ?> border-top: none;
        border-left: none;
        border-right: none;
    <?php } ?> border <?php echo empty($conf->global->THEME_SHOW_BORDER_ON_INPUT) ? '-bottom' : ''; ?>: solid 1px var(--inputbordercolor);
    }
...
if ($conf->browser->layout == 'phone') {
    ...
    .flexcontainer {
    <?php if (in_array($conf->browser->name, ['chrome', 'firefox'])) {
    ...
    .maxscreenheightless200 {
        max-height: <?php echo isset($_SESSION['dol_screenheight']) && !empty($_SESSION['dol_screenheight']) ? max(500, $_SESSION['dol_screenheight'] - 200) : 700; ?>px; /* we guarantee height of 500 */
    }

    .maxscreenheightless300 {
        max-height: <?php echo isset($_SESSION['dol_screenheight']) && !empty($_SESSION['dol_screenheight']) ? max(400, $_SESSION['dol_screenheight'] - 300) : 700; ?>px; /* we guarantee height of 500 */
    }
	...
	*/
    ...
    #id-left {
        padding-top: 20px;
        padding-bottom: 5px;
    <?php if (!empty($conf->global->MAIN_USE_TOP_MENU_SEARCH_DROPDOWN) && !empty($conf->global->MAIN_USE_TOP_MENU_QUICKADD_DROPDOWN)) { ?> padding-top: 8px;
    <?php } ?>
    }
...
div.statusref {
    ...
    <?php print $minwidthtmenu ? 'min-width: '.$minwidthtmenu.'px;' : ''; ?> text-align: center;
        vertical-align: bottom;
    <?php if (empty($conf->global->MAIN_MENU_INVERT)) { ?> float: <?php print $left; ?>;
	...
	$divalreadydefined = ['home', 'companies', 'products', 'mrp', 'commercial', 'externalsite', 'accountancy', 'project', 'tools', 'members', 'agenda', 'ftp', 'holiday', 'hrm', 'bookmark', 'cashdesk', 'takepos', 'ecm', 'geoipmaxmind', 'gravatar', 'clicktodial', 'paypal', 'stripe', 'webservices', 'website'];
	// Put here list of menu entries we are sure we don't want
	$divnotrequired = ['multicurrency', 'salaries', 'ticket', 'margin', 'opensurvey', 'paybox', 'expensereport', 'incoterm', 'prelevement', 'propal', 'workflow', 'notification', 'supplier_proposal', 'cron', 'product', 'productbatch', 'expedition'];
...
div.tabsAction a.butActionDelete:last-child, div.tabsAction a.butAction:last-child {
    ...
    div.pagination li.pagination span {
        ...
        text-decoration: none;
        background-repeat: repeat-x;
    <?php if (empty($conf->global->THEME_DARKMODEENABLED)) { ?> color: #000;
    <?php } ?>
    }
 ...
 */
    ...
    @media only screen and (max-width: 767px) {
        ...
        .thumbstat {
            flex: 1 1 110px;
            margin-bottom: 8px;
            min-width: <?php echo isset($_SESSION['dol_screenwidth']) && !empty($_SESSION['dol_screenwidth']) ?min(160, round($_SESSION['dol_screenwidth'] / 2 - 20)) : 150; ?>px; /* on screen < 320, we guaranty to have 2 columns */
        }

        .thumbstat150 {
            flex: 1 1 110px;
            margin-bottom: 8px;
            min-width: <?php echo isset($_SESSION['dol_screenwidth']) && !empty($_SESSION['dol_screenwidth']) ?min(160, round($_SESSION['dol_screenwidth'] / 2 - 20)) : 160; ?>px; /* on screen < 320, we guaranty to have 2 columns */
            max-width: <?php echo isset($_SESSION['dol_screenwidth']) && !empty($_SESSION['dol_screenwidth']) ?min(161, round($_SESSION['dol_screenwidth'] / 2 - 20)) : 161; ?>px; /* on screen < 320, we guaranty to have 2 columns */
...
*/
    ...
    div.scroll2 {
        width: <?php print isset($_SESSION['dol_screenwidth']) && !empty($_SESSION['dol_screenwidth']) ?max($_SESSION['dol_screenwidth'] - 830, 450) : '450'; ?>px !important;
    }
    ...
    <?php if (!empty($conf->global->MAIN_JQUERY_JNOTIFY_BOTTOM)) { ?> top: auto !important;
...
.ui-corner-all { white-space: nowrap; } */
    ...
    <?php if (empty($conf->global->THEME_SHOW_BORDER_ON_INPUT)) { ?> border-left: none;
        border-top: none;
        border-right: none;
    <?php } ?>
    }
