        ...
        if ($action == 'COMPANY_MODIFY') {
            ...
            if ($object->client != 0) {
                ...
                if ($customer) {
                    ...
                    $langcleaned = $object->default_lang ? [substr($object->default_lang, 0, 2)] : null;
            ...
            if (!empty($object->stripe_card_ref)) {
                ...
                if ($stripecu) {
                    ...
                    if (empty($stripeacc)) {                // If the Stripe connect account not set, we use common API usage
                        $customer = Customer::retrieve($stripecu);
                    } else {
                        $customer = Customer::retrieve($stripecu, ["stripe_account" => $stripeacc]);
                    }

                    if ($customer) {
                        $card = $stripe->cardStripe($customer, $object, $stripeacc, $servicestatus);
                        if ($card) {
                            $card->metadata = ['dol_id' => $object->id, 'dol_version' => DOL_VERSION, 'dol_entity' => $conf->entity, 'ipaddress' => (empty($_SERVER['REMOTE_ADDR']) ? '' : $_SERVER['REMOTE_ADDR'])];
                            try {
                                $card->save();
                            } catch (Exception $e) {
                                $ok = -1;
