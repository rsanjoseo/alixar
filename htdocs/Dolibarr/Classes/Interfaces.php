...
class Interfaces
{
    ...
    public function run_triggers($action, $object, $user, $langs, $conf)
    {
        ...
        if (!is_object($object) || !is_object($conf)) {    // Error
            ...
            DolibarrFunctions::dol_syslog(get_class($this) . '::run_triggers ' . $this->error, LOG_ERR);
            $this->errors[] = $this->error;
            return -1;
        }
        if (!is_object($langs)) {    // Warning
            DolibarrFunctions::dol_syslog(get_class($this) . '::run_triggers was called with wrong parameters action=' . $action . ' object=' . is_object($object) . ' user=' . is_object($user) . ' langs=' . is_object($langs) . ' conf=' . is_object($conf), LOG_WARNING);
        }
        if (!is_object($user)) {        // Warning
            DolibarrFunctions::dol_syslog(get_class($this) . '::run_triggers was called with wrong parameters action=' . $action . ' object=' . is_object($object) . ' user=' . is_object($user) . ' langs=' . is_object($langs) . ' conf=' . is_object($conf), LOG_WARNING);
            $user = new User($this->db);
        }
        //DolibarrFunctions::dol_syslog(get_class($this)."::run_triggers action=".$action." Launch run_triggers", LOG_DEBUG);
            ...
            if (is_resource($handle)) {
                ...
                while (($file = readdir($handle)) !== false) {
                    ...
                    if (is_readable($newdir . "/" . $file) && preg_match('/^interface_([0-9]+)_([^_]+)_(.+)\.class\.php$/i', $file, $reg)) {
                        ...
                        if (!$qualified) {
                            //DolibarrFunctions::dol_syslog(get_class($this)."::run_triggers action=".$action." Triggers for file '".$file."' need module to be enabled", LOG_DEBUG);
                            continue;
                        }
                        ...
                        if (in_array($modName, $modules)) {    // $modules = list of modName already loaded
                            $langs->load("errors");
                            DolibarrFunctions::dol_syslog(get_class($this) . "::run_triggers action=" . $action . " " . $langs->trans("ErrorDuplicateTrigger", $newdir . "/" . $file, $fullpathfiles[$modName]), LOG_WARNING);
                            continue;
                        }
                        ...
                        try {
                            ...
                            DolibarrFunctions::dol_syslog('ko for ' . $modName . " " . $e->getMessage() . "\n", LOG_ERR);
                        }
        ...
        foreach ($orders as $key => $value) {
            ...
            if ($objMod) {
                ...
                if (method_exists($objMod, 'runTrigger')) {    // New method to implement
                    //DolibarrFunctions::dol_syslog(get_class($this)."::run_triggers action=".$action." Launch runTrigger for file '".$files[$key]."'", LOG_DEBUG);
                    $result = $objMod->runTrigger($action, $object, $user, $langs, $conf);
                } elseif (method_exists($objMod, 'run_trigger')) {    // Deprecated method
                    DolibarrFunctions::dol_syslog(get_class($this) . "::run_triggers action=" . $action . " Launch old method run_trigger (rename your trigger into runTrigger) for file '" . $files[$key] . "'", LOG_WARNING);
                    $result = $objMod->run_trigger($action, $object, $user, $langs, $conf);
                } else {
                    DolibarrFunctions::dol_syslog(get_class($this) . "::run_triggers action=" . $action . " A trigger was declared for class " . get_class($objMod) . " but method runTrigger was not found", LOG_ERR);
                }
                ...
                if ($result < 0) {
                    // Action KO
                    //DolibarrFunctions::dol_syslog("Error in trigger ".$action." - result = ".$result." - Nb of error string returned = ".count($objMod->errors), LOG_ERR);
                    $nbtotal++;
                    $nbko++;
                    ...
                    //DolibarrFunctions::dol_syslog("Error in trigger ".$action." - Nb of error string returned = ".count($this->errors), LOG_ERR);
                }
            } else {
                DolibarrFunctions::dol_syslog(get_class($this) . "::run_triggers action=" . $action . " Failed to instantiate trigger for file '" . $files[$key] . "'", LOG_ERR);
            }
        }

        if ($nbko) {
            DolibarrFunctions::dol_syslog(get_class($this) . "::run_triggers action=" . $action . " Files found: " . $nbfile . ", Files launched: " . $nbtotal . ", Done: " . $nbok . ", Failed: " . $nbko . " - Nb of error string returned in this->errors = " . count($this->errors), LOG_ERR);
            return -$nbko;
        } else {
            //DolibarrFunctions::dol_syslog(get_class($this)."::run_triggers Files found: ".$nbfile.", Files launched: ".$nbtotal.", Done: ".$nbok.", Failed: ".$nbko, LOG_DEBUG);
            return $nbok;
        }
    }
