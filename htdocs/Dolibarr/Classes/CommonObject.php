...
abstract class CommonObject
{
    ...
    public $next_prev_filter; // Date last change (tms field)
    /**
    ...
    {
        if (!empty($this->conf->global->MAIN_FIRST_TO_UPPER)) {
        ...
        if (!empty($this->conf->global->MAIN_ALL_TO_UPPER)) {
        ...
        if (!empty($this->conf->global->MAIN_ALL_TOWN_TO_UPPER)) {
    ...
    public function getBannerAddress($htmlkey, $object)
    {
        ...
        $outdone = 0;
        $coords = $this->getFullAddress(1, ', ', (!empty($this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) ? $this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT : 0));
        if ($coords) {
            if (!empty($this->conf->use_javascript_ajax)) {
                // Add picto with tooltip on map
                $namecoords = '';
                if ($this->element == 'contact' && !empty($this->conf->global->MAIN_SHOW_COMPANY_NAME_IN_BANNER_ADDRESS)) {
                    $namecoords .= $object->name . '<br>';
                }
                $namecoords .= $this->getFullName($this->langs, 1) . '<br>' . $coords;
                // hideonsmatphone because copyToClipboard call jquery dialog that does not work with jmobile
                $out .= '<a href="#" class="hideonsmartphone" onclick="return copyToClipboard(\'' . dol_escape_js($namecoords) . '\',\'' . dol_escape_js($this->langs->trans("HelpCopyToClipboard")) . '\');">';
                $out .= img_picto($this->langs->trans("Address"), 'map-marker-alt');
                $out .= '</a> ';
            }
            ...
            if (is_array($arrayoflangcode) && count($arrayoflangcode)) {
                ...
                if (!empty($extralanguages->attributes[$elementforaltlanguage]['address']) || !empty($extralanguages->attributes[$elementforaltlanguage]['town'])) {
                    ...
                    foreach ($arrayoflangcode as $extralangcode) {
                        ...
                        $coords = $this->getFullAddress(1, ', ', $this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT, $extralangcode);
        ...
        if (!in_array($this->country_code, $countriesusingstate) && empty($this->conf->global->MAIN_FORCE_STATE_INTO_ADDRESS)   // If MAIN_FORCE_STATE_INTO_ADDRESS is on, state is already returned previously with getFullAddress
            && empty($this->conf->global->SOCIETE_DISABLE_STATE) && $this->state) {
            if (!empty($this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && $this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1 && $this->region) {
        ...
        if (!empty($this->phone) && empty($this->phone_pro)) {        // For objects that store pro phone into ->phone
            $out .= dol_print_phone($this->phone, $this->country_code, $contactid, $thirdpartyid, 'AC_TEL', '&nbsp;', 'phone', $this->langs->trans("PhonePro"));
            $outdone++;
        }
        if (!empty($this->phone_pro)) {
            $out .= dol_print_phone($this->phone_pro, $this->country_code, $contactid, $thirdpartyid, 'AC_TEL', '&nbsp;', 'phone', $this->langs->trans("PhonePro"));
            $outdone++;
        }
        if (!empty($this->phone_mobile)) {
            $out .= dol_print_phone($this->phone_mobile, $this->country_code, $contactid, $thirdpartyid, 'AC_TEL', '&nbsp;', 'mobile', $this->langs->trans("PhoneMobile"));
            $outdone++;
        }
        if (!empty($this->phone_perso)) {
            $out .= dol_print_phone($this->phone_perso, $this->country_code, $contactid, $thirdpartyid, 'AC_TEL', '&nbsp;', 'phone', $this->langs->trans("PhonePerso"));
            $outdone++;
        }
        if (!empty($this->office_phone)) {
            $out .= dol_print_phone($this->office_phone, $this->country_code, $contactid, $thirdpartyid, 'AC_TEL', '&nbsp;', 'phone', $this->langs->trans("PhonePro"));
            $outdone++;
        }
        if (!empty($this->user_mobile)) {
            $out .= dol_print_phone($this->user_mobile, $this->country_code, $contactid, $thirdpartyid, 'AC_TEL', '&nbsp;', 'mobile', $this->langs->trans("PhoneMobile"));
            $outdone++;
        }
        if (!empty($this->fax)) {
            $out .= dol_print_phone($this->fax, $this->country_code, $contactid, $thirdpartyid, 'AC_FAX', '&nbsp;', 'fax', $this->langs->trans("Fax"));
            $outdone++;
        }
        if (!empty($this->office_fax)) {
            $out .= dol_print_phone($this->office_fax, $this->country_code, $contactid, $thirdpartyid, 'AC_FAX', '&nbsp;', 'fax', $this->langs->trans("Fax"));
            $outdone++;
        }
        ...
        if (!empty($this->conf->socialnetworks->enabled)) {
            $outsocialnetwork = '';
     ...
     *
     * @param Translate $this-     >langs     Language object for translation of civility (used only if option is 1)
    ...
    public function getFullName($langs, $option = 0, $nameorder = -1, $maxlen = 0)
    {
        ...
        if (!empty($option) && !empty($this->civility_code)) {
            if ($this->langs->transnoentitiesnoconv("Civility" . $this->civility_code) != "Civility" . $this->civility_code) {
                $ret .= $this->langs->transnoentitiesnoconv("Civility" . $this->civility_code) . ' ';
            } else {
                $ret .= $this->civility_code . ' ';
            }
        }

        $ret .= DolibarrFunctions::dolGetFirstLastname($firstname, $lastname, $nameorder);

        return DolibarrFunctions::dol_trunc($ret, $maxlen);
    }
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                if (!$list) {
                    ...
                    $libelle_type = ($this->langs->trans($transkey) != $transkey ? $this->langs->trans($transkey) : $obj->libelle);
                    $tab[$i] = [
                        'source' => $obj->source,
                        'socid' => $obj->socid,
                        'id' => $obj->id,
    ...
    {
        // phpcs:enable
        // global $this->langs, $this->conf;
        $this->langs = Translator::getInstance();

        //        if (!is_object($this->langs)) {    // If lang was not defined, we set it. It is required by run_triggers.
    ...
    public function listeTypeContacts($source = 'internal', $option = 0, $activeonly = 0, $code = '', $element = '', $excludeelement = '')
    {
        ...
        $this->langs->loadLangs(['bills', 'contracts', 'interventions', 'orders', 'projects', 'propal', 'ticket', 'agenda']);
        ...
        if ($resql) {
            $num = $this->db->num_rows($resql);
            if ($num > 0) {
                $this->langs->loadLangs(["propal", "orders", "bills", "suppliers", "contracts", "supplier_proposal"]);
                ...
                while ($obj = $this->db->fetch_object($resql)) {
                    ...
                    if (!empty($this->conf->{$modulename}->enabled)) {
                        $libelle_element = $this->langs->trans('ContactDefault_' . $obj->element);
    ...
    public function fetch_thirdparty($force_thirdparty_id = 0)
    {
        ...
        if ($idtofetch) {
            ...
            if (!empty($this->conf->global->PRODUIT_MULTIPRICES) && empty($this->thirdparty->price_level)) {
    ...
    public function fetch_barcode()
    {
        ...
        if (empty($idtype) && $idtype != '0') {    // If type of barcode no set, we try to guess. If set to '0' it means we forced to have type remain not defined
            if ($this->element == 'product' && !empty($this->conf->global->PRODUIT_DEFAULT_BARCODE_TYPE)) {
                $idtype = $this->conf->global->PRODUIT_DEFAULT_BARCODE_TYPE;
        ...
        foreach ($this->fields as $field => $info) {
            if ($this->isDate($info)) {
                if (is_null($obj->{$field}) || $obj->{$field} === '' || $obj->{$field} === '0000-00-00 00:00:00' || $obj->{$field} === '1000-01-01 00:00:00') {
                    ...
                    $this->{$field} = $this->db->jdate($obj->{$field});
                }
                ...
                if ($numrows) {
                    ...
                    foreach ($tab as $key => $value) {
                        ...
                        if ($key != 'rowid' && $key != 'tms' && $key != 'fk_member' && !is_int($key)) {
                            // we can add this attribute to object
                            if (!empty($extrafields) && in_array($extrafields->attributes[$this->table_element]['type'][$key], ['date', 'datetime'])) {
                    ...
                    foreach ($tab as $key => $value) {
                        if (!empty($extrafields) && !empty($extrafields->attributes[$this->table_element]['computed'][$key])) {
                            //var_dump($this->conf->disable_compute);
                            if (empty($this->conf->disable_compute)) {
    ...
    public function setDocModel($user, $modelpdf)
    {
        ...
        $newmodelpdf = DolibarrFunctions::dol_trunc($modelpdf, 255);
    ...
    public function updateRangOfLine($rowid, $rang)
    {
        ...
        if (in_array($this->table_element_line, ['bom_bomline', 'ecm_files', 'emailcollector_emailcollectoraction'])) {
    ...
    public function updateLineUp($rowid, $rang)
    {
        if ($rang > 1) {
            $fieldposition = 'rang';
            if (in_array($this->table_element_line, ['ecm_files', 'emailcollector_emailcollectoraction'])) {
                $fieldposition = 'position';
            }
    ...
    public function update_price($exclspec = 0, $roundingadjust = 'none', $nodatabaseupdate = 0, $seller = null)
    {
        ...
        if (!empty($MODULE)) {
            if (!empty($this->conf->global->$MODULE)) {
                $modsactivated = explode(',', $this->conf->global->$MODULE);
                foreach ($modsactivated as $mod) {
                    if ($this->conf->$mod->enabled) {
        ...
        if ($forcedroundingmode == 'auto' && isset($this->conf->global->MAIN_ROUNDOFTOTAL_NOT_TOTALOFROUND)) {
            $forcedroundingmode = $this->conf->global->MAIN_ROUNDOFTOTAL_NOT_TOTALOFROUND;
        ...
        if ($resql) {
            ...
            if (!empty($this->linkedObjectsIds)) {
                ...
                foreach ($tmparray as $objecttype => $objectids) {       // $objecttype is a module name ('facture', 'mymodule', ...) or a module name with a suffix ('project_task', 'mymodule_myobj', ...)
                    ...
                    if ($this->conf->$module->enabled && (($element != $this->element) || $alsosametype)) {
                        if ($loadalsoobjects) {
    ...
    public function getCanvas($id = 0, $ref = '')
    {
        ...
        if (!empty($this->conf->global->MAIN_DISABLE_CANVAS)) {
    ...
    {
        global $user, $object, $hookmanager, $extrafields;
        global $form;
        ...
        $dirtpls = array_merge($this->conf->modules_parts['tpl'], [$defaulttpldir]);
        foreach ($dirtpls as $module => $reldir) {
            ...
            if (empty($this->conf->file->strict_mode)) {
                $res = @include $tpl;
            } else {
    ...
    {
        global $hookmanager, $user, $form, $extrafields, $object;
        // TODO We should not use global var for this
        global $inputalsopricewithtax, $usemargins, $disableedit, $disablemove, $disableremove, $outputalsopricetotalwithtax;

        // Define usemargins
        $usemargins = 0;
        if (!empty($this->conf->margin->enabled) && !empty($this->element) && in_array($this->element, ['facture', 'facturerec', 'propal', 'commande'])) {
            $usemargins = 1;
        }
        ...
        foreach ($this->lines as $line) {
            ...
            if (is_object($hookmanager)) {   // Old code is commented on preceding line.
                if (empty($line->fk_parent_line)) {
                    $parameters = ['line' => $line, 'num' => $num, 'i' => $i, 'dateSelector' => $dateSelector, 'seller' => $seller, 'buyer' => $buyer, 'selected' => $selected, 'table_element_line' => $line->table_element];
                    $reshook = $hookmanager->executeHooks('printObjectLine', $parameters, $this, $action); // Note that $action and $object may have been modified by some hooks
                } else {
                    $parameters = ['line' => $line, 'num' => $num, 'i' => $i, 'dateSelector' => $dateSelector, 'seller' => $seller, 'buyer' => $buyer, 'selected' => $selected, 'table_element_line' => $line->table_element, 'fk_parent_line' => $line->fk_parent_line];
    ...
    {
        global $user, $object, $hookmanager;
        global $form;
        ...
        if ($action != 'editline' || $selected != $line->id) {
            ...
            if ($line->fk_product > 0) {
                ...
                if (!empty($this->conf->global->MAIN_MULTILANGS)) {
                    ...
                    if (!empty($this->conf->global->PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE) && empty($newlang) && is_object($this->thirdparty)) {
                        $newlang = $this->thirdparty->default_lang; // To use language of customer
                    }
                    if (!empty($newlang)) {
                        $outputlangs = new Translate("", $this->conf);
    ...
    {
        global $hookmanager, $form;

        print '<tr class="liste_titre">';
        print '<td>' . $this->langs->trans('Ref') . '</td>';
        print '<td>' . $this->langs->trans('Description') . '</td>';
        print '<td class="right">' . $this->langs->trans('VATRate') . '</td>';
        print '<td class="right">' . $this->langs->trans('PriceUHT') . '</td>';
        if (!empty($this->conf->multicurrency->enabled)) {
            print '<td class="right">' . $this->langs->trans('PriceUHTCurrency') . '</td>';
        }
        print '<td class="right">' . $this->langs->trans('Qty') . '</td>';
        if (!empty($this->conf->global->PRODUCT_USE_UNITS)) {
            print '<td class="left">' . $this->langs->trans('Unit') . '</td>';
        }
        print '<td class="right">' . $this->langs->trans('ReductionShort') . '</td>';
            ...
            foreach ($this->lines as $line) {
                ...
                if (is_object($hookmanager)) {   // Old code is commented on preceding line.
                    if (empty($line->fk_parent_line)) {
                        ...
                        $parameters = ['line' => $line, 'i' => $i, 'restrictlist' => $restrictlist, 'selectedLines' => $selectedLines, 'fk_parent_line' => $line->fk_parent_line];
    ...
    public function printOriginLine($line, $var, $restrictlist = '', $defaulttpldir = '/core/tpl', $selectedLines = [])
    {
        ...
        if (($line->info_bits & 2) == 2) {  // TODO Not sure this is used for source object
            ...
            $this->tpl['label'] .= ($line->product_type == -1 ? '&nbsp;' : ($line->product_type == 1 ? img_object($this->langs->trans(''), 'service') : img_object($this->langs->trans(''), 'product')));
            if (!empty($line->desc)) {
        ...
        if (!empty($line->desc)) {
            if ($line->desc == '(CREDIT_NOTE)') {  // TODO Not sure this is used for source object
                ...
                $this->tpl['description'] = $this->langs->transnoentities("DiscountFromCreditNote", $discount->getNomUrl(0));
            } elseif ($line->desc == '(DEPOSIT)') {  // TODO Not sure this is used for source object
                $discount = new DiscountAbsolute($this->db);
                $discount->fetch($line->fk_remise_except);
                $this->tpl['description'] = $this->langs->transnoentities("DiscountFromDeposit", $discount->getNomUrl(0));
            } elseif ($line->desc == '(EXCESS RECEIVED)') {
                $discount = new DiscountAbsolute($this->db);
                $discount->fetch($line->fk_remise_except);
                $this->tpl['description'] = $this->langs->transnoentities("DiscountFromExcessReceived", $discount->getNomUrl(0));
            } elseif ($line->desc == '(EXCESS PAID)') {
                $discount = new DiscountAbsolute($this->db);
                $discount->fetch($line->fk_remise_except);
                $this->tpl['description'] = $this->langs->transnoentities("DiscountFromExcessPaid", $discount->getNomUrl(0));
            } else {
                $this->tpl['description'] = DolibarrFunctions::dol_trunc($line->desc, 60);
            }
        } else {
        ...
        if (!empty($this->conf->global->PRODUCT_USE_UNITS)) {
            $this->tpl['unit'] = $this->langs->transnoentities($line->getLabelOfUnit('long'));
        }
     ...
     * 3) If not but a constant $this->conf->global->OBJECTELEMENT_FIELDNAME is set, we return it (It is better to use the dedicated table).
    ...
    public function getDefaultCreateValueFor($fieldname, $alternatevalue = null)
    {
        ...
        if (isset($this->conf->global->$keyforfieldname)) {
            return $this->conf->global->$keyforfieldname;
        }
    ...
    public function setValuesForExtraLanguages($onlykey = '')
    {
        ...
        foreach ($_POST as $postfieldkey => $postfieldvalue) {
            ...
            /*if ($nofillrequired) {
				$this->langs->load('errors');
				setEventMessages($this->langs->trans('ErrorFieldsRequired').' : '.implode(', ', $error_field_required), null, 'errors');
				return -1;
			}*/
        }
    ...
    {
        $savDisableCompute = $this->conf->disable_compute;
        $this->conf->disable_compute = 1;

        $ret = $this->fetch($id);

        $this->conf->disable_compute = $savDisableCompute;
    ...
    public function insertExtraLanguages($trigger = '', $userused = null)
    {
        ...
        if (!empty($this->conf->global->MAIN_EXTRALANGUAGES_DISABLED)) {
    ...
    public function showInputField($val, $key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = 0, $nonewbutton = 0)
    {
        ...
        if ($computed) {
            if (!preg_match('/^search_/', $keyprefix)) {
                return '<span class="opacitymedium">' . $this->langs->trans("AutomaticallyCalculated") . '</span>';
            } else {
                return '';
            }
        }
        ...
        if (in_array($type, ['date'])) {
        ...
        } elseif (in_array($type, ['mail', 'phone', 'url'])) {
            $out = '<input type="text" class="flat ' . $morecss . '" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . dol_escape_htmltag($value) . '" ' . ($moreparam ? $moreparam : '') . ($autofocusoncreate ? ' autofocus' : '') . '>';
            ...
            if (!preg_match('/search_/', $keyprefix)) {        // If keyprefix is search_ or search_options_, we must just use a simple text field
                require_once DOL_DOCUMENT_ROOT . '/core/class/doleditor.class.php';
                $doleditor = new DolEditor($keyprefix . $key . $keysuffix, $value, '', 200, 'dolibarr_notes', 'In', false, false, !empty($this->conf->fckeditor->enabled) && $this->conf->global->FCKEDITOR_ENABLE_SOCIETE, ROWS_5, '90%');
                $out = $doleditor->Create(1);
            } else {
                $out = '<input type="text" class="flat ' . $morecss . ' maxwidthonsmartphone" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . dol_escape_htmltag($value) . '" ' . ($moreparam ? $moreparam : '') . '>';
            }
            ...
            $out = '<input type="text" class="flat ' . $morecss . ' maxwidthonsmartphone" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . $value . '" ' . ($moreparam ? $moreparam : '') . '> ' . $this->langs->getCurrencySymbol($this->conf->currency);
            ...
            $out = '';
            if (!empty($this->conf->use_javascript_ajax) && empty($this->conf->global->MAIN_EXTRAFIELDS_DISABLE_SELECT2)) {
            ...
            if (is_array($param['options'])) {
                ...
                if (in_array($InfoFieldList[0], ['tablewithentity'])) {
                    $sqlwhere .= " AND entity = " . ((int) $this->conf->entity);
                }
                $sql .= $sqlwhere;
                //print $sql;
                ...
                if ($resql) {
                    ...
                    while ($i < $num) {
                        ...
                        $labeltoshow = DolibarrFunctions::dol_trunc($labeltoshow, 45);

                        if ($value == $obj->rowid) {
                            foreach ($fields_label as $field_toshow) {
                                $translabel = $this->langs->trans($obj->$field_toshow);
                                if ($translabel != $obj->$field_toshow) {
                                    $labeltoshow = DolibarrFunctions::dol_trunc($translabel) . ' ';
                                } else {
                                    $labeltoshow = DolibarrFunctions::dol_trunc($obj->$field_toshow) . ' ';
                                }
                            }
                            ...
                            if (!$notrans) {
                                ...
                                if ($translabel != $obj->{$InfoFieldList[1]}) {
                                    $labeltoshow = DolibarrFunctions::dol_trunc($translabel, 18);
                                } else {
                                    $labeltoshow = DolibarrFunctions::dol_trunc($obj->{$InfoFieldList[1]});
                                }
                            }
                        ...
                        if (is_array($value_arr) && in_array($obj->rowid, $value_arr)) {
                            foreach ($fields_label as $field_toshow) {
                                ...
                                if ($translabel != $obj->$field_toshow) {
                                    ...
                                    $labeltoshow = DolibarrFunctions::dol_trunc($obj->$field_toshow, 18) . ' ';
                                }
                            }
                            ...
                            if (!$notrans) {
                                ...
                                if ($translabel != $obj->{$InfoFieldList[1]}) {
                                    ...
                                    $labeltoshow = DolibarrFunctions::dol_trunc($obj->{$InfoFieldList[1]}, 18);
                                }
                            }
                            if (empty($labeltoshow)) {
                                $labeltoshow = '(not defined)';
            ...
            if (!empty($param_list_array[2])) {        // If the entry into $fields is set to add a create button
                if (!GETPOSTISSET('backtopage') && empty($val['disabled']) && empty($nonewbutton)) {    // To avoid to open several times the 'Create Object' button and to avoid to have button if field is protected by a "disabled".
                    ...
                    $out .= '<a class="butActionNew" title="' . $this->langs->trans("New") . '" href="' . $url_path . '?action=create&backtopage=' . urlencode($_SERVER['PHP_SELF'] . ($paramforthenewlink ? '?' . $paramforthenewlink : '')) . '"><span class="fa fa-plus-circle valignmiddle"></span></a>';
                }
            }
    ...
    public function showOutputField($val, $key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = '')
    {
        ...
        if ($key == 'ref' && method_exists($this, 'getNomUrl')) {
            ...
            if (!is_null($value) && $value !== '') {
                $value = price($value, 0, $this->langs, 0, 0, -1, $this->conf->currency);
            }
        } elseif ($type == 'select') {
                ...
                if (is_array($fields_label) && count($fields_label) > 1) {
                    foreach ($fields_label as $field_toshow) {
                        ...
                        if ($translabel != $field_toshow) {
                            $value .= DolibarrFunctions::dol_trunc($translabel, 18) . ' ';
                        } else {
                    ...
                    if (is_array($value_arr) && in_array($obj->rowid, $value_arr)) {
                        if (is_array($fields_label) && count($fields_label) > 1) {
                            foreach ($fields_label as $field_toshow) {
                                ...
                                if ($translabel != $field_toshow) {
                                    $toprint[] = '<li class="select2-search-choice-dolibarr noborderoncategories" style="background: #bbb">' . DolibarrFunctions::dol_trunc($translabel, 18) . '</li>';
                                } else {
    ...
    public function validateField($val, $fieldKey, $fieldValue)
    {
        ...
        if (!isset($val[$fieldKey])) {
            $this->setFieldError($fieldKey, $this->langs->trans('FieldNotFoundInObject'));
            return false;
        }
        ...
        $validate = new Validate($this->db, $this->langs);
        ...
        if (in_array($type, ['date', 'datetime', 'timestamp'])) {
            ...
            if (!isset($param['options'][$fieldValue])) {
                $this->error = $this->langs->trans('RequireValidValue');
                return false;
            } else {
                return true;
            }
    ...
    {
        if (empty($msg)) {
            $msg = $this->langs->trans("UnknowError");
        }
    ...
    {
        global $action, $form, $hookmanager;
            ...
            if (key_exists('label', $extrafields->attributes[$this->table_element]) && is_array($extrafields->attributes[$this->table_element]['label']) && count($extrafields->attributes[$this->table_element]['label']) > 0) {
                ...
                foreach ($extrafields->attributes[$this->table_element]['label'] as $key => $label) {
                    ...
                    if (!empty($extrafields->attributes[$this->table_element]['langfile'][$key])) {
                        $this->langs->load($extrafields->attributes[$this->table_element]['langfile'][$key]);
                    }
                    ...
                    if ($extrafields->attributes[$this->table_element]['type'][$key] == 'separate') {
                        ...
                        if ($display_type == 'card') {
                            if (!empty($this->conf->global->MAIN_EXTRAFIELDS_USE_TWO_COLUMS) && ($e % 2) == 0) {
                                $colspan = '0';
                            }
                        ...
                        if (in_array($extrafields->attributes[$this->table_element]['type'][$key], ['html', 'text', 'varchar', 'select', 'int'])) {
                            if ($action == 'create') {
                                $value = (GETPOSTISSET($keyprefix . 'options_' . $key . $keysuffix) || $value) ? $value : $extrafields->attributes[$this->table_element]['default'][$key];
                            }
                        }

                        $labeltoshow = $this->langs->trans($label);
                        $helptoshow = $this->langs->trans($extrafields->attributes[$this->table_element]['help'][$key]);
                        ...
                        if ($display_type == 'card') {
                            ...
                            if (!empty($this->conf->global->MAIN_VIEW_LINE_NUMBER) && ($action == 'view' || $action == 'editline')) {
                                $out .= '<td></td>';
                            }
                        ...
                        if (!empty($this->conf->global->MAIN_EXTRAFIELDS_USE_TWO_COLUMS) && (($e % 2) == 1)) {
    ...
    public function defineBuyPrice($unitPrice = 0.0, $discountPercent = 0.0, $fk_product = 0)
    {
        ...
        if (($unitPrice > 0) && (isset($this->conf->global->ForceBuyingPriceIfNull) && $this->conf->global->ForceBuyingPriceIfNull > 0)) {
            // When ForceBuyingPriceIfNull is set
            $buyPrice = $unitPrice * (1 - $discountPercent / 100);
        } else {
            // Get cost price for margin calculation
            if (!empty($fk_product) && $fk_product > 0) {
                if (isset($this->conf->global->MARGIN_TYPE) && $this->conf->global->MARGIN_TYPE == 'costprice') {
                ...
                } elseif (isset($this->conf->global->MARGIN_TYPE) && $this->conf->global->MARGIN_TYPE == 'pmp') {
                    require_once DOL_DOCUMENT_ROOT . '/Modules/Products/class/product.class.php';
                    $product = new Product($this->db);
                    $result = $product->fetch($fk_product);
                    if ($result <= 0) {
                ...
                if (empty($buyPrice) && isset($this->conf->global->MARGIN_TYPE) && in_array($this->conf->global->MARGIN_TYPE, ['1', 'pmp', 'costprice'])) {
    ...
    public function show_photos($modulepart, $sdir, $size = 0, $nbmax = 0, $nbbyrow = 5, $showfilename = 0, $showaction = 0, $maxHeight = 120, $maxWidth = 160, $nolink = 0, $notitle = 0, $usesharelink = 0)
    {
        ...
        if ($modulepart == 'product') {
            if (!empty($this->conf->global->PRODUCT_USE_OLD_PATH_FOR_PHOTO)) {
        ...
        /*if (! empty($this->conf->global->PRODUCT_USE_OLD_PATH_FOR_PHOTO))    // For backward compatiblity, we scan also old dirs
		 {
            ...
            foreach ($filearray as $key => $val) {
                ...
                if (image_format_supported($file) >= 0) {
                    ...
                    if ($size == 1 || $size == 'small') {   // Format vignette
                        ...
                        $alt = $this->langs->transnoentitiesnoconv('File') . ': ' . $relativefile;
                        $alt .= ' - ' . $this->langs->transnoentitiesnoconv('Size') . ': ' . $imgarray['width'] . 'x' . $imgarray['height'];
                        if ($notitle) {
                            $alt = '';
                        }
        ...
        foreach ($this->fields as $field => $info) {    // Loop on definition of fields
            ...
            if ($this->isDate($info)) {
                ...
                if ($field == 'entity' && is_null($this->{$field})) {
                    $queryarray[$field] = $this->conf->entity;
                } else {
                    // $this->{$field} may be null, '', 0, '0', 123, '123'
                    if ((isset($this->{$field}) && $this->{$field} != '') || !empty($info['notnull'])) {
                        if (!isset($this->{$field})) {
    ...
    public function isObjectUsed($id = 0)
    {
        ...
        foreach ($arraytoscan as $table => $elementname) {
            ...
            if ($resql) {
                ...
                if ($obj->nb > 0) {
                    $this->langs->load("errors");
                    //print 'Found into table '.$table.', type '.$this->langs->transnoentitiesnoconv($elementname).', haschild='.$haschild;
                    $haschild += $obj->nb;
                    if (is_numeric($elementname)) {    // old usage
                        $this->errors[] = $this->langs->transnoentities("ErrorRecordHasAtLeastOneChildOfType", method_exists($this, 'getNomUrl') ? $this->getNomUrl() : $this->ref, $table);
                    } else // new usage: $elementname=Translation key
                    {
                        $this->errors[] = $this->langs->transnoentities("ErrorRecordHasAtLeastOneChildOfType", method_exists($this, 'getNomUrl') ? $this->getNomUrl() : $this->ref, $this->langs->transnoentitiesnoconv($elementname));
                    }
    ...
    public function deleteEcmFiles($mode = 0)
    {
        ...
        if ($mode == 0) {
            ...
            $sql .= " AND filepath = '" . $this->db->escape($element) . "/" . $this->db->escape($this->ref) . "' AND entity = " . ((int) $this->conf->entity); // No need of getEntity here
            $sql .= ")";
    ...
    protected function commonGenerateDocument($modelspath, $modele, $outputlangs, $hidedetails, $hidedesc, $hideref, $moreparams = null)
    {
        ...
        if (empty($reshook)) {
            ...
            $filefound = '';
            $dirmodels = ['/'];
            if (is_array($this->conf->modules_parts['models'])) {
                $dirmodels = array_merge($dirmodels, $this->conf->modules_parts['models']);
            }
            ...
            if ($filefound) {
                ...
                if ($obj->type == 'odt' && empty($srctemplatepath)) {
                    ...
                    if ($varfortemplatedir && !empty($this->conf->global->$varfortemplatedir)) {
                        $dirtoscan = $this->conf->global->$varfortemplatedir;
                ...
                if (in_array(get_class($this), ['Adherent'])) {
                    $arrayofrecords = []; // The write_file of templates of adherent class need this var
                ...
                if ($resultwritefile > 0) {
                    ...
                    if (!empty($obj->result['fullpath'])) {
                        ...
                        if (!preg_match('/[\\/]temp[\\/]|[\\/]thumbs|\.meta$/', $rel_dir)) {     // If not a tmp dir
                            ...
                            $setsharekey = false;
                            if ($this->element == 'propal') {
                                $useonlinesignature = $this->conf->global->MAIN_FEATURES_LEVEL; // Replace this with 1 when feature to make online signature is ok
                                if ($useonlinesignature) {
                                    $setsharekey = true;
                                }
                                if (!empty($this->conf->global->PROPOSAL_ALLOW_EXTERNAL_DOWNLOAD)) {
                                    $setsharekey = true;
                                }
                            }
                            if ($this->element == 'commande' && !empty($this->conf->global->ORDER_ALLOW_EXTERNAL_DOWNLOAD)) {
                                $setsharekey = true;
                            }
                            if ($this->element == 'facture' && !empty($this->conf->global->INVOICE_ALLOW_EXTERNAL_DOWNLOAD)) {
                                $setsharekey = true;
                            }
                            if ($this->element == 'bank_account' && !empty($this->conf->global->BANK_ACCOUNT_ALLOW_EXTERNAL_DOWNLOAD)) {
                                $setsharekey = true;
                            }
                            if ($this->element == 'contrat' && !empty($this->conf->global->CONTRACT_ALLOW_EXTERNAL_DOWNLOAD)) {
                                $setsharekey = true;
                            }
                            if ($this->element == 'supplier_proposal' && !empty($this->conf->global->SUPPLIER_PROPOSAL_ALLOW_EXTERNAL_DOWNLOAD)) {
                                $setsharekey = true;
                            }
                ...
                if (!$filefound) {
                    $this->error = $this->langs->trans("Error") . ' Failed to load doc generator with modelpaths=' . $modelspath . ' - modele=' . $modele;
                    dol_print_error('', $this->error);
                } else {
                    $this->error = $this->langs->trans("Error") . " " . $this->langs->trans("ErrorFileDoesNotExists", $filefound);
                    dol_print_error('', $this->error);
                }
                return -1;
            }
