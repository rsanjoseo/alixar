...
class MenuManager
{
    ...
    {
        global $user, $dolibarr_main_db_name;
        ...
        if ($mode == 'top') {
            if (empty($noout)) {
                $this->print_start_menu_array_empty();
            }

            $usemenuhider = 1;

            // Show/Hide vertical menu
            if ($mode != 'jmobile' && $mode != 'topnb' && $usemenuhider && empty($this->conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
                $showmode = 1;
            ...
            $this->menu->add('/index.php', $this->langs->trans("Home"), 0, $showmode, $this->atarget, 'home', '', 10, $id, $idsel, $classname);

            // Sort on position
            $this->menu->liste = DolibarrFunctions::dol_sort_array($this->menu->liste, 'position');
            ...
            $showmode = 1;
            if (empty($noout)) {
                $this->print_start_menu_entry_empty('', 'class="tmenuend"', $showmode);
            }
            if (empty($noout)) {
                $this->print_end_menu_entry_empty($showmode);
            }

            if (empty($noout)) {
                $this->print_end_menu_array_empty();
            }
        ...
        if ($mode == 'jmobile') {     // Used to get menu in xml ul/li
            ...
            $substitarray = getCommonSubstitutionArray($this->langs, 0, null, null);
            ...
            foreach ($this->menu->liste as $key => $val) {        // $val['url','titre','level','enabled'=0|1|2,'target','mainmenu','leftmenu'
                ...
                if ($val['enabled'] == 1) {
                    $relurl = DolibarrFunctions::dol_buildpath($val['url'], 1);
                    ...
                    $submenu->add(BASE_URI . "/admin/index.php?leftmenu=setup", $this->langs->trans("Setup"), 0);
                    $submenu->add("/admin/company.php", $this->langs->trans("MenuCompanySetup"), 1);
                    $submenu->add("/admin/modules.php", $this->langs->trans("Modules"), 1);
                    $submenu->add("/admin/menus.php", $this->langs->trans("Menus"), 1);
                    $submenu->add("/admin/ihm.php", $this->langs->trans("GUISetup"), 1);
                    $submenu->add("/admin/translation.php?mainmenu=home", $this->langs->trans("Translation"), 1);
                    $submenu->add("/admin/defaultvalues.php?mainmenu=home", $this->langs->trans("DefaultValues"), 1);

                    $submenu->add("/admin/boxes.php?mainmenu=home", $this->langs->trans("Boxes"), 1);
                    $submenu->add("/admin/delais.php?mainmenu=home", $this->langs->trans("Alerts"), 1);
                    $submenu->add("/admin/proxy.php?mainmenu=home", $this->langs->trans("Security"), 1);
                    $submenu->add("/admin/limits.php?mainmenu=home", $this->langs->trans("MenuLimits"), 1);
                    $submenu->add("/admin/pdf.php?mainmenu=home", $this->langs->trans("PDF"), 1);
                    $submenu->add("/admin/mails.php?mainmenu=home", $this->langs->trans("Emails"), 1);
                    $submenu->add("/admin/sms.php?mainmenu=home", $this->langs->trans("SMS"), 1);
                    $submenu->add("/admin/dict.php?mainmenu=home", $this->langs->trans("DictionarySetup"), 1);
                    $submenu->add("/admin/const.php?mainmenu=home", $this->langs->trans("OtherSetup"), 1);

                    //if ($tmpmainmenu.'-'.$tmpleftmenu == 'home-all') { var_dump($submenu); exit; }
                    //if ($tmpmainmenu=='accountancy') { var_dump($submenu->liste); exit; }
                    $nexturl = DolibarrFunctions::dol_buildpath($submenu->liste[0]['url'], 1);
                    ...
                    if (($canonrelurl != $canonnexturl && !in_array($val['mainmenu'], ['tools']))
                        ...
                        print '<a href="' . $relurl . '">';
                        if ($this->langs->trans(ucfirst($val['mainmenu']) . "Dashboard") == ucfirst($val['mainmenu']) . "Dashboard") {  // No translation
                            if (in_array($val['mainmenu'], ['cashdesk', 'websites'])) {
                                print $this->langs->trans("Access");
                            } else {
                                print $this->langs->trans("Dashboard");
                            }
                        } else {
                            print $this->langs->trans(ucfirst($val['mainmenu']) . "Dashboard");
                        }
                        print '</a>';
                        print '</li>' . "\n";
                    }
                    ...
                    foreach ($submenu->liste as $key2 => $val2) {        // $val['url','titre','level','enabled'=0|1|2,'target','mainmenu','leftmenu'
                        $showmenu = true;
                        if (!empty($this->conf->global->MAIN_MENU_HIDE_UNAUTHORIZED) && empty($val2['enabled'])) {
                            $showmenu = false;
                        }
                        ...
                        if ($showmenu) {        // Visible (option to hide when not allowed is off or allowed)
                            ...
                            $relurl2 = DolibarrFunctions::dol_buildpath($val2['url'], 1);
                ...
                for ($i = 0; $i < $num; $i++) {
                    ...
                    if ($menu_array[$i]['level'] == 0) {
                        if ($menu_array[$i]['enabled']) {
                            print '<div class="menu_titre">' . $tabstring . '<a class="vmenu" href="' . DolibarrFunctions::dol_buildpath($menu_array[$i]['url'], 1) . '"' . ($menu_array[$i]['target'] ? ' target="' . $menu_array[$i]['target'] . '"' : '') . '>' . $menu_array[$i]['titre'] . '</a></div>' . "\n";
                        } else {
                    ...
                    if ($menu_array[$i]['level'] > 0) {
                        $cssmenu = '';
                        if ($menu_array[$i]['url']) {
                            $cssmenu = ' menu_contenu' . DolibarrFunctions::dol_string_nospecial(preg_replace('/\.php.*$/', '', $menu_array[$i]['url']));
                        }
                        ...
                        if ($menu_array[$i]['enabled']) {
                            print $tabstring;
                            if ($menu_array[$i]['url']) {
                                print '<a class="vsmenu"  itle="' . DolibarrFunctions::dol_escape_htmltag($menu_array[$i]['titre']) . '" href="' . DolibarrFunctions::dol_buildpath($menu_array[$i]['url'], 1) . '"' . ($menu_array[$i]['target'] ? ' target="' . $menu_array[$i]['target'] . '"' : '') . '>';
                            } else {
                                print '<span class="vsmenu" title="' . DolibarrFunctions::dol_escape_htmltag($menu_array[$i]['titre']) . '">';
                            }
                            if ($menu_array[$i]['url']) {
    ...
    function print_start_menu_array_empty()
    {
        ...
        print '<ul role="navigation" class="tmenu"' . (empty($this->conf->global->MAIN_OPTIMIZEFORTEXTBROWSER) ? '' : ' title="Top menu"') . '>';
    }
    ...
    function print_text_menu_entry_empty($text, $showmode, $url, $id, $idsel, $classname, $atarget)
    {
        ...
        if ($showmode == 2) {
            ...
            print '<a class="tmenudisabled" id="mainmenua_' . $idsel . '" href="#" title="' . DolibarrFunctions::dol_escape_htmltag($this->langs->trans("NotAllowed")) . '">';
        }
    }
