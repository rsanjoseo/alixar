...
class FormCompany extends Form
{
    ...
    public function effectif_array($mode = 0, $filter = '')
    {
        ...
        $sql .= " ORDER BY id ASC";
        DolibarrFunctions::dol_syslog(get_class($this) . '::effectif_array', LOG_DEBUG);
    ...
    public function form_prospect_level($page, $selected = '', $htmlname = 'prospect_level_id', $empty = 0)
    {
        ...
        DolibarrFunctions::dol_syslog(get_class($this) . '::form_prospect_level', LOG_DEBUG);
        ...
        if ($resql) {
            ...
            while ($obj = $this->db->fetch_object($resql)) {
                $level = $this->langs->trans($obj->code);

                if ($level == $obj->code) {
                    $level = $this->langs->trans($obj->label);
                }
        ...
        if (!empty($htmlname) && $this->user->admin) {
            print ' ' . DolibarrFunctions::info_admin($this->langs->trans("YouCanChangeValuesForThisListFromDictionarySetup"), 1);
        }
        print '<input type="submit" class="button button-save valignmiddle" value="' . $this->langs->trans("Modify") . '">';
        print '</form>';
    }
    ...
    public function select_state($selected = 0, $country_codeid = 0, $htmlname = 'state_id')
    {
        ...
        DolibarrFunctions::dol_syslog(get_class($this) . "::select_departement selected=" . $selected . ", country_codeid=" . $country_codeid, LOG_DEBUG);
        ...
        if ($result) {
            ...
            $i = 0;
            DolibarrFunctions::dol_syslog(get_class($this) . "::select_departement num=" . $num, LOG_DEBUG);
            if ($num) {
                $country = '';
                while ($i < $num) {
                    ...
                    if ($obj->code == '0') {        // Le code peut etre une chaine
                        ...
                        if (!empty($this->conf->global->MAIN_SHOW_STATE_CODE) &&
                            ($this->conf->global->MAIN_SHOW_STATE_CODE == 1 || $this->conf->global->MAIN_SHOW_STATE_CODE == 2 || $this->conf->global->MAIN_SHOW_STATE_CODE === 'all')) {
                            if (!empty($this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && $this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1) {
                                $out .= $obj->region_name . ' - ' . $obj->code . ' - ' . ($this->langs->trans($obj->code) != $obj->code ? $this->langs->trans($obj->code) : ($obj->name != '-' ? $obj->name : ''));
                            } else {
                                $out .= $obj->code . ' - ' . ($this->langs->trans($obj->code) != $obj->code ? $this->langs->trans($obj->code) : ($obj->name != '-' ? $obj->name : ''));
                            }
                        } else {
                            if (!empty($this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && $this->conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1) {
                                $out .= $obj->region_name . ' - ' . ($this->langs->trans($obj->code) != $obj->code ? $this->langs->trans($obj->code) : ($obj->name != '-' ? $obj->name : ''));
                            } else {
                                $out .= ($this->langs->trans($obj->code) != $obj->code ? $this->langs->trans($obj->code) : ($obj->name != '-' ? $obj->name : ''));
                            }
                        }
    ...
    public function select_region($selected = '', $htmlname = 'region_id')
    {
        ...
        DolibarrFunctions::dol_syslog(get_class($this) . "::select_region", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            ...
            if ($num) {
                ...
                while ($i < $num) {
                    ...
                    if ($obj->code == 0) {
                        ...
                        if ($country == '' || $country != $obj->country) {
                            // Show break
                            $key = $this->langs->trans("Country" . strtoupper($obj->country_code));
    ...
    public function select_civility($selected = '', $htmlname = 'civility_id', $morecss = 'maxwidth150', $addjscombo = 1)
    {
        ...
        DolibarrFunctions::dol_syslog("Form::select_civility", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            ...
            if ($num) {
                while ($i < $num) {
                    ...
                    $out .= ($this->langs->trans("Civility" . $obj->code) != "Civility" . $obj->code ? $this->langs->trans("Civility" . $obj->code) : ($obj->label != '-' ? $obj->label : ''));
                    $out .= '</option>';
                    $i++;
                }
            }
    ...
    public function select_juridicalstatus($selected = '', $country_codeid = 0, $filter = '', $htmlname = 'forme_juridique_code', $morecss = '')
    {
        ...
        DolibarrFunctions::dol_syslog(get_class($this) . "::select_juridicalstatus", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            ...
            if ($num) {
                ...
                while ($i < $num) {
                    ...
                    if ($obj->code) {        // We exclude empty line, we will add it later
                        $labelcountry = (($this->langs->trans("Country" . $obj->country_code) != "Country" . $obj->country_code) ? $this->langs->trans("Country" . $obj->country_code) : $obj->country);
                        $labeljs = (($this->langs->trans("JuridicalStatus" . $obj->code) != "JuridicalStatus" . $obj->code) ? $this->langs->trans("JuridicalStatus" . $obj->code) : ($obj->label != '-' ? $obj->label : '')); // $obj->label is already in output charset (converted by database driver)
                ...
                $arraydata = DolibarrFunctions::dol_sort_array($arraydata, 'label_sort', 'ASC');
    ...
    {
        if (!empty($this->conf->use_javascript_ajax) && !empty($this->conf->global->COMPANY_USE_SEARCH_TO_SELECT)) {
            // Use Ajax search
            $minLength = (is_numeric($this->conf->global->COMPANY_USE_SEARCH_TO_SELECT) ? $this->conf->global->COMPANY_USE_SEARCH_TO_SELECT : 2);
            ...
            print ajax_autocompleter(($socid ? $socid : -1), $htmlname, DOL_URL_ROOT . '/Modules/Societes/ajax/ajaxcompanies.php', '', $minLength, 0);
            return $socid;
        } else {
    ...
    public function selectTypeContact($object, $selected, $htmlname = 'type', $source = 'internal', $sortorder = 'position', $showempty = 0, $morecss = '', $output = 1, $forcehidetooltip = 0)
    {
        ...
        if (is_object($object) && method_exists($object, 'liste_type_contact')) {
            ...
            $out .= "</select>";
            if ($this->user->admin && empty($forcehidetooltip)) {
                $out .= ' ' . DolibarrFunctions::info_admin($this->langs->trans("YouCanChangeValuesForThisListFromDictionarySetup"), 1);
            }
     ...
     */
    public function showRoles($htmlname, Contact $contact, $rendermode = 'view', $selected = [])
    {
        if ($rendermode === 'view') {
            $toprint = [];
    ...
    public function get_input_id_prof($idprof, $htmlname, $preselected, $country_code, $morecss = 'maxwidth100onsmartphone quatrevingtpercent')
    {
        // phpcs:enable
        $formlength = 0;
        if (empty($this->conf->global->MAIN_DISABLEPROFIDRULES)) {
            if ($country_code == 'FR') {
                if (isset($idprof)) {
                    if ($idprof == 1) {
                        $formlength = 9;
    ...
    {
        // phpcs:enable
        $tax = DolibarrFunctions::get_localtax_by_third($local);
    ...
    {
        if (!empty($this->conf->global->SOCIETE_DISABLE_PROSPECTS) && !empty($this->conf->global->SOCIETE_DISABLE_CUSTOMERS) && empty($this->conf->fournisseur->enabled)) {
            return '';
        }
        ...
        if ($typeinput == 'form') {
            if ($allowempty || ($selected == '' || $selected == '-1')) {
                ...
                if (is_numeric($allowempty)) {
                    ...
                    $out .= $this->langs->trans($allowempty);
                }
                $out .= '</option>';
            }
            if (empty($this->conf->global->SOCIETE_DISABLE_PROSPECTS)) {
                $out .= '<option value="2"' . ($selected == 2 ? ' selected' : '') . '>' . $this->langs->trans('Prospect') . '</option>';
            }
            if (empty($this->conf->global->SOCIETE_DISABLE_PROSPECTS) && empty($this->conf->global->SOCIETE_DISABLE_CUSTOMERS) && empty($this->conf->global->SOCIETE_DISABLE_PROSPECTSCUSTOMERS)) {
                $out .= '<option value="3"' . ($selected == 3 ? ' selected' : '') . '>' . $this->langs->trans('ProspectCustomer') . '</option>';
            }
            if (empty($this->conf->global->SOCIETE_DISABLE_CUSTOMERS)) {
                $out .= '<option value="1"' . ($selected == 1 ? ' selected' : '') . '>' . $this->langs->trans('Customer') . '</option>';
            }
            $out .= '<option value="0"' . ((string) $selected == '0' ? ' selected' : '') . '>' . $this->langs->trans('NorProspectNorCustomer') . '</option>';
        } elseif ($typeinput == 'list') {
            $out .= '<option value="-1"' . (($selected == '' || $selected == '-1') ? ' selected' : '') . '>&nbsp;</option>';
            if (empty($this->conf->global->SOCIETE_DISABLE_PROSPECTS)) {
                $out .= '<option value="2,3"' . ($selected == '2,3' ? ' selected' : '') . '>' . $this->langs->trans('Prospect') . '</option>';
            }
            if (empty($this->conf->global->SOCIETE_DISABLE_CUSTOMERS)) {
                $out .= '<option value="1,3"' . ($selected == '1,3' ? ' selected' : '') . '>' . $this->langs->trans('Customer') . '</option>';
            }
            if (!empty($this->conf->fournisseur->enabled)) {
                $out .= '<option value="4"' . ($selected == '4' ? ' selected' : '') . '>' . $this->langs->trans('Supplier') . '</option>';
            }
            $out .= '<option value="0"' . ($selected == '0' ? ' selected' : '') . '>' . $this->langs->trans('Other') . '</option>';
    ...
    public function formThirdpartyType($page, $selected = '', $htmlname = 'socid', $filter = '', $nooutput = 0)
    {
        ...
        if ($htmlname != "none") {
            ...
            $sortparam = (empty($this->conf->global->SOCIETE_SORT_ON_TYPEENT) ? 'ASC' : $this->conf->global->SOCIETE_SORT_ON_TYPEENT); // NONE means we keep sort of original array, so we sort on position. ASC, means next function will sort on label.
    ...
    public function typent_array($mode = 0, $filter = '')
    {
        ...
        $sql .= " WHERE active = 1 AND (fk_country IS NULL OR fk_country = " . (empty($this->mysoc->country_id) ? '0' : $this->mysoc->country_id) . ")";
        if ($filter) {
            $sql .= " " . $filter;
        }
        $sql .= " ORDER by position, id";
        DolibarrFunctions::dol_syslog(get_class($this) . '::typent_array', LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            $num = $this->db->num_rows($resql);
            $i = 0;
            ...
            while ($i < $num) {
                ...
                if ($this->langs->trans($objp->code) != $objp->code) {
                    $effs[$key] = $this->langs->trans($objp->code);
                } else {
