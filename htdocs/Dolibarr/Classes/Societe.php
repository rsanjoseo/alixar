...
class Societe extends CommonObject
{
    ...
    public $fields = [
        ...
        'code_compta' => ['type' => 'varchar(24)', 'label' => 'CodeCompta', 'enabled' => 1, 'visible' => -1, 'position' => 65],
        'code_compta_fournisseur' => ['type' => 'varchar(24)', 'label' => 'CodeComptaSupplier', 'enabled' => 1, 'visible' => -1, 'position' => 70],
        ...
        'note_private' => ['type' => 'text', 'label' => 'NotePublic', 'enabled' => 1, 'visible' => 0, 'position' => 225],
        'note_public' => ['type' => 'text', 'label' => 'NotePrivate', 'enabled' => 1, 'visible' => 0, 'position' => 230],
        'prefix_comm' => ['type' => 'varchar(5)', 'label' => 'Prefix comm', 'enabled' => '$conf->global->SOCIETE_USEPREFIX', 'visible' => -1, 'position' => 235],
        ...
        'order_min_amount' => ['type' => 'double(24,8)', 'label' => 'Order min amount', 'enabled' => '!empty($conf->commande->enabled) && !empty($conf->global->ORDER_MANAGE_MIN_AMOUNT)', 'visible' => -1, 'position' => 315, 'isameasure' => 1],
        'supplier_order_min_amount' => ['type' => 'double(24,8)', 'label' => 'Supplier order min amount', 'enabled' => '!empty($conf->commande->enabled) && !empty($conf->global->ORDER_MANAGE_MIN_AMOUNT)', 'visible' => -1, 'position' => 320, 'isameasure' => 1],
        'cond_reglement_supplier' => ['type' => 'integer', 'label' => 'Cond reglement supplier', 'enabled' => 1, 'visible' => -1, 'position' => 325],
        ...
        'price_level' => ['type' => 'integer', 'label' => 'Price level', 'enabled' => '$conf->global->PRODUIT_MULTIPRICES || $conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES', 'visible' => -1, 'position' => 365],
        ...
        'fk_account' => ['type' => 'integer', 'label' => 'AccountingAccount', 'enabled' => 1, 'visible' => -1, 'position' => 450],
    ...
    public function id_prof_check($idprof, $soc)
    {
        ...
        if ($idprof == 1 && $soc->country_code == 'ES') {
            ...
            if (preg_match('/^[XYZ]{1}/', $string)) {
                if ($num[8] == substr('TRWAGMYFPDXBNJZSQVHLCKE', substr(str_replace(['X', 'Y', 'Z'], ['0', '1', '2'], $string), 0, 8) % 23, 1)) {
                    return 3;
                } else {
                    return -3;
                }
     ...
     * @return    int                    <0 if KO, >0 if OK
     */
    public function create_individual(User $user, $no_email = 0, $tags = [])
    {
        global $conf;
