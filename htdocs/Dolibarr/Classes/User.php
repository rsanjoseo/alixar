...
class User extends CommonObject
{
    ...
    public function hasRight($module, $permlevel1, $permlevel2 = '')
    {
        ...
        if (!in_array($module, $this->conf->modules)) {
            return 0;
        }
    ...
    public function addrights($rid, $allmodule = '', $allperms = '', $entity = 0, $notrigger = 0)
    {
        ...
        if (!$error && !$notrigger) {
            ...
            $this->context = ['audit' => $this->langs->trans("PermissionsAdd") . ($rid ? ' (id=' . $rid . ')' : '')];
    ...
    public function delrights($rid, $allmodule = '', $allperms = '', $entity = 0, $notrigger = 0)
    {
        ...
        if (!$error && !$notrigger) {
            $this->langs->load("other");
            $this->context = ['audit' => $this->langs->trans("PermissionsDelete") . ($rid ? ' (id=' . $rid . ')' : '')];
    ...
    public function getrights($moduletag = '', $forcereload = 0)
    {
        ...
        if (!empty($this->conf->global->MULTICOMPANY_BACKWARD_COMPATIBILITY)) {
            $sql .= " AND r.entity IN (0," . (!empty($this->conf->multicompany->enabled) && !empty($this->conf->global->MULTICOMPANY_TRANSVERSE_MODE) ? "1," : "") . $this->conf->entity . ")";
        } else {
            $sql .= " AND ur.entity = " . ((int) $this->conf->entity);
        }
        ...
        if (!empty($this->conf->global->MULTICOMPANY_BACKWARD_COMPATIBILITY)) {
            if (!empty($this->conf->multicompany->enabled) && !empty($this->conf->global->MULTICOMPANY_TRANSVERSE_MODE)) {
                $sql .= " AND gu.entity IN (0," . $this->conf->entity . ")";
            } else {
                $sql .= " AND r.entity = " . ((int) $this->conf->entity);
            }
        } else {
            $sql .= " AND gr.entity = " . ((int) $this->conf->entity);
            $sql .= " AND gu.entity = " . ((int) $this->conf->entity);
            $sql .= " AND r.entity = " . ((int) $this->conf->entity);
        }
    ...
    public function fetch($id = '', $login = '', $sid = '', $loadpersonalconf = 0, $entity = -1, $email = '')
    {
        ...
        if ($entity < 0) {
            if ((empty($this->conf->multicompany->enabled) || empty($this->conf->global->MULTICOMPANY_TRANSVERSE_MODE)) && (!empty($user->entity))) {
                $sql .= " WHERE u.entity IN (0, " . ((int) $this->conf->entity) . ")";
            } else {
            ...
            if (!empty($this->conf->multicompany->enabled) && !empty($this->conf->global->MULTICOMPANY_TRANSVERSE_MODE)) {
                ...
                $sql .= " WHERE u.entity IN (0, " . ((int) (($entity != '' && $entity >= 0) ? $entity : $this->conf->entity)) . ")"; // search in entity provided in parameter
            }
        }
        ...
        if ($result) {
            ...
            if ($obj) {
                ...
                //$this->country = $obj->country_id?($this->langs->trans('Country'.$obj->country_code)!='Country'.$obj->country_code?$this->langs->transnoentities('Country'.$obj->country_code):$obj->country):'';
                ...
                if (empty($this->conf->multicompany->enabled) && $this->admin && $this->entity == 1) {
                    $this->entity = 0;
                }
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        if (!empty($this->conf->global->USER_MAIL_REQUIRED) && !isValidEMail($this->email)) {
            ...
            $this->error = $this->langs->trans("ErrorBadEMail", $this->email);
            return -1;
        }
        if (empty($this->login)) {
            $this->langs->load("errors");
            $this->error = $this->langs->trans("ErrorFieldRequired", $this->langs->transnoentitiesnoconv("Login"));
            return -1;
        } elseif (preg_match('/[,@<>"\']/', $this->login)) {
            $this->langs->load("errors");
            $this->error = $this->langs->trans("ErrorBadCharIntoLoginName");
            return -1;
        }
        ...
        if ($result) {
            ...
            if (!empty($this->conf->global->MAIN_DEFAULT_WAREHOUSE_USER) && !empty($this->conf->global->STOCK_USERSTOCK_AUTOCREATE)) {
                require_once DOL_DOCUMENT_ROOT . '/Modules/Products/stock/class/entrepot.class.php';
                $this->langs->load("stocks");
                $entrepot = new Entrepot($this->db);
                $entrepot->label = $this->langs->trans("PersonalStock", $this->getFullName($this->langs));
                $entrepot->libelle = $entrepot->label; // For backward compatibility
                $entrepot->description = $this->langs->trans("ThisWarehouseIsPersonalStock", $this->getFullName($this->langs));
                $entrepot->statut = 1;
    ...
    public function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0, $nosynccontact = 0)
    {
        ...
        if (empty($this->login)) {
            $this->langs->load("errors");
            $this->error = $this->langs->trans("ErrorFieldRequired", 'Login');
            return -1;
        } elseif (preg_match('/[,@<>"\']/', $this->login)) {
            $this->langs->load("errors");
            $this->error = $this->langs->trans("ErrorBadCharIntoLoginName");
    ...
    public function setPassword($user, $password = '', $changelater = 0, $notrigger = 0, $nosyncmember = 0, $passwordalreadycrypted = 0)
    {
        ...
        if (!$changelater) {
            ...
            $sql .= " pass_temp = null";
            if (!empty($this->conf->global->DATABASE_PWD_ENCRYPTED)) {
                $sql .= ", pass = null";
            } else {
    ...
    public function send_password($user, $password = '', $changelater = 0)
    {
        ...
        if ($mailfile->sendfile()) {
            ...
            $this->langs->trans("errors");
            $this->error = $this->langs->trans("ErrorFailedToSendPassword") . ' ' . $mailfile->error;
            return -1;
        }
    }
    ...
    public function SetInGroup($group, $entity, $notrigger = 0)
    {
        ...
        if ($result) {
            if (!$error && !$notrigger) {
                ...
                $this->context = ['audit' => $this->langs->trans("UserSetInGroup"), 'newgroupid' => $group];
    ...
    public function RemoveFromGroup($group, $entity, $notrigger = 0)
    {
        ...
        if ($result) {
            if (!$error && !$notrigger) {
                ...
                $this->context = ['audit' => $this->langs->trans("UserRemovedFromGroup"), 'oldgroupid' => $group];
    ...
    public function getNomUrl($withpictoimg = 0, $option = '', $infologin = 0, $notooltip = 0, $maxlen = 24, $hidethirdpartylogo = 0, $mode = '', $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        if (!empty($this->conf->global->MAIN_OPTIMIZEFORTEXTBROWSER) && $withpictoimg) {
            $withpictoimg = 0;
        }
        ...
        $label .= img_picto('', $this->picto) . ' <u class="paddingrightonly">' . $this->langs->trans("User") . '</u>';
        $label .= ' ' . $this->getLibStatut(4);
        $label .= '<br><b>' . $this->langs->trans('Name') . ':</b> ' . dol_string_nohtmltag($this->getFullName($this->langs, ''));
        if (!empty($this->login)) {
            $label .= '<br><b>' . $this->langs->trans('Login') . ':</b> ' . dol_string_nohtmltag($this->login);
        }
        if (!empty($this->job)) {
            $label .= '<br><b>' . $this->langs->trans("Job") . ':</b> ' . dol_string_nohtmltag($this->job);
        }
        $label .= '<br><b>' . $this->langs->trans("Email") . ':</b> ' . dol_string_nohtmltag($this->email);
        if (!empty($this->office_phone) || !empty($this->office_fax) || !empty($this->fax)) {
            ...
            $label .= '<br><b>' . $this->langs->trans('Phone') . ':</b> ' . implode('&nbsp;', $phonelist);
        }
        if (!empty($this->admin)) {
            $label .= '<br><b>' . $this->langs->trans("Administrator") . '</b>: ' . yn($this->admin);
        }
        if (!empty($this->accountancy_code) || $option == 'accountancy') {
            $label .= '<br><b>' . $this->langs->trans("AccountancyCode") . '</b>: ' . $this->accountancy_code;
        }
        $company = '';
        if (!empty($this->socid)) {    // Add thirdparty for external users
            ...
            $company = ' (' . $this->langs->trans("Company") . ': ' . dol_string_nohtmltag($thirdpartystatic->name) . ')';
        }
        $type = ($this->socid ? $this->langs->trans("External") . $company : $this->langs->trans("Internal"));
        $label .= '<br><b>' . $this->langs->trans("Type") . ':</b> ' . dol_string_nohtmltag($type);
        $label .= '</div>';
        if ($infologin > 0) {
            $label .= '<br>';
            $label .= '<br><u>' . $this->langs->trans("Session") . '</u>';
            $label .= '<br><b>' . $this->langs->trans("IPAddress") . '</b>: ' . dol_string_nohtmltag(getUserRemoteIP());
            if (!empty($this->conf->global->MAIN_MODULE_MULTICOMPANY)) {
                $label .= '<br><b>' . $this->langs->trans("ConnectedOnMultiCompany") . ':</b> ' . $this->conf->entity . ' (User entity ' . $this->entity . ')';
            }
            $label .= '<br><b>' . $this->langs->trans("AuthenticationMode") . ':</b> ' . dol_string_nohtmltag($_SESSION["dol_authmode"] . (empty($dolibarr_main_demo) ? '' : ' (demo)'));
            $label .= '<br><b>' . $this->langs->trans("ConnectedSince") . ':</b> ' . dol_print_date($this->datelastlogin, "dayhour", 'tzuser');
            $label .= '<br><b>' . $this->langs->trans("PreviousConnexion") . ':</b> ' . dol_print_date($this->datepreviouslogin, "dayhour", 'tzuser');
            $label .= '<br><b>' . $this->langs->trans("CurrentTheme") . ':</b> ' . dol_string_nohtmltag($this->conf->theme);
            $label .= '<br><b>' . $this->langs->trans("CurrentMenuManager") . ':</b> ' . dol_string_nohtmltag($menumanager->name);
            $s = picto_from_langcode($this->langs->getDefaultLang());
            $label .= '<br><b>' . $this->langs->trans("CurrentUserLanguage") . ':</b> ' . dol_string_nohtmltag(($s ? $s . ' ' : '') . $this->langs->getDefaultLang());
            $label .= '<br><b>' . $this->langs->trans("Browser") . ':</b> ' . dol_string_nohtmltag($this->conf->browser->name . ($this->conf->browser->version ? ' ' . $this->conf->browser->version : '') . ' (' . $_SERVER['HTTP_USER_AGENT'] . ')');
            $label .= '<br><b>' . $this->langs->trans("Layout") . ':</b> ' . dol_string_nohtmltag($this->conf->browser->layout);
            $label .= '<br><b>' . $this->langs->trans("Screen") . ':</b> ' . dol_string_nohtmltag($_SESSION['dol_screenwidth'] . ' x ' . $_SESSION['dol_screenheight']);
            if ($this->conf->browser->layout == 'phone') {
                $label .= '<br><b>' . $this->langs->trans("Phone") . ':</b> ' . $this->langs->trans("Yes");
            }
            if (!empty($_SESSION["disablemodules"])) {
                $label .= '<br><b>' . $this->langs->trans("DisabledModules") . ':</b> <br>' . dol_string_nohtmltag(join(', ', explode(',', $_SESSION["disablemodules"])));
            }
        }
        if ($infologin < 0) {
            $label = '';
        ...
        if (empty($notooltip)) {
            if (!empty($this->conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
                ...
                $label = $this->langs->trans("ShowUser");
        ...
        if ($withpictoimg) {
            ...
            if (abs($withpictoimg) == 1) {
                $paddafterimage = 'style="margin-' . ($this->langs->trans("DIRECTION") == 'rtl' ? 'left' : 'right') . ': 3px;"';
            }
            // Only picto
            if ($withpictoimg > 0) {
        ...
        if ($withpictoimg > -2 && $withpictoimg != 2) {
            ...
            if ($mode == 'login') {
                ...
                $result .= dol_string_nohtmltag($this->getFullName($this->langs, '', ($mode == 'firstelselast' ? 3 : ($mode == 'firstname' ? 2 : -1)), $maxlen));
            }
            if (empty($this->conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
                $result .= '</span>';
            }
    ...
    public function LibStatut($status, $mode = 0)
    {
        ...
        if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
            //            global $this->langs;
            $this->langs->load("mymodule");
            $this->labelStatus[self::STATUS_ENABLED] = $this->langs->trans('Enabled');
            $this->labelStatus[self::STATUS_DISABLED] = $this->langs->trans('Disabled');
            $this->labelStatusShort[self::STATUS_ENABLED] = $this->langs->trans('Enabled');
            $this->labelStatusShort[self::STATUS_DISABLED] = $this->langs->trans('Disabled');
        }
        ...
        return DolibarrFunctions::dolGetStatus($this->labelStatus[$status], $this->labelStatusShort[$status], '', $statusType, $mode);
    }
    ...
    public function _load_ldap_dn($info, $mode = 0)
    {
        ...
        $dn = '';
        if ($mode == 0) {
            $dn = $this->conf->global->LDAP_KEY_USERS . "=" . $info[$this->conf->global->LDAP_KEY_USERS] . "," . $this->conf->global->LDAP_USER_DN;
        } elseif ($mode == 1) {
            $dn = $this->conf->global->LDAP_USER_DN;
        } elseif ($mode == 2) {
            $dn = $this->conf->global->LDAP_KEY_USERS . "=" . $info[$this->conf->global->LDAP_KEY_USERS];
        }
        return $dn;
    }
    ...
    public function _load_ldap_info()
    {
        ...
        // Object classes
        $info["objectclass"] = explode(',', $this->conf->global->LDAP_USER_OBJECT_CLASS);

        $this->fullname = $this->getFullName($this->langs);
        ...
        foreach ($ldapkey as $constname => $varname) {
            if (!empty($this->$varname) && !empty($this->conf->global->$constname)) {
                $info[$this->conf->global->$constname] = $this->$varname;

                // Check if it is the LDAP key and if its value has been changed
                if (!empty($this->conf->global->LDAP_KEY_USERS) && $this->conf->global->LDAP_KEY_USERS == $this->conf->global->$constname) {
        ...
        foreach ($socialnetworks as $key => $value) {
            if ($this->socialnetworks[$value['label']] && !empty($this->conf->global->{'LDAP_FIELD_' . strtoupper($value['label'])})) {
                $info[$this->conf->global->{'LDAP_FIELD_' . strtoupper($value['label'])}] = $this->socialnetworks[$value['label']];
            }
        }
        if ($this->address && !empty($this->conf->global->LDAP_FIELD_ADDRESS)) {
            $info[$this->conf->global->LDAP_FIELD_ADDRESS] = $this->address;
        }
        if ($this->zip && !empty($this->conf->global->LDAP_FIELD_ZIP)) {
            $info[$this->conf->global->LDAP_FIELD_ZIP] = $this->zip;
        }
        if ($this->town && !empty($this->conf->global->LDAP_FIELD_TOWN)) {
            $info[$this->conf->global->LDAP_FIELD_TOWN] = $this->town;
        }
        if ($this->note_public && !empty($this->conf->global->LDAP_FIELD_DESCRIPTION)) {
            $info[$this->conf->global->LDAP_FIELD_DESCRIPTION] = dol_string_nohtmltag($this->note_public, 2);
        }
        if ($this->socid > 0) {
            $soc = new Societe($this->db);
            $soc->fetch($this->socid);

            $info[$this->conf->global->LDAP_FIELD_COMPANY] = $soc->name;
            if ($soc->client == 1) {
        ...
        if (!empty($this->pass)) {
            if (!empty($this->conf->global->LDAP_FIELD_PASSWORD)) {
                $info[$this->conf->global->LDAP_FIELD_PASSWORD] = $this->pass; // this->pass = mot de passe non crypte
            }
            if (!empty($this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED)) {
                $info[$this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED] = dol_hash($this->pass, 4); // Create OpenLDAP MD5 password (TODO add type of encryption)
            }
        } elseif ($this->conf->global->LDAP_SERVER_PROTOCOLVERSION !== '3') {
            // Set LDAP password if possible
            // If ldap key is modified and LDAPv3 we use ldap_rename function for avoid lose encrypt password
            if (!empty($this->conf->global->DATABASE_PWD_ENCRYPTED)) {
                // Just for the default MD5 !
                if (empty($this->conf->global->MAIN_SECURITY_HASH_ALGO)) {
                    if ($this->pass_indatabase_crypted && !empty($this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED)) {
                        $info[$this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED] = dol_hash($this->pass_indatabase_crypted, 5); // Create OpenLDAP MD5 password from Dolibarr MD5 password
                    }
                }
            } elseif (!empty($this->pass_indatabase)) {
                // Use $this->pass_indatabase value if exists
                if (!empty($this->conf->global->LDAP_FIELD_PASSWORD)) {
                    $info[$this->conf->global->LDAP_FIELD_PASSWORD] = $this->pass_indatabase; // $this->pass_indatabase = mot de passe non crypte
                }
                if (!empty($this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED)) {
                    $info[$this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED] = dol_hash($this->pass_indatabase, 4); // md5 for OpenLdap TODO add type of encryption
                }
            }
        }

        if ($this->conf->global->LDAP_SERVER_TYPE == 'egroupware') {
        ...
        if (!empty($this->conf->global->LDAP_FIELD_USERID)) {
            $info[$this->conf->global->LDAP_FIELD_USERID] = $this->id;
        }
        if (!empty($this->conf->global->LDAP_FIELD_GROUPID)) {
            $usergroup = new UserGroup($this->db);
            $groupslist = $usergroup->listGroupsForUser($this->id);
            $info[$this->conf->global->LDAP_FIELD_GROUPID] = '65534';
            if (!empty($groupslist)) {
                foreach ($groupslist as $groupforuser) {
                    $info[$this->conf->global->LDAP_FIELD_GROUPID] = $groupforuser->id; //Select first group in list
                    break;
                }
            }
        }
        if (!empty($this->conf->global->LDAP_FIELD_HOMEDIRECTORY) && !empty($this->conf->global->LDAP_FIELD_HOMEDIRECTORYPREFIX)) {
            $info[$this->conf->global->LDAP_FIELD_HOMEDIRECTORY] = "{$this->conf->global->LDAP_FIELD_HOMEDIRECTORYPREFIX}/$this->login";
        }
    ...
    public function update_ldap2dolibarr(&$ldapuser)
    {
        ...
        //        global $user, $this->conf;

        $socialnetworks = getArrayOfSocialNetworks();

        $this->firstname = $ldapuser->{$this->conf->global->LDAP_FIELD_FIRSTNAME};
        $this->lastname = $ldapuser->{$this->conf->global->LDAP_FIELD_NAME};
        $this->login = $ldapuser->{$this->conf->global->LDAP_FIELD_LOGIN};
        $this->pass = $ldapuser->{$this->conf->global->LDAP_FIELD_PASSWORD};
        $this->pass_indatabase_crypted = $ldapuser->{$this->conf->global->LDAP_FIELD_PASSWORD_CRYPTED};

        $this->office_phone = $ldapuser->{$this->conf->global->LDAP_FIELD_PHONE};
        $this->user_mobile = $ldapuser->{$this->conf->global->LDAP_FIELD_MOBILE};
        $this->office_fax = $ldapuser->{$this->conf->global->LDAP_FIELD_FAX};
        $this->email = $ldapuser->{$this->conf->global->LDAP_FIELD_MAIL};
        foreach ($socialnetworks as $key => $value) {
            $tmpkey = 'LDAP_FIELD_' . strtoupper($value['label']);
            $this->socialnetworks[$value['label']] = $ldapuser->{$this->conf->global->$tmpkey};
        }
        $this->ldap_sid = $ldapuser->{$this->conf->global->LDAP_FIELD_SID};

        $this->job = $ldapuser->{$this->conf->global->LDAP_FIELD_TITLE};
        $this->note_public = $ldapuser->{$this->conf->global->LDAP_FIELD_DESCRIPTION};
    ...
    {
        //        global $this->conf, $user, $this->langs;

        $this->langs->load("user");

        // Positionne le modele sur le nom du modele a utiliser
        if (!dol_strlen($modele)) {
            if (!empty($this->conf->global->USER_ADDON_PDF)) {
                $modele = $this->conf->global->USER_ADDON_PDF;
            } else {
                $modele = 'bluesky';
            }
        }
    ...
    public function user_get_property($rowid, $mode)
    {
        ...
        if ($resql) {
            ...
            if ($nump) {
                ...
                if ($mode == 'email') {
                    $user_property = DolibarrFunctions::dolGetFirstLastname($obj->firstname, $obj->lastname) . " <" . $obj->email . ">";
                } elseif ($mode == 'mobile') {
    ...
    public function findUserIdByEmail($email)
    {
        ...
        $sql = 'SELECT rowid';
        $sql .= ' FROM ' . MAIN_DB_PREFIX . 'user';
        if (!empty($this->conf->global->AGENDA_DISABLE_EXACT_USER_EMAIL_COMPARE_FOR_EXTERNAL_CALENDAR)) {
