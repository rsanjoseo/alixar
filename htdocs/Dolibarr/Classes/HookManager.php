...
class HookManager
{
     ...
     *  This makes $this->conf->hooks_modules loaded with an entry ('modulename'=>array(nameofcontext1,nameofcontext2,...))
    ...
    public function initHooks($arraycontext)
    {
        ...
        if (!is_array($this->conf->modules_parts['hooks']) || empty($this->conf->modules_parts['hooks'])) {
            return;
        }
        ...
        $arraytolog = [];
        foreach ($this->conf->modules_parts['hooks'] as $module => $hooks) {    // Loop on each module that brings hooks
            if (empty($this->conf->$module->enabled)) {
                continue;
            }
     ...
     */
    public function executeHooks($method, $parameters = [], &$object = '', &$action = '')
    {
