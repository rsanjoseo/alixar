...
class DefaultValues extends CommonObject
{
     ...
     *  'enabled' is a condition when the field must be managed (Example: 1 or '$this->conf->global->MY_SETUP_PARAM)
    ...
    public function __construct()
    {
        ...
        if (empty($this->conf->global->MAIN_SHOW_TECHNICAL_ID) && isset($this->fields['rowid'])) {
            $this->fields['rowid']['visible'] = 0;
        }
        if (empty($this->conf->multicompany->enabled) && isset($this->fields['entity'])) {
            ...
            foreach ($this->fields as $key => $val) {
                if (!empty($val['arrayofkeyval']) && is_array($val['arrayofkeyval'])) {
                    foreach ($val['arrayofkeyval'] as $key2 => $val2) {
                        $this->fields[$key]['arrayofkeyval'][$key2] = $this->langs->trans($val2);
                    }
                }
            }
        }
    ...
    public function fetchAll($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, array $filter = [], $filtermode = 'AND')
    {
        ...
        if (count($filter) > 0) {
            foreach ($filter as $key => $value) {
                ...
                } elseif (isset($this->fields[$key]) && in_array($this->fields[$key]['type'], ['date', 'datetime', 'timestamp'])) {
