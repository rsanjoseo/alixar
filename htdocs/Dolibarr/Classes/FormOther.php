...
class FormOther
{
     ...
     */
    public function getBoxesArea($user, $areacode)
    {
        ...
        $selectboxlist = '';
        $boxactivated = InfoBox::listBoxes($this->db, 'activated', $areacode, (empty($user->conf->confuserzone) ? null : $user), [], 0); // Search boxes of common+user (or common only if user has no specific setup)
        ...
        if (!empty($user->conf->confuserzone)) {
            $boxorder = '';
            $this->langs->load("boxes"); // Load label of boxes
            foreach ($boxactivated as $box) {
                if (!empty($boxidactivatedforuser[$box->id])) {
                    continue; // Already visible for user
                }
                $label = $this->langs->transnoentitiesnoconv($box->boxlabel);
                //if (preg_match('/graph/',$box->class)) $label.=' ('.$this->langs->trans("Graph").')';
                if (preg_match('/graph/', $box->class) && $this->conf->browser->layout != 'phone') {
            ...
            $selectboxlist .= Form::selectarray('boxcombo', $arrayboxtoactivatelabel, -1, $this->langs->trans("ChooseBoxToAdd") . '...', 0, 0, '', 0, 0, 0, 'ASC', 'maxwidth150onsmartphone hideonprint', 0, 'hidden selected', 0, 1);
            if (empty($this->conf->use_javascript_ajax)) {
                $selectboxlist .= ' <input type="submit" class="button" value="' . $this->langs->trans("AddBox") . '">';
            }
            $selectboxlist .= '</form>';
            if (!empty($this->conf->use_javascript_ajax)) {
        ...
        if ($nbboxactivated) {
            ...
            $this->langs->loadLangs(["boxes", "projects"]);
            ...
            $box_max_lines = 5;
            if (!empty($this->conf->global->MAIN_BOXES_MAXLINES)) {
                $box_max_lines = $this->conf->global->MAIN_BOXES_MAXLINES;
            }
    ...
    public function getHTMLScannerForm($jstoexecuteonadd = 'barcodescannerjs')
    {
        ...
        $out .= $this->langs->trans("QtyToAddAfterBarcodeScan") . ' <input type="text" name="barcodeproductqty" class="width50 right" value="1"><br>';
        $out .= '<textarea type="text" name="barcodelist" class="centpercent" autofocus rows="' . ROWS_3 . '"></textarea>';

        /*print '<br>'.$this->langs->trans("or").'<br>';
        ...
        */
        $out .= '<br>';
        $out .= '<center>';
        $out .= '<input type="submit" class="button marginleftonly marginrightonly" id ="exec' . dol_escape_js($jstoexecuteonadd) . '" name="addscan" value="' . $this->langs->trans("Add") . '">';
        $out .= '<input type="submit" class="button marginleftonly marginrightonly" name="cancel" value="' . $this->langs->trans("Cancel") . '">';
        $out .= '<br>';
    ...
    public function select_export_model($selected = '', $htmlname = 'exportmodelid', $type = '', $useempty = 0, $fk_user = null)
    {
        ...
        if (empty($this->conf->global->EXPORTS_SHARE_MODELS)) {    // EXPORTS_SHARE_MODELS means all templates are visible, whatever is owner.
        ...
        if ($result) {
            ...
            while ($i < $num) {
                ...
                $label = $obj->label;
                if ($obj->fk_user == 0) {
                    $label .= ' <span class="opacitymedium">(' . $this->langs->trans("Everybody") . ')</span>';
                } elseif ($obj->fk_user > 0) {
                    $tmpuser->fetch($obj->fk_user);
                    $label .= ' <span class="opacitymedium">(' . $tmpuser->getFullName($this->langs) . ')</span>';
                }
        ...
        if ($resql) {
            ...
            if ($num) {
                while ($i < $num) {
                    ...
                    $selectOptionValue = $obj->code . ' - ' . $obj->label . ' : ' . price($obj->price) . ' ' . $this->langs->trans("HT") . ' (' . $obj->organization . ')';
    ...
    public function select_categories($type, $selected = 0, $htmlname = 'search_categ', $nocateg = 0, $showempty = 1, $morecss = '')
    {
        ...
        if ($nocateg) {
            ...
            $moreforfilter .= '<option value="-2"' . ($selected == -2 ? ' selected' : '') . '>- ' . $this->langs->trans("NotCategorized") . ' -</option>';
        }
    ...
    public function select_salesrepresentatives($selected, $htmlname, $user, $showstatus = 0, $showempty = 1, $morecss = '', $norepresentative = 0)
    {
        ...
        $reshook = $this->hookmanager->executeHooks('addSQLWhereFilterOnSelectSalesRep', [], $this, $action);
        ...
        if (!empty($reshook)) {
            $sql_usr .= $this->hookmanager->resArray[0];
        }
        ...
        if (empty($user->rights->user->user->lire) && $user->socid) {
            ...
            //Add hook to filter on user (for exemple on usergroup define in custom modules)
            if (!empty($reshook)) {
                $sql_usr .= $this->hookmanager->resArray[1];
            }
        }
        ...
        if ($norepresentative) {
            ...
            $out .= '<option value="-2"' . ($selected == -2 ? ' selected' : '') . '>- ' . $this->langs->trans("NoSalesRepresentativeAffected") . ' -</option>';
        }
    ...
    public function selectProjectTasks($selectedtask = '', $projectid = 0, $htmlname = 'task_parent', $modeproject = 0, $modetask = 0, $mode = 0, $useempty = 0, $disablechildoftaskid = 0, $filteronprojstatus = '', $morecss = '')
    {
        ...
        $tasksarray = $task->getTasksArray($modetask ? $this->user : 0, $modeproject ? $this->user : 0, $projectid, 0, $mode, '', $filteronprojstatus);
        if ($tasksarray) {
            ...
            print '<div class="warning">' . $this->langs->trans("NoProject") . '</div>';
        }
    }
            ...
            if ($lines[$i]->fk_parent == $parent) {
                ...
                if ($parent == 0) {    // We are on a task at first level
                    if ($lines[$i]->fk_project != $lastprojectid) {    // Break found on project
                        ...
                        $labeltoshow = $this->langs->trans("Project") . ' ' . $lines[$i]->projectref;
                        if (empty($lines[$i]->public)) {
                            $labeltoshow .= ' <span class="opacitymedium">(' . $this->langs->trans("Visibility") . ': ' . $this->langs->trans("PrivateProject") . ')</span>';
                        } else {
                            $labeltoshow .= ' <span class="opacitymedium">(' . $this->langs->trans("Visibility") . ': ' . $this->langs->trans("SharedProject") . ')</span>';
                        }
     ...
     * @see showColor()
     */
    public function selectColor($set_color = '', $prefix = 'f_color', $form_name = '', $showcolorbox = 1, $arrayofcolors = '', $morecss = '', $setpropertyonselect = '')
    {
        // Deprecation warning
        if ($form_name) {
        ...
        if (!is_array($arrayofcolors) || count($arrayofcolors) < 1) {
            ...
            if (empty($this->conf->dol_use_jmobile) && !empty($this->conf->use_javascript_ajax)) {
                ...
                $out .= '<script type="text/javascript">
	             jQuery(document).ready(function(){
	                $(\'#colorpicker' . $prefix . '\').jPicker( {
		                window: {
		                  title: \'' . dol_escape_js($this->langs->trans("SelectAColor")) . '\', /* any title for the jPicker window itself - displays "Drag Markers To Pick A Color" if left null */
		                  effects:
		                    {
		                ...
		                localization: // alter these to change the text presented by the picker (e.g. different language)
		                  {
		                    text:
		                    {
		                      title: \'' . dol_escape_js($this->langs->trans("SelectAColor")) . '\',
		                      newColor: \'' . dol_escape_js($this->langs->trans("New")) . '\',
		                      currentColor: \'' . dol_escape_js($this->langs->trans("Current")) . '\',
		                      ok: \'' . dol_escape_js($this->langs->trans("Save")) . '\',
		                      cancel: \'' . dol_escape_js($this->langs->trans("Cancel")) . '\'
		                    }
		                  }
				        },
    ...
    public function CreateColorIcon($color, $module, $name, $x = '12', $y = '12')
    {
        ...
        $file = $this->conf->$module->dir_temp . '/' . $name . '.png';

        // On cree le repertoire contenant les icones
        if (!file_exists($this->conf->$module->dir_temp)) {
            dol_mkdir($this->conf->$module->dir_temp);
        }
    ...
    public function select_dayofweek($selected = '', $htmlname = 'weekid', $useempty = 0)
    {
        // phpcs:enable
        $week = [
            0 => $this->langs->trans("Day0"),
            1 => $this->langs->trans("Day1"),
            2 => $this->langs->trans("Day2"),
            3 => $this->langs->trans("Day3"),
            4 => $this->langs->trans("Day4"),
            5 => $this->langs->trans("Day5"),
            6 => $this->langs->trans("Day6"),
        ];
    ...
    public function select_month($selected = '', $htmlname = 'monthid', $useempty = 0, $longlabel = 0, $morecss = 'minwidth50 maxwidth75imp valignmiddle', $addjscombo = false)
    {
        ...
        if ($longlabel) {
            $monthArray = DolibarrDate::monthArray($this->langs, 0); // Get array
        } else {
            $monthArray = DolibarrDate::monthArray($this->langs, 1);
        }
    ...
    public function select_dictionary($htmlname, $dictionarytable, $keyfield = 'code', $labelfield = 'label', $selected = '', $useempty = 0, $moreattrib = '')
    {
        ...
        if ($result) {
            ...
            if ($num) {
                ...
                print $this->langs->trans("DictionaryEmpty");
            }
        } else {
            dol_print_error($this->db);
        }
    ...
    public function selectAutoManual($htmlname, $value = '', $option = 0, $disabled = false, $useempty = 0)
    {
        ...
        if (("$value" == 'automatic') || ($value == 1)) {
            $resultautomanual .= '<option value="' . $automatic . '" selected>' . $this->langs->trans("Automatic") . '</option>' . "\n";
            $resultautomanual .= '<option value="' . $manual . '">' . $this->langs->trans("Manual") . '</option>' . "\n";
        } else {
            $selected = (($useempty && $value != '0' && $value != 'manual') ? '' : ' selected');
            $resultautomanual .= '<option value="' . $automatic . '">' . $this->langs->trans("Automatic") . '</option>' . "\n";
            $resultautomanual .= '<option value="' . $manual . '"' . $selected . '>' . $this->langs->trans("Manual") . '</option>' . "\n";
        }
    ...
    {
        global $extrafields, $form;

        $YYYY = substr($this->langs->trans("Year"), 0, 1) . substr($this->langs->trans("Year"), 0, 1) . substr($this->langs->trans("Year"), 0, 1) . substr($this->langs->trans("Year"), 0, 1);
        $MM = substr($this->langs->trans("Month"), 0, 1) . substr($this->langs->trans("Month"), 0, 1);
        $DD = substr($this->langs->trans("Day"), 0, 1) . substr($this->langs->trans("Day"), 0, 1);
        $HH = substr($this->langs->trans("Hour"), 0, 1) . substr($this->langs->trans("Hour"), 0, 1);
        $MI = substr($this->langs->trans("Minute"), 0, 1) . substr($this->langs->trans("Minute"), 0, 1);
        $SS = substr($this->langs->trans("Second"), 0, 1) . substr($this->langs->trans("Second"), 0, 1);
        ...
        foreach ($object->fields as $key => $val) {
            if (!$val['measure']) {
                ...
                if (in_array($val['type'], ['html', 'text'])) {
                    continue;
                }
                if (in_array($val['type'], ['timestamp', 'date', 'datetime'])) {
                    $arrayofgroupby['t.' . $key . '-year'] = ['label' => $this->langs->trans($val['label']) . ' <span class="opacitymedium">(' . $YYYY . ')</span>', 'position' => $val['position'] . '-y'];
                    $arrayofgroupby['t.' . $key . '-month'] = ['label' => $this->langs->trans($val['label']) . ' <span class="opacitymedium">(' . $YYYY . '-' . $MM . ')</span>', 'position' => $val['position'] . '-m'];
                    $arrayofgroupby['t.' . $key . '-day'] = ['label' => $this->langs->trans($val['label']) . ' <span class="opacitymedium">(' . $YYYY . '-' . $MM . '-' . $DD . ')</span>', 'position' => $val['position'] . '-d'];
                } else {
                    $arrayofgroupby['t.' . $key] = ['label' => $this->langs->trans($val['label']), 'position' => (int) $val['position']];
                }
            }
        }
        ...
        if ($object->isextrafieldmanaged) {
            foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
                ...
                $arrayofgroupby['te.' . $key] = ['label' => $this->langs->trans($extrafields->attributes[$object->table_element]['label'][$key]), 'position' => 1000 + (int) $extrafields->attributes[$object->table_element]['pos'][$key]];
            }
        }
    ...
    public function selectXAxisField($object, $search_xaxis, &$arrayofxaxis)
    {
        ...
        foreach ($object->fields as $key => $val) {
            if (!$val['measure']) {
                ...
                if (in_array($val['type'], ['timestamp', 'date', 'datetime'])) {
                    $arrayofxaxis['t.' . $key . '-year'] = ['label' => $this->langs->trans($val['label']) . ' (' . $YYYY . ')', 'position' => $val['position'] . '-y'];
                    $arrayofxaxis['t.' . $key . '-month'] = ['label' => $this->langs->trans($val['label']) . ' (' . $YYYY . '-' . $MM . ')', 'position' => $val['position'] . '-m'];
                    $arrayofxaxis['t.' . $key . '-day'] = ['label' => $this->langs->trans($val['label']) . ' (' . $YYYY . '-' . $MM . '-' . $DD . ')', 'position' => $val['position'] . '-d'];
                } else {
                    $arrayofxaxis['t.' . $key] = ['label' => $this->langs->trans($val['label']), 'position' => (int) $val['position']];
                }
            }
        }
        ...
        if ($object->isextrafieldmanaged) {
            foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
                ...
                $arrayofxaxis['te.' . $key] = ['label' => $this->langs->trans($extrafields->attributes[$object->table_element]['label'][$key]), 'position' => 1000 + (int) $extrafields->attributes[$object->table_element]['pos'][$key]];
            }
        }
