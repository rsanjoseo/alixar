...
abstract class DolibarrFunctions
{
    /**
     ...
     * @return string
     */
    static public function getDolGlobalString($key)
    {
        global $conf;
     ...
     * @return int
     */
    static public function getDolGlobalInt($key)
    {
        global $conf;
    ...
    static public function getEntity($element, $shared = 1, $currentobject = null)
    {
        ...
        if (isset($mc) && is_object($mc)) { // Multicompany it's a private module
            ...
            $out = '';
            $addzero = ['user', 'usergroup', 'c_email_templates', 'email_template', 'default_values'];
     ...
     */
    static public function setEntity($currentobject)
    {
        global $conf, $mc;
     ...
     * @return    void
     */
    static public function dol_shutdown()
    {
     ...
     */
    static public function GETPOSTISSET($paramname)
    {
        $isset = false;
    ...
    static public function GETPOST($paramname, $check = 'alphanohtml', $method = 0, $filter = null, $options = null, $noreplace = 0)
    {
                                        ...
                                        if ($qualified) {
                                            ...
                                            foreach ($user->default_values[$relativepathstring]['sortorder'][$defkey] as $key => $val) {
                                                ...
                                                if ($paramname == 'sortfield') {
                                                    $out .= self::dol_string_nospecial($key, '', $forbidden_chars_to_replace);
                                                }
                                                if ($paramname == 'sortorder') {
                                                    $out .= self::dol_string_nospecial($val, '', $forbidden_chars_to_replace);
                                                }
                                            }
                                    ...
                                    if ($qualified) {
                                        ...
                                        if (isset($_POST['sall']) || isset($_POST['search_all']) || isset($_GET['sall']) || isset($_GET['search_all'])) {
                                            ...
                                            if (empty($conf->global->MAIN_DISABLE_DEFAULT_FILTER_FOR_QUICK_SEARCH)) {
                                                $forbidden_chars_to_replace = [" ", "'", "/", "\\", ":", "*", "?", "\"", "<", ">", "|", "[", "]", ";", "="]; // we accept _, -, . and ,
                                                $out = self::dol_string_nospecial($user->default_values[$relativepathstring]['filters'][$defkey][$paramname], '', $forbidden_chars_to_replace);
                                            }
                                        } else {
                                            $forbidden_chars_to_replace = [" ", "'", "/", "\\", ":", "*", "?", "\"", "<", ">", "|", "[", "]", ";", "="]; // we accept _, -, . and ,
                                            $out = self::dol_string_nospecial($user->default_values[$relativepathstring]['filters'][$defkey][$paramname], '', $forbidden_chars_to_replace);
        ...
        if (preg_match('/^array/', $check)) {    // If 'array' or 'array:restricthtml' or 'array:aZ09'
            ...
            $out = self::checkVal($out, $check, $filter, $options);
        }
        ...
        if ($paramname == 'backtopage' || $paramname == 'backtolist' || $paramname == 'backtourl') {
            ...
            $out = preg_replace(['/^[a-z]*\/\/+/i'], '', $out);    // We remove schema*// to remove external URL
        }
     ...
     *
     * @see            dol_sanitizeFilename(), self::dol_string_unaccent(), self::dol_string_nounprintableascii()
     */
    static public function dol_string_nospecial($str, $newstr = '_', $badcharstoreplace = '', $badcharstoremove = '')
    {
     ...
     */
    static public function checkVal($out = '', $check = 'alphanohtml', $filter = null, $options = null)
    {
        global $conf;
        ...
        switch ($check) {
            ...
            case 'alphawithlgt':        // No " and no ../ but we keep balanced < > tags with no special chars inside. Can be used for email string like "Name <email>"
                if (!is_array($out)) {
                    ...
                    do {
                        $oldstringtoclean = $out;
                        // Remove html tags
                        $out = self::dol_html_entity_decode($out, ENT_COMPAT | ENT_HTML5, 'UTF-8');
                        // '"' is dangerous because param in url can close the href= or src= and add javascript functions.
                        // '../' or '..\' is dangerous because it allows dir transversals
                        // Note &#38, '&#0000038', '&#x26'... is a simple char like '&' alone but there is no reason to accept such way to encode input data.
                        $out = str_ireplace(['&#38', '&#0000038', '&#x26', '&quot', '&#34', '&#0000034', '&#x22', '"', '&#47', '&#0000047', '&#92', '&#0000092', '&#x2F', '../', '..\\'], '', $out);
            ...
            case 'restricthtmlallowunvalid':
                do {
                    ...
                    $out = preg_replace_callback('/&#(x?[0-9][0-9a-f]+;?)/i', [DolibarrView::class, 'realCharForNumericEntities'], $out);
                    ...
                    $out = self::dol_string_onlythesehtmltags($out, 0, 1, 1);
                    ...
                    if (!empty($conf->global->MAIN_RESTRICTHTML_REMOVE_ALSO_BAD_ATTRIBUTES)) {
                        ...
                        $out = trim(self::dol_string_onlythesehtmlattributes($out));
                    }
    ...
    static public function dol_string_onlythesehtmltags($stringtoclean, $cleanalsosomestyles = 1, $removeclassattribute = 1, $cleanalsojavascript = 0, $allowiframe = 0)
    {
        ...
        $stringtoclean = self::dol_string_nounprintableascii($stringtoclean, 0);
     ...
     * @return string                    Cleaned string
     *
     * @see            dol_sanitizeFilename(), self::dol_string_unaccent(), self::dol_string_nospecial()
     */
    static public function dol_string_nounprintableascii($str, $removetabcrlf = 1)
    {
        if ($removetabcrlf) {
     ...
     *  Note. Not as secured as self::dol_string_onlythesehtmltags().
     *
     * @param string $stringtoclean      String to clean
     ...
     */
    static public function GETPOSTINT($paramname, $method = 0)
    {
     ...
     */
    static public function dol_getprefix($mode = '')
    {
        ...
        if ($mode == 'email') {
            ...
            return DolibarrSecurity::dol_hash(DOL_DOCUMENT_ROOT . DOL_URL_ROOT, '3');
        }
        ...
        if (isset($_SERVER["SERVER_NAME"]) && isset($_SERVER["DOCUMENT_ROOT"])) {
            return DolibarrSecurity::dol_hash($_SERVER["SERVER_NAME"] . $_SERVER["DOCUMENT_ROOT"] . DOL_DOCUMENT_ROOT . DOL_URL_ROOT, '3');
        }
     ...
     */
    static public function dol_include_once($relpath, $classname = '')
    {
        ...
        $fullpath = self::dol_buildpath($relpath);

        if (!file_exists($fullpath)) {
            self::dol_syslog('functions::dol_include_once Tried to load unexisting file: ' . $relpath, LOG_WARNING);
            return false;
        }
     ...
     */
    static public function dol_clone($object, $native = 0)
    {
        if (empty($native)) {
     ...
     */
    static public function dol_size($size, $type = '')
    {
        global $conf;
     ...
     *
     * @see            self::dol_string_nospecial(), self::dol_string_unaccent(), dol_sanitizePathName()
     */
    static public function dol_sanitizeFileName($str, $newstr = '_', $unaccent = 1)
    {
        ...
        $tmp = self::dol_string_nospecial($unaccent ? self::dol_string_unaccent($str) : $str, $newstr, $filesystem_forbidden_chars);
     ...
     * @return string                Cleaned string
     *
     * @see            dol_sanitizeFilename(), self::dol_string_nospecial()
     */
     ...
     * @return string                String cleaned (a-zA-Z_)
     *
     * @see            self::dol_string_nospecial(), self::dol_string_unaccent(), dol_sanitizeFileName()
     */
    static public function dol_sanitizePathName($str, $newstr = '_', $unaccent = 1)
    {
        // List of special chars for filenames in windows are defined on page https://docs.microsoft.com/en-us/windows/win32/fileio/naming-a-file
        // Char '>' '<' '|' '$' and ';' are special chars for shells.
        // Chars '--' can be used into filename to inject special paramaters like --use-compress-program to make command with file as parameter making remote execution of command
     ...
     * @return     string                            Escaped string.
     */
    static public function dol_sanitizeUrl($stringtoclean, $type = 1)
    {
        ...
        // We should use self::dol_string_nounprintableascii but function may not be yet loaded/available
     ...
     */
    static public function dol_escape_json($stringtoescape)
    {
     ...
     */
    static public function dol_strtolower($string, $encoding = "UTF-8")
    {
     ...
     * @return    string                            String converted
     */
    static public function dol_strtoupper($string, $encoding = "UTF-8")
    {
     ...
     * @return  string                      String converted
     */
    static public function dol_ucfirst($string, $encoding = "UTF-8")
    {
        if (function_exists('mb_substr')) {
     ...
     * @return  string                      String converted
     */
    static public function dol_ucwords($string, $encoding = "UTF-8")
    {
     ...
     */
    static public function dolButtonToOpenUrlInDialogPopup($name, $label, $buttonstring, $url, $disabled = '')
    {
        if (strpos($url, '?') > 0) {
        ...
        //print '<input type="submit" class="button bordertransp"'.$disabled.' value="'.DolibarrFunctions::dol_escape_htmltag($langs->trans("MediaFiles")).'" name="file_manager">';
        $out = '<a class="button bordertransp button_' . $name . '"' . $disabled . ' title="' . DolibarrFunctions::dol_escape_htmltag($label) . '">' . $buttonstring . '</a>';
        $out .= '<!-- Add js code to open dialog popup on dialog -->';
        $out .= '<script language="javascript">
				jQuery(document).ready(function () {
					jQuery(".button_' . $name . '").click(function () {
						console.log("Open popup with jQuery(...).dialog() on URL ' . DolibarrFunctions::dol_escape_js(DOL_URL_ROOT . $url) . '")
						var $dialog = $(\'<div></div>\').html(\'<iframe class="iframedialog" style="border: 0px;" src="' . DOL_URL_ROOT . $url . '" width="100%" height="98%"></iframe>\')
							.dialog({
								autoOpen: false,
							 	...
							 	width: \'80%\',
							 	title: "' . DolibarrFunctions::dol_escape_js($label) . '"
							});
						$dialog.dialog(\'open\');
					});
				});
     ...
     */
    static public function dol_escape_js($stringtoescape, $mode = 0, $noescapebackslashn = 0)
    {
        // escape quotes and backslashes, newlines, etc.
        $substitjs = ["&#039;" => "\\'", "\r" => '\\r'];
     ...
     */
    static public function dol_fiche_head($links = [], $active = '0', $title = '', $notab = 0, $picto = '', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limittoshow = 0, $moretabssuffix = '')
    {
     ...
     * @see        img_object(), DolibarrFunctions::img_picto_common()
     */
    static public function img_picto($titlealt, $picto, $moreatt = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0, $alt = '', $morecss = '', $marginleftonlyshort = 2)
    {
        global $conf, $langs;
        ...
        if ($pictoisfullpath) {
            ...
            if (empty($srconly) && in_array($pictowithouttext, [
                ...
                $fasize = '';
                $fa = 'fas';
                if (in_array($pictowithouttext, ['clock', 'establishment', 'generic', 'minus-square', 'object_generic', 'pdf', 'plus-square', 'timespent', 'note', 'off', 'on', 'object_bookmark', 'bookmark', 'vcard'])) {
                    $fa = 'far';
                }
                if (in_array($pictowithouttext, ['black-tie', 'github', 'skype', 'twitter', 'facebook', 'linkedin', 'instagram', 'snapchat', 'stripe', 'stripe-s', 'youtube', 'google-plus-g', 'whatsapp'])) {
                    $fa = 'fab';
                }
                ...
                $enabledisablehtml .= ($morecss ? ' ' . $morecss : '') . '" style="' . ($fasize ? ('font-size: ' . $fasize . ';') : '') . ($facolor ? (' color: ' . $facolor . ';') : '') . ($morestyle ? ' ' . $morestyle : '') . '"' . (($notitle || empty($titlealt)) ? '' : ' title="' . DolibarrFunctions::dol_escape_htmltag($titlealt) . '"') . ($moreatt ? ' ' . $moreatt : '') . '>';
        ...
        return '<img src="' . $fullpathpicto . '"' . ($notitle ? '' : ' alt="' . DolibarrFunctions::dol_escape_htmltag($alt) . '"') . (($notitle || empty($titlealt)) ? '' : ' title="' . DolibarrFunctions::dol_escape_htmltag($titlealt) . '"') . ($moreatt ? ' ' . $moreatt . ($morecss ? ' class="' . $morecss . '"' : '') : ' class="inline-block' . ($morecss ? ' ' . $morecss : '') . '"') . '>'; // Alt is used for accessibility, title for popup
    }
     ...
     */
    static public function dol_fiche_end($notab = 0)
    {
     ...
     * @return    string
     */
    static public function dol_get_fiche_end($notab = 0)
    {
        if (!$notab || $notab == -1) {
            return "\n</div>\n";
        } else {
     ...
     * @return    void
     */
    static public function dol_banner_tab($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '', $nodbprefix = 0, $morehtmlleft = '', $morehtmlstatus = '', $onlybanner = 0, $morehtmlright = '')
    {
        ...
        if ($object->element == 'product') {
            ...
            if (!$showimage) {
                if (!empty($conf->global->TICKET_NODISPLAYIFNOPHOTO)) {
                    ...
                    $nophoto = DolibarrFunctions::img_picto('No photo', 'object_ticket');
            ...
            if ($showimage) {
                ...
                if (empty($phototoshow)) {      // Show No photo link (picto of object)
                    ...
                    if ($object->element == 'action') {
                        ...
                        $nophoto = DolibarrFunctions::img_picto('No photo', 'title_agenda');
                    } else {
                        $width = 14;
                        $cssclass = 'photorefcenter';
                        $picto = $object->picto;
                        ...
                        $nophoto = DolibarrFunctions::img_picto('No photo', 'object_' . $picto);
                    }
                    $morehtmlleft .= '<!-- No photo to show -->';
                    $morehtmlleft .= '<div class="floatleft inline-block valignmiddle divphotoref"><div class="photoref">';
                    $morehtmlleft .= $nophoto;
        ...
        } elseif (in_array($object->element, ['facture', 'invoice', 'invoice_supplier', 'chargesociales', 'loan', 'tva', 'salary'])) {
        ...
        if (!empty($conf->accounting->enabled) && in_array($object->element, ['bank', 'paiementcharge', 'facture', 'invoice', 'invoice_supplier', 'expensereport', 'payment_various'])) {
     ...
     */
    static public function dol_bc($var, $moreclass = '')
    {
        global $bc;
        $ret = ' ' . $bc[$var];
        if ($moreclass) {
    ...
    static public function dol_format_address($object, $withcountry = 0, $sep = "\n", $outputlangs = '', $mode = 0, $extralangcode = '')
    {
        ...
        // Zip/Town/State
        if (isset($object->country_code) && in_array($object->country_code, ['AU', 'CA', 'US', 'CN']) || !empty($conf->global->MAIN_FORCE_STATE_INTO_ADDRESS)) {
     ...
     */
    static public function dol_strftime($fmt, $ts = false, $is_gmt = false)
    {
     ...
     * @return    string                Link
     */
    static public function dol_print_size($size, $shortvalue = 0, $shortunit = 0)
    {
        global $conf, $langs;
        $level = 1024;
     ...
     * @return    string                        HTML Link
     */
    static public function dol_print_email($email, $cid = 0, $socid = 0, $addlink = 0, $max = 64, $showinvalid = 1, $withpicto = 0)
    {
        global $conf, $user, $langs, $hookmanager;

        $newemail = DolibarrFunctions::dol_escape_htmltag($email);
        ...
        $rep = ($withpicto ? DolibarrFunctions::img_picto($langs->trans("EMail") . ' : ' . $email, 'object_email.png') . ' ' : '') . $newemail;
        //$rep .= '</div>';
        if ($hookmanager) {
     ...
     */
    static public function getArrayOfSocialNetworks()
    {
        global $conf, $db;
     ...
     * @return    string                            HTML Link
     */
    static public function dol_print_socialnetworks($value, $cid, $socid, $type, $dictsocialnetworks = [])
    {
     ...
     */
    static public function dol_print_phone($phone, $countrycode = '', $cid = 0, $socid = 0, $addlink = '', $separ = "&nbsp;", $withpicto = '', $titlealt = '', $adddivfloat = 0)
    {
        ...
        if (strtoupper($countrycode) == "FR") {
            // France
            if (DolibarrFunctions::dol_strlen($phone) == 10) {
                $newphone = substr($newphone, 0, 2) . $separ . substr($newphone, 2, 2) . $separ . substr($newphone, 4, 2) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 7) {
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 2) . $separ . substr($newphone, 5, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 9) {
                $newphone = substr($newphone, 0, 2) . $separ . substr($newphone, 2, 3) . $separ . substr($newphone, 5, 2) . $separ . substr($newphone, 7, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 11) {
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 2) . $separ . substr($newphone, 5, 2) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 12) {
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex :  +90 ABC_DEF_GHIJ
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +1 ABC_DEF_GHIJ
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +52 ABCD_EFG_HI
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 4) . $separ . substr($newphone, 7, 3) . $separ . substr($newphone, 10, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 11) {//ex: +52 AB_CD_EF_GH
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 2) . $separ . substr($newphone, 5, 2) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +52 ABC_DEF_GHIJ
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 3) . $separ . substr($newphone, 6, 3) . $separ . substr($newphone, 9, 4);
            }
        } elseif (strtoupper($countrycode) == "ML") {//Mali
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +223 AB_CD_EF_GH
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 2) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 2);
            }
        } elseif (strtoupper($countrycode) == "TH") {//Thaïlande
            if (DolibarrFunctions::dol_strlen($phone) == 11) {//ex: +66_ABC_DE_FGH
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 3) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 3);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +66_A_BCD_EF_GHI
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 1) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 3);
            }
        } elseif (strtoupper($countrycode) == "MU") {
            //Maurice
            if (DolibarrFunctions::dol_strlen($phone) == 11) {//ex: +230_ABC_DE_FG
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +230_ABCD_EF_GH
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 4) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 2);
            }
        } elseif (strtoupper($countrycode) == "ZA") {//Afrique du sud
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +27_AB_CDE_FG_HI
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 2) . $separ . substr($newphone, 5, 3) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 2);
            }
        } elseif (strtoupper($countrycode) == "SY") {//Syrie
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +963_AB_CD_EF_GH
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 2) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +963_AB_CD_EF_GHI
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 2) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 3);
            }
        } elseif (strtoupper($countrycode) == "AE") {//Emirats Arabes Unis
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +971_ABC_DEF_GH
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 3) . $separ . substr($newphone, 10, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +971_ABC_DEF_GHI
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 3) . $separ . substr($newphone, 10, 3);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 14) {//ex: +971_ABC_DEF_GHIK
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +213_ABC_DEF_GHI
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 3) . $separ . substr($newphone, 10, 3);
            }
        } elseif (strtoupper($countrycode) == "BE") {//Belgique
            if (DolibarrFunctions::dol_strlen($phone) == 11) {//ex: +32_ABC_DE_FGH
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 3) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 3);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +32_ABC_DEF_GHI
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 3) . $separ . substr($newphone, 6, 3) . $separ . substr($newphone, 9, 3);
            }
        } elseif (strtoupper($countrycode) == "PF") {//Polynésie française
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +689_AB_CD_EF_GH
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 2) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 2);
            }
        } elseif (strtoupper($countrycode) == "CO") {//Colombie
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +57_ABC_DEF_GH_IJ
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +962_A_BCD_EF_GH
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 1) . $separ . substr($newphone, 5, 3) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2);
            }
        } elseif (strtoupper($countrycode) == "JM") {//Jamaïque
            if (DolibarrFunctions::dol_strlen($newphone) == 12) {//ex: +1867_ABC_DEFG
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +261_AB_CD_EF_GHI
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 2) . $separ . substr($newphone, 6, 2) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 3);
            }
        } elseif (strtoupper($countrycode) == "GB") {//Royaume uni
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +44_ABCD_EFG_HIJ
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +41_AB_CDE_FG_HI
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 2) . $separ . substr($newphone, 5, 3) . $separ . substr($newphone, 8, 2) . $separ . substr($newphone, 10, 2);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 15) {// +41_AB_CDE_FGH_IJKL
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +216_AB_CDE_FGH
            ...
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +594_ABC_DE_FG_HI  (ABC=594 de nouveau)
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2) . $separ . substr($newphone, 11, 2);
            }
        } elseif (strtoupper($countrycode) == "GP") {//Guadeloupe
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +590_ABC_DE_FG_HI  (ABC=590 de nouveau)
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2) . $separ . substr($newphone, 11, 2);
            }
        } elseif (strtoupper($countrycode) == "MQ") {//Martinique
            if (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +596_ABC_DE_FG_HI  (ABC=596 de nouveau)
                $newphone = substr($newphone, 0, 4) . $separ . substr($newphone, 4, 3) . $separ . substr($newphone, 7, 2) . $separ . substr($newphone, 9, 2) . $separ . substr($newphone, 11, 2);
            }
        } elseif (strtoupper($countrycode) == "IT") {//Italie
            if (DolibarrFunctions::dol_strlen($phone) == 12) {//ex: +39_ABC_DEF_GHI
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 3) . $separ . substr($newphone, 6, 3) . $separ . substr($newphone, 9, 3);
            } elseif (DolibarrFunctions::dol_strlen($phone) == 13) {//ex: +39_ABC_DEF_GH_IJ
                $newphone = substr($newphone, 0, 3) . $separ . substr($newphone, 3, 3) . $separ . substr($newphone, 6, 3) . $separ . substr($newphone, 9, 2) . $separ . substr($newphone, 11, 2);
            }
        } elseif (strtoupper($countrycode) == "AU") {
            //Australie
            if (DolibarrFunctions::dol_strlen($phone) == 12) {
                //ex: +61_A_BCDE_FGHI
        ...
        if (empty($reshook)) {
            ...
            $rep .= ($withpicto ? DolibarrFunctions::img_picto($titlealt, 'object_' . $picto . '.png') . ' ' : '') . $newphone;
            if ($adddivfloat) {
                $rep .= '</div>';
            } else {
                $rep .= '</span>';
     ...
     */
    static public function dol_print_ip($ip, $mode = 0)
    {
        global $conf, $langs;
        ...
        if ($mode != 2) {
            ...
            if ($countrycode) {    // If success, countrycode is us, fr, ...
                if (file_exists(DOL_DOCUMENT_ROOT . '/theme/common/flags/' . $countrycode . '.png')) {
                    $ret .= ' ' . DolibarrFunctions::img_picto($countrycode . ' ' . $langs->trans("AccordingToGeoIPDatabase"), DOL_URL_ROOT . '/theme/common/flags/' . $countrycode . '.png', '', 1);
                } else {
                    $ret .= ' (' . $countrycode . ')';
                }
            } else {
     ...
     */
    static public function dolGetCountryCodeFromIp($ip)
    {
        global $conf;
     ...
     */
    static public function dol_user_country()
    {
     ...
     */
    static public function dol_print_address($address, $htmlid, $element, $id, $noprint = 0, $charfornl = '')
    {
        global $conf, $user, $langs, $hookmanager;
     ...
     * @see isValidMXRecord()
     */
    static public function isValidEmail($address, $acceptsupervisorkey = 0)
    {
     ...
     * @see isValidEmail()
     */
    static public function isValidMXRecord($domain)
    {
     ...
     *
     * @return    string                        Return img tag
     * @see    DolibarrFunctions::img_picto(), DolibarrFunctions::img_picto_common()
     */
    static public function img_object($titlealt, $picto, $moreatt = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0)
    {
        if (strpos($picto, '^') === 0) {
            return self::img_picto($titlealt, str_replace('^', '', $picto), $moreatt, $pictoisfullpath, $srconly, $notitle);
        } else {
            return self::img_picto($titlealt, 'object_' . $picto, $moreatt, $pictoisfullpath, $srconly, $notitle);
        }
    }
     ...
     *
     * @return     string                        Return img tag
     * @see        img_object(), DolibarrFunctions::img_picto()
     */
    static public function img_weather($titlealt, $picto, $moreatt = '', $pictoisfullpath = 0, $morecss = '')
    {
        ...
        return self::img_picto($titlealt, $path, $moreatt, 1, 0, 0, '', $morecss);
    }
    ...
    static public function img_action($titlealt, $numaction, $picto = '')
    {
        ...
        return self::img_picto($titlealt, !empty($picto) ? $picto : 'stcomm' . $numaction . '.png');
    }
    ...
    static public function img_pdf($titlealt = 'default', $size = 3)
    {
        ...
        return self::img_picto($titlealt, 'pdf' . $size . '.png');
    }
    ...
    static public function img_edit_add($titlealt = 'default', $other = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'edit_add.png', $other);
    }
    ...
    static public function img_edit_remove($titlealt = 'default', $other = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'edit_remove.png', $other);
    }
    ...
    static public function img_edit($titlealt = 'default', $float = 0, $other = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'edit.png', ($float ? 'style="float: ' . ($langs->tab_translate["DIRECTION"] == 'rtl' ? 'left' : 'right') . '"' : "") . ($other ? ' ' . $other : ''));
    }
    ...
    static public function img_view($titlealt = 'default', $float = 0, $other = 'class="valignmiddle"')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'view.png', $moreatt);
    }
    ...
    static public function img_delete($titlealt = 'default', $other = 'class="pictodelete"', $morecss = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'delete.png', $other, false, 0, 0, '', $morecss);
    }
    ...
    static public function img_printer($titlealt = "default", $other = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'printer.png', $other);
    }
    ...
    static public function img_split($titlealt = 'default', $other = 'class="pictosplit"')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'split.png', $other);
    }
    ...
    static public function img_help($usehelpcursor = 1, $usealttitle = 1)
    {
        ...
        if ($usealttitle) {
            if (is_string($usealttitle)) {
                $usealttitle = DolibarrFunctions::dol_escape_htmltag($usealttitle);
            } else {
                $usealttitle = $langs->trans('Info');
            }
        }

        return DolibarrFunctions::img_picto($usealttitle, 'info.png', 'style="vertical-align: middle;' . ($usehelpcursor == 1 ? ' cursor: help' : ($usehelpcursor == 2 ? ' cursor: pointer' : '')) . '"');
    }
    ...
    static public function img_info($titlealt = 'default')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'info.png', 'style="vertical-align: middle;"');
    }
    ...
    static public function img_warning($titlealt = 'default', $moreatt = '', $morecss = 'pictowarning')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'warning.png', 'class="' . $morecss . '"' . ($moreatt ? ($moreatt == '1' ? ' style="float: right"' : ' ' . $moreatt) : ''));
    }
    ...
    static public function img_error($titlealt = 'default')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, 'error.png');
    }
    ...
    static public function img_next($titlealt = 'default', $moreatt = '')
    {
        ...
        //return DolibarrFunctions::img_picto($titlealt, 'next.png', $moreatt);
        return '<span class="fa fa-chevron-right paddingright paddingleft" title="' . DolibarrFunctions::dol_escape_htmltag($titlealt) . '"></span>';
    }
    ...
    static public function img_left($titlealt = 'default', $selected = 0, $moreatt = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, ($selected ? '1leftarrow_selected.png' : '1leftarrow.png'), $moreatt);
    }
    ...
    static public function img_right($titlealt = 'default', $selected = 0, $moreatt = '')
    {
        ...
        return DolibarrFunctions::img_picto($titlealt, ($selected ? '1rightarrow_selected.png' : '1rightarrow.png'), $moreatt);
    }
    ...
    static public function img_allow($allow, $titlealt = 'default')
    {
        ...
        if ($allow == 1) {
            return DolibarrFunctions::img_picto($titlealt, 'tick.png');
        }
     ...
     * @return string                Return img tag
     */
    static public function img_credit_card($brand, $morecss = null)
    {
        if (is_null($morecss)) {
            $morecss = 'fa-2x';
        }
     ...
     * @return string                Return img tag
     */
    static public function img_mime($file, $titlealt = '', $morecss = '')
    {
        ...
        //return DolibarrFunctions::img_picto_common($titlealt, 'mime/'.$mimeimg, 'class="'.$morecss.'"');
     ...
     * @return string            Retourne tag img
     */
    static public function img_search($titlealt = 'default', $other = '')
    {
        global $conf, $langs;
        ...
        $img = DolibarrFunctions::img_picto($titlealt, 'search.png', $other, false, 1);

        $input = '<input type="image" class="liste_titre" name="button_search" src="' . $img . '" ';
        $input .= 'value="' . DolibarrFunctions::dol_escape_htmltag($titlealt) . '" title="' . DolibarrFunctions::dol_escape_htmltag($titlealt) . '" >';
     ...
     * @return string            Retourne tag img
     */
    static public function img_searchclear($titlealt = 'default', $other = '')
    {
        global $conf, $langs;
        ...
        $img = DolibarrFunctions::img_picto($titlealt, 'searchclear.png', $other, false, 1);
    ...
    static public function info_admin($text, $infoonimgalt = 0, $nodiv = 0, $admin = '1', $morecss = '', $textfordropdown = '')
    {
        ...
        if ($infoonimgalt) {
            ...
            $result = ($nodiv ? '' : '<div class="' . $class . ' hideonsmartphone' . ($morecss ? ' ' . $morecss : '') . ($textfordropdown ? ' hidden' : '') . '">') . '<span class="fa fa-info-circle" title="' . DolibarrFunctions::dol_escape_htmltag($admin ? $langs->trans('InfoAdmin') : $langs->trans('Note')) . '"></span> ' . $text . ($nodiv ? '' : '</div>');

            if ($textfordropdown) {
                $tmpresult .= '<span class="' . $class . 'text opacitymedium cursorpointer">' . $langs->trans($textfordropdown) . ' ' . DolibarrFunctions::img_picto($langs->trans($textfordropdown), '1downarrow') . '</span>';
     ...
     */
    static public function dol_print_error($db = '', $error = '', $errors = null)
    {
        ...
        if (is_object($db)) {
            if ($_SERVER['DOCUMENT_ROOT']) {  // Mode web
                ...
                $out .= "<b>" . $langs->trans("RequestLastAccessInError") . ":</b> " . ($db->lastqueryerror() ? DolibarrFunctions::dol_escape_htmltag($db->lastqueryerror()) : $langs->trans("ErrorNoRequestInError")) . "<br>\n";
                $out .= "<b>" . $langs->trans("ReturnCodeLastAccessInError") . ":</b> " . ($db->lasterrno() ? DolibarrFunctions::dol_escape_htmltag($db->lasterrno()) : $langs->trans("ErrorNoRequestInError")) . "<br>\n";
                $out .= "<b>" . $langs->trans("InformationLastAccessInError") . ":</b> " . ($db->lasterror() ? DolibarrFunctions::dol_escape_htmltag($db->lasterror()) : $langs->trans("ErrorNoRequestInError")) . "<br>\n";
                $out .= "<br>\n";
            } else // Mode CLI
            {
                // No DolibarrFunctions::dol_escape_htmltag for output, we are in CLI mode
        ...
        if ($error || $errors) {
            ...
            foreach ($errors as $msg) {
                ...
                if ($_SERVER['DOCUMENT_ROOT']) {  // Mode web
                    $out .= "<b>" . $langs->trans("Message") . ":</b> " . DolibarrFunctions::dol_escape_htmltag($msg) . "<br>\n";
                } else // Mode CLI
                {
     ...
     * @return    void
     */
    static public function dol_print_error_email($prefixcode, $errormessage = '', $errormessages = [], $morecss = 'error', $email = '')
    {
        global $langs, $conf;
    ...
    static public function getTitleFieldOfList($name, $thead = 0, $file = "", $field = "", $begin = "", $moreparam = "", $moreattrib = "", $sortfield = "", $sortorder = "", $prefix = "", $disablesortlink = 0, $tooltip = '', $forcenowrapcolumntitle = 0)
    {
        ...
        //$out .= (($field && empty($conf->global->MAIN_DISABLE_WRAPPING_ON_COLUMN_TITLE) && preg_match('/^[a-zA-Z_0-9\s\.\-:&;]*$/', $name)) ? ' title="'.DolibarrFunctions::dol_escape_htmltag($langs->trans($name)).'"' : '');
        $out .= ($name && empty($conf->global->MAIN_DISABLE_WRAPPING_ON_COLUMN_TITLE) && empty($forcenowrapcolumntitle) && !self::dol_textishtml($name)) ? ' title="' . DolibarrFunctions::dol_escape_htmltag($langs->trans($name)) . '"' : '';
        $out .= '>';
        ...
        if (empty($thead) && $field && empty($disablesortlink)) {    // If this is a sort field
            ...
            //$out .= (empty($conf->global->MAIN_DISABLE_WRAPPING_ON_COLUMN_TITLE) ? ' title="'.DolibarrFunctions::dol_escape_htmltag($langs->trans($name)).'"' : '');
            $out .= '>';
        }
     ...
     */
    static public function print_fiche_titre($title, $mesg = '', $picto = 'generic', $pictoisfullpath = 0, $id = '')
    {
     ...
     */
    static public function load_fiche_titre($titre, $morehtmlright = '', $picto = 'generic', $pictoisfullpath = 0, $id = '', $morecssontable = '', $morehtmlcenter = '')
    {
        global $conf;
        ...
        if ($picto) {
            $return .= '<td class="nobordernopadding widthpictotitle valignmiddle col-picto">' . DolibarrFunctions::img_picto('', $picto, 'class="valignmiddle widthpictotitle pictotitle"', $pictoisfullpath) . '</td>';
        }
        $return .= '<td class="nobordernopadding valignmiddle col-title">';
        $return .= '<div class="titre inline-block">' . $titre . '</div>';
        $return .= '</td>';
        if (DolibarrFunctions::dol_strlen($morehtmlcenter)) {
            $return .= '<td class="nobordernopadding center valignmiddle">' . $morehtmlcenter . '</td>';
        }
        if (DolibarrFunctions::dol_strlen($morehtmlright)) {
     ...
     * @return    void
     */
    static public function print_barre_liste($titre, $page, $file, $options = '', $sortfield = '', $sortorder = '', $morehtmlcenter = '', $num = -1, $totalnboflines = '', $picto = 'generic', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limit = -1, $hideselectlimit = 0, $hidenavigation = 0, $pagenavastextinput = 0, $morehtmlrightbeforearrow = '')
    {
        global $conf, $langs;
        ...
        if ($picto && $titre) {
            print '<td class="nobordernopadding widthpictotitle valignmiddle col-picto">' . DolibarrFunctions::img_picto('', $picto, 'class="valignmiddle pictotitle widthpictotitle"', $pictoisfullpath) . '</td>';
        }
     ...
     * @return    void
     */
    static public function print_fleche_navigation($page, $file, $options = '', $nextpage = 0, $betweenarrows = '', $afterarrows = '', $limit = -1, $totalnboflines = 0, $hideselectlimit = 0, $beforearrows = '')
    {
        global $conf, $langs;
        ...
        if ((int) $limit > 0 && empty($hideselectlimit)) {
            ...
            print '<select class="flat selectlimit" name="limit" title="' . DolibarrFunctions::dol_escape_htmltag($langs->trans("MaxNbOfRecordPerPage")) . '">';
            ...
            foreach ($tmpchoice as $val) {
                ...
                if ($key != '' && $val != '') {
                    ...
                    print '<option name="' . $key . '"' . $selected . '>' . DolibarrFunctions::dol_escape_htmltag($val) . '</option>' . "\n";
                }
            }
            print '</select>';
        ...
        if ($page > 0) {
            print '<li class="pagination paginationpage paginationpageleft"><a class="paginationprevious" href="' . $file . '?page=' . ($page - 1) . $options . '"><i class="fa fa-chevron-left" title="' . DolibarrFunctions::dol_escape_htmltag($langs->trans("Previous")) . '"></i></a></li>';
        }
        if ($betweenarrows) {
        ...
        if ($nextpage > 0) {
            print '<li class="pagination paginationpage paginationpageright"><a class="paginationnext" href="' . $file . '?page=' . ($page + 1) . $options . '"><i class="fa fa-chevron-right" title="' . DolibarrFunctions::dol_escape_htmltag($langs->trans("Next")) . '"></i></a></li>';
        }
        if ($afterarrows) {
     ...
     */
    static public function vatrate($rate, $addpercent = false, $info_bits = 0, $usestarfornpr = 0)
    {
        $morelabel = '';
     ...
     */
    static public function price($amount, $form = 0, $outlangs = '', $trunc = 1, $rounding = -1, $forcerounding = -1, $currency_code = '')
    {
        global $langs, $conf;
        ...
        if (DolibarrFunctions::dol_strlen($decpart) > $nbdecimal) {
            $nbdecimal = DolibarrFunctions::dol_strlen($decpart);
        }
        // Si on depasse max
        ...
        if ($currency_code) {
            ...
            $listofcurrenciesbefore = ['AUD', 'CAD', 'CNY', 'COP', 'CLP', 'GBP', 'HKD', 'MXN', 'PEN', 'USD'];
    ...
    static public function price2num($amount, $rounding = '', $option = 0)
    {
        ...
        if ($option != 1) {    // If not a PHP number or unknown, we change or clean format
            ...
            if (is_numeric($amount)) {
                ...
                $nbofdec = max(0, DolibarrFunctions::dol_strlen($temps) - 2); // -2 to remove "0."
        ...
        if ($rounding) {
            ...
            if (DolibarrFunctions::dol_strlen($nbofdectoround)) {
     ...
     */
    static public function showDimensionInBestUnit($dimension, $unit, $type, $outputlangs, $round = -1, $forceunitoutput = 'no')
    {
     ...
     * @see get_default_tva()
     */
    static public function get_localtax($vatrate, $local, $thirdparty_buyer = "", $thirdparty_seller = "", $vatnpr = 0)
    {
     ...
     */
    static public function getTaxesFromId($vatrate, $buyer = null, $seller = null, $firstparamisid = 1)
    {
        global $db, $mysoc;
     ...
     * @see getTaxesFromId()
     */
    static public function getLocalTaxesFromRate($vatrate, $local, $buyer, $seller, $firstparamisid = 0)
    {
        global $db, $mysoc;
     ...
     */
    static public function get_product_vat_for_country($idprod, $thirdpartytouse, $idprodfournprice = 0)
    {
        global $db, $conf, $mysoc;
     ...
     */
    static public function get_product_localtax_for_country($idprod, $local, $thirdpartytouse)
    {
        global $db, $mysoc;
     ...
     */
    static public function get_default_tva(Societe $thirdparty_seller, Societe $thirdparty_buyer, $idprod = 0, $idprodfournprice = 0)
    {
        global $conf;
     ...
     */
    static public function get_default_npr(Societe $thirdparty_seller, Societe $thirdparty_buyer, $idprod = 0, $idprodfournprice = 0)
    {
        global $db;
     ...
     */
    static public function get_default_localtax($thirdparty_seller, $thirdparty_buyer, $local, $idprod = 0)
    {
        global $mysoc;
     ...
     */
    static public function get_exdir($num, $level, $alpha, $withoutslash, $object, $modulepart = '')
    {
        global $conf;
     ...
     */
    static public function dol_mkdir($dir, $dataroot = '', $newmask = null)
    {
        global $conf;
     ...
     */
    static public function picto_required()
    {
     ...
     */
    static public function dol_nl2br($stringtoencode, $nl2brmode = 0, $forxml = false)
    {
        if (!$nl2brmode) {
     ...
     * @return    string                        String decoded
     */
    static public function dol_htmlentitiesbr_decode($stringtodecode, $pagecodeto = 'UTF-8')
    {
        $ret = self::dol_html_entity_decode($stringtodecode, ENT_COMPAT | ENT_HTML5, $pagecodeto);
     ...
     * @return    string                        String decoded
     */
    static public function dol_htmlcleanlastbr($stringtodecode)
    {
     ...
     */
    static public function dol_string_is_good_iso($s, $clean = 0)
    {
        $len = DolibarrFunctions::dol_strlen($s);
        $out = '';
        $ok = 1;
     ...
     */
    static public function dol_nboflines($s, $maxchar = 0)
    {
        if ($s == '') {
            return 0;
        }
     ...
     * @see    dol_nboflines(), dolGetFirstLineOfText()
     */
    static public function dol_nboflines_bis($text, $maxlinesize = 0, $charset = 'UTF-8')
    {
        $repTable = ["\t" => " ", "\n" => "<br>", "\r" => " ", "\0" => " ", "\x0B" => " "];
        if (self::dol_textishtml($text)) {
            $repTable = ["\t" => " ", "\n" => " ", "\r" => " ", "\0" => " ", "\x0B" => " "];
        ...
        if ($maxlinesize) {
            foreach ($a as $line) {
                if (DolibarrFunctions::dol_strlen($line) > $maxlinesize) {
                    //$line_dec = html_entity_decode(strip_tags($line));
                    $line_dec = html_entity_decode($line);
                    if (DolibarrFunctions::dol_strlen($line_dec) > $maxlinesize) {
     ...
     * @see    self::dol_textishtml()
     */
    static public function dol_concatdesc($text1, $text2, $forxml = false, $invert = false)
    {
        if (!empty($invert)) {
            $tmp = $text1;
            $text1 = $text2;
        ...
        $ret = '';
        $ret .= (!self::dol_textishtml($text1) && self::dol_textishtml($text2)) ? dol_nl2br(DolibarrFunctions::dol_escape_htmltag($text1, 0, 1, '', 1), 0, $forxml) : $text1;
        $ret .= (!empty($text1) && !empty($text2)) ? ((self::dol_textishtml($text1) || self::dol_textishtml($text2)) ? ($forxml ? "<br \>\n" : "<br>\n") : "\n") : "";
        $ret .= (self::dol_textishtml($text1) && !self::dol_textishtml($text2)) ? dol_nl2br(DolibarrFunctions::dol_escape_htmltag($text2, 0, 1, '', 1), 0, $forxml) : $text2;
        return $ret;
    }
     ...
     */
    static public function make_substitutions($text, $substitutionarray, $outputlangs = null, $converttextinhtmlifnecessary = 0)
    {
        global $conf, $langs;
     ...
     */
    static public function complete_substitutions_array(&$substitutionarray, $outputlangs, $object = null, $parameters = null, $callfunc = "completesubstitutionarray")
    {
        global $conf, $user;
     ...
     * @return    void
     */
    static public function print_date_range($date_start, $date_end, $format = '', $outputlangs = '')
    {
     ...
     */
    static public function dolGetFirstLastname($firstname, $lastname, $nameorder = -1)
    {
        global $conf;
            ...
            if (!empty($conf->use_javascript_ajax) && empty($conf->global->MAIN_DISABLE_JQUERY_JNOTIFY) && empty($keepembedded)) {
                $return = '<script>
					$(document).ready(function() {
						...
						if (block) {
							$.dolEventValid("","' . DolibarrFunctions::dol_escape_js($out) . '");
						} else {
							/* jnotify(message, preset of message type, keepmessage) */
							$.jnotify("' . DolibarrFunctions::dol_escape_js($out) . '",
							"' . ($style == "ok" ? 3000 : $style) . '",
							' . ($style == "ok" ? "false" : "true") . ',
							{ remove: function (){} } )
						}
     ...
     */
    static public function dol_sort_array(&$array, $index, $order = 'asc', $natsort = 0, $case_sensitive = 0, $keepindex = 0)
    {
        // Clean parameters
     ...
     */
    static public function ascii_check($str)
    {
     ...
     */
    static public function dol_getIdFromCode($db, $key, $tablename, $fieldkey = 'code', $fieldid = 'id', $entityfilter = 0)
    {
        global $cache_codes;
     ...
     */
    static public function verifCond($strRights)
    {
     ...
     * This static public function is called by verifCond() or trans() and transnoentitiesnoconv().
     *
     ...
     */
    static public function dol_eval($s, $returnvalue = 0, $hideerrors = 1)
    {
        ...
        $forbiddenphpstrings = ["exec(", "passthru(", "shell_exec(", "system(", "proc_open(", "popen(", "eval(", "dol_eval(", "executeCLI("];
        $forbiddenphpstrings = array_merge($forbiddenphpstrings, ["fopen(", "file_put_contents(", "fputs(", "fputscsv(", "fwrite(", "fpassthru(", "unlink(", "mkdir(", "rmdir(", "symlink(", "touch(", "umask("]);
        $forbiddenphpstrings = array_merge($forbiddenphpstrings, ['function(', '$$', 'call_user_func(']);
     ...
     */
    static function picto_from_langcode($codelang, $moreatt = '', $notitlealt = 0)
    {
        if (empty($codelang)) {
            return '';
        }
        ...
        return self::img_picto_common($codelang, 'flags/' . strtolower($flagImage) . '.png', $moreatt, 0, $notitlealt);
    }
     ...
     * @see        img_object(), DolibarrFunctions::img_picto()
     */
    static public function img_picto_common($titlealt, $picto, $moreatt = '', $pictoisfullpath = 0, $notitle = 0)
    {
        global $conf;
        ...
        return self::img_picto($titlealt, $path, $moreatt, 1, 0, $notitle);
    }
     ...
     */
    static function dolGetStatus($statusLabel = '', $statusLabelShort = '', $html = '', $statusType = 'status0', $displayMode = 0, $url = '', $params = [])
    {
        $return = '';
        $dolGetBadgeParams = [];
        ...
        if ($displayMode == 0) {
            ...
            if (!empty($statusImg[$statusType])) {
                $htmlImg = DolibarrFunctions::img_picto($statusLabel, $statusImg[$statusType]);
            } else {
                $htmlImg = DolibarrFunctions::img_picto($statusLabel, $statusType);
            }
     ...
     */
    static function showValueWithClipboardCPButton($valuetocopy, $showonlyonhover = 1, $texttoshow = '')
    {
        /*
	global $conf;
	...
	if (!empty($conf->dol_no_mouse_hover)) {
        ...
        if ($texttoshow) {
            $result = '<span class="clipboardCP' . ($showonlyonhover ? ' clipboardCPShowOnHover' : '') . '"><' . $tag . ' class="clipboardCPValue hidewithsize">' . DolibarrFunctions::dol_escape_htmltag($valuetocopy, 1, 1) . '</' . $tag . '><span class="clipboardCPValueToPrint">' . DolibarrFunctions::dol_escape_htmltag($texttoshow, 1, 1) . '</span><span class="clipboardCPButton far fa-clipboard opacitymedium paddingleft paddingright"></span><span class="clipboardCPText opacitymedium"></span></span>';
        } else {
            $result = '<span class="clipboardCP' . ($showonlyonhover ? ' clipboardCPShowOnHover' : '') . '"><' . $tag . ' class="clipboardCPValue">' . DolibarrFunctions::dol_escape_htmltag($valuetocopy, 1, 1) . '</' . $tag . '><span class="clipboardCPButton far fa-clipboard opacitymedium paddingleft paddingright"></span><span class="clipboardCPText opacitymedium"></span></span>';
        }
    ...
    function showDirectDownloadLink($object)
    {
        ...
        $out .= DolibarrFunctions::img_picto($langs->trans("PublicDownloadLinkDesc"), 'globe') . ' <span class="opacitymedium">' . $langs->trans("DirectDownloadLink") . '</span><br>';
        if ($url) {
    ...
    function getAdvancedPreviewUrl($modulepart, $relativepath, $alldata = 0, $param = '')
    {
        ...
        if ($alldata == 1) {
            if ($isAllowedForPreview) {
                return ['target' => '_blank', 'css' => 'documentpreview', 'url' => DOL_URL_ROOT . '/document.php?modulepart=' . $modulepart . '&attachment=0&file=' . urlencode($relativepath) . ($param ? '&' . $param : ''), 'mime' => dol_mimetype($relativepath)];
            } else {
                return [];
            }
        }
        ...
        if ($isAllowedForPreview) {
            return 'javascript:document_preview(\'' . DolibarrFunctions::dol_escape_js(DOL_URL_ROOT . '/document.php?modulepart=' . $modulepart . '&attachment=0&file=' . urlencode($relativepath) . ($param ? '&' . $param : '')) . '\', \'' . dol_mimetype($relativepath) . '\', \'' . DolibarrFunctions::dol_escape_js($langs->trans('Preview')) . '\')';
        } else {
            return '';
        }
    }
     ...
     * @return string               html button
     */
    function dolGetButtonAction($label, $html = '', $actionType = 'default', $url = '', $id = '', $userRight = 1, $params = [])
    {
    ...
    function getFieldErrorIcon($fieldValidationErrorMsg)
    {
        ...
        if (!empty($fieldValidationErrorMsg)) {
            $out .= '<span class="field-error-icon classfortooltip" title="' . DolibarrFunctions::dol_escape_htmltag($fieldValidationErrorMsg, 1) . '"  role="alert" >'; // role alert is used for accessibility
