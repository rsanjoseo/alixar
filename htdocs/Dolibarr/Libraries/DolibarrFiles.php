...
abstract class DolibarrFiles
{
     ...
     */
    static function dol_is_dir($folder)
    {
        $newfolder = DolibarrFunctions::dol_osencode($folder);
        if (is_dir($newfolder)) {
            return true;
        } else {
            return false;
     ...
     */
    static function dol_is_file($pathoffile)
    {
        $newpathoffile = DolibarrFunctions::dol_osencode($pathoffile);
    ...
    {
        $tmpprot = ['file', 'http', 'https', 'ftp', 'zlib', 'data', 'ssh', 'ssh2', 'ogg', 'expect'];
    ...
    function dol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = '', $refname = '', $mode = 'read')
    {
        ...
        if ($modulepart == 'medias' && !empty($dolibarr_main_data_root)) {
            ...
            $original_file = $conf->societe->multidir_output[$entity] . '/Modules/Contacts/' . $original_file;
            ...
            if ($fuser->rights->projet->{$lire} || preg_match('/^specimen/i', $original_file)) {
                ...
                if ($refname && !preg_match('/^specimen/i', $original_file)) {
                    ...
                    $accessallowed = checkUserAccessToObject($user, ['projet_task'], $tmptask->id, 'projet&project', '', '', 'rowid', '');
                }
            }
            $original_file = $conf->projet->dir_output . '/' . $original_file;
            $sqlprotectagainstexternals = "SELECT fk_soc as fk_soc FROM " . MAIN_DB_PREFIX . "projet WHERE ref='" . $db->escape($refname) . "' AND entity IN (" . getEntity('project') . ")";
