...
class DolibarrCompany
{
     ...
     */
    static function getCountry($searchkey, $withcode = '', $dbtouse = 0, $outputlangs = '', $entconv = 1, $searchlabel = '')
    {
        // global $db, $langs;
        ...
        if ($resql) {
            ...
            if ($obj) {
                ...
                if ($withcode == 1) {
                    ...
                    $result = ['id' => $obj->rowid, 'code' => $obj->code, 'label' => $label];
                } else {
                    $result = $label;
                }
            } else {
     ...
     */
    static function getState($id, $withcode = '', $dbtouse = 0, $withregion = 0, $outputlangs = '', $entconv = 1)
    {
        // global $db, $langs;
        $db = DolibarrGlobals::getDb();
        $langs = DolibarrGlobals::getLangs();
        ...
        DolibarrFunctions::dol_syslog("Company.lib::getState", LOG_DEBUG);
        $resql = $dbtouse->query($sql);
        if ($resql) {
            $obj = $dbtouse->fetch_object($resql);
            if ($obj) {
                ...
                if ($withcode == 1) {
                    ...
                    if ($withregion == 1) {
                        return ['id' => $obj->id, 'code' => $obj->code, 'label' => $label, 'region_code' => $obj->region_code, 'region' => $obj->region_name];
                    } else {
                        return ['id' => $obj->id, 'code' => $obj->code, 'label' => $label];
                    }
                } else {
                    if ($withregion == 1) {
    ...
    function societe_prepare_head(Societe $object)
    {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/index.php?module=societes&controller=card&socid=' . $object->id;
        ...
        if (empty($conf->global->MAIN_SUPPORT_SHARED_CONTACT_BETWEEN_THIRDPARTIES)) {
            if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB) && $user->rights->societe->contact->lire) {
                ...
                $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/contact.php?socid=' . $object->id;
            ...
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/societecontact.php?socid=' . $object->id;
            $nbContact = count($object->liste_contact(-1, 'internal')) + count($object->liste_contact(-1, 'external'));
            $head[$h][1] = $langs->trans("ContactsAddresses");
            if ($nbContact > 0) {
                $head[$h][1] .= '<span class="badge marginleftonlyshort">' . $nbContact . '</span>';
        ...
        if ($object->client == 1 || $object->client == 2 || $object->client == 3) {
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/card.php?socid=' . $object->id;
            $head[$h][1] = '';
            ...
            if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
                $langs->load("products");
                // price
                $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/price.php?socid=' . $object->id;
        ...
        if ($supplier_module_enabled == 1 && $object->fournisseur && !empty($user->rights->fournisseur->lire)) {
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=' . $object->id;
        ...
        if (!empty($conf->projet->enabled) && (!empty($user->rights->projet->lire))) {
            ...
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/project.php?socid=' . $object->id;
        ...
        if (!empty($conf->global->ACCOUNTING_ENABLE_LETTERING)) {
            ...
            if (!empty($conf->accounting->enabled) && $object->client > 0) {
                $head[$h][0] = DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/thirdparty_lettering_customer.php?socid=' . $object->id;
            ...
            if (!empty($conf->accounting->enabled) && $object->fournisseur > 0) {
                $head[$h][0] = DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/thirdparty_lettering_supplier.php?socid=' . $object->id;
        ...
        if ((!empty($conf->commande->enabled) || !empty($conf->propal->enabled) || !empty($conf->facture->enabled) || !empty($conf->ficheinter->enabled) || (!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled))
            ...
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/consumption.php?socid=' . $object->id;
        ...
        if (empty($conf->global->SOCIETE_DISABLE_BANKACCOUNT)) {
            ...
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . urlencode($object->id);
            $head[$h][1] = $title;
        ...
        if (!empty($conf->website->enabled) && (!empty($conf->global->WEBSITE_USE_WEBSITE_ACCOUNTS)) && (!empty($user->rights->societe->lire))) {
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/website.php?id=' . urlencode($object->id);
        ...
        if (getDolGlobalString('PARTNERSHIP_IS_MANAGED_FOR') == 'thirdparty') {
            if (!empty($user->rights->partnership->read)) {
                ...
                $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/partnership.php?socid=' . $object->id;
        ...
        if ($user->socid == 0) {
            ...
            if (!empty($conf->notification->enabled)) {
                ...
                $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/notify/card.php?socid=' . urlencode($object->id);
            ...
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/note.php?id=' . urlencode($object->id);
            ...
            $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/document.php?socid=' . $object->id;
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/agenda.php?socid=' . $object->id;
        ...
        if (!empty($conf->agenda->enabled) && (!empty($user->rights->agenda->myactions->read) || !empty($user->rights->agenda->allactions->read))) {
            ...
            if (!is_null($dataretrieved)) {
                ...
                if ($resql) {
                    ...
                    DolibarrFunctions::dol_syslog('Failed to count actioncomm ' . $db->lasterror(), LOG_ERR);
                }
        ...
        // Log
        /*$head[$h][0] = DOL_URL_ROOT.'/Modules/Societes/info.php?socid='.$object->id;
    ...
    function societe_admin_prepare_head()
    {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/admin/societe.php';
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/admin/societe_extrafields.php';
        $head[$h][1] = $langs->trans("ExtraFieldsThirdParties");
        $head[$h][2] = 'attributes';
        $h++;

        $head[$h][0] = DOL_URL_ROOT . '/Modules/Societes/admin/contact_extrafields.php';
    ...
    function getFormeJuridiqueLabel($code)
    {
        ...
        DolibarrFunctions::dol_syslog("Company.lib::getFormeJuridiqueLabel", LOG_DEBUG);
        $resql = $db->query($sql);
        if ($resql) {
            $num = $db->num_rows($resql);
    ...
    function show_projects($conf, $langs, $db, $object, $backtopage = '', $nocreatelink = 0, $morehtmlright = '')
    {
        ...
        if (!empty($conf->projet->enabled) && $user->rights->projet->lire) {
            ...
            if (!empty($conf->projet->enabled) && $user->rights->projet->creer && empty($nocreatelink)) {
                $newcardbutton .= dolGetButtonTitle($langs->trans('AddProject'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $object->id . '&amp;action=create&amp;backtopage=' . urlencode($backtopage));
            }
    ...
    function show_contacts($conf, $langs, $db, $object, $backtopage = '')
    {
        ...
        $contactstatic->fields = [
            'name' => ['type' => 'varchar(128)', 'label' => 'Name', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'showoncombobox' => 1, 'index' => 1, 'position' => 10, 'searchall' => 1],
            'poste' => ['type' => 'varchar(128)', 'label' => 'PostOrFunction', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'showoncombobox' => 2, 'index' => 1, 'position' => 20],
            'address' => ['type' => 'varchar(128)', 'label' => 'Address', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'showoncombobox' => 3, 'index' => 1, 'position' => 30],
            'role' => ['type' => 'checkbox', 'label' => 'Role', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'showoncombobox' => 4, 'index' => 1, 'position' => 40],
            'statut' => ['type' => 'integer', 'label' => 'Status', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'default' => 0, 'index' => 1, 'position' => 50, 'arrayofkeyval' => [0 => $contactstatic->LibStatut(0, 1), 1 => $contactstatic->LibStatut(1, 1)]],
        ];

        // Definition of fields for list
        $arrayfields = [
            't.rowid' => ['label' => "TechnicalID", 'checked' => (!empty($conf->global->MAIN_SHOW_TECHNICAL_ID) ? 1 : 0), 'enabled' => (!empty($conf->global->MAIN_SHOW_TECHNICAL_ID) ? 1 : 0), 'position' => 1],
            't.name' => ['label' => "Name", 'checked' => 1, 'position' => 10],
            't.poste' => ['label' => "PostOrFunction", 'checked' => 1, 'position' => 20],
            't.address' => ['label' => (empty($conf->dol_optimize_smallscreen) ? $langs->trans("Address") . ' / ' . $langs->trans("Phone") . ' / ' . $langs->trans("Email") : $langs->trans("Address")), 'checked' => 1, 'position' => 30],
            'sc.role' => ['label' => "ContactByDefaultFor", 'checked' => 1, 'position' => 40],
            't.statut' => ['label' => "Status", 'checked' => 1, 'position' => 50, 'class' => 'center'],
        ];
        // Extra fields
        ...
        if ($user->rights->societe->contact->creer) {
            ...
            $newcardbutton .= dolGetButtonTitle($addcontact, '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Contacts/card.php?socid=' . $object->id . '&amp;action=create&amp;backtopage=' . urlencode($backtopage));
        }
        ...
        if ($search_name) {
            $sql .= natural_search(['t.lastname', 't.firstname'], $search_name);
        }
        if ($search_poste) {
        ...
        DolibarrFunctions::dol_syslog('core/lib/company.lib.php :: show_contacts', LOG_DEBUG);
        $result = $db->query($sql);
        if (!$result) {
            dol_print_error($db);
        }
        ...
        foreach ($contactstatic->fields as $key => $val) {
            ...
            if (!empty($arrayfields['t.' . $key]['checked']) || !empty($arrayfields['sc.' . $key]['checked'])) {
                ...
                if (in_array($key, ['statut'])) {
                    print $form->selectarray('search_status', ['-1' => '', '0' => $contactstatic->LibStatut(0, 1), '1' => $contactstatic->LibStatut(1, 1)], $search_status);
        ...
        if ($num || (GETPOST('button_search') || GETPOST('button_search.x') || GETPOST('button_search_x'))) {
            ...
            while ($i < $num) {
                ...
                if (!empty($conf->agenda->enabled) && $user->rights->agenda->myactions->create) {
                    print '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&actioncode=&contactid=' . $obj->rowid . '&socid=' . $object->id . '&backtopage=' . urlencode($backtopage) . '">';
                ...
                // Edit
                if ($user->rights->societe->contact->creer) {
                    print '<a class="editfielda paddingleft" href="' . DOL_URL_ROOT . '/Modules/Contacts/card.php?action=edit&token=' . newToken() . '&id=' . $obj->rowid . '&backtopage=' . urlencode($backtopage) . '">';
                    print img_edit();
                    print '</a>';
                }
     ...
     * @return    string|void                           Return html part or void if noprint is 1
     */
    function show_actions_done($conf, $langs, $db, $filterobj, $objcon = '', $noprint = 0, $actioncode = '', $donetodo = 'done', $filters = [], $sortfield = 'a.datep,a.id', $sortorder = 'DESC', $module = '')
    {
        global $user, $conf;
        global $form;
        ...
        if (!empty($conf->agenda->enabled)) {
            ...
            if (is_object($filterobj) && in_array(get_class($filterobj), ['Societe', 'Client', 'Fournisseur'])) {
        ...
        if ($sql) {
            ...
            DolibarrFunctions::dol_syslog("company.lib::show_actions_done", LOG_DEBUG);
            $resql = $db->query($sql);
            if ($resql) {
                $i = 0;
                $num = $db->num_rows($resql);
    ...
    function show_subsidiaries($conf, $langs, $db, $object)
    {
        ...
        if ($num) {
            ...
            while ($i < $num) {
                ...
                print '<td class="center">';
                print '<a class="editfielda" href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&socid=' . ((int) $obj->rowid) . '&action=edit&token=' . newToken() . '">';
                print img_edit();
                print '</a></td>';
