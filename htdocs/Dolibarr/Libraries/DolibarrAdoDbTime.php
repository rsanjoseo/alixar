...
abstract class DolibarrAdoDbTime
{
    public array $_month_table_normal = ["", 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    public array $_month_table_leaf = ["", 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];

    static public function adodb_date_test_date($y1, $m, $d = 13)
    {
        $h = round(rand() % 24);
        $t = self::adodb_mktime($h, 0, 0, $m, $d, $y1);
        $rez = self::adodb_date('Y-n-j H:i:s', $t);
        if ($h == 0) {
            $h = '00';
        } else {
            if ($h < 10) {
     ...
     */
    static public function adodb_mktime($hr, $min, $sec, $mon = false, $day = false, $year = false, $is_dst = false, $is_gmt = false)
    {
     ...
     */
    static public function adodb_date($fmt, $d = false, $is_gmt = false)
    {
        static $daylight;
        ...
        if (!isset($daylight)) {
            $daylight = function_exists('self::adodb_daylight_sv');
            if (empty($jan1_1971)) {
        ...
        $arr = self::_adodb_getdate($d, true, $is_gmt);

        if ($daylight) {
            self::adodb_daylight_sv($arr, $is_gmt);
        }
        ...
        for ($i = 0; $i < $max; $i++) {
            switch ($fmt[$i]) {
                ...
                case 'r': // Thu, 21 Dec 2000 16:01:07 +0200
                    ...
                    $dates .= gmdate('D', $_day_power * (3 + self::adodb_dow($year, $month, $day))) . ', '
                    ...
                    $gmt = self::adodb_get_gmt_diff($year, $month, $day);

                    $dates .= ' ' . self::adodb_tz_offset($gmt, $isphp5);
                    break;
                ...
                case 'z':
                    ...
                    break;
                case 'w':
                    $dates .= self::adodb_dow($year, $month, $day);
                    break;
                case 'l':
                    $dates .= gmdate('l', $_day_power * (3 + self::adodb_dow($year, $month, $day)));
                    break;
                case 'D':
                    $dates .= gmdate('D', $_day_power * (3 + self::adodb_dow($year, $month, $day)));
                    break;
     ...
     */
    static public function _adodb_getdate($origd = false, $fast = false, $is_gmt = false)
    {
        static $YRS;
        global $_month_table_normal, $_month_table_leaf;

        $d = $origd - ($is_gmt ? 0 : self::adodb_get_gmt_diff_ts($origd));
        $_day_power = 86400;
        $_hour_power = 3600;
        $_min_power = 60;
        ...
        if ($d < 0) {
            ...
            */
            ...
            for (; --$a >= 0;) {
                ...
                if ($leaf = self::_adodb_is_leap_year($a)) {
                    $d += $d366;
                } else {
                    $d += $d365;
                }
        ...
        $dow = self::adodb_dow($year, $month, $day);
    ...
    static public function adodb_get_gmt_diff_ts($ts)
    {
        if (0 <= $ts && $ts <= 0x7FFFFFFF) { // check if number in 32-bit signed range) {
            ...
            $m = $arr['mon'];
            $d = $arr['mday'];
            return self::adodb_get_gmt_diff($y, $m, $d);
        } else {
            return self::adodb_get_gmt_diff(false, false, false);
        }
    }
     ...
     */
    static public function adodb_get_gmt_diff($y, $m, $d)
    {
        static $TZ, $tzo;
        global $ADODB_DATETIME_CLASS;
     ...
     */
    static public function _adodb_is_leap_year($year)
    {
        if ($year % 4 != 0) {
            return false;
        }
     ...
     */
    static public function adodb_dow($year, $month, $day)
    {
        /*
    ...
    static public function adodb_tz_offset($gmt, $isphp5)
    {
        $zhrs = abs($gmt) / 3600;
        $hrs = floor($zhrs);
        if ($isphp5) {
     ...
     * Test Suite
     */
    static public function adodb_date_test()
    {

        for ($m = -24; $m <= 24; $m++) {
            echo "$m :", self::adodb_date('d-m-Y', self::adodb_mktime(0, 0, 0, 1 + $m, 20, 2040)), "<br>";
        }
        ...
        $fmt = 'Y-m-d H:i:s';
        echo '<pre>';
        echo 'adodb: ', self::adodb_date($fmt, $t), '<br>';
        echo 'php  : ', date($fmt, $t), '<br>';
        echo '</pre>';

        self::adodb_date_test_strftime('%Y %m %x %X');
        self::adodb_date_test_strftime("%A %d %B %Y");
        self::adodb_date_test_strftime("%H %M S");

        $t = self::adodb_mktime(0, 0, 0);
        if (!(self::adodb_date('Y-m-d') == date('Y-m-d'))) {
            print 'Error in ' . self::adodb_mktime(0, 0, 0) . '<br>';
        }

        $t = self::adodb_mktime(0, 0, 0, 6, 1, 2102);
        if (!(self::adodb_date('Y-m-d', $t) == '2102-06-01')) {
            print 'Error in ' . self::adodb_date('Y-m-d', $t) . '<br>';
        }

        $t = self::adodb_mktime(0, 0, 0, 2, 1, 2102);
        if (!(self::adodb_date('Y-m-d', $t) == '2102-02-01')) {
            print 'Error in ' . self::adodb_date('Y-m-d', $t) . '<br>';
        }

        print "<p>Testing gregorian <=> julian conversion<p>";
        $t = self::adodb_mktime(0, 0, 0, 10, 11, 1492);
        //http://www.holidayorigins.com/html/columbus_day.html - Friday check
        if (!(self::adodb_date('D Y-m-d', $t) == 'Fri 1492-10-11')) {
            print 'Error in Columbus landing<br>';
        }

        $t = self::adodb_mktime(0, 0, 0, 2, 29, 1500);
        if (!(self::adodb_date('Y-m-d', $t) == '1500-02-29')) {
            print 'Error in julian leap years<br>';
        }

        $t = self::adodb_mktime(0, 0, 0, 2, 29, 1700);
        if (!(self::adodb_date('Y-m-d', $t) == '1700-03-01')) {
            print 'Error in gregorian leap years<br>';
        }

        print self::adodb_mktime(0, 0, 0, 10, 4, 1582) . ' ';
        print self::adodb_mktime(0, 0, 0, 10, 15, 1582);
        $diff = (self::adodb_mktime(0, 0, 0, 10, 15, 1582) - self::adodb_mktime(0, 0, 0, 10, 4, 1582));
        if ($diff != 3600 * 24) {
            print " <b>Error in gregorian correction = " . ($diff / 3600 / 24) . " days </b><br>";
        }

        print " 15 Oct 1582, Fri=" . (self::adodb_dow(1582, 10, 15) == 5 ? 'Fri' : '<b>Error</b>') . "<br>";
        print " 4 Oct 1582, Thu=" . (self::adodb_dow(1582, 10, 4) == 4 ? 'Thu' : '<b>Error</b>') . "<br>";

        print "<p>Testing overflow<p>";

        $t = self::adodb_mktime(0, 0, 0, 3, 33, 1965);
        if (!(self::adodb_date('Y-m-d', $t) == '1965-04-02')) {
            print 'Error in day overflow 1 <br>';
        }
        $t = self::adodb_mktime(0, 0, 0, 4, 33, 1971);
        if (!(self::adodb_date('Y-m-d', $t) == '1971-05-03')) {
            print 'Error in day overflow 2 <br>';
        }
        $t = self::adodb_mktime(0, 0, 0, 1, 60, 1965);
        if (!(self::adodb_date('Y-m-d', $t) == '1965-03-01')) {
            print 'Error in day overflow 3 ' . self::adodb_date('Y-m-d', $t) . ' <br>';
        }
        $t = self::adodb_mktime(0, 0, 0, 12, 32, 1965);
        if (!(self::adodb_date('Y-m-d', $t) == '1966-01-01')) {
...
&nbsp; \"$s2\" (adodb_date len=" . strlen($s2) . ")</b></pre><br>";
    ...
    static public function adodb_date_test_strftime($fmt)
    {
        $s1 = strftime($fmt);
        $s2 = self::adodb_strftime($fmt);
    ...
    static public function adodb_date_gentable($out=true)
    {
    ...
    */
    ...
    static public function adodb_strftime($fmt, $ts = false, $is_gmt = false)
    {
    ...
    static public function adodb_time()
    {
        $d = new DateTime();
        return $d->format('U');
    }
     ...
     */
    static public function adodb_is_leap_year($year)
    {
        return self::_adodb_is_leap_year(self::adodb_year_digit_check($year));
    }
     ...
     */
    static public function adodb_year_digit_check($y)
    {
        if ($y < 100) {
     ...
     */
    static public function adodb_getdate($d = false, $fast = false)
    {
        if ($d === false) {
            return getdate();
        }
        ...
        return self::_adodb_getdate($d);
    }

    static public function adodb_validdate($y, $m, $d)
    {
        global $_month_table_normal, $_month_table_leaf;

        if (self::_adodb_is_leap_year($y)) {
    ...
    static public function adodb_gmdate($fmt, $d = false)
    {
        return self::adodb_date($fmt, $d, true);
    }

    static public function adodb_date2($fmt, $d = false, $is_gmt = false)
    {
        if ($d !== false) {
            if (!preg_match(
                "|^([0-9]{4})[-/\.]?([0-9]{1,2})[-/\.]?([0-9]{1,2})[ -]?(([0-9]{1,2}):?([0-9]{1,2}):?([0-9\.]{1,4}))?|",
                ($d), $rr)) {
                return self::adodb_date($fmt, false, $is_gmt);
            }
            ...
            // h-m-s-MM-DD-YY
            if (!isset($rr[5])) {
                $d = self::adodb_mktime(0, 0, 0, $rr[2], $rr[3], $rr[1], false, $is_gmt);
            } else {
                $d = @self::adodb_mktime($rr[5], $rr[6], $rr[7], $rr[2], $rr[3], $rr[1], false, $is_gmt);
            }
        }

        return self::adodb_date($fmt, $d, $is_gmt);
    }
     ...
     * Note that $is_dst is not implemented and is ignored.
     */
    static public function adodb_gmmktime($hr, $min, $sec, $mon = false, $day = false, $year = false, $is_dst = false)
    {
    ...
    static public function adodb_gmstrftime($fmt, $ts = false)
    {
