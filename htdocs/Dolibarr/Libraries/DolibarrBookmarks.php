 ...
 */
abstract class DolibarrBookmarks
{
    /**
     ...
     * @return    string
     */
    static function printDropdownBookmarksList()
    {
        ...
        // require_once DOL_DOCUMENT_ROOT . '/Modules/Bookmarks/class/bookmark.class.php';
        ...
        if (!empty($_SERVER["QUERY_STRING"])) {
            $url .= (DolibarrFunctions::dol_escape_htmltag($_SERVER["QUERY_STRING"]) ? '?' . DolibarrFunctions::dol_escape_htmltag($_SERVER["QUERY_STRING"]) : '');
        } else {
            ...
            if (is_array($_POST)) {
                foreach ($_POST as $key => $val) {
                    if (preg_match('/^search_/', $key) && $val != '') {
                        $tmpurl .= ($tmpurl ? '&' : '') . http_build_query([$key => $val]);
                    }
                }
            }
            $url .= ($tmpurl ? '?' . $tmpurl : '');
        ...
        $searchForm .= '<input type="hidden" name="token" value="' . DolibarrFunctions::newToken() . '">';
        ...
        $listbtn .= DolibarrFunctions::img_picto('', 'bookmark', 'class="paddingright"') . $langs->trans('Bookmarks') . '</a>';
        ...
        if (!empty($user->rights->bookmark->creer)) {
            if (!preg_match('/bookmarks\/card.php/', $_SERVER['PHP_SELF'])) {
                ...
                $newbtn .= '<a class="top-menu-dropdown-link" title="' . $langs->trans('AddThisPageToBookmarks') . '" href="' . DolibarrFunctions::dol_escape_htmltag($urltoadd) . '" >';
                $newbtn .= DolibarrFunctions::img_picto('', 'add', '', false, 0, 0, '', 'paddingright') . DolibarrFunctions::dol_escape_htmltag($langs->trans('AddThisPageToBookmarks')) . '</a>';
            }
        }
        ...
        $sql .= " AND entity IN (" . DolibarrFunctions::getEntity('bookmarks') . ")";
        ...
        if ($resql = $db->query($sql)) {
            if (empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
                ...
                while ((empty($conf->global->BOOKMARKS_SHOW_IN_MENU) || $i < $conf->global->BOOKMARKS_SHOW_IN_MENU) && $obj = $db->fetch_object($resql)) {
                    $bookmarkList .= '<a class="dropdown-item bookmark-item' . (strpos($obj->url, 'http') === 0 ? ' bookmark-item-external' : '') . '" id="bookmark-item-' . $obj->rowid . '" data-id="' . $obj->rowid . '" ' . ($obj->target == 1 ? ' target="_blank"' : '') . ' href="' . DolibarrFunctions::dol_escape_htmltag($obj->url) . '" >';
                    $bookmarkList .= DolibarrFunctions::dol_escape_htmltag($obj->title);
                    $bookmarkList .= '</a>';
                    $i++;
                }
                $bookmarkList .= '</div>';
                ...
                $searchForm .= '<option value="listbookmark" class="optionblue" rel="' . DolibarrFunctions::dol_escape_htmltag(DOL_URL_ROOT . '/bookmarks/list.php') . '" ';
                $searchForm .= ' data-html="' . DolibarrFunctions::dol_escape_htmltag(DolibarrFunctions::img_picto('', 'bookmark') . ' ' . ($user->rights->bookmark->creer ? $langs->trans('EditBookmarks') : $langs->trans('ListOfBookmarks')) . '...') . '">';
                $searchForm .= DolibarrFunctions::dol_escape_htmltag($user->rights->bookmark->creer ? $langs->trans('EditBookmarks') : $langs->trans('ListOfBookmarks')) . '...</option>';
                // Url to go on create new bookmark page
                if (!empty($user->rights->bookmark->creer)) {
                    if (!preg_match('/bookmarks\/card.php/', $_SERVER['PHP_SELF'])) {
                        $urltoadd = DOL_URL_ROOT . '/bookmarks/card.php?action=create&amp;url=' . urlencode($url);
                        $searchForm .= '<option value="newbookmark" class="optionblue" rel="' . DolibarrFunctions::dol_escape_htmltag($urltoadd) . '"';
                        $searchForm .= ' data-html="' . DolibarrFunctions::dol_escape_htmltag(DolibarrFunctions::img_picto('', 'bookmark') . ' ' . $langs->trans('AddThisPageToBookmarks') . '...') . '">' . DolibarrFunctions::dol_escape_htmltag($langs->trans('AddThisPageToBookmarks') . '...') . '</option>';
                    }
                }
                $i = 0;
                while ((empty($conf->global->BOOKMARKS_SHOW_IN_MENU) || $i < $conf->global->BOOKMARKS_SHOW_IN_MENU) && $obj = $db->fetch_object($resql)) {
                    $searchForm .= '<option name="bookmark' . $obj->rowid . '" value="' . $obj->rowid . '" ' . ($obj->target == 1 ? ' target="_blank"' : '') . ' rel="' . DolibarrFunctions::dol_escape_htmltag($obj->url) . '" >';
                    $searchForm .= DolibarrFunctions::dol_escape_htmltag($obj->title);
                    $searchForm .= '</option>';
                    $i++;
                }
                $searchForm .= '</select>';
