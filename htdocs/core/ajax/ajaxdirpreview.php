...
if ($type == 'directory') {
    ...
    $maxlengthname = 40;
    $excludefiles = ['^SPECIMEN\.pdf$', '^\.', '(\.meta|_preview.*\.png)$', '^temp$', '^payments$', '^CVS$', '^thumbs$'];
    ...
    if (in_array($module, $automodules)) {
        ...
        if (($section === '0' || empty($section)) && ($module != 'medias')) {
            ...
            $filearray = dol_dir_list($upload_dir, "files", 0, '', ['^\.', '(\.meta|_preview.*\.png)$', '^temp$', '^CVS$'], $sortfield, $sorting, 1);
        }
...
if ($useajax || $action == 'delete') {
    ...
    $form = new Form($db);
    $formquestion['urlfile'] = ['type' => 'hidden', 'value' => $urlfile, 'name' => 'urlfile']; // We must always put field, even if empty because it is filled by javascript later
    $formquestion['section'] = ['type' => 'hidden', 'value' => $section, 'name' => 'section']; // We must always put field, even if empty because it is filled by javascript later
    $formquestion['section_id'] = ['type' => 'hidden', 'value' => $section_id, 'name' => 'section_id']; // We must always put field, even if empty because it is filled by javascript later
    $formquestion['section_dir'] = ['type' => 'hidden', 'value' => $section_dir, 'name' => 'section_dir']; // We must always put field, even if empty because it is filled by javascript later
    $formquestion['sortfield'] = ['type' => 'hidden', 'value' => $sortfield, 'name' => 'sortfield']; // We must always put field, even if empty because it is filled by javascript later
    $formquestion['sortorder'] = ['type' => 'hidden', 'value' => $sortorder, 'name' => 'sortorder']; // We must always put field, even if empty because it is filled by javascript later
    if (!empty($action) && $action == 'file_manager') {
        $formquestion['file_manager'] = ['type' => 'hidden', 'value' => 1, 'name' => 'file_manager'];
    }
    if (!empty($websitekey)) {
        $formquestion['website'] = ['type' => 'hidden', 'value' => $websitekey, 'name' => 'website'];
    }
    if (!empty($pageid) && $pageid > 0) {
        $formquestion['pageid'] = ['type' => 'hidden', 'value' => $pageid, 'name' => 'pageid'];
    }
