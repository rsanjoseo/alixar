...
class modTax extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        // Config pages
        $this->config_page_url = ["taxes.php"];
        ...
        $this->export_permission[$r] = [["tax", "charges", "export"]];
        $this->export_fields_array[$r] = ['cc.libelle' => "Type", 'c.rowid' => "IdSocialContribution", 'c.libelle' => "Label", 'c.date_ech' => 'DateDue', 'c.periode' => 'Period', 'c.amount' => "AmountExpected", "c.paye" => "Status", 'p.rowid' => 'PaymentId', 'p.datep' => 'DatePayment', 'p.amount' => 'AmountPayment', 'p.num_paiement' => 'Numero'];
        $this->export_TypeFields_array[$r] = ['cc.libelle' => "List:c_chargesociales:libelle:id", 'c.libelle' => "Text", 'c.date_ech' => 'Date', 'c.periode' => 'Period', 'c.amount' => "Numeric", "c.paye" => "Boolean", 'p.datep' => 'Date', 'p.amount' => 'Numeric', 'p.num_paiement' => 'Numeric'];
        $this->export_entities_array[$r] = ['cc.libelle' => "tax_type", 'c.rowid' => "tax", 'c.libelle' => 'tax', 'c.date_ech' => 'tax', 'c.periode' => 'tax', 'c.amount' => "tax", "c.paye" => "tax", 'p.rowid' => 'payment', 'p.datep' => 'payment', 'p.amount' => 'payment', 'p.num_paiement' => 'payment'];
        ...
        $this->import_tables_array[$r] = ['t' => MAIN_DB_PREFIX . 'chargesociales'];
        $this->import_fields_array[$r] = [
            't.libelle' => "Label*", 't.fk_type' => "Type",
            't.amount' => "Amount*", 't.date_ech' => "DateDue*", 't.periode' => "PeriodEndDate*",
        ];

        $this->import_convertvalue_array[$r] = [
            't.fk_type' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Compta/sociales/class/cchargesociales.class.php', 'class' => 'Cchargesociales', 'method' => 'fetch', 'element' => 'Cchargesociales'],
        ];
        $this->import_examplevalues_array[$r] = [
            't.libelle' => "Social/fiscal contribution", 't.fk_type' => "TAXPRO (must be id or code found into dictionary)",
        ...
        $this->import_icon[$r] = 'tax';
        $this->import_entities_array[$r] = []; // We define here only fields that use another icon that the one defined into import_icon
        $this->import_tables_array[$r] = ['t' => MAIN_DB_PREFIX . 'tva'];
        $this->import_fields_array[$r] = [
            't.datep' => "DatePayment*", 't.datev' => "DateValue*", 't.label' => "Label*", 't.fk_typepayment' => "PaymentMode*",
            't.amount' => "Amount*", 't.num_payment' => 'Numero',
        ];

        $this->import_convertvalue_array[$r] = [
            't.fk_typepayment' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Compta/paiement/class/cpaiement.class.php', 'class' => 'Cpaiement', 'method' => 'fetch', 'element' => 'Cpaiement'],
        ];
        $this->import_examplevalues_array[$r] = [
            't.label' => "VAT Payment 1st quarter 2016", 't.fk_typepayment' => "CHQ (must be id or code found into dictionary)",
