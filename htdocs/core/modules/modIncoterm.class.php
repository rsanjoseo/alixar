...
class modIncoterm extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->need_dolibarr_version = [3, 0]; // Minimum version of Dolibarr required by module
        $this->langfiles = ["incoterm"];

        $this->const = [
            ['INCOTERM_ACTIVATE', 'chaine', 0, 'Description de INCOTERM_ACTIVATE'],
        ];
        ...
        $this->dictionaries = [
            'langs' => 'incoterm',
            'tabname' => [MAIN_DB_PREFIX . "c_incoterms"], // List of tables we want to see into dictonnary editor
            'tablib' => ["Incoterms"], // Label of tables
            'tabsql' => ['SELECT rowid, code, libelle, active FROM ' . MAIN_DB_PREFIX . 'c_incoterms'], // Request to select fields
            'tabsqlsort' => ["rowid ASC"], // Sort order
            'tabfield' => ["code,libelle"], // List of fields (result of select to show dictionary)
