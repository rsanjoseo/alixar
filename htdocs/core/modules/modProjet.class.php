...
class modProjet extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->config_page_url = ["project.php@projet"];
        $this->picto = 'project';

        // Data directories to create when module is enabled
        $this->dirs = ["/Modules/Projects/temp"];

        // Dependencies
        $this->hidden = false; // A condition to hide module
        $this->depends = []; // List of module class names as string that must be enabled if this module is enabled
        $this->requiredby = ['modEventOrganization']; // List of module ids to disable if this one is disabled
        ...
        // Boxes
        $this->boxes = [
            0 => ['file' => 'box_project.php', 'enabledbydefaulton' => 'Home'],
            1 => ['file' => 'box_task.php', 'enabledbydefaulton' => 'Home'],
            2 => ['file' => 'box_validated_projects.php', 'enabledbydefaulton' => 'Home'],
            3 => ['file' => 'box_funnel_of_prospection.php', 'enabledbydefaulton' => 'Home'],
        ];
        ...
        $this->export_permission[$r] = [["projet", "export"]];
        $this->export_dependencies_array[$r] = ['projecttask' => 'pt.rowid', 'task_time' => 'ptt.rowid'];
        ...
        $this->export_fields_array[$r] = array_merge($this->export_fields_array[$r], ['pt.rowid' => 'TaskId', 'pt.ref' => 'RefTask', 'pt.label' => 'LabelTask', 'pt.dateo' => "TaskDateStart", 'pt.datee' => "TaskDateEnd", 'pt.duration_effective' => "DurationEffective", 'pt.planned_workload' => "PlannedWorkload", 'pt.progress' => "Progress", 'pt.description' => "TaskDescription"]);
        $this->export_entities_array[$r] = array_merge($this->export_entities_array[$r], ['pt.rowid' => 'projecttask', 'pt.ref' => 'projecttask', 'pt.label' => 'projecttask', 'pt.dateo' => "projecttask", 'pt.datee' => "projecttask", 'pt.duration_effective' => "projecttask", 'pt.planned_workload' => "projecttask", 'pt.progress' => "projecttask", 'pt.description' => "projecttask"]);
        ...
        if (empty($conf->global->PROJECT_HIDE_TASKS)) {
            ...
            // End add extra fields
            $this->import_fieldshidden_array[$r] = ['t.fk_user_creat' => 'user->id', 'extra.fk_object' => 'lastrowid-' . MAIN_DB_PREFIX . 'projet_task']; // aliastable.field => ('user->id' or 'lastrowid-'.tableparent)
            $this->import_convertvalue_array[$r] = [
                't.fk_projet' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Projects/class/project.class.php', 'class' => 'Project', 'method' => 'fetch', 'element' => 'Project'],
                't.ref' => ['rule' => 'getrefifauto'],
            ];
            //$this->import_convertvalue_array[$r]=array('s.fk_soc'=>array('rule'=>'lastrowid',table='t');
            $this->import_regex_array[$r] = ['t.dateo' => '^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$', 't.datee' => '^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$', 't.datec' => '^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]( [0-9][0-9]:[0-9][0-9]:[0-9][0-9])?$'];
            $this->import_examplevalues_array[$r] = ['t.fk_projet' => 'MyProjectRef', 't.ref' => "auto or TK2010-1234", 't.label' => "My task", 't.progress' => "0 (not started) to 100 (finished)", 't.datec' => '1972-10-10', 't.note_private' => "My private note", 't.note_public' => "My public note"];
        }
    }
