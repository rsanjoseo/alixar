...
class modPrelevement extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->dirs = ["/prelevement/temp", "/prelevement/receipts"];

        // Dependencies
        $this->hidden = false; // A condition to hide module
        $this->depends = ["modFacture", "modBanque"]; // List of module class names as string that must be enabled if this module is enabled
        ...
        // Config pages
        $this->config_page_url = ["prelevement.php"];
    ...
    public function init($options = '')
    {
        ...
        $sql = [
            "DELETE FROM " . MAIN_DB_PREFIX . "document_model WHERE nom = '" . $this->db->escape($this->const[0][2]) . "' AND type = 'bankaccount' AND entity = " . ((int) $this->conf->entity),
            "INSERT INTO " . MAIN_DB_PREFIX . "document_model (nom, type, entity) VALUES('" . $this->db->escape($this->const[0][2]) . "','bankaccount'," . ((int) $this->conf->entity) . ")",
        ];
