...
class modReception extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        // Config pages
        $this->config_page_url = ["reception_setup.php"];

        // Dependencies
        $this->depends = ["modFournisseur"];
        $this->requiredby = [];
        $this->conflictwith = [];
        $this->langfiles = ['receptions'];
        ...
        $this->export_permission[$r] = [["reception", "reception", "export"]];
        ...
        if ($idcontacts && !empty($this->conf->global->RECEPTION_ADD_CONTACTS_IN_EXPORT)) {
        ...
        $this->export_dependencies_array[$r] = ['reception_line' => 'ed.rowid', 'product' => 'ed.rowid']; // To add unique key if we ask a field of a child to avoid the DISTINCT to discard them
        if ($idcontacts && !empty($this->conf->global->RECEPTION_ADD_CONTACTS_IN_EXPORT)) {
    ...
    public function init($options = '')
    {
        ...
        $sql = [
            "DELETE FROM " . MAIN_DB_PREFIX . "document_model WHERE nom = '" . $this->db->escape($this->const[0][2]) . "' AND type = 'reception' AND entity = " . ((int) $this->conf->entity),
            "INSERT INTO " . MAIN_DB_PREFIX . "document_model (nom, type, entity) VALUES('" . $this->db->escape($this->const[0][2]) . "','reception'," . ((int) $this->conf->entity) . ")",
        ];
