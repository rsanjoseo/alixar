...
class modResource extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->config_page_url = ["resource.php"];

        // Dependencies
        // List of modules id that must be enabled if this module is enabled
        $this->depends = [];
        // List of modules id to disable if this one is disabled
        $this->requiredby = ['modPlace'];
        // Minimum version of PHP required by module
        $this->phpmin = [5, 6];

        $this->langfiles = ["resource"]; // langfiles@resource
        // Constants
        ...
        $this->export_permission[$r] = [["resource", "read"]];
        $this->export_fields_array[$r] = ['r.rowid' => 'IdResource', 'r.ref' => 'ResourceFormLabel_ref', 'c.code' => 'ResourceTypeCode', 'c.label' => 'ResourceType', 'r.description' => 'ResourceFormLabel_description', 'r.note_private' => "NotePrivate", 'r.note_public' => "NotePublic", 'r.asset_number' => 'AssetNumber', 'r.datec' => "DateCreation", 'r.tms' => "DateLastModification"];
        $this->export_TypeFields_array[$r] = ['r.rowid' => 'List:resource:ref', 'r.ref' => 'Text', 'r.asset_number' => 'Text', 'r.description' => 'Text', 'c.code' => 'Text', 'c.label' => 'List:c_type_resource:label', 'r.datec' => 'Date', 'r.tms' => 'Date', 'r.note_private' => 'Text', 'r.note_public' => 'Text'];
        $this->export_entities_array[$r] = ['r.rowid' => 'resource', 'r.ref' => 'resource', 'c.code' => 'resource', 'c.label' => 'resource', 'r.description' => 'resource', 'r.note_private' => "resource", 'r.resource' => "resource", 'r.asset_number' => 'resource', 'r.datec' => "resource", 'r.tms' => "resource"];
        $keyforselect = 'resource';
        $keyforelement = 'resource';
        $keyforaliasextra = 'extra';
        include DOL_DOCUMENT_ROOT . '/core/extrafieldsinexport.inc.php';

        $this->export_dependencies_array[$r] = ['resource' => ['r.rowid']]; // We must keep this until the aggregate_array is used. To add unique key if we ask a field of a child to avoid the DISTINCT to discard them.
        ...
        $this->import_tables_array[$r] = ['r' => MAIN_DB_PREFIX . 'resource', 'extra' => MAIN_DB_PREFIX . 'resource_extrafields']; // List of tables to insert into (insert done in same order)
        $this->import_fields_array[$r] = ['r.ref' => "ResourceFormLabel_ref*", 'r.fk_code_type_resource' => 'ResourceTypeCode', 'r.description' => 'ResourceFormLabel_description', 'r.note_private' => "NotePrivate", 'r.note_public' => "NotePublic", 'r.asset_number' => 'AssetNumber', 'r.datec' => 'DateCreation'];
        // Add extra fields
        $sql = "SELECT name, label, fieldrequired FROM " . MAIN_DB_PREFIX . "extrafields WHERE elementtype = 'resource' AND entity IN (0," . $this->conf->entity . ")";
        ...
        $this->import_fieldshidden_array[$r] = ['r.fk_user_author' => 'user->id', 'extra.fk_object' => 'lastrowid-' . MAIN_DB_PREFIX . 'resource']; // aliastable.field => ('user->id' or 'lastrowid-'.tableparent)
        $this->import_convertvalue_array[$r] = [
            'r.fk_code_type_resource' => ['rule' => 'fetchidfromcodeorlabel', 'classfile' => '/core/class/ctyperesource.class.php', 'class' => 'Ctyperesource', 'method' => 'fetch', 'dict' => 'DictionaryResourceType'],
        ];
        //$this->import_convertvalue_array[$r]=array('s.fk_soc'=>array('rule'=>'lastrowid',table='t');
        $this->import_regex_array[$r] = ['s.datec' => '^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]( [0-9][0-9]:[0-9][0-9]:[0-9][0-9])?$'];
        $this->import_examplevalues_array[$r] = ['r.ref' => "REF1", 'r.fk_code_type_resource' => "Code from dictionary resource type", 'r.datec' => "2017-01-01 or 2017-01-01 12:30:00"];
        $this->import_updatekeys_array[$r] = ['r.rf' => 'ResourceFormLabel_ref'];
    }
