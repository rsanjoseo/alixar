...
class modCron extends DolibarrModules
{
    ...
    public function __construct()
    {
        ...
        // Config pages
        //-------------
        $this->config_page_url = ["cron.php@cron"];

        // Dependancies
        //-------------
        $this->hidden = !empty($this->conf->global->MODULE_CRON_DISABLED); // A condition to disable module
        ...
        //------
        $this->boxes = [
            0 => ['file' => 'box_scheduled_jobs.php', 'enabledbydefaulton' => 'Home'],
        ];
        ...
        $this->menu[$r] = [
            ...
            'titre' => 'CronList',
            'url' => '/Modules/Cron/list.php?leftmenu=admintools',
            'langs' => 'cron', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 200,
            'enabled' => '$this->conf->cron->enabled && preg_match(\'/^(admintools|all)/\', $leftmenu)', // Define condition to show or hide menu entry. Use '$this->conf->mymodule->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
