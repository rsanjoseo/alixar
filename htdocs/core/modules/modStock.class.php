...
class modStock extends DolibarrModules
{
    ...
    public function __construct()
    {
        ...
        $this->dirs = ["/stock/temp"];

        $this->config_page_url = ["stock.php"];

        // Dependencies
        $this->hidden = false; // A condition to hide module
        $this->depends = ["modProduct"]; // List of module class names as string that must be enabled if this module is enabled
        $this->requiredby = ["modProductBatch"]; // List of module ids to disable if this one is disabled
        ...
        $this->langfiles = ["stocks"];

        // Constants
        $this->const = [];
        $r = 0;

        $this->const[$r] = ['STOCK_ALLOW_NEGATIVE_TRANSFER', 'chaine', '1', '', 1];
        ...
        $this->export_permission[$r] = [["stock", "lire"]];
        ...
        $this->export_aggregate_array[$r] = ['ps.reel' => 'SUM']; // TODO Not used yet
        $this->export_dependencies_array[$r] = ['stock' => ['p.rowid', 'e.rowid']]; // We must keep this until the aggregate_array is used. To have a unique key, if we ask a field of a child, to avoid the DISTINCT to discard them.
        $keyforselect = 'product';
        $keyforelement = 'product';
        $keyforaliasextra = 'extra';
        include DOL_DOCUMENT_ROOT . '/core/extrafieldsinexport.inc.php';
        $this->export_fields_array[$r] = array_merge($this->export_fields_array[$r], ['ps.reel' => 'Stock']);
        ...
        if (!empty($this->conf->productbatch->enabled)) {
            ...
            $this->export_aggregate_array[$r] = ['ps.reel' => 'SUM']; // TODO Not used yet
            $this->export_dependencies_array[$r] = ['stockbatch' => ['pb.rowid'], 'batch' => ['pb.rowid']]; // We must keep this until the aggregate_array is used. To add unique key if we ask a field of a child to avoid the DISTINCT to discard them.
        ...
        $this->export_aggregate_array[$r] = ['sm.value' => 'SUM']; // TODO Not used yet
        $this->export_dependencies_array[$r] = ['movement' => ['sm.rowid']]; // We must keep this until the aggregate_array is used. To add unique key if we ask a field of a child to avoid the DISTINCT to discard them.
        ...
        $this->import_tables_array[$r] = ['e' => MAIN_DB_PREFIX . 'entrepot'];
        $this->import_tables_creator_array[$r] = ['e' => 'fk_user_author'];
        $this->import_fields_array[$r] = [
            'e.ref' => "LocationSummary*",
        ...
        $this->import_convertvalue_array[$r] = [
            'e.fk_departement' => ['rule' => 'fetchidfromcodeid', 'classfile' => '/core/class/cstate.class.php', 'class' => 'Cstate', 'method' => 'fetch', 'dict' => 'DictionaryStateCode'],
            'e.fk_pays' => ['rule' => 'fetchidfromcodeid', 'classfile' => '/core/class/ccountry.class.php', 'class' => 'Ccountry', 'method' => 'fetch', 'dict' => 'DictionaryCountry'],
            'e.fk_parent' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Products/stock/class/entrepot.class.php', 'class' => 'Entrepot', 'method' => 'fetch', 'element' => 'ref'],
        ];
        $this->import_regex_array[$r] = ['e.statut' => '^[0|1]'];
        ...
        $this->import_entities_array[$r] = []; // We define here only fields that use another icon that the one defined into import_icon
        $this->import_tables_array[$r] = ['ps' => MAIN_DB_PREFIX . 'product_stock'];
        $this->import_fields_array[$r] = ['ps.fk_product' => "Product*", 'ps.fk_entrepot' => "Warehouse*", 'ps.reel' => "Stock*"];

        $this->import_convertvalue_array[$r] = [
            'ps.fk_product' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Products/class/product.class.php', 'class' => 'Product', 'method' => 'fetch', 'element' => 'product'],
            'ps.fk_entrepot' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Products/stock/class/entrepot.class.php', 'class' => 'Entrepot', 'method' => 'fetch', 'element' => 'ref'],
        ];
        $this->import_examplevalues_array[$r] = [
            'ps.fk_product' => "id or ref of product", 'ps.fk_entrepot' => "id or ref of warehouse", 'ps.reel' => "10",
        ];
        $this->import_updatekeys_array[$r] = ['ps.fk_product' => 'Product', 'ps.fk_entrepot' => "Warehouse"];
        $this->import_run_sql_after_array[$r] = [    // Because we may change data that are denormalized, we must update dernormalized data after.
