...
class modFicheinter extends DolibarrModules
{
    ...
    public function __construct()
    {
        ...
        $this->dirs = ["/ficheinter/temp"];

        // Dependencies
        $this->depends = ["modSociete"];
        $this->requiredby = [];
        $this->conflictwith = [];
        $this->langfiles = ["bills", "companies", "interventions"];
        ...
        if (!isset($this->conf->ficheinter) || !isset($this->conf->ficheinter->enabled)) {
            $this->conf->ficheinter = new stdClass();
            $this->conf->ficheinter->enabled = 0;
        }
        ...
        // Boxes
        $this->boxes = [0 => ['file' => 'box_ficheinter.php', 'enabledbydefaulton' => 'Home']];
        ...
        $this->export_permission[$r] = [["ficheinter", "export"]];
        ...
        $this->export_dependencies_array[$r] = ['inter_line' => 'fd.rowid']; // To add unique key if we ask a field of a child to avoid the DISTINCT to discard them
    ...
    public function init($options = '')
    {
        ...
        $sql = [
            "DELETE FROM " . MAIN_DB_PREFIX . "document_model WHERE nom = '" . $this->db->escape($this->const[0][2]) . "' AND type = 'ficheinter' AND entity = " . ((int) $this->conf->entity),
            "INSERT INTO " . MAIN_DB_PREFIX . "document_model (nom, type, entity) VALUES('" . $this->db->escape($this->const[0][2]) . "','ficheinter'," . ((int) $this->conf->entity) . ")",
        ];
