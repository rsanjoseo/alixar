...
class modUser extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->export_sql_end[$r] .= ' WHERE u.entity IN (' . DolibarrFunctions::getEntity('user') . ')';
        ...
        $this->import_tables_array[$r] = ['u' => MAIN_DB_PREFIX . 'user', 'extra' => MAIN_DB_PREFIX . 'user_extrafields']; // List of tables to insert into (insert done in same order)
        ...
        $this->import_fieldshidden_array[$r] = ['u.fk_user_creat' => 'user->id', 'extra.fk_object' => 'lastrowid-' . MAIN_DB_PREFIX . 'user']; // aliastable.field => ('user->id' or 'lastrowid-'.tableparent)
        ...
        $this->import_examplevalues_array[$r] = [
            ...
            'u.fk_soc' => '0 (internal user) or company name (external user)', 'u.datec' => DolibarrFunctions::dol_print_date(DolibarrFunctions::dol_now(), '%Y-%m-%d'), 'u.address' => "61 jump street",
