...
class modSociete extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->config_page_url = ["societe.php@societe"];
        // Name of image file used for this module.
        $this->picto = 'company';

        // Data directories to create when module is enabled
        $this->dirs = ["/Modules/Societes/temp"];

        // Dependencies
        $this->hidden = false; // A condition to hide module
        $this->depends = []; // List of module class names as string that must be enabled if this module is enabled
        $this->requiredby = ["modExpedition", "modFacture", "modFournisseur", "modFicheinter", "modPropale", "modContrat", "modCommande"]; // List of module ids to disable if this one is disabled
        ...
        $this->langfiles = ["companies", 'bills', "compta", "admin", "banks"];
        ...
        */
        ...
        // Boxes
        $this->boxes = [
            0 => ['file' => 'box_clients.php', 'enabledbydefaulton' => 'Home'],
            1 => ['file' => 'box_prospect.php', 'enabledbydefaulton' => 'Home'],
            2 => ['file' => 'box_contacts.php', 'enabledbydefaulton' => 'Home'],
            3 => ['file' => 'box_activity.php', 'enabledbydefaulton' => 'Home', 'note' => '(WarningUsingThisBoxSlowDown)'],
            4 => ['file' => 'box_goodcustomers.php', 'enabledbydefaulton' => 'Home', 'note' => '(WarningUsingThisBoxSlowDown)'],
        ];
        ...
        */
        ...
        $this->export_permission[$r] = [["societe", "export"]];
        ...
        $this->export_fields_array[$r] += ['u.login' => 'SaleRepresentativeLogin', 'u.firstname' => 'SaleRepresentativeFirstname', 'u.lastname' => 'SaleRepresentativeLastname'];
        ...
        $this->export_entities_array[$r] = ['u.login' => 'user', 'u.firstname' => 'user', 'u.lastname' => 'user']; // We define here only fields that use another picto
        $this->export_examplevalues_array[$r] = ['s.client' => '0 (no customer no prospect)/1 (customer)/2 (prospect)/3 (customer and prospect)', 's.fournisseur' => '0 (not a supplier) or 1 (supplier)'];
        ...
        $this->export_permission[$r] = [["societe", "contact", "export"]];
        $this->export_fields_array[$r] = [
        ...
        $this->import_fields_array[$r] = [//field order as per structure of table llx_societe
                                          's.nom' => "Name*",
        ...
        $this->import_convertvalue_array[$r] = [//field order as per structure of table llx_societe
                                                's.code_client' => ['rule' => 'getcustomercodeifauto'],
                                                's.code_fournisseur' => ['rule' => 'getsuppliercodeifauto'],
                                                's.code_compta' => ['rule' => 'getcustomeraccountancycodeifauto'],
                                                's.code_compta_fournisseur' => ['rule' => 'getsupplieraccountancycodeifauto'],
                                                's.fk_departement' => [
                                                ...
                                                's.capital' => ['rule' => 'numeric'],
                                                's.fk_stcomm' => ['rule' => 'zeroifnull'],
                                                's.parent' => [
                                                    'rule' => 'fetchidfromref',
                                                ...
                                                's.outstanding_limit' => ['rule' => 'numeric'],
                                                's.fk_account' => [
                                                    'rule' => 'fetchidfromcodeid',
                                                    'classfile' => '/Modules/Compta/bank/class/account.class.php',
                                                    'class' => 'Account',
                                                    'method' => 'fetch',
                                                    'element' => 'BankAccount'
                                                    //          ),
        ...
        $this->import_regex_array[$r] = [//field order as per structure of table llx_societe
                                         's.status' => '^[0|1]',
        ...
        $this->import_examplevalues_array[$r] = [//field order as per structure of table llx_societe
                                                 's.nom' => "TPBigCompany",
        ...
        $this->import_updatekeys_array[$r] = [
            's.nom' => 'Name',
            's.code_client' => 'CustomerCode',
            's.code_fournisseur' => 'SupplierCode',
            's.code_compta' => 'CustomerAccountancyCode',
        ...
        $this->import_entities_array[$r] = ['s.fk_soc' => 'company']; // We define here only fields that use a different icon than the one defined in import_icon
        $this->import_tables_array[$r] = [
            's' => MAIN_DB_PREFIX . 'socpeople',
            'extra' => MAIN_DB_PREFIX . 'socpeople_extrafields',
        ]; // List of tables to insert into (insert done in same order)
        $this->import_fields_array[$r] = [//field order as per structure of table llx_socpeople
                                          's.rowid' => 'Id',
                                          's.datec' => "DateCreation",
        ...
        $this->import_tables_array[$r] = ['sr' => MAIN_DB_PREFIX . 'societe_rib'];
        $this->import_fields_array[$r] = [//field order as per structure of table llx_societe_rib
                                          'sr.label' => "Label*",
        ...
        $this->import_convertvalue_array[$r] = [
            'sr.fk_soc' => [
                'rule' => 'fetchidfromref',
                'classfile' => '/Modules/Societes/class/societe.class.php',
                'class' => 'Societe',
                'method' => 'fetch',
                'element' => 'ThirdParty',
            ],
        ...
        $this->import_icon[$r] = 'company';
        $this->import_entities_array[$r] = ['sr.fk_user' => 'user']; // We define here only fields that use another icon that the one defined into import_icon
        $this->import_tables_array[$r] = ['sr' => MAIN_DB_PREFIX . 'societe_commerciaux'];
        $this->import_fields_array[$r] = ['sr.fk_soc' => "ThirdPartyName*", 'sr.fk_user' => "User*"];

        $this->import_convertvalue_array[$r] = [
            'sr.fk_soc' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Societes/class/societe.class.php', 'class' => 'Societe', 'method' => 'fetch', 'element' => 'ThirdParty'],
            'sr.fk_user' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Users/class/user.class.php', 'class' => 'User', 'method' => 'fetch', 'element' => 'User'],
        ];
        $this->import_examplevalues_array[$r] = ['sr.fk_soc' => "MyBigCompany", 'sr.fk_user' => "login"];
    }
