...
class printing_printgcp extends PrintingDriver
{
    ...
    public function __construct($db)
    {
        ...
        if (!$conf->oauth->enabled) {
            ...
            if ($this->google_id != '' && $this->google_secret != '') {
                $this->conf[] = ['varname' => 'PRINTGCP_INFO', 'info' => 'GoogleAuthConfigured', 'type' => 'info'];
                $this->conf[] = [
                ...
                if ($token_ok) {
                    ...
                    $this->conf[] = ['varname' => 'TOKEN_REFRESH', 'info' => ((!empty($refreshtoken)) ? 'Yes' : 'No'), 'type' => 'info'];
                    $this->conf[] = ['varname' => 'TOKEN_EXPIRED', 'info' => ($expire ? 'Yes' : 'No'), 'type' => 'info'];
                    $this->conf[] = ['varname' => 'TOKEN_EXPIRE_AT', 'info' => ($expiredat), 'type' => 'info'];
                }
                /*
                ...
                $this->conf[] = ['varname' => 'PRINTGCP_INFO', 'info' => 'GoogleAuthNotConfigured', 'type' => 'info'];
            }
        }
        // do not display submit button
        $this->conf[] = ['enabled' => 0, 'type' => 'submit'];
    }
    ...
    public function sendPrintToPrinter($printerid, $printjobtitle, $filepath, $contenttype)
    {
        ...
        if (!$handle) {
            return ['status' => 0, 'errorcode' => '', 'errormessage' => 'Could not read the file.'];
        }
        // Read file content
