...
class pdf_vinci extends ModelePDFMo
{
    ...
    public function write_file($object, $outputlangs = '', $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        // Load translation files required by the page
        $outputlangs->loadLangs(["main", "orders", "companies", "bills", "dict", "products"]);
        ...
        if (!empty($conf->global->PDF_USE_ALSO_LANGUAGE_CODE) && $outputlangs->defaultlang != $conf->global->PDF_USE_ALSO_LANGUAGE_CODE) {
            ...
            $outputlangsbis->loadLangs(["main", "orders", "companies", "bills", "dict", "products"]);
        }
            ...
            if (file_exists($dir)) {
                ...
                for ($i = 0; $i < $nblines; $i++) {
                    ...
                    if ($this->getColumnStatus('dim')) {
                        $array = array_filter([$prod->length, $prod->width, $prod->height]);
                        $dim = implode("x", $array);
    ...
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
    {
        ...
        // Load translation files required by the page
        $outputlangs->loadLangs(["main", "orders", "companies", "bills", "sendings"]);
        ...
        $array = array_filter([$prodToMake->length, $prodToMake->width, $prodToMake->height]);
        $dim = implode("x", $array);
        if (!empty($dim)) {
            $posy += 5;
            $pdf->SetFont('', 'B', $default_font_size + 3);
