...
class modMailmanSpip extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->depends = ['modAdherent']; // List of module class names as string that must be enabled if this module is enabled
        ...
        // Config pages
        $this->config_page_url = ['mailman.php'];

        // Constants
        $this->const = [];
        $this->const[1] = ["ADHERENT_MAILMAN_UNSUB_URL", "chaine", "http://lists.example.com/cgi-bin/mailman/admin/%LISTE%/members?adminpw=%MAILMAN_ADMINPW%&user=%EMAIL%", "Url de désinscription aux listes mailman"];
        $this->const[2] = ["ADHERENT_MAILMAN_URL", "chaine", "http://lists.example.com/cgi-bin/mailman/admin/%LISTE%/members?adminpw=%MAILMAN_ADMINPW%&send_welcome_msg_to_this_batch=1&subscribees=%EMAIL%", "Url pour les inscriptions mailman"];
        $this->const[3] = ["ADHERENT_MAILMAN_LISTS", "chaine", "", "Mailing-list to subscribe new members to"];
