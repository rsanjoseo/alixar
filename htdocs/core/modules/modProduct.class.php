...
class modProduct extends DolibarrModules
{
    ...
    public function __construct()
    {
        ...
        $this->dirs = ["/Modules/Products/temp"];

        // Dependencies
        $this->hidden = false; // A condition to hide module
        $this->depends = []; // List of module class names as string that must be enabled if this module is enabled
        $this->requiredby = ["modStock", "modBarcode", "modProductBatch", "modVariants"]; // List of module ids to disable if this one is disabled
        ...
        // Config pages
        $this->config_page_url = ["product.php@product"];
        $this->langfiles = ["products", "companies", "stocks", "bills"];
        ...
        // Boxes
        $this->boxes = [
            0 => ['file' => 'box_produits.php', 'enabledbydefaulton' => 'Home'],
            1 => ['file' => 'box_produits_alerte_stock.php', 'enabledbydefaulton' => ''],
            2 => ['file' => 'box_graph_product_distribution.php', 'enabledbydefaulton' => 'Home'],
        ];
        ...
        */
        ...
        $usenpr = 0;
        if (is_object($this->mysoc)) {
            $usenpr = $this->mysoc->useNPR();
        }
        ...
        $this->export_permission[$r] = [["produit", "export"]];
        ...
        if (is_object($this->mysoc) && $usenpr) {
        ...
        if (!empty($this->conf->global->PRODUIT_SOUSPRODUITS)) {
            ...
            $keyforaliasextra = 'extra';
            include DOL_DOCUMENT_ROOT . '/core/extrafieldsinexport.inc.php';
            $this->export_fields_array[$r] = array_merge($this->export_fields_array[$r], ['p2.rowid' => "Id", 'p2.ref' => "Ref", 'p2.label' => "Label", 'p2.description' => "Description"]);
        ...
        $this->import_tables_array[$r] = ['p' => MAIN_DB_PREFIX . 'product', 'extra' => MAIN_DB_PREFIX . 'product_extrafields'];
        $this->import_tables_creator_array[$r] = ['p' => 'fk_user_author']; // Fields to store import user id
        ...
        $this->import_convertvalue_array[$r] = [
            ...
            'p.accountancy_code_sell' => ['rule' => 'accountingaccount'],
            'p.accountancy_code_sell_intra' => ['rule' => 'accountingaccount'],
            'p.accountancy_code_sell_export' => ['rule' => 'accountingaccount'],
            'p.accountancy_code_buy' => ['rule' => 'accountingaccount'],
            'p.accountancy_code_buy_intra' => ['rule' => 'accountingaccount'],
            'p.accountancy_code_buy_export' => ['rule' => 'accountingaccount'],
        ];
        ...
        if (!empty($this->conf->stock->enabled)) {//if Stock module enabled
            ...
            $this->import_regex_array[$r] = array_merge($this->import_regex_array[$r], [
                'p.tobatch' => '^[0|1|2]$',
            ]);

            $this->import_convertvalue_array[$r] = array_merge($this->import_convertvalue_array[$r], [
                'p.fk_default_warehouse' => [
                    'rule' => 'fetchidfromref',
                    'classfile' => '/Modules/Products/stock/class/entrepot.class.php',
                    'class' => 'Entrepot',
                    'method' => 'fetch',
                    'element' => 'Warehouse',
                ],
        ...
        if (is_object($this->mysoc) && $usenpr) {
            $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], ['p.recuperableonly' => 'NPR']);
        }
        if (is_object($this->mysoc) && $this->mysoc->useLocalTax(1)) {
            $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], ['p.localtax1_tx' => 'LT1', 'p.localtax1_type' => 'LT1Type']);
        }
        if (is_object($this->mysoc) && $this->mysoc->useLocalTax(2)) {
        ...
        $this->import_fieldshidden_array[$r] = ['extra.fk_object' => 'lastrowid-' . MAIN_DB_PREFIX . 'product']; // aliastable.field => ('user->id' or 'lastrowid-'.tableparent)
        $this->import_regex_array[$r] = [
            'p.ref' => '[^ ]',
        ...
        if (is_object($this->mysoc) && $usenpr) {
            $import_sample = array_merge($import_sample, ['p.recuperableonly' => '0']);
        }
        if (is_object($this->mysoc) && $this->mysoc->useLocalTax(1)) {
            $import_sample = array_merge($import_sample, ['p.localtax1_tx' => '', 'p.localtax1_type' => '']);
        }
        if (is_object($this->mysoc) && $this->mysoc->useLocalTax(2)) {
        ...
        if ((!empty($this->conf->fournisseur->enabled) && empty($this->conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($this->conf->supplier_order->enabled) || !empty($this->conf->supplier_invoice->enabled)) {
            ...
            $this->import_tables_array[$r] = ['sp' => MAIN_DB_PREFIX . 'product_fournisseur_price', 'extra' => MAIN_DB_PREFIX . 'product_fournisseur_price_extrafields'];
            $this->import_tables_creator_array[$r] = ['sp' => 'fk_user'];
            $this->import_fields_array[$r] = [//field order as per structure of table llx_product_fournisseur_price, without optional fields
            ...
            if (is_object($this->mysoc) && $usenpr) {
                $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], ['sp.recuperableonly' => 'VATNPR']);
            }
            if (is_object($this->mysoc) && $this->mysoc->useLocalTax(1)) {
                $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], ['sp.localtax1_tx' => 'LT1', 'sp.localtax1_type' => 'LT1Type']);
            }
            if (is_object($this->mysoc) && $this->mysoc->useLocalTax(2)) {
                $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], ['sp.localtax2_tx' => 'LT2', 'sp.localtax2_type' => 'LT2Type']);
            }
            $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], [
            ...
            // End add extra fields
            $this->import_fieldshidden_array[$r] = ['extra.fk_object' => 'lastrowid-' . MAIN_DB_PREFIX . 'product_fournisseur_price']; // aliastable.field => ('user->id' or 'lastrowid-'.tableparent)

            $this->import_convertvalue_array[$r] = [
                'sp.fk_soc' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Societes/class/societe.class.php', 'class' => 'Societe', 'method' => 'fetch', 'element' => 'ThirdParty'],
                'sp.fk_product' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Products/class/product.class.php', 'class' => 'Product', 'method' => 'fetch', 'element' => 'Product'],
            ];
            ...
            if (is_object($this->mysoc) && $usenpr) {
                $this->import_examplevalues_array[$r] = array_merge($this->import_examplevalues_array[$r], ['sp.recuperableonly' => '']);
            }
            if (is_object($this->mysoc) && $this->mysoc->useLocalTax(1)) {
                $this->import_examplevalues_array[$r] = array_merge($this->import_examplevalues_array[$r], ['sp.localtax1_tx' => 'LT1', 'sp.localtax1_type' => 'LT1Type']);
            }
            if (is_object($this->mysoc) && $this->mysoc->useLocalTax(2)) {
                $this->import_examplevalues_array[$r] = array_merge($this->import_examplevalues_array[$r], ['sp.localtax2_tx' => 'LT2', 'sp.localtax2_type' => 'LT2Type']);
            }
        ...
        if (!empty($this->conf->global->PRODUIT_MULTIPRICES)) {
            ...
            $this->import_icon[$r] = $this->picto;
            $this->import_entities_array[$r] = []; // We define here only fields that use another icon that the one defined into import_icon
            $this->import_tables_array[$r] = ['pr' => MAIN_DB_PREFIX . 'product_price'];
            $this->import_tables_creator_array[$r] = ['pr' => 'fk_user_author']; // Fields to store import user id
            $this->import_fields_array[$r] = [
                'pr.fk_product' => "ProductOrService*",
                'pr.price_base_type' => "PriceBase", 'pr.price_level' => "PriceLevel",
                'pr.price' => "PriceLevelUnitPriceHT", 'pr.price_ttc' => "PriceLevelUnitPriceTTC",
                'pr.price_min' => "MinPriceLevelUnitPriceHT", 'pr.price_min_ttc' => "MinPriceLevelUnitPriceTTC",
                'pr.date_price' => 'DateCreation*',
            ...
            if (is_object($this->mysoc) && $usenpr) {
                $this->import_fields_array[$r] = array_merge($this->import_fields_array[$r], ['pr.recuperableonly' => 'NPR']);
            }
            $this->import_regex_array[$r] = ['pr.datec' => '^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$', 'pr.recuperableonly' => '^[0|1]$'];
            $this->import_convertvalue_array[$r] = [
                'pr.fk_product' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Products/class/product.class.php', 'class' => 'Product', 'method' => 'fetch', 'element' => 'Product'],
            ];
            $this->import_examplevalues_array[$r] = [
                'pr.fk_product' => "ref:PRODUCT_REF or id:123456",
        ...
        if (!empty($this->conf->global->MAIN_MULTILANGS)) {
            ...
            $this->import_fields_array[$r] = ['l.fk_product' => 'ProductOrService*', 'l.lang' => 'Language*', 'l.label' => 'TranslatedLabel', 'l.description' => 'TranslatedDescription'];
            //$this->import_fields_array[$r]['l.note']='TranslatedNote';
            $this->import_convertvalue_array[$r] = [
                'l.fk_product' => ['rule' => 'fetchidfromref', 'classfile' => '/Modules/Products/class/product.class.php', 'class' => 'Product', 'method' => 'fetch', 'element' => 'Product'],
            ];
            $this->import_examplevalues_array[$r] = ['l.fk_product' => 'ref:PRODUCT_REF or id:123456', 'l.lang' => 'en_US', 'l.label' => 'Label in en_US', 'l.description' => 'Desc in en_US'];
            $this->import_updatekeys_array[$r] = ['l.fk_product' => 'ProductOrService', 'l.lang' => 'Language'];
        }
    }
