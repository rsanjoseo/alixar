...
class modPropale extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->dirs = ["/propale/temp"];

        // Dependencies
        $this->hidden = false; // A condition to hide module
        $this->depends = ["modSociete"]; // List of module class names as string that must be enabled if this module is enabled
        ...
        $this->config_page_url = ["propal.php"];
        $this->langfiles = ["propal", "bills", "companies", "deliveries", "products"];
        ...
        // Boxes
        $this->boxes = [
            0 => ['file' => 'box_graph_propales_permonth.php', 'enabledbydefaulton' => 'Home'],
            1 => ['file' => 'box_propales.php', 'enabledbydefaulton' => 'Home'],
        ];
        ...
        $this->export_permission[$r] = [["propale", "export"]];
        ...
        $this->export_dependencies_array[$r] = ['propal_line' => 'cd.rowid', 'product' => 'cd.rowid']; // To add unique key if we ask a field of a child to avoid the DISTINCT to discard them
        $keyforselect = 'propal';
        ...
        $this->import_convertvalue_array[$r] = [
            'cd.fk_propal' => [
                'rule' => 'fetchidfromref',
                'file' => '/Modules/Comm/propal/class/propal.class.php',
                'class' => 'Propal',
                'method' => 'fetch',
            ],
        ];
