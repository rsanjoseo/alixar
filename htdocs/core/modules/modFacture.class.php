...
class modFacture extends DolibarrModules
{
    ...
    public function __construct()
    {
        ...
        $this->dirs = ["/facture/temp"];

        // Dependencies
        $this->depends = ['always' => "modSociete"];
        $this->requiredby = ["modComptabilite", "modAccounting"];
        $this->conflictwith = [];
        $this->langfiles = ["bills", "companies", "compta", "products"];
        ...
        $this->boxes = [
            0 => ['file' => 'box_factures_imp.php', 'enabledbydefaulton' => 'Home'],
            1 => ['file' => 'box_factures.php', 'enabledbydefaulton' => 'Home'],
            2 => ['file' => 'box_graph_invoices_permonth.php', 'enabledbydefaulton' => 'Home'],
            3 => ['file' => 'box_customers_outstanding_bill_reached.php', 'enabledbydefaulton' => 'Home'],
        ];
        ...
        $this->export_permission[$r] = [["facture", "facture", "export", "other"]];
        ...
        $this->export_special_array[$r] = ['none.rest' => 'getRemainToPay'];
        $this->export_dependencies_array[$r] = ['invoice_line' => 'fd.rowid', 'product' => 'fd.rowid', 'none.rest' => ['f.rowid', 'f.total_ttc', 'f.close_code']]; // To add unique key if we ask a field of a child to avoid the DISTINCT to discard them
        ...
        $this->export_icon[$r] = 'invoice';
        $this->export_permission[$r] = [["facture", "facture", "export"]];
        $this->export_fields_array[$r] = [
        ...
        $this->export_special_array[$r] = ['none.rest' => 'getRemainToPay'];
        $this->export_dependencies_array[$r] = ['payment' => 'p.rowid', 'none.rest' => ['f.rowid', 'f.total_ttc', 'f.close_code']]; // To add unique key if we ask a field of a child to avoid the DISTINCT to discard them, or just to have field we need
        $keyforselect = 'facture';
        $keyforelement = 'invoice';
        $keyforaliasextra = 'extra';
        include DOL_DOCUMENT_ROOT . '/core/extrafieldsinexport.inc.php';
