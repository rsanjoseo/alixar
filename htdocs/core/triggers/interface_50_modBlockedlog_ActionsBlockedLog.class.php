...
class InterfaceActionsBlockedLog extends DolibarrTriggers
{
    ...
    public function runTrigger($action, $object, User $user, $langs, Conf $conf)
    {
        ...
        if ($action === 'BILL_VALIDATE' || (($action === 'BILL_DELETE' || $action === 'BILL_SENTBYMAIL') && $object->statut != 0)
            ...
            || (in_array($object->element, ['facture', 'supplier_invoice']) && $action === 'DOC_DOWNLOAD' && $object->statut != 0)
            || (in_array($object->element, ['facture', 'supplier_invoice']) && $action === 'DOC_PREVIEW' && $object->statut != 0)
        ...
        } elseif (strpos($action, 'PAYMENT') !== false && !in_array($action, ['PAYMENT_ADD_TO_BANK'])) {
            $qualified++;
            $amounts = (double) $object->amount;
        }
