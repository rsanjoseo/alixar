...
class box_scheduled_jobs extends ModeleBoxes
{
    ...
    public function loadBox($max = 5)
    {
        ...
        $langs->load("cron");
        $this->info_box_head = ['text' => $langs->trans("BoxScheduledJobs", $max)];

        if ($user->rights->cron->read) {
            include_once DOL_DOCUMENT_ROOT . '/Modules/Cron/class/cronjob.class.php';
            ...
            if ($result) {
                ...
                $this->info_box_contents[$line][] = [
                    'td' => 'class="center"',
                    'textnoformat' => ($nbjobsinerror ? '<a href="' . DOL_URL_ROOT . '/Modules/Cron/list.php?search_lastresult=' . urlencode('<>0') . '"><div class="badge badge-danger"><i class="fa fa-exclamation-triangle"></i> ' . $nbjobsinerror . '</div></a>' : '<div class="center badge-status4">0</div>'),
                ];
            } else {
