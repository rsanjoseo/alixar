    ...
    public function loadBox($max = 5)
    {
        ...
        if ($user->rights->ticket->read) {
            ...
            if ($resql) {
                ...
                while ($i < $num) {
                    ...
                    while ($minimumdatecformated < $objp->datec) {
                        $dataseries[] = ['label' => dol_print_date($minimumdatecformated, 'day'), 'data' => 0];
                        $minimumdatec = date_add($minimumdatec, $intervaltoadd);
                        $minimumdatecformated = date('Y-m-d', date_timestamp_get($minimumdatec));
                    }
                    $dataseries[] = ['label' => dol_print_date($objp->datec, 'day'), 'data' => $objp->nb];
                    $minimumdatec = date_add($minimumdatec, $intervaltoadd);
                    $minimumdatecformated = date('Y-m-d', date_timestamp_get($minimumdatec));
                    $i++;
                }
            ...
            if (!$mesg) {
                ...
                $px1->setShowLegend(0);
                $px1->SetType(['bars']);
                $px1->SetLegend([$langs->trans('BoxNumberOfTicketByDay')]);
