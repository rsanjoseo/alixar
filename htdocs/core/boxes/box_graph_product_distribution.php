...
class box_graph_product_distribution extends ModeleBoxes
{
    ...
    public $depends = ["product|service", "facture|propal|commande"];
    ...
    public function loadBox($max = 5)
    {
        ...
        if (in_array('DOLUSERCOOKIE_box_' . $this->boxcode, $autosetarray)) {
            ...
            $tmparray = (!empty($_COOKIE['DOLUSERCOOKIE_box_' . $this->boxcode]) ? json_decode($_COOKIE['DOLUSERCOOKIE_box_' . $this->boxcode], true) : []);
        ...
        if (!empty($conf->propal->enabled) && !empty($user->rights->propale->lire)) {
            ...
            if ($showpropalnb) {
                $langs->load("propal");
                include_once DOL_DOCUMENT_ROOT . '/Modules/Comm/propal/class/propalestats.class.php';
        ...
        if (!empty($conf->commande->enabled) && !empty($user->rights->commande->lire)) {
            ...
            if ($showordernb) {
                $langs->load("orders");
                include_once DOL_DOCUMENT_ROOT . '/Modules/Commande/class/commandestats.class.php';
        ...
        if (!empty($conf->facture->enabled) && !empty($user->rights->facture->lire)) {
            ...
            if ($showinvoicenb) {
                $langs->load("bills");
                include_once DOL_DOCUMENT_ROOT . '/Modules/Compta/facture/class/facturestats.class.php';
