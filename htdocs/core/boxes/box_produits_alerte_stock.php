...
class box_produits_alerte_stock extends ModeleBoxes
{
    ...
    public function __construct($db, $param = '')
    {
        ...
        $tmpentry = ['enabled' => ((!empty($conf->product->enabled) || !empty($conf->service->enabled)) && !empty($conf->stock->enabled)), 'perms' => !empty($user->rights->stock->lire), 'module' => 'product|service|stock'];
    ...
    public function loadBox($max = 5)
    {
        ...
        $this->info_box_head = ['text' => $langs->trans("BoxTitleProductsAlertStock", $max)];
        ...
        if (($user->rights->produit->lire || $user->rights->service->lire) && $user->rights->stock->lire) {
            ...
            if (is_object($hookmanager)) {
                $parameters = ['boxproductalertstocklist' => 1];
