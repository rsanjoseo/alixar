...
function shipping_prepare_head($object)
{
    ...
    $langs->loadLangs(["sendings", "deliveries"]);
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Expedition/document.php?id=' . $object->id;
...
function delivery_prepare_head($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Expedition/document.php?id=' . $tmpobject->id;
    $head[$h][1] = $langs->trans('Documents');
    if (($nbFiles + $nbLinks) > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">' . ($nbFiles + $nbLinks) . '</span>';
    }
    ...
    if ($resql) {
        ...
        if ($num) {
            ...
            while ($i < $num) {
                ...
                print '<a href="' . DOL_URL_ROOT . '/Modules/Expedition/card.php?id=' . $objp->expedition_id . '">' . img_object($langs->trans("ShowSending"), 'sending') . ' ' . $objp->exp_ref . '<a>';
                print '</td>';
