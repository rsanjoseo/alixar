...
function bank_prepare_head(Account $object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/bank/card.php?id=' . $object->id;
    $head[$h][1] = $langs->trans("BankAccount");
    $head[$h][2] = 'bankname';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/bankentries_list.php?id=" . $object->id;
    ...
    //    {
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/treso.php?account=" . $object->id;
    $head[$h][1] = $langs->trans("PlannedTransactions");
    $head[$h][2] = 'cash';
    $h++;
    //    }

    $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/annuel.php?account=" . $object->id;
    $head[$h][1] = $langs->trans("IOMonthlyReporting");
    $head[$h][2] = 'annual';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/graph.php?account=" . $object->id;
    ...
    if ($object->courant != Account::TYPE_CASH) {
        ...
        $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/releve.php?account=" . ((int) $object->id);
    ...
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/document.php?account=" . $object->id;
    ...
    /*$head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/info.php?id=" . $object->id;
...
function account_statement_prepare_head($object, $num)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/bank/releve.php?account=' . $object->id . '&num=' . $num;
    ...
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Compta/bank/account_statement_document.php?account=" . $object->id . "&num=" . $num;
    $head[$h][1] = $langs->trans("Documents");
    if (($nbFiles + $nbLinks) > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">' . ($nbFiles + $nbLinks) . '</span>';
    }
...
function various_payment_prepare_head($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/card.php?id=' . $object->id;
    ...
    $nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
    $nbLinks = Link::count($db, $object->element, $object->id);
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/document.php?id=' . $object->id;
    $head[$h][1] = $langs->trans('Documents');
    if (($nbFiles + $nbLinks) > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">' . ($nbFiles + $nbLinks) . '</span>';
    }
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/info.php?id=' . $object->id;
    $head[$h][1] = $langs->trans("Info");
    $head[$h][2] = 'info';
    $h++;
