...
function propal_prepare_head($object)
{
    ...
    $langs->loadLangs(['propal', 'compta', 'companies']);

    $h = 0;
    $head = [];

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/propal/card.php?id=' . $object->id;
    ...
    if ((empty($conf->commande->enabled) && ((!empty($conf->expedition->enabled) && !empty($conf->expedition_bon->enabled) && $user->rights->expedition->lire)
        ...
        $langs->load("sendings");
        $text = '';
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Expedition/propal.php?id=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/propal/contact.php?id=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/propal/note.php?id=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/propal/document.php?id=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/propal/info.php?id=' . $object->id;
...
function propal_admin_prepare_head()
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/admin/propal_extrafields.php';
    $head[$h][1] = $langs->trans("ExtraFields");
    $head[$h][2] = 'attributes';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/admin/propaldet_extrafields.php';
...
function getCustomerProposalPieChart($socid = 0)
{
    ...
    $listofstatus = [Propal::STATUS_DRAFT, Propal::STATUS_VALIDATED, Propal::STATUS_SIGNED, Propal::STATUS_NOTSIGNED, Propal::STATUS_BILLED];
    ...
    if ($resql) {
        ...
        foreach ($listofstatus as $status) {
            $dataseries[] = [$propalstatic->LibStatut($status, 1), (isset($vals[$status]) ? (int) $vals[$status] : 0)];
