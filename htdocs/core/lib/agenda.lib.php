 ...
 * @return    void
 */
function print_actions_filter($form, $canedit, $status, $year, $month, $day, $showbirthday, $filtera, $filtert, $filterd, $pid, $socid, $action, $showextcals = [], $actioncode = '', $usergroupid = '', $excludetype = '', $resourceid = 0)
{
    ...
    // Hooks
    $parameters = ['canedit' => $canedit, 'pid' => $pid, 'socid' => $socid];
    $object = null;
...
function show_array_actions_to_do($max = 5)
{
    ...
    include_once DOL_DOCUMENT_ROOT . '/Modules/Societes/class/client.class.php';
    ...
    if ($resql) {
        ...
        print '<th colspan="2" class="right"><a class="commonlink" href="' . DOL_URL_ROOT . '/Modules/Comm/action/list.php?action=show_list&status=todo">' . $langs->trans("FullList") . '</a></th>';
        print '</tr>';
...
function show_array_last_actions_done($max = 5)
{
    ...
    if ($resql) {
        ...
        print '<th colspan="2" class="right"><a class="commonlink" href="' . DOL_URL_ROOT . '/Modules/Comm/action/list.php?action=show_list&status=done">' . $langs->trans("FullList") . '</a></th>';
        print '</tr>';
...
function actions_prepare_head($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/card.php?id=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/document.php?id=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/info.php?id=' . $object->id;
...
function calendars_prepare_head($param)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/list.php?action=show_list' . ($param ? '&' . $param : '');
    $head[$h][1] = $langs->trans("ViewList");
    $head[$h][2] = 'cardlist';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_month' . ($param ? '&' . $param : '');
    $head[$h][1] = $langs->trans("ViewCal");
    $head[$h][2] = 'cardmonth';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_week' . ($param ? '&' . $param : '');
    $head[$h][1] = $langs->trans("ViewWeek");
    $head[$h][2] = 'cardweek';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_day' . ($param ? '&' . $param : '');
    ...
    if (!empty($conf->global->AGENDA_SHOW_PERTYPE)) {
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/pertype.php' . ($param ? '?' . $param : '');
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Comm/action/peruser.php' . ($newparam ? '?' . $newparam : '');
