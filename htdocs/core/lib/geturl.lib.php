 ...
 */
function getURLContent($url, $postorget = 'GET', $param = '', $followlocation = 1, $addheaders = [], $allowedschemes = ['http', 'https'], $localurl = 0)
{
    ...
    do {
        ...
        if (in_array($hosttocheck, ['metadata.google.internal'])) {
            $info['http_code'] = 400;
        ...
        if (in_array($hosttocheck, ['localhost', 'localhost.domain'])) {
            $iptocheck = '127.0.0.1';
        } elseif (in_array($hosttocheck, ['ip6-localhost', 'ip6-loopback'])) {
            $iptocheck = '::1';
        } else {
        ...
        if ($iptocheck) {
            ...
            if (in_array($iptocheck, ['100.100.100.200'])) {
                $info['http_code'] = 400;
            ...
            if (defined('CURLOPT_CONNECT_TO')) {
                $connect_to = [sprintf("%s:%d:%s:%d", $newUrlArray['host'], empty($newUrlArray['port']) ? '' : $newUrlArray['port'], $iptocheck, empty($newUrlArray['port']) ? '' : $newUrlArray['port'])];
