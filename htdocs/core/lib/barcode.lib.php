...
function barcode_encode_ean($ean, $encoding = "EAN-13")
{
    ...
    if (preg_match("/[^0-9]/i", $ean)) {
        return ["error" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $ean . " (not a numeric)", "text" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $ean . " (not a numeric)"];
    }
    ...
    if (strlen($ean) < 12 || strlen($ean) > 13) {
        return ["error" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $ean . " (must have 12/13 numbers)", "text" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $ean . " (must have 12/13 numbers)"];
    }
...
function barcode_encode_upc($upc, $encoding = "UPC")
{
    ...
    if (preg_match("/[^0-9]/i", $upc)) {
        return ["error" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $upc . " (not a numeric)", "text" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $upc . " (not a numeric)"];
    }
    $encoding = strtoupper($encoding);
    if (strlen($upc) < 11 || strlen($upc) > 12) {
        return ["error" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $upc . " (must have 11/12 numbers)", "text" => "Invalid encoding/code. encoding=" . $encoding . " code=" . $upc . " (must have 11/12 numbers)"];
    }
...
function barcode_outimage($text, $bars, $scale = 1, $mode = "png", $total_y = 0, $space = '')
{
    ...
    if (!$space) {
        $space = ['top' => 2 * $scale, 'bottom' => 2 * $scale, 'left' => 2 * $scale, 'right' => 2 * $scale];
    }
