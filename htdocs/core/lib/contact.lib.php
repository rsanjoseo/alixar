...
function contact_prepare_head(Contact $object)
{
    ...
    $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/card.php?id=' . $object->id;
    ...
    if ((!empty($conf->ldap->enabled) && !empty($conf->global->LDAP_CONTACT_ACTIVE))
        ...
        $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/ldap.php?id=' . $object->id;
        $head[$tab][1] = $langs->trans("LDAPCard");
        $head[$tab][2] = 'ldap';
        $tab++;
    }

    $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/perso.php?id=' . $object->id;
    ...
    if (!empty($conf->commande->enabled) || !empty($conf->propal->enabled) || !empty($conf->facture->enabled) || !empty($conf->ficheinter->enabled) || (!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled)) {
        $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/consumption.php?id=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/note.php?id=' . $object->id;
    ...
    $upload_dir = $conf->societe->dir_output . "/Modules/Contacts/" . dol_sanitizeFileName($object->ref);
    $nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
    $nbLinks = Link::count($db, $object->element, $object->id);
    $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/document.php?id=' . $object->id;
    ...
    // Agenda / Events
    $head[$tab][0] = DOL_URL_ROOT . '/Modules/Contacts/agenda.php?id=' . $object->id;
    ...
    // Log
    /*
    $head[$tab][0] = DOL_URL_ROOT.'/Modules/Contacts/info.php?id='.$object->id;
