...
function categories_prepare_head(Categorie $object, $type)
{
    ...
    $langs->loadLangs(['categories', 'products']);

    $h = 0;
    $head = [];

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Categories/viewcat.php?id=' . $object->id . '&amp;type=' . $type;
    $head[$h][1] = $langs->trans("Category");
    $head[$h][2] = 'card';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Categories/photos.php?id=' . $object->id . '&amp;type=' . $type;
    $head[$h][1] = $langs->trans("Photos");
    $head[$h][2] = 'photos';
    $h++;

    if (!empty($conf->global->MAIN_MULTILANGS)) {
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Categories/traduction.php?id=' . $object->id . '&amp;type=' . $type;
        $head[$h][1] = $langs->trans("Translation");
        $head[$h][2] = 'translation';
        $h++;
    }

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Categories/info.php?id=' . $object->id . '&amp;type=' . $type;
...
function categoriesadmin_prepare_head()
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Categories/admin/categorie.php';
    $head[$h][1] = $langs->trans("Setup");
    $head[$h][2] = 'setup';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Categories/admin/categorie_extrafields.php';
