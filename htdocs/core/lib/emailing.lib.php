...
function emailing_prepare_head(Mailing $object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Comm/mailing/card.php?id=" . $object->id;
    ...
    if (empty($conf->global->MAIN_USE_ADVANCED_PERMS) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && $user->rights->mailing->mailing_advance->recipient)) {
        $head[$h][0] = DOL_URL_ROOT . "/Modules/Comm/mailing/cibles.php?id=" . $object->id;
    ...
    if (!empty($conf->global->EMAILING_USE_ADVANCED_SELECTOR)) {
        $head[$h][0] = DOL_URL_ROOT . "/Modules/Comm/mailing/advtargetemailing.php?id=" . $object->id;
        $head[$h][1] = $langs->trans("MailAdvTargetRecipients");
        $head[$h][2] = 'advtargets';
        $h++;
    }

    $head[$h][0] = DOL_URL_ROOT . "/Modules/Comm/mailing/info.php?id=" . $object->id;
