...
function facturefourn_prepare_head($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?facid=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/facture/contact.php?facid=' . $object->id;
    ...
    if (!empty($conf->paymentbybanktransfer->enabled)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/facture/prelevement.php?facid=' . $object->id . '&type=bank-transfer';
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/facture/note.php?facid=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/facture/document.php?facid=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/facture/info.php?facid=' . $object->id;
...
function ordersupplier_prepare_head($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?id=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB)) {
        $nbContact = count($object->liste_contact(-1, 'internal')) + count($object->liste_contact(-1, 'external'));
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/commande/contact.php?id=' . $object->id;
        $head[$h][1] = $langs->trans('ContactsAddresses');
        if ($nbContact > 0) {
            $head[$h][1] .= '<span class="badge marginleftonlyshort">' . $nbContact . '</span>';
        }
    ...
    if (!empty($conf->stock->enabled) && (!empty($conf->global->STOCK_CALCULATE_ON_SUPPLIER_DISPATCH_ORDER) || !empty($conf->global->STOCK_CALCULATE_ON_RECEPTION) || !empty($conf->global->STOCK_CALCULATE_ON_RECEPTION_CLOSE))) {
        $langs->load("stocks");
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/commande/dispatch.php?id=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/commande/note.php?id=' . $object->id;
        $head[$h][1] = $langs->trans("Notes");
        if ($nbNote > 0) {
            $head[$h][1] .= '<span class="badge marginleftonlyshort">' . $nbNote . '</span>';
        }
    ...
    require_once DOL_DOCUMENT_ROOT . '/core/lib/files.lib.php';
    require_once DOL_DOCUMENT_ROOT . '/core/class/link.class.php';
    $upload_dir = $conf->fournisseur->dir_output . "/Modules/Commande/" . dol_sanitizeFileName($object->ref);
    $nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
    $nbLinks = Link::count($db, $object->element, $object->id);
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/commande/document.php?id=' . $object->id;
    $head[$h][1] = $langs->trans('Documents');
    if (($nbFiles + $nbLinks) > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">' . ($nbFiles + $nbLinks) . '</span>';
    }
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Fourn/commande/info.php?id=' . $object->id;
