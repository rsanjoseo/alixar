...
function project_prepare_head(Project $project, $moreparam = '')
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/card.php?id=' . ((int) $project->id) . ($moreparam ? '&' . $moreparam : '');
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/contact.php?id=' . ((int) $project->id) . ($moreparam ? '&' . $moreparam : '');
    ...
    if (empty($conf->global->PROJECT_HIDE_TASKS)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . ((int) $project->id) . ($moreparam ? '&' . $moreparam : '');
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/time.php?withproject=1&projectid=' . ((int) $project->id) . ($moreparam ? '&' . $moreparam : '');
    ...
    if (((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled))
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/element.php?id=' . $project->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/note.php?id=' . $project->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/document.php?id=' . $project->id;
    ...
    if (!empty($conf->global->PROJECT_ALLOW_COMMENT_ON_PROJECT)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/comment.php?id=' . $project->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/info.php?id=' . $project->id;
...
function task_prepare_head($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/task.php?id=' . $object->id . (GETPOST('withproject') ? '&withproject=1' : '');
    $head[$h][1] = $langs->trans("Task");
    $head[$h][2] = 'task_task';
    $h++;

    $nbContact = count($object->liste_contact(-1, 'internal')) + count($object->liste_contact(-1, 'external'));
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/contact.php?id=' . $object->id . (GETPOST('withproject') ? '&withproject=1' : '');
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/time.php?id=' . urlencode($object->id) . (GETPOST('withproject') ? '&withproject=1' : '');
    $head[$h][1] = $langs->trans("TimeSpent");
    if ($nbTimeSpent > 0) {
        $head[$h][1] .= '<span class="badge marginleftonlyshort">...</span>';
    }
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/note.php?id=' . urlencode($object->id) . (GETPOST('withproject') ? '&withproject=1' : '');
        $head[$h][1] = $langs->trans('Notes');
        if ($nbNote > 0) {
            $head[$h][1] .= '<span class="badge marginleftonlyshort">' . $nbNote . '</span>';
        }
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/document.php?id=' . $object->id . (GETPOST('withproject') ? '&withproject=1' : '');
    ...
    if (!empty($conf->global->PROJECT_ALLOW_COMMENT_ON_TASK)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/tasks/comment.php?id=' . $object->id . (GETPOST('withproject') ? '&withproject=1' : '');
        $head[$h][1] = $langs->trans("CommentLink");
        if ($nbComments > 0) {
            $head[$h][1] .= '<span class="badge marginleftonlyshort">' . $nbComments . '</span>';
        }
...
function project_timesheet_prepare_head($mode, $fuser = null)
{
    ...
    if (empty($conf->global->PROJECT_DISABLE_TIMESHEET_PERMONTH)) {
        $head[$h][0] = DOL_URL_ROOT . "/Modules/Projects/activity/permonth.php" . ($param ? '?' . $param : '');
    ...
    if (empty($conf->global->PROJECT_DISABLE_TIMESHEET_PERWEEK)) {
        $head[$h][0] = DOL_URL_ROOT . "/Modules/Projects/activity/perweek.php" . ($param ? '?' . $param : '');
    ...
    if (empty($conf->global->PROJECT_DISABLE_TIMESHEET_PERTIME)) {
        $head[$h][0] = DOL_URL_ROOT . "/Modules/Projects/activity/perday.php" . ($param ? '?' . $param : '');
	...
	{
		$head[$h][0] = DOL_URL_ROOT."/Modules/Projects/activity/perline.php".($param?'?'.$param:'');
...
function project_admin_prepare_head()
{
    ...
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Projects/admin/project.php";
    ...
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Projects/admin/project_extrafields.php";
    $head[$h][1] = $langs->trans("ExtraFieldsProject");
    $head[$h][2] = 'attributes';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/admin/project_task_extrafields.php';
    ...
    if (!empty($conf->global->MAIN_FEATURES_LEVEL) && $conf->global->MAIN_FEATURES_LEVEL >= 2) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Projects/admin/website.php';
 ...
 */
function projectLinesa(&$inc, $parent, &$lines, &$level, $var, $showproject, &$taskrole, $projectsListId = '', $addordertick = 0, $projectidfortotallink = 0, $filterprogresscalc = '', $showbilltime = 0, $arrayfields = [])
{
        ...
        if ($lines[$i]->fk_parent == $parent || $level < 0) {       // if $level = -1, we dont' use sublevel recursion, we show all lines
            ...
            if ($showline) {
                ...
                if (count($arrayfields) > 0 && !empty($arrayfields['t.label']['checked'])) {
                    ...
                    //else print '<a href="'.DOL_URL_ROOT.'/Modules/Projects/tasks/task.php?id='.$lines[$i]->id.'&withproject=1">';
                    for ($k = 0; $k < $level; $k++) {
                ...
                if (count($arrayfields) > 0 && !empty($arrayfields['t.duration_effective']['checked'])) {
                    ...
                    if ($showlineingray) {
                        ...
                        print '<a href="' . DOL_URL_ROOT . '/Modules/Projects/tasks/time.php?id=' . $lines[$i]->id . ($showproject ? '' : '&withproject=1') . '">';
                    }
                    if ($lines[$i]->duration) {
                ...
                // Fields from hook
                $parameters = ['arrayfields' => $arrayfields, 'obj' => $lines[$i]];
    ...
    if (($total_projectlinesa_planned > 0 || $total_projectlinesa_spent > 0 || $total_projectlinesa_tobill > 0 || $total_projectlinesa_billed > 0)
        ...
        if (count($arrayfields) > 0 && !empty($arrayfields['t.duration_effective']['checked'])) {
            ...
            if ($projectidfortotallink > 0) {
                print '<a href="' . DOL_URL_ROOT . '/Modules/Projects/tasks/time.php?projectid=' . $projectidfortotallink . ($showproject ? '' : '&withproject=1') . '">';
            }
            ...
            if (empty($mine) || !empty($tasksrole[$lines[$i]->id])) {
				...
				*/
                ...
                if (!empty($arrayfields['timeconsumed']['checked'])) {
                    ...
                    if ($lines[$i]->duration) {
                        print '<a href="' . DOL_URL_ROOT . '/Modules/Projects/tasks/time.php?id=' . $lines[$i]->id . '">';
 ...
 * @return  array                                Array with time spent for $fuser for each day of week on tasks in $lines and substasks
 */
function projectLinesPerMonth(&$inc, $firstdaytoshow, $fuser, $parent, $lines, &$level, &$projectsrole, &$tasksrole, $mine, $restricteditformytask, &$isavailable, $oldprojectforbreak = 0, $TWeek = [])
{
    global $conf, $db, $user, $langs;
    global $form, $formother, $projectstatic, $taskstatic, $thirdpartystatic;
 ...
 * @return    void
 */
function print_projecttasks_array($db, $form, $socid, $projectsListId, $mytasks = 0, $status = -1, $listofoppstatus = [], $hiddenfields = [])
{
    global $langs, $conf, $user;
    ...
    if ($resql) {
        ...
        print '<tr class="liste_titre">';
        print_liste_field_titre($title . '<a href="' . DOL_URL_ROOT . '/Modules/Projects/list.php?search_status=' . ((int) $status) . '"><span class="badge marginleftonlyshort">' . $num . '</span></a>', $_SERVER["PHP_SELF"], "", "", "", "", $sortfield, $sortorder);
                        ...
                        if (empty($conf->global->USE_COLOR_FOR_PROSPECTION_STATUS)) {
                            ...
                            if ($oppStatusCode) {
                                if (!empty($oppStatusColor)) {
                                    print '<a href="' . dol_buildpath('/Modules/Projects/list.php?search_opp_status=' . $objp->opp_status, 1) . '" style="display: inline-block; width: 4px; border: 5px solid rgb(' . $oppStatusColor . '); border-radius: 2px;" title="' . $langs->trans("OppStatus" . $oppStatusCode) . '"></a>';
                                } else {
                                    print '<a href="' . dol_buildpath('/Modules/Projects/list.php?search_opp_status=' . $objp->opp_status, 1) . '" title="' . $langs->trans("OppStatus" . $oppStatusCode) . '">' . $oppStatusCode . '</a>';
                                }
                            }
                        }
                        print '</td>';
...
function getTaskProgressView($task, $label = true, $progressNumber = true, $hideOnProgressNull = false, $spaced = false)
{
    ...
    if ($progressNumber !== false) {
        ...
        if ($progressNumber !== true) {
            ...
            $url = DOL_URL_ROOT . '/Modules/Projects/tasks/time.php?id=' . $task->id;
