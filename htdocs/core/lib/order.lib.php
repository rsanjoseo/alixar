...
function commande_prepare_head(Commande $object)
{
    ...
    if (!empty($conf->commande->enabled) && $user->rights->commande->lire) {
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Commande/card.php?id=' . $object->id;
    ...
    if (empty($conf->global->MAIN_DISABLE_CONTACTS_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Commande/contact.php?id=' . $object->id;
    ...
    if (($conf->expedition_bon->enabled && $user->rights->expedition->lire)
        ...
        $nbReceiption = 0;
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Expedition/shipment.php?id=' . $object->id;
        $text = '';
    ...
    if (empty($conf->global->MAIN_DISABLE_NOTES_TAB)) {
        ...
        $head[$h][0] = DOL_URL_ROOT . '/Modules/Commande/note.php?id=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Commande/document.php?id=' . $object->id;
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Commande/info.php?id=' . $object->id;
...
function getCustomerOrderPieChart($socid = 0)
{
    ...
    if ($resql) {
        ...
        $listofstatus = [0, 1, 2, 3, -1];
        foreach ($listofstatus as $status) {
            $dataseries[] = [$commandestatic->LibStatut($status, 0, 1, 1), (isset($vals[$status]) ? (int) $vals[$status] : 0)];
