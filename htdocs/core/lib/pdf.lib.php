...
function pdf_getInstance($format = '', $metric = 'mm', $pagetype = 'P')
{
    ...
    if (!empty($conf->global->PDF_SECURITY_ENCRYPTION)) {
		...
		*/
        ...
        $pdfrights = (!empty($conf->global->PDF_SECURITY_ENCRYPTION_RIGHTS) ? json_decode($conf->global->PDF_SECURITY_ENCRYPTION_RIGHTS, true) : ['modify', 'copy']); // Json format in llx_const
...
function pdf_build_address($outputlangs, $sourcecompany, $targetcompany = '', $targetcontact = '', $usecontact = 0, $mode = 'source', $object = null)
{
    ...
    if (is_object($hookmanager)) {
        $parameters = ['sourcecompany' => &$sourcecompany, 'targetcompany' => &$targetcompany, 'targetcontact' => &$targetcontact, 'outputlangs' => $outputlangs, 'mode' => $mode, 'usecontact' => $usecontact];
        $action = '';
...
function pdf_writelinedesc(&$pdf, $object, $i, $outputlangs, $w, $h, $posx, $posy, $hideref = 0, $hidedesc = 0, $issupplierline = 0)
{
    ...
    if (is_object($hookmanager)) {   // Old code is commented on preceding line. Reproduct this test in the pdf_xxx function if you don't want your hook to run
        ...
        $parameters = ['pdf' => $pdf, 'i' => $i, 'outputlangs' => $outputlangs, 'w' => $w, 'h' => $h, 'posx' => $posx, 'posy' => $posy, 'hideref' => $hideref, 'hidedesc' => $hidedesc, 'issupplierline' => $issupplierline, 'special_code' => $special_code];
        $action = '';
...
function pdf_getlinedesc($object, $i, $outputlangs, $hideref = 0, $hidedesc = 0, $issupplierline = 0)
{
    ...
    if ($issupplierline) {
        ...
        if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
            include_once DOL_DOCUMENT_ROOT . '/Modules/Products/class/productcustomerprice.class.php';
        }
    }
    ...
    if (empty($hideref)) {
        if ($issupplierline) {
            ...
            if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
                ...
                $filter = ['fk_product' => $idprod, 'fk_soc' => $object->socid];
...
function pdf_getlinenum($object, $i, $outputlangs, $hidedetails = 0)
{
    ...
    if (is_object($hookmanager)) {   // Old code is commented on preceding line. Reproduct this test in the pdf_xxx function if you don't want your hook to run
        ...
        $parameters = ['i' => $i, 'outputlangs' => $outputlangs, 'hidedetails' => $hidedetails, 'special_code' => $special_code];
        $action = '';
...
function pdf_getlinetotalexcltax($object, $i, $outputlangs, $hidedetails = 0)
{
    ...
    if (is_object($hookmanager)) {   // Old code is commented on preceding line. Reproduct this test in the pdf_xxx function if you don't want your hook to run
        ...
        $parameters = ['i' => $i, 'outputlangs' => $outputlangs, 'hidedetails' => $hidedetails, 'special_code' => $special_code, 'sign' => $sign];
        $action = '';
...
function pdf_getLinkedObjects($object, $outputlangs)
{
    ...
    foreach ($object->linkedObjects as $objecttype => $objects) {
        if ($objecttype == 'facture') {
            ...
            $outputlangs->loadLangs(["orders", "sendings"]);
    ...
    if (is_object($hookmanager)) {
        $parameters = ['linkedobjects' => $linkedobjects, 'outputlangs' => $outputlangs];
        $action = '';
