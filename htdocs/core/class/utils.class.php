...
class Utils
{
        ...
        foreach ($choicesarray as $choice) {
            ...
            if ($choice == 'logfile' || $choice == 'logfiles') {
                ...
                if (!empty($conf->syslog->enabled)) {
                    ...
                    if (!$alreadyincluded) {
                        $filesarray[] = ['fullname' => $filelog, 'type' => 'file'];
                    }
                }
            }
            ...
            if (is_array($filesarray) && count($filesarray)) {
                foreach ($filesarray as $key => $value) {
                    ...
                    if ($filesarray[$key]['type'] == 'dir') {
                        ...
                        if (!in_array($filesarray[$key]['fullname'], [$conf->api->dir_temp, $conf->user->dir_temp])) {        // The 2 directories $conf->api->dir_temp and $conf->user->dir_temp are recreated at end, so we do not count them
                            $count += $result;
    ...
    public function dumpDatabase($compression = 'none', $type = 'auto', $usedefault = 1, $file = 'auto', $keeplastnfiles = 0, $execmethod = 0)
    {
        ...
        if (!in_array($compression, ['none', 'gz', 'bz', 'zip'])) {
            $langs->load("errors");
        ...
        if (!in_array($type, ['postgresql', 'pgsql', 'mysql', 'mysqli', 'mysqlnobin'])) {
            $langs->load("errors");
        ...
        if ($type == 'mysql' || $type == 'mysqli') {
            ...
            $this->error = $errormsg;
            $this->result = ["commandbackuplastdone" => $command . " " . $paramcrypted, "commandbackuptorun" => ""];
        ...
        if ($type == 'mysqlnobin') {
            ...
            $this->output = "";
            $this->result = ["commandbackuplastdone" => "", "commandbackuptorun" => ""];
        }
        ...
        if ($type == 'postgresql' || $type == 'pgsql') {
            ...
            $this->output = "";
            $this->result = ["commandbackuplastdone" => "", "commandbackuptorun" => $command . " " . $paramcrypted];
        }
...
";
    ...
    public function generateDoc($module)
    {
        ...
        if (count($arrayversion)) {
            ...
            if ($dirofmoduledoc) {
                ...
                if ($fhandle) {
                    $specs = dol_dir_list(dol_buildpath(strtolower($module) . '/doc', 0), 'files', 1, '(\.md|\.asciidoc)$', ['\/temp\/']);
    ...
    public function compressSyslogs()
    {
        ...
        $tabfiles[] = ['name' => $mainlog, 'path' => $mainlogdir];
