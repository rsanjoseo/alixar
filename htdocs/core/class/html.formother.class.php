    ...
    public function selectGroupByField($object, $search_groupby, &$arrayofgroupby, $morecss = 'minwidth200 maxwidth250')
    {
        ...
        foreach ($object->fields as $key => $val) {
            if (!$val['measure']) {
                ...
                if (in_array($val['type'], ['timestamp', 'date', 'datetime'])) {
                    $arrayofgroupby['t.' . $key . '-year'] = ['label' => $langs->trans($val['label']) . ' <span class="opacitymedium">(' . $YYYY . ')</span>', 'position' => $val['position'] . '-y'];
                    $arrayofgroupby['t.' . $key . '-month'] = ['label' => $langs->trans($val['label']) . ' <span class="opacitymedium">(' . $YYYY . '-' . $MM . ')</span>', 'position' => $val['position'] . '-m'];
                    $arrayofgroupby['t.' . $key . '-day'] = ['label' => $langs->trans($val['label']) . ' <span class="opacitymedium">(' . $YYYY . '-' . $MM . '-' . $DD . ')</span>', 'position' => $val['position'] . '-d'];
                } else {
                    $arrayofgroupby['t.' . $key] = ['label' => $langs->trans($val['label']), 'position' => (int) $val['position']];
                }
            }
        }
        ...
        if ($object->isextrafieldmanaged) {
            foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
                ...
                $arrayofgroupby['te.' . $key] = ['label' => $langs->trans($extrafields->attributes[$object->table_element]['label'][$key]), 'position' => 1000 + (int) $extrafields->attributes[$object->table_element]['pos'][$key]];
            }
        }
    ...
    public function selectXAxisField($object, $search_xaxis, &$arrayofxaxis)
    {
        ...
        foreach ($object->fields as $key => $val) {
            if (!$val['measure']) {
                ...
                if (in_array($val['type'], ['timestamp', 'date', 'datetime'])) {
                    $arrayofxaxis['t.' . $key . '-year'] = ['label' => $langs->trans($val['label']) . ' (' . $YYYY . ')', 'position' => $val['position'] . '-y'];
                    $arrayofxaxis['t.' . $key . '-month'] = ['label' => $langs->trans($val['label']) . ' (' . $YYYY . '-' . $MM . ')', 'position' => $val['position'] . '-m'];
                    $arrayofxaxis['t.' . $key . '-day'] = ['label' => $langs->trans($val['label']) . ' (' . $YYYY . '-' . $MM . '-' . $DD . ')', 'position' => $val['position'] . '-d'];
                } else {
                    $arrayofxaxis['t.' . $key] = ['label' => $langs->trans($val['label']), 'position' => (int) $val['position']];
                }
            }
        }
        ...
        if ($object->isextrafieldmanaged) {
            foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
                ...
                $arrayofxaxis['te.' . $key] = ['label' => $langs->trans($extrafields->attributes[$object->table_element]['label'][$key]), 'position' => 1000 + (int) $extrafields->attributes[$object->table_element]['pos'][$key]];
            }
        }
