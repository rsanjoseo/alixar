...
class DolGraph
{
    ...
    public function __construct($library = 'auto')
    {
        ...
        $this->bordercolor = [235, 235, 224];
        $this->datacolor = [[120, 130, 150], [160, 160, 180], [190, 190, 220]];
        $this->bgcolor = [235, 235, 224];
     ...
     * @return    void
     */
    public function SetBgColor($bg_color = [255, 255, 255])
    {
        // phpcs:enable
     ...
     * @return    void
     */
    public function SetBgColorGrid($bg_colorgrid = [255, 255, 255])
    {
        // phpcs:enable
        global $theme_bgcolor, $theme_bgcoloronglet;
    ...
    public function draw($file, $fileurl = '')
    {
        ...
        call_user_func_array([$this, $call], [$file, $fileurl]);
    }
    ...
    private function draw_jflot($file, $fileurl)
    {
        ...
        while ($i < $nblot) {    // Loop on each serie
            $values = []; // Array with horizontal y values (specific values of a serie) for each abscisse x
            $serie[$i] = "var d" . $i . " = [];\n";
            ...
            if (isset($this->type[$firstlot]) && in_array($this->type[$firstlot], ['pie', 'piesemicircle', 'polar'])) {
        ...
        $tag = dol_escape_htmltag(dol_string_unaccent(dol_string_nospecial(basename($file), '_', ['-', '.'])));
        ...
        if (isset($this->type[$firstlot]) && in_array($this->type[$firstlot], ['pie', 'piesemicircle', 'polar'])) {
            ...
            foreach ($this->datacolor as $val) {
                if (is_array($val)) {
                    ...
                    $datacolor[] = "#" . str_replace(['#', '-'], '', $val); // If $val is '124' or '#124'
                }
            }
    ...
    private function draw_chart($file, $fileurl)
    {
        ...
        $i = $firstlot;
        while ($i < $nblot) {    // Loop on each serie
            $values = []; // Array with horizontal y values (specific values of a serie) for each abscisse x (with x=0,1,2,...)
            $serie[$i] = "";
        ...
        if (isset($this->type[$firstlot]) && (in_array($this->type[$firstlot], ['pie', 'polar', 'piesemicircle']))) {
            ...
            $this->stringtoshow .= '
    			...
    			type: \'' . (in_array($type, ['pie', 'piesemicircle']) ? 'doughnut' : 'polarArea') . '\',
