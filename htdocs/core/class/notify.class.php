...
class Notify
{
     ...
     */
    public function getNotificationsArray($notifcode, $socid = 0, $object = null, $userid = 0, $scope = ['thirdparty', 'user', 'global'])
    {
        global $conf, $user;
                    ...
                    while ($i < $num) {
                        ...
                        if ($obj) {
                            ...
                            if (empty($resarray[$newval2])) {
                                $resarray[$newval2] = ['type' => 'tocontact', 'code' => trim($obj->code), 'emaildesc' => 'Contact id ' . $obj->rowid, 'email' => $newval2, 'contactid' => $obj->rowid, 'isemailvalid' => $isvalid];
                            }
                        }
                        $i++;
                    }
                ...
                if ($resql) {
                    ...
                    while ($i < $num) {
                        ...
                        if ($obj) {
                            ...
                            $isvalid = isValidEmail($newval2);
                            if (empty($resarray[$newval2])) {
                                $resarray[$newval2] = ['type' => 'touser', 'code' => trim($obj->code), 'emaildesc' => 'User id ' . $obj->rowid, 'email' => $newval2, 'userid' => $obj->rowid, 'isemailvalid' => $isvalid];
                            }
                        }
                        $i++;
                    }
                ...
                foreach ($conf->global as $key => $val) {
                    ...
                    foreach ($tmpemail as $key2 => $val2) {
                        ...
                        if ($newval2) {
                            ...
                            if (empty($resarray[$newval2])) {
                                $resarray[$newval2] = ['type' => 'tofixedemail', 'code' => trim($key), 'emaildesc' => trim($val2), 'email' => $newval2, 'isemailvalid' => $isvalid];
                            }
                        }
                    }
                }
    ...
    public function send($notifcode, $object, $filename_list = [], $mimetype_list = [], $mimefilename_list = [])
    {
        ...
        $hookmanager->initHooks(['notification']);
                ...
                while ($i < $num && !$error) {    // For each notification couple defined (third party/actioncode)
                    ...
                    if (dol_strlen($obj->email)) {
                        ...
                        if ($obj->default_lang && $obj->default_lang != $langs->defaultlang) {
                            ...
                            $outputlangs->loadLangs(["main", "other"]);
                        }
                        ...
                        $parameters = ['notifcode' => $notifcode, 'sendto' => $sendto, 'replyto' => $replyto, 'file' => $filename_list, 'mimefile' => $mimetype_list, 'filename' => $mimefilename_list];
                        if (!isset($action)) {
                            $action = '';
                        }
