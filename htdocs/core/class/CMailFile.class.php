...
class CMailFile
{
     ...
     */
    public function __construct($subject, $to, $from, $msg, $filename_list = [], $mimetype_list = [], $mimefilename_list = [], $addr_cc = "", $addr_bcc = "", $deliveryreceipt = 0, $msgishtml = 0, $errors_to = '', $css = '', $trackid = '', $moreinheader = '', $sendcontext = 'standard', $replyto = '')
    {
        ...
        if ($this->sendmode == 'mail') {
            ...
            if (!empty($conf->global->$keyforsslseflsigned)) {
                $smtps->setOptions(['ssl' => ['verify_peer' => false, 'verify_peer_name' => false, 'allow_self_signed' => true]]);
            }
    ...
    public function sendfile()
    {
        ...
        if (empty($conf->global->MAIN_DISABLE_ALL_MAILS)) {
            ...
            if ($this->sendmode == 'mail') {
                ...
                if (!empty($conf->global->$keyforsslseflsigned)) {
                    $this->transport->setStreamOptions(['ssl' => ['allow_self_signed' => true, 'verify_peer' => false]]);
                }
