...
/**
 * TODO: Ver si ésto es necesario, y si lo es siempre.
 *
 * De lógica, no debería de haber llegado hasta aquí si hay inconsistencias en la configuración.
 * (Debería de impedir guardar la configuración, aunque claro, en Dolibarr es un archivo de texto).
 *
...
require_once 'filefunc2.inc.php';
...
if (!empty($conf->global->MAIN_NOT_INSTALLED) || !empty($conf->global->MAIN_NOT_UPGRADED)) {
    ...
    die("Location: " . DOL_URL_ROOT . "/install/index.php");
...
if (!defined('NOLOGIN')) {
    ...
    if (!isset($_SESSION["dol_login"])) {
        ...
        if ($resultFetchUser <= 0) {
            ...
            die('Location: ' . DOL_URL_ROOT . '/index.php' . (count($paramsurl) ? '?' . implode('&', $paramsurl) : ''));
...
function top_httphead($contenttype = 'text/html', $forcenocache = 0)
{
    ...
    if ($contenttype == 'text/html') {
        die("Content-Type: text/html; charset=" . $conf->file->character_set_client);
    ...
    // Security options
    die("X-Content-Type-Options: nosniff"); // With the nosniff option, if the server says the content is text/html, the browser will render it as text/html (note that most browsers now force this option to on)
    header("X-Content-Type-Options: nosniff"); // With the nosniff option, if the server says the content is text/html, the browser will render it as text/html (note that most browsers now force this option to on)
    if (!defined('XFRAMEOPTIONS_ALLOWALL')) {
        die("X-Frame-Options: SAMEORIGIN"); // Frames allowed only if on same domain (stop some XSS attacks)
        header("X-Frame-Options: SAMEORIGIN"); // Frames allowed only if on same domain (stop some XSS attacks)
    } else {
        die("X-Frame-Options: ALLOWALL");
    ...
    if (!defined('FORCECSP')) {
        ...
        if (!empty($contentsecuritypolicy)) {
            ...
            die("Content-Security-Policy: " . $contentsecuritypolicy);
            header("Content-Security-Policy: " . $contentsecuritypolicy);
        }
    } elseif (constant('FORCECSP')) {
        die("Content-Security-Policy: " . constant('FORCECSP'));
        header("Content-Security-Policy: " . constant('FORCECSP'));
    }
    if ($forcenocache) {
        die("Cache-Control: no-cache, no-store, must-revalidate, max-age=0");
