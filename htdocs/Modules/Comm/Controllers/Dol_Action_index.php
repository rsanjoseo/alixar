...
if (GETPOST("viewlist", 'alpha') || $action == 'show_list') {
    ...
    //print $param;
    header("Location: " . DOL_URL_ROOT . '/Modules/Comm/action/list.php?' . $param);
    exit;
}
...
if (GETPOST("viewperuser", 'alpha') || $action == 'show_peruser') {
    ...
    //print $param;
    header("Location: " . DOL_URL_ROOT . '/Modules/Comm/action/peruser.php?' . $param);
    exit;
}
...
$viewmode .= '<a class="btnTitle' . ($action == 'show_month' ? ' btnTitleSelected' : '') . ' reposition" href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_month&year=' . (isset($object->datep) ? dol_print_date($object->datep, '%Y') : $year) . '&month=' . (isset($object->datep) ? dol_print_date($object->datep, '%m') : $month) . '&day=' . (isset($object->datep) ? dol_print_date($object->datep, '%d') : $day) . $paramnoactionodate . '">';
//$viewmode .= '<span class="fa paddingleft imgforviewmode valignmiddle btnTitle-icon">';
...
$viewmode .= '<a class="btnTitle' . ($action == 'show_week' ? ' btnTitleSelected' : '') . ' reposition" href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_week&year=' . (isset($object->datep) ? dol_print_date($object->datep, '%Y') : $year) . '&month=' . (isset($object->datep) ? dol_print_date($object->datep, '%m') : $month) . '&day=' . (isset($object->datep) ? dol_print_date($object->datep, '%d') : $day) . $paramnoactionodate . '">';
//$viewmode .= '<span class="fa paddingleft imgforviewmode valignmiddle btnTitle-icon">';
...
$viewmode .= '<a class="btnTitle' . ($action == 'show_day' ? ' btnTitleSelected' : '') . ' reposition" href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_day&year=' . (isset($object->datep) ? dol_print_date($object->datep, '%Y') : $year) . '&month=' . (isset($object->datep) ? dol_print_date($object->datep, '%m') : $month) . '&day=' . (isset($object->datep) ? dol_print_date($object->datep, '%d') : $day) . $paramnoactionodate . '">';
//$viewmode .= '<span class="fa paddingleft imgforviewmode valignmiddle btnTitle-icon">';
...
$viewmode .= '<a class="btnTitle reposition" href="' . DOL_URL_ROOT . '/Modules/Comm/action/peruser.php?action=show_peruser&year=' . (isset($object->datep) ? dol_print_date($object->datep, '%Y') : $year) . '&month=' . (isset($object->datep) ? dol_print_date($object->datep, '%m') : $month) . '&day=' . (isset($object->datep) ? dol_print_date($object->datep, '%d') : $day) . $paramnoactionodate . '">';
//$viewmode .= '<span class="fa paddingleft imgforviewmode valignmiddle btnTitle-icon">';
...
if (count($listofextcals)) {
    require_once DOL_DOCUMENT_ROOT . '/Modules/Comm/action/class/ical.class.php';
...
function show_day_events($db, $day, $month, $year, $monthshown, $style, &$eventarray, $maxprint = 0, $maxnbofchar = 16, $newparam = '', $showinfo = 0, $minheight = 60, $nonew = 0)
{
    ...
    $urltoshow = DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_day&day=' . str_pad($day, 2, "0", STR_PAD_LEFT) . '&month=' . str_pad($month, 2, "0", STR_PAD_LEFT) . '&year=' . $year . $newparam;
    $urltocreate = '';
    if ($user->rights->agenda->myactions->create || $user->rights->agenda->allactions->create) {
        $newparam .= '&month=' . str_pad($month, 2, "0", STR_PAD_LEFT) . '&year=' . $year;
        $hourminsec = '100000';
        $urltocreate = DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&datep=' . sprintf("%04d%02d%02d", $year, $month, $day) . $hourminsec . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . ($newparam ? '?' . $newparam : ''));
    }
    ...
    include_once DOL_DOCUMENT_ROOT . '/Modules/Holiday/class/holiday.class.php';
        ...
        if ($day == $jour && $month == $mois && $year == $annee) {
            foreach ($eventarray[$daykey] as $index => $event) {
                if ($i < $maxprint || $maxprint == 0 || !empty($conf->global->MAIN_JS_SWITCH_AGENDA)) {
                    ...
                    print '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=' . $action . '&maxprint=0&month=' . $monthshown . '&year=' . $year;
