...
$arrayfields = [
    ...
    'a.fk_element' => ['label' => "LinkedObject", 'checked' => 1, 'enabled' => (!empty($conf->global->AGENDA_SHOW_LINKED_OBJECT))],
...
if (GETPOST("viewcal") || GETPOST("viewweek") || GETPOST("viewday")) {
    ...
    //print $param;
    header("Location: " . DOL_URL_ROOT . '/Modules/Comm/action/index.php?' . $param);
    exit;
}
...
$viewmode = '';
$viewmode .= '<a class="btnTitle btnTitleSelected reposition" href="' . DOL_URL_ROOT . '/Modules/Comm/action/list.php?action=show_list&restore_lastsearch_values=1' . $paramnoactionodate . '">';
...
$viewmode .= '<a class="btnTitle reposition marginrightonly" href="' . DOL_URL_ROOT . '/Modules/Comm/action/peruser.php?action=show_peruser&year=' . dol_print_date($object->datep, '%Y') . '&month=' . dol_print_date($object->datep, '%m') . '&day=' . dol_print_date($object->datep, '%d') . $paramnoactionodate . '">';
//$viewmode .= '<span class="fa paddingleft imgforviewmode valignmiddle btnTitle-icon">';
...
$url = DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create';
