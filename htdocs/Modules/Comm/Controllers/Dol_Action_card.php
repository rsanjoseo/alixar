...
require_once DOL_DOCUMENT_ROOT . '/Modules/Comm/action/class/actioncommreminder.class.php';
...
if (!empty($conf->global->AGENDA_REMINDER_BROWSER)) {
    $TRemindTypes['browser'] = ['label' => $langs->trans('BrowserPush'), 'disabled' => (empty($conf->global->AGENDA_REMINDER_BROWSER) ? 1 : 0)];
}
if (!empty($conf->global->AGENDA_REMINDER_EMAIL)) {
    $TRemindTypes['email'] = ['label' => $langs->trans('EMail'), 'disabled' => (empty($conf->global->AGENDA_REMINDER_EMAIL) ? 1 : 0)];
}
...
if (empty($reshook) && (GETPOST('addassignedtouser') || GETPOST('updateassignedtouser'))) {
    ...
    if (GETPOST('assignedtouser') > 0) {
        ...
        $assignedtouser[GETPOST('assignedtouser')] = ['id' => GETPOST('assignedtouser'), 'transparency' => GETPOST('transparency'), 'mandatory' => 1];
...
if (empty($reshook) && $action == 'add') {
    ...
    if (empty($backtopage)) {
        if ($socid > 0) {
            $backtopage = DOL_URL_ROOT . '/Modules/Societes/agenda.php?socid=' . $socid;
        } else {
            $backtopage = DOL_URL_ROOT . '/Modules/Comm/action/index.php';
        }
    }
        ...
        if ($idaction > 0) {
            if (!$object->error) {
                ...
                if (!empty($backtopage)) {
                    ...
                    header("Location: " . DOL_URL_ROOT . '/Modules/Comm/action/card.php?id=' . $idaction . ($moreparam ? '&' . $moreparam : ''));
                } else {
                    header("Location: " . DOL_URL_ROOT . '/Modules/Comm/action/index.php' . ($moreparam ? '?' . $moreparam : ''));
                }
                exit;
            } else {
                // If error
...
if ($action == 'create') {
    ...
    if (empty($donotclearsession)) {
        ...
        if ($assignedtouser) {
            $listofuserid[$assignedtouser] = ['id' => $assignedtouser, 'mandatory' => 0, 'transparency' => $object->transparency]; // Owner first
        }
    ...
    if ($conf->societe->enabled) {
        ...
        print $form->selectcontacts(GETPOST('socid', 'int'), $preselectedids, 'socpeopleassigned[]', 1, '', '', 0, 'minwidth300 quatrevingtpercent', false, 0, [], false, 'multiple', 'contactid');
        print '</td></tr>';
    }
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print '&nbsp;<a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . (empty($societe->id) ? '' : $societe->id) . '&action=create&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create') . '">';
        ...
        print '$(document).ready(function () {
	               $("#projectid").change(function () {
                        var url = "' . DOL_URL_ROOT . '/Modules/Projects/ajax/projects.php?mode=gettasks&socid="+$("#projectid").val()+"&projectid="+$("#projectid").val();
...
if ($id > 0) {
    ...
    if ($action == 'edit') {
        ...
        if ($conf->societe->enabled) {
            ...
            print '<div class="maxwidth200onsmartphone">';
            print img_picto('', 'contact', 'class="paddingrightonly"') . $form->selectcontacts($object->socid, array_keys($object->socpeopleassigned), 'socpeopleassigned[]', 1, '', '', 1, 'quatrevingtpercent', false, 0, 0, [], 'multiple', 'contactid');
            print '</div>';
            print '</td>';
            print '</tr>';
        }
        ...
        if (!empty($conf->projet->enabled)) {
            ...
            if ($numprojet == 0) {
                print ' &nbsp; <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $object->socid . '&action=create&token=' . newToken() . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=edit') . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddProject") . '"></span></a>';
            }
            print '</td></tr>';
        }
        ...
        if (!empty($object->fk_element) && !empty($object->elementtype)) {
            ...
            if ($object->elementtype == 'task' && !empty($conf->projet->enabled)) {
                ...
                $url = DOL_URL_ROOT . '/Modules/Comm/action/card.php' . $urloption;
        ...
        $linkback .= '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/list.php?action=show_list&restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
        $linkback .= '</li>';
        ...
        $linkback .= '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_month&year=' . dol_print_date($object->datep, '%Y') . '&month=' . dol_print_date($object->datep, '%m') . '&day=' . dol_print_date($object->datep, '%d') . '">' . $langs->trans("ViewCal") . '</a>';
        $linkback .= '</li>';
        $linkback .= '<li class="noborder litext">';
        $linkback .= img_picto($langs->trans("ViewWeek"), 'object_calendarweek', 'class="hideonsmartphone pictoactionview"');
        $linkback .= '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_week&year=' . dol_print_date($object->datep, '%Y') . '&month=' . dol_print_date($object->datep, '%m') . '&day=' . dol_print_date($object->datep, '%d') . '">' . $langs->trans("ViewWeek") . '</a>';
        $linkback .= '</li>';
        $linkback .= '<li class="noborder litext">';
        $linkback .= img_picto($langs->trans("ViewDay"), 'object_calendarday', 'class="hideonsmartphone pictoactionview"');
        $linkback .= '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/index.php?action=show_day&year=' . dol_print_date($object->datep, '%Y') . '&month=' . dol_print_date($object->datep, '%m') . '&day=' . dol_print_date($object->datep, '%d') . '">' . $langs->trans("ViewDay") . '</a>';
        $linkback .= '</li>';
        $linkback .= '<li class="noborder litext">';
        $linkback .= img_picto($langs->trans("ViewPerUser"), 'object_calendarperuser', 'class="hideonsmartphone pictoactionview"');
        $linkback .= '<a href="' . DOL_URL_ROOT . '/Modules/Comm/action/peruser.php?action=show_peruser&year=' . dol_print_date($object->datep, '%Y') . '&month=' . dol_print_date($object->datep, '%m') . '&day=' . dol_print_date($object->datep, '%d') . '">' . $langs->trans("ViewPerUser") . '</a>';
