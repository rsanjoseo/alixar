...
/**
 *      \file       htdocs/Modules/Categories/index.php
 *      \ingroup    category
...
$hookmanager->initHooks(['categoryindex']);
...
$arrayofjs = ['/includes/jquery/plugins/jquerytreeview/jquery.treeview.js', '/includes/jquery/plugins/jquerytreeview/lib/jquery.cookie.js'];
...
if (!empty($user->rights->categorie->creer)) {
    $newcardbutton .= dolGetButtonTitle($langs->trans('NewCategory'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Categories/card.php?action=create&type=' . $type . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?type=' . $type . $moreparam) . $moreparam);
}
...
foreach ($fulltree as $key => $val) {
    ...
    $entry .= '<a href="' . DOL_URL_ROOT . '/Modules/Categories/viewcat.php?id=' . $val['id'] . '&type=' . $type . $moreparam . '&backtolist=' . urlencode($_SERVER["PHP_SELF"] . '?type=' . $type) . '">' . img_view() . '</a>';
    $entry .= '</td>';
    $entry .= '<td class="right" width="20px;">';
    if ($user->rights->categorie->creer) {
        $entry .= '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Categories/edit.php?id=' . $val['id'] . '&type=' . $type . $moreparam . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?type=' . $type) . '">' . img_edit() . '</a>';
    }
    $entry .= '</td>';
    $entry .= '<td class="right" width="20px;">';
    if ($user->rights->categorie->supprimer) {
        $entry .= '<a class="deletefilelink" href="' . DOL_URL_ROOT . '/Modules/Categories/viewcat.php?action=delete&token=' . newToken() . '&id=' . $val['id'] . '&type=' . $type . $moreparam . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?type=' . $type . $moreparam) . '&backtolist=' . urlencode($_SERVER["PHP_SELF"] . '?type=' . $type . $moreparam) . '">' . img_delete() . '</a>';
    }
    $entry .= '</td>';
    ...
    $data[] = ['rowid' => $val['rowid'], 'fk_menu' => $val['fk_parent'], 'entry' => $entry];
}
