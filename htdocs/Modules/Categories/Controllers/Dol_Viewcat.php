...
/**
 *       \file       htdocs/Modules/Categories/viewcat.php
 *       \ingroup    category
...
$hookmanager->initHooks(['categorycard', 'globalcard']);
...
if ($user->rights->categorie->supprimer && $action == 'confirm_delete' && $confirm == 'yes') {
    if ($object->delete($user) >= 0) {
        if ($backtopage) {
            ...
            header("Location: " . DOL_URL_ROOT . '/Modules/Categories/index.php?type=' . $type);
            exit;
        }
    } else {
...
if (!empty($user->rights->categorie->creer)) {
    $link = DOL_URL_ROOT . '/Modules/Categories/card.php';
    $link .= '?action=create';
    $link .= '&type=' . $type;
...
if ($cats < 0) {
    ...
    foreach ($fulltree as $key => $val) {
        ...
        $entry .= '<a href="' . DOL_URL_ROOT . '/Modules/Categories/viewcat.php?id=' . $val['id'] . '&type=' . $type . '&backtolist=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $id . '&type=' . $type) . '">' . img_view() . '</a>';
        $entry .= '</td>';
        $entry .= '<td class="right" width="20px;">';
        $entry .= '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Categories/edit.php?id=' . $val['id'] . '&type=' . $type . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $id . '&type=' . $type) . '">' . img_edit() . '</a>';
        $entry .= '</td>';
        $entry .= '<td class="right" width="20px;">';
        $entry .= '<a class="deletefilelink" href="' . DOL_URL_ROOT . '/Modules/Categories/viewcat.php?action=delete&token=' . newToken() . '&id=' . $val['id'] . '&type=' . $type . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $id . '&type=' . $type) . '&backtolist=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $id . '&type=' . $type) . '">' . img_delete() . '</a>';
        $entry .= '</td>';
