...
$langsLoad = ['projects', 'bills', 'orders'];
...
if (!empty($project_ref) && !empty($withproject)) {
    if ($projectstatic->fetch(0, $project_ref) > 0) {
        ...
        if (count($tasksarray) > 0) {
            ...
            header("Location: " . DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $projectstatic->id . ($withproject ? '&withproject=1' : '') . (empty($mode) ? '' : '&mode=' . $mode));
            exit;
        }
    }
}
...
if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0) {
    ...
    if (empty($projectidforalltimes)) {
        ...
        $linkback = $withproject ? '<a href="' . DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $projectstatic->id . '">' . $langs->trans("BackToList") . '</a>' : '';
    ...
    if ($projectstatic->id > 0) {
        ...
        $arrayfields = [];
        $arrayfields['t.task_date'] = ['label' => $langs->trans("Date"), 'checked' => 1];
        if ((empty($id) && empty($ref)) || !empty($projectidforalltimes)) {    // Not a dedicated task
            $arrayfields['t.task_ref'] = ['label' => $langs->trans("RefTask"), 'checked' => 1];
            $arrayfields['t.task_label'] = ['label' => $langs->trans("LabelTask"), 'checked' => 1];
        }
        $arrayfields['author'] = ['label' => $langs->trans("By"), 'checked' => 1];
        $arrayfields['t.note'] = ['label' => $langs->trans("Note"), 'checked' => 1];
        $arrayfields['t.task_duration'] = ['label' => $langs->trans("Duration"), 'checked' => 1];
        $arrayfields['value'] = ['label' => $langs->trans("Value"), 'checked' => 1, 'enabled' => (empty($conf->salaries->enabled) ? 0 : 1)];
        $arrayfields['valuebilled'] = ['label' => $langs->trans("Billed"), 'checked' => 1, 'enabled' => (((!empty($conf->global->PROJECT_HIDE_TASKS) || empty($conf->global->PROJECT_BILL_TIME_SPENT)) ? 0 : 1) && $projectstatic->usage_bill_time)];
        // Extra fields
        ...
        foreach ($tasks as $task_time) {
            ...
            */
            ...
            if ($action == 'splitline' && $_GET['lineid'] == $task_time->rowid) {
                ...
                // Fields from hook
                $parameters = ['arrayfields' => $arrayfields, 'obj' => $task_time];
                $reshook = $hookmanager->executeHooks('printFieldListValue', $parameters); // Note that $action and $object may have been modified by hook
                print $hookmanager->resPrint;
