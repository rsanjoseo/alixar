...
if ($action == 'confirm_delete' && $confirm == "yes" && $user->rights->projet->supprimer) {
    ...
    if ($object->delete($user) > 0) {
        header('Location: ' . DOL_URL_ROOT . '/Modules/Projects/tasks.php?restore_lastsearch_values=1&id=' . $projectstatic->id . ($withproject ? '&withproject=1' : ''));
        exit;
    } else {
        setEventMessages($object->error, $object->errors, 'errors');
        $action = '';
...
if (!empty($project_ref) && !empty($withproject)) {
    if ($projectstatic->fetch('', $project_ref) > 0) {
        ...
        if (count($tasksarray) > 0) {
            ...
            header("Location: " . DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $projectstatic->id . (empty($mode) ? '' : '&mode=' . $mode));
        }
    }
}
...
if ($id > 0 || !empty($ref)) {
    ...
    */
    ...
    if ($action == 'edit' && $user->rights->projet->creer) {
        ...
        $linkback = $withproject ? '<a href="' . DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $projectstatic->id . '&restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>' : '';
