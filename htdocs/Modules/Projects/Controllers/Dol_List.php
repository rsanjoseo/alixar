...
$arrayfields['s.nom'] = ['label' => $langs->trans("ThirdParty"), 'checked' => 1, 'position' => 21, 'enabled' => (empty($conf->societe->enabled) ? 0 : 1)];
$arrayfields['commercial'] = ['label' => $langs->trans("SaleRepresentativesOfThirdParty"), 'checked' => 0, 'position' => 23];
$arrayfields['opp_weighted_amount'] = ['label' => $langs->trans('OpportunityWeightedAmountShort'), 'checked' => 0, 'position' => 116, 'enabled' => (empty($conf->global->PROJECT_USE_OPPORTUNITIES) ? 0 : 1), 'position' => 106];
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all) {
    ...
    header("Location: " . DOL_URL_ROOT . '/Modules/Projects/card.php?id=' . $obj->id);
    exit;
}
...
$url = DOL_URL_ROOT . '/Modules/Projects/card.php?action=create';
if (!empty($socid)) {
    $url .= '&socid=' . $socid;
}
