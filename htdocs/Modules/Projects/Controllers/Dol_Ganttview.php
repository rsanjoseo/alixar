...
if (!empty($conf->use_javascript_ajax)) {
    $arrayofjs = [
        ...
        '/Modules/Projects/jsgantt_language.js.php?lang=' . $langs->defaultlang,
    ];
}
...
$linktocreatetask = dolGetButtonTitle($langs->trans('AddTask'), '', 'fa fa-plus-circle paddingleft', DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $object->id . '&action=create' . $param . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id), '', $linktocreatetaskUserRight, $linktocreatetaskParam);

$linktotasks = dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-list-alt paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $object->id, '', 1, ['morecss' => 'reposition']);
$linktotasks .= dolGetButtonTitle($langs->trans('ViewGantt'), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Projects/ganttview.php?id=' . $object->id . '&withproject=1', '', 1, ['morecss' => 'reposition marginleftonly btnTitleSelected']);
...
if (count($tasksarray) > 0) {
    ...
    foreach ($tasksarray as $key => $val) {    // Task array are sorted by "project, position, date"
        ...
        if ($s) {
            $tasks[$taskcursor]['task_resources'] = '<a href="' . DOL_URL_ROOT . '/Modules/Projects/tasks/contact.php?id=' . $val->id . '&withproject=1" title="' . dol_escape_htmltag($s) . '">' . $langs->trans("List") . '</a>';
        }
    ...
    if (!empty($conf->use_javascript_ajax)) {
        ...
        include_once DOL_DOCUMENT_ROOT . '/Modules/Projects/ganttchart.inc.php';
        print '</div>' . "\n";
