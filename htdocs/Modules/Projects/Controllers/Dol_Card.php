...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Projects/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Projects/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
...
if ($action == 'create' && $user->rights->projet->creer) {
    ...
    if ($conf->societe->enabled) {
        ...
        $text = img_picto('', 'company') . $form->select_company(GETPOST('socid', 'int'), 'socid', $filteronlist, 'SelectThirdParty', 1, 0, [], 0, 'minwidth300 widthcentpercentminusxx');
        ...
        if (!GETPOSTISSET('backtopage')) {
            print ' <a href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=create&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create') . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddThirdParty") . '"></span></a>';
        }
        print '</td></tr>';
    }
    ...
    if ($action == 'clone') {
        $formquestion = [
            ...
            ['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company(GETPOST('socid', 'int') > 0 ? GETPOST('socid', 'int') : $object->socid, 'socid', '', "None", 0, 0, null, 0, 'minwidth200')],
    ...
    if ($action == 'edit' && $userWrite > 0) {
        ...
        if ($conf->societe->enabled) {
            ...
            $text = $form->select_company($object->thirdparty->id, 'socid', $filteronlist, 'None', 1, 0, [], 0, 'minwidth300');
            if (empty($conf->global->PROJECT_CAN_ALWAYS_LINK_TO_ALL_SUPPLIERS) && empty($conf->dol_use_jmobile)) {
                $texthelp = $langs->trans("IfNeedToUseOtherObjectKeepEmpty");
        ...
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Projects/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
    ...
    if (empty($reshook)) {
        if ($action != "edit" && $action != 'presend') {
            ...
            /*if ($conf->agenda->enabled && ! empty($conf->global->MAIN_ADD_EVENT_ON_ELEMENT_CARD)) 				// Add hidden condition because this is not a
                ...
                // page.
            {
                print '<a class="butAction" href="'.DOL_URL_ROOT.'/Modules/Comm/action/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '&amp;projectid=' . $object->id . '">' . $langs->trans("AddAction") . '</a>';
            }*/
            ...
            if (!empty($conf->global->PROJECT_SHOW_CREATE_OBJECT_BUTTON)) {
                if (!empty($conf->propal->enabled) && $user->rights->propal->creer) {
                    $langs->load("propal");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Comm/propal/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddProp") . '</a>';
                }
                if (!empty($conf->commande->enabled) && $user->rights->commande->creer) {
                    $langs->load("orders");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Commande/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("CreateOrder") . '</a>';
                }
                if (!empty($conf->facture->enabled) && $user->rights->facture->creer) {
                    $langs->load("bills");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("CreateBill") . '</a>';
                }
                ...
                if (!empty($conf->supplier_order->enabled) && ($user->rights->fournisseur->commande->creer || $user->rights->supplier_order->creer)) {
                    ...
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddSupplierOrder") . '</a>';
                }
                if (!empty($conf->supplier_invoice->enabled) && ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer)) {
                    $langs->load("suppliers");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddSupplierInvoice") . '</a>';
                }
                if (!empty($conf->ficheinter->enabled) && $user->rights->ficheinter->creer) {
                    $langs->load("interventions");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fichinter/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddIntervention") . '</a>';
                }
                if (!empty($conf->contrat->enabled) && $user->rights->contrat->creer) {
                    $langs->load("contracts");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Contrats/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddContract") . '</a>';
                }
                if (!empty($conf->expensereport->enabled) && $user->rights->expensereport->creer) {
                    $langs->load("trips");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddTrip") . '</a>';
                }
                if (!empty($conf->don->enabled) && $user->rights->don->creer) {
                    $langs->load("donations");
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Don/card.php?action=create&projectid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("AddDonation") . '</a>';
                }
            }
    ...
    if ($action != 'presend') {
        ...
        $morehtmlright = '<a href="' . DOL_URL_ROOT . '/Modules/Projects/info.php?id=' . $object->id . '">';
