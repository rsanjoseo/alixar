...
$arrayfields = [
    't.ref' => ['label' => $langs->trans("RefTask"), 'checked' => 1, 'position' => 1],
    't.label' => ['label' => $langs->trans("LabelTask"), 'checked' => 1, 'position' => 2],
    't.description' => ['label' => $langs->trans("Description"), 'checked' => 0, 'position' => 3],
    't.dateo' => ['label' => $langs->trans("DateStart"), 'checked' => 1, 'position' => 4],
    't.datee' => ['label' => $langs->trans("Deadline"), 'checked' => 1, 'position' => 5],
    't.planned_workload' => ['label' => $langs->trans("PlannedWorkload"), 'checked' => 1, 'position' => 6],
    't.duration_effective' => ['label' => $langs->trans("TimeSpent"), 'checked' => 1, 'position' => 7],
    't.progress_calculated' => ['label' => $langs->trans("ProgressCalculated"), 'checked' => 1, 'position' => 8],
    't.progress' => ['label' => $langs->trans("ProgressDeclared"), 'checked' => 1, 'position' => 9],
    't.progress_summary' => ['label' => $langs->trans("TaskProgressSummary"), 'checked' => 1, 'position' => 10],
    'c.assigned' => ['label' => $langs->trans("TaskRessourceLinks"), 'checked' => 1, 'position' => 11],
];
    ...
    if (!$cancel) {
        ...
        if (!$error) {
            if (!empty($backtopage)) {
                ...
                header("Location: " . DOL_URL_ROOT . '/Modules/Projects/tasks/list.php' . (empty($mode) ? '' : '?mode=' . $mode));
                exit;
            }
            $id = $projectid;
        }
...
if ($action == 'create' && $user->rights->projet->creer && (empty($object->thirdparty->id) || $userWrite > 0)) {
    ...
    $linktocreatetask = dolGetButtonTitle($langs->trans('AddTask'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Projects/tasks.php?action=create' . $param . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id), '', $linktocreatetaskUserRight, $linktocreatetaskParam);
    ...
    $linktotasks = dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-list-alt imgforviewmode', DOL_URL_ROOT . '/Modules/Projects/tasks.php?id=' . $object->id, '', 1, ['morecss' => 'reposition btnTitleSelected']);
    $linktotasks .= dolGetButtonTitle($langs->trans('ViewGantt'), '', 'fa fa-stream imgforviewmode', DOL_URL_ROOT . '/Modules/Projects/ganttview.php?id=' . $object->id . '&withproject=1', '', 1, ['morecss' => 'reposition marginleftonly']);
