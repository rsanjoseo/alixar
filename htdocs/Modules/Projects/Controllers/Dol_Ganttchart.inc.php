...
function constructGanttLine($tarr, $task, $task_dependencies, $level = 0, $project_id = null)
{
    ...
    if ($task["task_id"] < 0) {
        //$link=DOL_URL_ROOT.'/Modules/Projects/tasks.php?withproject=1&id='.abs($task["task_id"]);
        $link = '';
    } else {
        $link = DOL_URL_ROOT . '/Modules/Projects/tasks/contact.php?withproject=1&id=' . $task["task_id"];
    }

    // Name
    //$name='<a href="'.DOL_URL_ROOT.'/Modules/Projects/task/tasks.php?id='.$task['task_id'].'">'.$task['task_name'].'</a>';
    $name = $task['task_name'];
