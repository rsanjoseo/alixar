...
function conferenceorboothPrepareHead($object, $with_project = 0)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorbooth_card.ph?id=' . $object->id . $withProjectUrl;
    $head[$h][1] = $langs->trans("Card");
    $head[$h][2] = 'card';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorbooth_contact.php?id=' . $object->id . $withProjectUrl;
    $head[$h][1] = $langs->trans("ContactsAddresses");
    $head[$h][2] = 'contact';
    $h++;

    $head[$h][0] = DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorboothattendee_list.php?conforboothid=' . $object->id . $withProjectUrl;
    ...
    if (!is_null($dataretrieved)) {
        ...
        $result = $attendees->fetchAll('', '', 0, 0, ['t.fk_actioncomm' => $object->id]);
    ...
    $head[$h][0] = dol_buildpath("/Modules/Eventorganization/conferenceorbooth_document.php", 1) . '?id=' . $object->id . $withProjectUrl;
    ...
    //$this->tabs = array(
    //	'entity:+tabname:Title:@eventorganization:/Modules/Eventorganization/mypage.php?id=__ID__'
    //); // to add new tab
    //$this->tabs = array(
    //	'entity:-tabname:Title:@eventorganization:/Modules/Eventorganization/mypage.php?id=__ID__'
    //); // to remove a tab
...
function conferenceorboothProjectPrepareHead($object)
{
    ...
    $head[$h][0] = dol_buildpath("/Modules/Eventorganization/conferenceorbooth_list.php", 1) . '?projectid=' . $object->id;
    ...
    if (!is_null($dataretrieved)) {
        ...
        $result = $conforbooth->fetchAll('', '', 0, 0, ['t.fk_project' => $object->id]);
        if (!is_array($result) && $result < 0) {
    ...
    $head[$h][0] = dol_buildpath("/Modules/Eventorganization/conferenceorboothattendee_list.php", 1) . '?fk_project=' . $object->id . '&withproject=1';
    $head[$h][1] = $langs->trans("Attendees");
    $head[$h][2] = 'attendees';
    // Enable caching of conf or booth count attendees
    $nbAttendees = 0;
    ...
    if (!is_null($dataretrieved)) {
        ...
        require_once DOL_DOCUMENT_ROOT . '/Modules/Eventorganization/class/conferenceorboothattendee.class.php';
        $attendees = new ConferenceOrBoothAttendee($db);
        $result = $attendees->fetchAll('', '', 0, 0, ['t.fk_project' => $object->id]);
        if (!is_array($result) && $result < 0) {
            setEventMessages($attendees->error, $attendees->errors, 'errors');
        } else {
            $nbAttendees = count($result);
...
function conferenceorboothattendeePrepareHead($object)
{
    ...
    $head[$h][0] = DOL_URL_ROOT . "/Modules/Eventorganization/conferenceorboothattendee_card.php?id=" . ((int) $object->id) . ($object->fk_actioncomm > 0 ? '&conforboothid=' . ((int) $object->fk_actioncomm) : '') . ($object->fk_project > 0 ? '&withproject=1&fk_project=' . ((int) $object->fk_project) : '');
    $head[$h][1] = $langs->trans("Card");
    $head[$h][2] = 'card';
    $h++;
