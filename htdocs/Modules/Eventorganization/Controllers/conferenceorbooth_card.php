...
/**
 *    \file       htdocs/Modules/Eventorganization/conferenceorbooth_card.php
...
$hookmanager->initHooks(['conferenceorboothcard', 'globalcard']); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
    ...
    $backurlforlist = dol_buildpath('/Modules/Eventorganization/conferenceorbooth_list.php', 1);
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = dol_buildpath('/Modules/Eventorganization/conferenceorbooth_card.php', 1) . '?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    // Object card
    // ------------------------------------------------------------
    http://dolibarr.local/Modules/Eventorganization/conferenceorbooth_list.php?projectid=7
    $linkback = '<a href="' . dol_buildpath('/Modules/Eventorganization/conferenceorbooth_list.php', 1) . '?projectid=' . $object->fk_project . $withProjectUrl . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['conferenceorbooth']);
