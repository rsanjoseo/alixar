...
$langs->loadLangs(["eventorganization", "other", "projects"]);
...
$hookmanager->initHooks(['conferenceorboothattendeelist']); // Note that conf->hooks_modules contains array
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: " . dol_buildpath('/Modules/Eventorganization/conferenceorboothattendee_card.php', 1) . '?id=' . $id);
    exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorboothattendee_card.php?action=create' . (!empty($confOrBooth->id) ? '&conforboothid=' . $confOrBooth->id : '') . (!empty($projectstatic->id) ? '&fk_project=' . $projectstatic->id : '') . $withProjectUrl . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?projectid=' . $projectstatic->id . (empty($confOrBooth->id) ? '' : '&conforboothid=' . $confOrBooth->id) . $withProjectUrl), '', $permissiontoadd);
