...
$hookmanager->initHooks(['conferenceorboothlist']); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
    ...
    include DOL_DOCUMENT_ROOT . '/Modules/Eventorganization/core/actions_massactions_mail.inc.php';
...
if ($projectid > 0) {
    ...
    //print '</span>';
    print '</td><td>';
    $link_subscription = $dolibarr_main_url_root . '/public/Modules/Eventorganization/attendee_register.php?id=' . ((int) $project->id) . '&type=global';
    $encodedsecurekey = dol_hash($conf->global->EVENTORGANIZATION_SECUREKEY . 'conferenceorbooth' . ((int) $project->id), 'md5');
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: " . DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorbooth_card.php?id=' . ((int) $id));
    exit;
}
...
if (GETPOST('nomassaction', 'int') || in_array($massaction, ['presend', 'presend_attendees', 'predelete'])) {
    $arrayofmassactions = [];
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorbooth_card.php?action=create' . (!empty($project->id) ? '&withproject=1&fk_project=' . $project->id : '') . (!empty($project->socid) ? '&fk_soc=' . $project->socid : '') . '&backtopage=' . urlencode($_SERVER['PHP_SELF']) . (!empty($project->id) ? '?projectid=' . $project->id : ''), '', $permissiontoadd);
...
include DOL_DOCUMENT_ROOT . '/Modules/Eventorganization/tpl/massactions_mail_pre.tpl.php';
...
foreach ($object->fields as $key => $val) {
    ...
    } elseif (in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && !in_array($key, ['rowid', 'ref']) && $val['label'] != 'TechnicalID') {
...
while ($i < ($limit ? min($num, $limit) : $num)) {
    ...
    foreach ($object->fields as $key => $val) {
        ...
        if (in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && !in_array($key, ['rowid', 'ref', 'status'])) {
            $cssforfield .= ($cssforfield ? ' ' : '') . 'right';
        }
