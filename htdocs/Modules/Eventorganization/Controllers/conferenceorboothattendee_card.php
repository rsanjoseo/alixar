...
require_once DOL_DOCUMENT_ROOT . '/Modules/Eventorganization/lib/eventorganization_conferenceorbooth.lib.php';
require_once DOL_DOCUMENT_ROOT . '/core/lib/project.lib.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Projects/class/project.class.php';

// Load translation files required by the page
$langs->loadLangs(["eventorganization", "other", "projects", "companies"]);
...
$hookmanager->initHooks(['conferenceorboothattendeecard', 'globalcard']); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
    ...
    if (!empty($withproject)) {
        $backurlforlist = DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorboothattendee_list.php?withproject=1';
    } else {
        $backurlforlist = DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorboothattendee_list.php';
    }
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Eventorganization/conferenceorboothattendee_card.php?fk_project=' . ((int) $fk_project) . '&id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
...
if (!empty($withproject)) {
    ...
    //print '</span>';
    print '</td><td>';
    $link_subscription = $dolibarr_main_url_root . '/public/Modules/Eventorganization/attendee_register.php?id=' . $projectstatic->id . '&type=global';
    $encodedsecurekey = dol_hash($conf->global->EVENTORGANIZATION_SECUREKEY . 'conferenceorbooth' . $projectstatic->id, 'md5');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    // Object card
    // ------------------------------------------------------------
    $linkback = '<a href="' . dol_buildpath('/Modules/Eventorganization/conferenceorboothattendee_list.php', 1) . '?restore_lastsearch_values=1' . ($confOrBooth->id > 0 ? '&conforboothid=' . ((int) $confOrBooth->id) : '') . $moreparam . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['conferenceorboothattendee']);
