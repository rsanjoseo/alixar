...
/**
 * \file        htdocs/Modules/Accountancy/expensereport/index.php
...
if (($action == 'clean' || $action == 'validatehistory') && $user->rights->accounting->bind->write) {
    ...
    dol_syslog("htdocs/Modules/Accountancy/customer/index.php fixaccountancycode", LOG_DEBUG);
...
if ($action == 'validatehistory') {
    ...
    dol_syslog('htdocs/Modules/Accountancy/expensereport/index.php');
...
dol_syslog('/Modules/Accountancy/expensereport/index.php:: sql=' . $sql);
$resql = $db->query($sql);
if ($resql) {
    ...
    while ($row = $db->fetch_row($resql)) {
        ...
        print '<td>';
        if ($row[0] == 'tobind') {
            print $langs->trans("UseMenuToSetBindindManualy", DOL_URL_ROOT . '/Modules/Accountancy/expensereport/list.php?search_year=' . $y, $langs->transnoentitiesnoconv("ToBind"));
        } else {
            print $row[1];
        }
        print '</td>';
