...
/**
 * \file        htdocs/Modules/Accountancy/admin/subaccount.php
...
$langs->loadLangs(["compta", "bills", "admin", "accountancy", "salaries", "hrm", "errors"]);
...
if ($resql) {
    ...
    while ($i < min($num, $limit)) {
        ...
        if (!empty($arrayfields['type']['checked'])) {
            ...
            // Customer
            if ($obj->type == 1) {
                $s .= '<a class="customer-back" style="padding-left: 6px; padding-right: 6px" title="' . $langs->trans("Customer") . '" href="' . DOL_URL_ROOT . '/Modules/Comm/card.php?socid=' . $obj->rowid . '">' . $langs->trans("Customer") . '</a>';
            } elseif ($obj->type == 2) {
                // Supplier
                $s .= '<a class="vendor-back" style="padding-left: 6px; padding-right: 6px" title="' . $langs->trans("Supplier") . '" href="' . DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=' . $obj->rowid . '">' . $langs->trans("Supplier") . '</a>';
            } elseif ($obj->type == 3) {
                // User
                $s .= '<a class="user-back" style="padding-left: 6px; padding-right: 6px" title="' . $langs->trans("Employee") . '" href="' . DOL_URL_ROOT . '/Modules/Users/card.php?id=' . $obj->id . '">' . $langs->trans("Employee") . '</a>';
            }
            print $s;
            print '</td>';
            if (!$i) {
        ...
        // Customer
        if ($obj->type == 1) {
            $e .= '<a class="editfielda" title="' . $langs->trans("Customer") . '" href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=edit&token=' . newToken() . '&socid=' . $obj->rowid . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '">' . img_edit() . '</a>';
        } elseif ($obj->type == 2) {
            // Supplier
            $e .= '<a class="editfielda" title="' . $langs->trans("Supplier") . '" href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=edit&token=' . newToken() . '&socid=' . $obj->rowid . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '">' . img_edit() . '</a>';
        } elseif ($obj->type == 3) {
            // User
            $e .= '<a class="editfielda" title="' . $langs->trans("Employee") . '" href="' . DOL_URL_ROOT . '/Modules/Users/card.php?action=edit&token=' . newToken() . '&id=' . $obj->rowid . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '">' . img_edit() . '</a>';
        }
        print $e;
        print '</td>' . "\n";
        if (!$i) {
