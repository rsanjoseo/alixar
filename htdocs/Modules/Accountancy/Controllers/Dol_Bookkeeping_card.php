...
/**
 * \file        htdocs/Modules/Accountancy/bookkeeping/card.php
...
if ($action == 'create') {
    ...
    */
    ...
    if (!empty($object->piece_num)) {
        $backlink = '<a href="' . DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/list.php?restore_lastsearch_values=1">' . $langs->trans('BackToList') . '</a>';
            ...
            if (count($object->linesmvt) > 0) {
                ...
                foreach ($object->linesmvt as $line) {
                    ...
                    if ($action == 'update' && $line->id == $id) {
                        ...
                        print '<td>';
                        print $formaccounting->select_account((GETPOSTISSET("accountingaccount_number") ? GETPOST("accountingaccount_number", "alpha") : $line->numero_compte), 'accountingaccount_number', 1, [], 1, 1, '');
                        print '</td>';
                        print '<td>';
                ...
                if (empty($object->date_export) && empty($object->date_validation)) {
                    if ($action == "" || $action == 'add') {
                        ...
                        print '<td>';
                        print $formaccounting->select_account('', 'accountingaccount_number', 1, [], 1, 1, '');
                        print '</td>';
                        print '<td>';
                        // TODO For the moment we keep a free input text instead of a combo. The select_auxaccount has problem because:
                        // It does not use the setup of "key pressed" to select a thirdparty and this hang browser on large databases.
                ...
                if ($mode == '_tmp' && $action == '') {
                    ...
                    print ' &nbsp; ';
                    print '<a class="button button-cancel" href="' . DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/list.php">' . $langs->trans("Cancel") . '</a>';
