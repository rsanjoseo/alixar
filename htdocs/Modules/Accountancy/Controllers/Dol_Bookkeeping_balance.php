...
/**
 *  \file        htdocs/Modules/Accountancy/bookkeeping/balance.php
...
$hookmanager->initHooks(['balancelist']);  // Note that conf->hooks_modules contains array
...
if ($action == 'export_csv') {
    ...
    $filename = 'balance';
    $type_export = 'balance';
    include DOL_DOCUMENT_ROOT . '/Modules/Accountancy/tpl/export_journal.tpl.php';
...
if ($action != 'export_csv') {
    ...
    print $formaccounting->select_account($search_accountancy_code_start, 'search_accountancy_code_start', $langs->trans('From'), [], 1, 1, '', 'accounts');
    print ' ';
    print $formaccounting->select_account($search_accountancy_code_end, 'search_accountancy_code_end', $langs->trans('to'), [], 1, 1, '', 'accounts');
    print '</td>';
    ...
    foreach ($object->lines as $line) {
        ...
        if (empty($accountingaccountstatic->label) && $accountingaccountstatic->id > 0) {
            $link = '<a class="editfielda reposition" href="' . DOL_URL_ROOT . '/Modules/Accountancy/admin/card.php?action=update&token=' . newToken() . '&id=' . $accountingaccountstatic->id . '">' . img_edit() . '</a>';
        } elseif ($accounting_account == 'NotDefined') {
            $link = '<a href="' . DOL_URL_ROOT . '/Modules/Accountancy/admin/card.php?action=create&token=' . newToken() . '&accountingaccount=' . length_accountg($line->numero_compte) . '">' . img_edit_add() . '</a>';
        ...
        if ($line->numero_compte) {
            $urlzoom = DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/listbyaccount.php?search_accountancy_code_start=' . urlencode($line->numero_compte) . '&search_accountancy_code_end=' . urlencode($line->numero_compte);
