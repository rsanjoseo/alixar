...
/**
 * \file    htdocs/Modules/Accountancy/customer/index.php
...
if ($action == 'validatehistory') {
    ...
    dol_syslog('htdocs/Modules/Accountancy/customer/index.php');
    $result = $db->query($sql);
    if (!$result) {
        $error++;
        setEventMessages($db->lasterror(), null, 'errors');
...
dol_syslog('htdocs/Modules/Accountancy/customer/index.php', LOG_DEBUG);
$resql = $db->query($sql);
if ($resql) {
    ...
    while ($row = $db->fetch_row($resql)) {
        ...
        print '<td>';
        if ($row[0] == 'tobind') {
            print $langs->trans("UseMenuToSetBindindManualy", DOL_URL_ROOT . '/Modules/Accountancy/customer/list.php?search_year=' . $y, $langs->transnoentitiesnoconv("ToBind"));
        } else {
            print $row[1];
        }
        print '</td>';
