...
/**
 * \file        htdocs/Modules/Accountancy/bookkeeping/list.php
 * \ingroup        Accountancy (Double entries)
 * \brief        List operation of book keeping
 */
require '../../main.inc.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Accountancy/class/accountancyexport.class.php';
require_once DOL_DOCUMENT_ROOT . '/core/lib/accounting.lib.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Accountancy/class/bookkeeping.class.php';
...
if (!in_array($action, ['export_file', 'delmouv', 'delmouvconfirm']) && !GETPOSTISSET('begin') && !GETPOSTISSET('formfilteraction') && GETPOST('page', 'int') == '' && !GETPOST('noreset', 'int') && $user->rights->accounting->mouvements->export) {
...
$arrayfields = [
    ...
    't.debit' => ['label' => $langs->trans("Debit"), 'checked' => 1],
    't.credit' => ['label' => $langs->trans("Credit"), 'checked' => 1],
    ...
    't.date_validated' => ['label' => $langs->trans("DateValidation"), 'checked' => 1],
];
        ...
        if (!empty($accountancyexport->errors)) {
            ...
            if (is_array($object->lines)) {
                foreach ($object->lines as $movement) {
                    ...
                    dol_syslog("/Modules/Accountancy/bookkeeping/list.php Function export_file Specify movements as exported", LOG_DEBUG);
...
if (empty($reshook)) {
    ...
    $newcardbutton .= dolGetButtonTitle($langs->trans('ViewFlatList'), '', 'fa fa-list paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/list.php?' . $param, '', 1, ['morecss' => 'marginleftonly btnTitleSelected']);
    $newcardbutton .= dolGetButtonTitle($langs->trans('GroupByAccountAccounting'), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/listbyaccount.php?' . $param, '', 1, ['morecss' => 'marginleftonly']);
    $newcardbutton .= dolGetButtonTitle($langs->trans('GroupBySubAccountAccounting'), '', 'fa fa-align-left vmirror paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/listbysubaccount.php?' . $param, '', 1, ['morecss' => 'marginleftonly']);
...
if (!empty($arrayfields['t.numero_compte']['checked'])) {
    ...
    print '<div class="nowrap">';
    print $formaccounting->select_account($search_accountancy_code_start, 'search_accountancy_code_start', $langs->trans('From'), [], 1, 1, 'maxwidth200', 'account');
    print '</div>';
    print '<div class="nowrap">';
    print $formaccounting->select_account($search_accountancy_code_end, 'search_accountancy_code_end', $langs->trans('to'), [], 1, 1, 'maxwidth200', 'account');
    print '</div>';
    print '</td>';
}
// Subledger account
...
while ($i < min($num, $limit)) {
    ...
    if (empty($line->date_export) && empty($line->date_validation)) {
        if ($user->rights->accounting->mouvements->creer) {
            print '<a class="editfielda paddingleft marginrightonly" href="' . DOL_URL_ROOT . '/Modules/Accountancy/bookkeeping/card.php?piece_num=' . $line->piece_num . $param . '&page=' . $page . ($sortfield ? '&sortfield=' . $sortfield : '') . ($sortorder ? '&sortorder=' . $sortorder : '') . '">' . img_edit() . '</a>';
        }
    }
