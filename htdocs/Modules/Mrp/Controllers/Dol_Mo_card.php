...
require_once DOL_DOCUMENT_ROOT . '/Modules/Mrp/lib/mrp_mo.lib.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Mrp/mo_card.php?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action != 'presend') {
        ...
        $morehtmlright = '<a href="' . dol_buildpath('/Modules/Mrp/mo_agenda.php', 1) . '?id=' . $object->id . '">';
