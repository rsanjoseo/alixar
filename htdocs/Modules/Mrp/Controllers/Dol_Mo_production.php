...
dol_include_once('/Modules/Mrp/class/mo.class.php');
dol_include_once('/Modules/Bom/class/bom.class.php');
dol_include_once('/Modules/Mrp/lib/mrp_mo.lib.php');
...
if (empty($reshook)) {
    ...
    $backurlforlist = dol_buildpath('/Modules/Mrp/mo_list.php', 1);
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        ...
        if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
            ...
            $backtopage = DOL_URL_ROOT . '/Modules/Mrp/mo_production.php?id=' . ($id > 0 ? $id : '__ID__');
        }
    }
...
llxHeader('', $langs->trans('Mo'), $help_url, '', 0, 0, ['/Modules/Mrp/js/lib_dispatch.js.php']);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    // Object card
    // ------------------------------------------------------------
    $linkback = '<a href="' . dol_buildpath('/Modules/Mrp/mo_list.php', 1) . '?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if (in_array($action, ['consumeorproduce', 'consumeandproduceall', 'addconsumeline', 'addproduceline'])) {
    ...
    if (!empty($object->table_element_line)) {
        ...
        if ($action == 'addconsumeline') {
            ...
            print '<tr class="liste_titre">';
            print '<td>';
            print $form->select_produits('', 'productidtoadd', '', 0, 0, -1, 2, '', 0, [], 0, '1', 0, 'maxwidth300');
            print '</td>';
            // Qty
            ...
            foreach ($object->lines as $line) {
                if ($line->role == 'toconsume') {
                    ...
                    foreach ($arrayoflines as $line2) {
                        ...
                        print '<a href="' . DOL_URL_ROOT . '/Modules/Products/stock/movement_list.php?search_ref=' . $tmpstockmovement->id . '">' . img_picto($langs->trans("StockMovement"), 'movement', 'class="paddingright"') . '</a>';
