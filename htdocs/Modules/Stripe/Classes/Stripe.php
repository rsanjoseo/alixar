...
//require_once DOL_DOCUMENT_ROOT . '/Modules/Stripe/config.php'; // This set stripe global env
...
class Stripe extends CommonObject
{
    ...
    public function getStripeCustomerAccount($id, $status = 0, $site_account = '')
    {
        ...
        $societeaccount = new SocieteAccount();
    ...
    public function getPaymentMethodStripe($paymentmethod, $key = '', $status = 0)
    {
        ...
        try {
            ...
            if (empty($key)) {                // If the Stripe connect account not set, we use common API usage
                ...
                $stripepaymentmethod = \Stripe\PaymentMethod::retrieve('' . $paymentmethod->id . '', ["stripe_account" => $key]);
            }
        } catch (Exception $e) {
            $this->error = $e->getMessage();
        }
        ...
        if ($resql) {
            ...
            if ($num) {
                ...
                if ($cardref) {
                    ...
                    $dataforcard = [
                        "source" => ['object' => 'card', 'exp_month' => $exp_date_month, 'exp_year' => $exp_date_year, 'number' => $number, 'cvc' => $cvc, 'name' => $cardholdername],
                        "metadata" => ['dol_id' => $object->id, 'dol_version' => DOL_VERSION, 'dol_entity' => $conf->entity, 'ipaddress' => $ipaddress],
                    ];
