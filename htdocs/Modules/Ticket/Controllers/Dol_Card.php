...
require_once DOL_DOCUMENT_ROOT . '/Modules/Tickets/class/actions_ticket.class.php';
...
$hookmanager->initHooks(['ticketcard', 'globalcard']);
...
$url_page_current = DOL_URL_ROOT . '/Modules/Tickets/card.php';
...
if (empty($reshook)) {
    ...
    $backurlforlist = DOL_URL_ROOT . '/Modules/Tickets/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Tickets/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_delete_ticket' && GETPOST('confirm', 'alpha') == "yes" && $user->rights->ticket->delete) {
        if ($object->fetch(GETPOST('id', 'int'), '', GETPOST('track_id', 'alpha')) >= 0) {
            if ($object->delete($user) > 0) {
                ...
                Header("Location: " . DOL_URL_ROOT . "/Modules/Ticket/list.php");
                exit;
            } else {
                $langs->load("errors");
    ...
    if ($action == 'confirm_reopen' && $user->rights->ticket->manage && !GETPOST('cancel')) {
        ...
        if (!GETPOST('cancel')) {
            ...
            if ($ret > 0) {
                ...
                dol_include_once('/Modules/Tickets/class/utils_diff.class.php');
...
if ($action == 'create' || $action == 'presend') {
    ...
    $formticket->param = ['origin' => GETPOST('origin'), 'originid' => GETPOST('originid')];
    ...
    if ($res > 0) {
        ...
        if (!empty($conf->societe->enabled)) {
            ...
            if ($action == 'editcustomer') {
                $morehtmlref .= $form->form_thirdparty($url_page_current . '?track_id=' . $object->track_id, $object->socid, 'editcustomer', '', 1, 0, 0, [], 1);
            } else {
                $morehtmlref .= $form->form_thirdparty($url_page_current . '?track_id=' . $object->track_id, $object->socid, 'none', '', 1, 0, 0, [], 1);
            }
        }
        ...
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Tickets/list.php?restore_lastsearch_values=1"><strong>' . $langs->trans("BackToList") . '</strong></a> ';
        ...
        if ($action != 'presend' && $action != 'presend_addmessage' && $action != 'editline') {
            ...
            if (empty($reshook)) {
                ...
                if ($object->fk_soc > 0 && $object->fk_statut < Ticket::STATUS_CLOSED && $user->rights->ficheinter->creer) {
                    print '<div class="inline-block divButAction"><a class="butAction" href="' . dol_buildpath('/Modules/Fichinter/card.php', 1) . '?action=create&token=' . newToken() . '&socid=' . $object->fk_soc . '&origin=ticket_ticket&originid=' . $object->id . '">' . $langs->trans('TicketAddIntervention') . '</a></div>';
                }
        ...
        if ($action == 'presend' || $action == 'presend_addmessage') {
            ...
            $arrayoffamiliestoexclude = ['objectamount'];
        ...
        if (!empty($conf->global->TICKET_SHOW_MESSAGES_ON_CARD)) {
            ...
            $messagingUrl = DOL_URL_ROOT . '/Modules/Tickets/agenda.php?track_id=' . $object->track_id;
            ...
            $btnstatus = $object->fk_statut < Ticket::STATUS_CLOSED && $action != "presend" && $action != "presend_addmessage" && $action != "add_message";;
            $url = dol_buildpath('/Modules/Comm/action/card.php', 1) . '?action=create&datep=' . date('YmdHi') . '&origin=ticket&originid=' . $object->id . '&projectid=' . $object->fk_project . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?track_id=' . $object->track_id);
        ...
        if ($action != 'presend' && $action != 'presend_addmessage' && $action != 'add_message') {
            ...
            $linktoelem = $form->showLinkToObjectBlock($object, null, ['ticket']);
            ...
            if (empty($conf->global->TICKET_SHOW_MESSAGES_ON_CARD)) {
                ...
                $morehtmlcenter .= dolGetButtonTitle($langs->trans('FullConversation'), '', 'fa fa-comments imgforviewmode', DOL_URL_ROOT . '/Modules/Tickets/messaging.php?id=' . $object->id);
                $morehtmlcenter .= ' ';
                $morehtmlcenter .= dolGetButtonTitle($langs->trans('FullList'), '', 'fa fa-list-alt imgforviewmode', DOL_URL_ROOT . '/Modules/Tickets/agenda.php?id=' . $object->id);
                $morehtmlcenter .= '</div>';
