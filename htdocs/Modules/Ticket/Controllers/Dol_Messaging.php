...
$linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Tickets/list.php"><strong>' . $langs->trans("BackToList") . '</strong></a> ';
...
if (!empty($object->id)) {
    ...
    $messagingUrl = DOL_URL_ROOT . '/Modules/Tickets/messaging.php?track_id=' . $object->track_id;
    $morehtmlright .= dolGetButtonTitle($langs->trans('ShowAsConversation'), '', 'fa fa-comments imgforviewmode', $messagingUrl, '', 1, ['morecss' => 'btnTitleSelected']);
    ...
    $btnstatus = $object->fk_statut < Ticket::STATUS_CLOSED && $action != "presend" && $action != "presend_addmessage";
    $url = DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&datep=' . date('YmdHi') . '&origin=ticket&originid=' . $object->id . '&projectid=' . $object->fk_project . '&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?track_id=' . $object->track_id);
