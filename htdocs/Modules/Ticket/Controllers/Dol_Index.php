...
require_once DOL_DOCUMENT_ROOT . '/Modules/Tickets/class/ticketstats.class.php';
...
$hookmanager->initHooks(['ticketsindex']);
...
if ($result) {
    ...
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_NOT_READ]), 'data' => round($tick['unread'])];
    $colorseries[Ticket::STATUS_NOT_READ] = '-' . $badgeStatus0;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_READ]), 'data' => round($tick['read'])];
    $colorseries[Ticket::STATUS_READ] = $badgeStatus1;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_ASSIGNED]), 'data' => round($tick['assigned'])];
    $colorseries[Ticket::STATUS_ASSIGNED] = $badgeStatus3;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_IN_PROGRESS]), 'data' => round($tick['inprogress'])];
    $colorseries[Ticket::STATUS_IN_PROGRESS] = $badgeStatus4;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_WAITING]), 'data' => round($tick['waiting'])];
    $colorseries[Ticket::STATUS_WAITING] = '-' . $badgeStatus4;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_NEED_MORE_INFO]), 'data' => round($tick['needmoreinfo'])];
    $colorseries[Ticket::STATUS_NEED_MORE_INFO] = '-' . $badgeStatus3;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_CANCELED]), 'data' => round($tick['canceled'])];
    $colorseries[Ticket::STATUS_CANCELED] = $badgeStatus9;
    $dataseries[] = ['label' => $langs->transnoentitiesnoconv($tickesupstatic->statuts_short[Ticket::STATUS_CLOSED]), 'data' => round($tick['closed'])];
...
if ($result) {
    ...
    print '<th class="right" colspan="2"><a href="' . DOL_URL_ROOT . '/Modules/Tickets/list.php?search_fk_statut[]=' . Ticket::STATUS_NOT_READ . '">' . $langs->trans("FullList") . '</th>';
    print '</tr>';
    if ($num > 0) {
        while ($i < $num) {
