...
$langs->loadLangs(["ticket", "companies", "other", "projects"]);
...
if ($socid > 0) {
    $hookmanager->initHooks(['thirdpartyticket']);
} elseif ($projectid > 0) {
    $hookmanager->initHooks(['projectticket']);
} else {
    $hookmanager->initHooks(['ticketlist']);
}
...
$url_page_current = DOL_URL_ROOT . '/Modules/Tickets/list.php';
...
foreach ($search as $key => $val) {
    if ($key == 'fk_statut' && !empty($search['fk_statut'])) {
        ...
        foreach ($search['fk_statut'] as $key2 => $val2) {
            if (in_array($val2, ['openall', 'closeall'])) {
                continue;
            }
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all) {
    ...
    $id = $obj->rowid;
    header("Location: " . DOL_URL_ROOT . '/Modules/Tickets/card.php?id=' . $id);
    exit;
}
...
$url = DOL_URL_ROOT . '/Modules/Tickets/card.php?action=create' . ($socid ? '&socid=' . $socid : '') . ($projectid ? '&origin=projet_project&originid=' . $projectid : '');
if (!empty($socid)) {
    $url .= '&socid=' . $socid;
}
...
while ($i < min($num, $limit)) {
    ...
    foreach ($object->fields as $key => $val) {
        ...
        if (in_array($key, ['ref', 'fk_project'])) {
