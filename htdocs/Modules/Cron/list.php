...
require_once DOL_DOCUMENT_ROOT . '/Modules/Cron/class/cronjob.class.php';
...
if (empty($reshook)) {
    ...
    if ($action == 'confirm_execute' && $confirm == "yes" && $user->rights->cron->execute) {
        if (!empty($conf->global->CRON_KEY) && $conf->global->CRON_KEY != $securitykey) {
            ...
            header("Location: " . DOL_URL_ROOT . '/Modules/Cron/list.php?' . $param . ($sortfield ? '&sortfield=' . $sortfield : '') . ($sortorder ? '&sortorder=' . $sortorder : '')); // Make a redirect to avoid to run twice the job when using back
            exit;
        }
    }
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), $langs->trans('CronCreateJob'), 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Cron/card.php?action=create&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?mode=modulesetup'), '', $user->rights->cron->create);
...
print $form->selectarray('search_status', ['0' => $langs->trans("Disabled"), '1' => $langs->trans("Scheduled")], $search_status, 1);
