...
require_once DOL_DOCUMENT_ROOT . '/Modules/Bom/lib/bom.lib.php';
...
if (empty($reshook)) {
    ...
    $backurlforlist = DOL_URL_ROOT . '/Modules/Bom/bom_list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Bom/bom_card.php?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    // Object card
    // ------------------------------------------------------------
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Bom/bom_list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    */
    ...
    if (!empty($object->table_element_line)) {
        ...
        if (!empty($object->lines)) {
            $object->printObjectLines($action, $mysoc, null, GETPOST('lineid', 'int'), 1, '/Modules/Bom/tpl');
        }
        ...
        if ($object->status == 0 && $permissiontoadd && $action != 'selectlines') {
            if ($action != 'editline') {
                ...
                if (empty($reshook)) {
                    $object->formAddObjectLine(1, $mysoc, null, '/Modules/Bom/tpl');
                }
            }
        }
        ...
        if (empty($reshook)) {
            ...
            if ($conf->mrp->enabled) {
                if ($object->status == $object::STATUS_VALIDATED && !empty($user->rights->mrp->write)) {
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Mrp/mo_card.php?action=create&fk_bom=' . $object->id . '&token=' . newToken() . '&backtopageforcancel=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $object->id) . '">' . $langs->trans("CreateMO") . '</a>' . "\n";
                }
            }
    ...
    if ($action != 'presend') {
        ...
        $morehtmlright = '<a href="' . DOL_URL_ROOT . '/Modules/Bom/bom_agenda.php?id=' . $object->id . '">';
