...
if (!empty($conf->product->enabled) || !empty($conf->service->enabled)) {
    ...
    if (!empty($conf->global->ENTREPOT_EXTRA_STATUS)) {
        ...
        $form->select_produits(GETPOST('idprod', 'int'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, [], $buyer->id, '1', 0, 'maxwidth500', 0, 'warehouseopen,warehouseinternal', GETPOST('combinations', 'array'));
    } else {
        $form->select_produits(GETPOST('idprod', 'int'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, [], $buyer->id, '1', 0, 'maxwidth500', 0, '', GETPOST('combinations', 'array'));
    }
...
if (is_object($objectline)) {
    print $objectline->showOptionals($extrafields, 'edit', ['style' => $bcnd[$var], 'colspan' => $coldisplay], '', '', 1, 'line');
}
?>
