...
$hookmanager->initHooks(['doncard', 'globalcard']);
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Don/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Don/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
...
if (!empty($id) && $action != 'edit') {
    ...
    if ($resql) {
        ...
        while ($i < $num) {
            ...
            print '<a href="' . DOL_URL_ROOT . '/Modules/Don/payment/card.php?id=' . $objp->rowid . '">' . img_object($langs->trans("Payment"), "payment") . ' ' . $objp->rowid . '</a></td>';
    ...
    if ($object->statut == $object::STATUS_VALIDATED && $object->paid == 0 && $user->rights->don->creer) {
        if ($remaintopay == 0) {
            ...
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Don/payment/payment.php?rowid=' . $object->id . '&action=create&token=' . newToken() . '">' . $langs->trans('DoPayment') . '</a></div>';
        }
    }
