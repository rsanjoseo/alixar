...
print '<td>';
print $form->select_company($conf->global->{'CASHDESK_ID_THIRDPARTY' . $terminaltouse}, 'socid', '(s.client IN (1, 3) AND s.status = 1)', 1, 0, 0, [], 0);
print '</td></tr>';
...
if (!empty($conf->stock->enabled)) {
    ...
    if (!$disabled) {
        print img_picto('', 'bank_account', 'class="pictofixedwidth"');
        print $formproduct->selectWarehouses($conf->global->{'CASHDESK_ID_WAREHOUSE' . $terminal}, 'CASHDESK_ID_WAREHOUSE' . $terminal, '', 1, $disabled, 0, '', 0, 0, [], 'maxwidth250');
        print ' <a href="' . DOL_URL_ROOT . '/Modules/Products/stock/card.php?action=create&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?&terminal=' . $terminal) . '"><span class="fa fa-plus-circle valignmiddle"></span></a>';
    } else {
