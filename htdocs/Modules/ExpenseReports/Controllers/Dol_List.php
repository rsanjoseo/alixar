...
if ($resql) {
    ...
    if ($id > 0) {        // For user tab
        ...
        if ($action != 'edit') {
            ...
            // Buttons for actions
            if ($canedit) {
                print '<a href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/card.php?action=create&fk_user_author=' . $fuser->id . '" class="butAction">' . $langs->trans("AddTrip") . '</a>';
            } else {
        ...
        $url = DOL_URL_ROOT . '/Modules/ExpenseReports/card.php?action=create';
        if (!empty($socid)) {
            $url .= '&socid=' . $socid;
        }
        ...
        while ($i < min($num, $limit)) {
            ...
            if (!empty($arrayfields['d.ref']['checked'])) {
                ...
                if (!empty($obj->note_private) || !empty($obj->note_public)) {
                    ...
                    print '<a href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/note.php?id=' . $obj->rowid . '">' . img_picto($langs->trans("ViewPrivateNote"), 'object_generic') . '</a>';
                    print '</span>';
                }
                print '</td>';
