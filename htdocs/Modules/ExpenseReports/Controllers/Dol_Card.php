...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/ExpenseReports/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/ExpenseReports/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == "confirm_validate" && GETPOST("confirm", 'alpha') == "yes" && $id > 0 && $user->rights->expensereport->creer) {
        ...
        if (!$error && $result > 0 && $object->fk_user_validator > 0) {
            ...
            if ($emailTo && $emailFrom) {
                ...
                // CONTENT
                $link = $urlwithroot . '/Modules/ExpenseReports/card.php?id=' . $object->id;
...
if ($action == 'create') {
    ...
    if ($result > 0) {
        ...
        if ($action == 'edit' && ($object->status < 3 || $object->status == 99)) {
            ...
            $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
            ...
            if (!empty($object->lines)) {
                ...
                foreach ($object->lines as &$line) {
                    ...
                    if ($action == 'editline' && $line->rowid == GETPOST('rowid', 'int')) {
                        ...
                        $tredited = 'tredited';
                        include DOL_DOCUMENT_ROOT . '/Modules/ExpenseReports/tpl/expensereport_addfile.tpl.php';
                        include DOL_DOCUMENT_ROOT . '/Modules/ExpenseReports/tpl/expensereport_linktofile.tpl.php';
                        ...
                        if (!empty($conf->global->MAIN_USE_EXPENSE_IK)) {
                            print '<td class="fk_c_exp_tax_cat">';
                            $params = ['fk_expense' => $object->id, 'fk_expense_det' => $line->rowid, 'date' => $line->dates];
	...
	*/
    ...
    if ($user->rights->expensereport->to_paid && !empty($conf->banque->enabled) && $object->status == ExpenseReport::STATUS_APPROVED) {
        ...
        if ($remaintopay == 0) {
            ...
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/payment/payment.php?id=' . $object->id . '&amp;action=create">' . $langs->trans('DoPayment') . '</a></div>';
        }
    }
