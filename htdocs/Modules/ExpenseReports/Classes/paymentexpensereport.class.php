...
class PaymentExpenseReport extends CommonObject
{
    ...
    public function addPaymentToBank($user, $mode, $label, $accountid, $emetteur_nom, $emetteur_banque)
    {
        ...
        if (!empty($conf->banque->enabled)) {
            ...
            if ($bank_line_id > 0) {
                ...
                $url = '';
                if ($mode == 'payment_expensereport') {
                    $url = DOL_URL_ROOT . '/Modules/ExpenseReports/payment/card.php?rowid=';
                }
                if ($url) {
    ...
    public function getNomUrl($withpicto = 0, $maxlen = 0)
    {
        ...
        if (!empty($this->id)) {
            $link = '<a href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/payment/card.php?id=' . $this->id . '" title="' . dol_escape_htmltag($label, 1) . '" class="classfortooltip">';
            $linkend = '</a>';
