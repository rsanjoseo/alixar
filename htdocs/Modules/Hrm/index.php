...
$langs->loadLangs(['users', 'holidays', 'trips', 'boxes']);
...
if (!empty($setupcompanynotcomplete)) {
    ...
    print '<br><div class="warning"><a href="' . BASE_URI . '?module=Admin&controller=company&mainmenu=home' . (empty($setupcompanynotcomplete) ? '' : '&action=edit&token=' . newToken()) . '">' . $warnpicto . ' ' . $langs->trans("WarningMandatorySetupNotComplete") . '</a></div>';
...
if (!empty($conf->holiday->enabled) && $user->rights->holiday->read) {
    ...
    if ($result) {
        ...
        print '<th class="right" colspan="2"><a href="' . DOL_URL_ROOT . '/Modules/Holiday/list.php?sortfield=cp.tms&sortorder=DESC">' . $langs->trans("FullList") . '</th>';
        print '</tr>';
        if ($num) {
            while ($i < $num && $i < $max) {
...
if (!empty($conf->expensereport->enabled) && $user->rights->expensereport->lire) {
    ...
    if ($result) {
        ...
        print '<th class="right" colspan="2"><a href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/list.php?sortfield=d.tms&sortorder=DESC">' . $langs->trans("FullList") . '</th>';
        print '</tr>';
        if ($num) {
