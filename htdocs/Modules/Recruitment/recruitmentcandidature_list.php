...
$hookmanager->initHooks(['recruitmentcandidaturelist']); // Note that conf->hooks_modules contains array
...
while ($i < ($limit ? min($num, $limit) : $num)) {
    ...
    foreach ($object->fields as $key => $val) {
        ...
        if (in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && $key != 'status') {
            $cssforfield .= ($cssforfield ? ' ' : '') . 'right';
        }
