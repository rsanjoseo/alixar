...
require_once $dolibarr_main_document_root . '/Modules/Compta/facture/class/facture.class.php';
require_once $dolibarr_main_document_root . '/Modules/Comm/propal/class/propal.class.php';
require_once $dolibarr_main_document_root . '/Modules/Contrats/class/contrat.class.php';
require_once $dolibarr_main_document_root . '/Modules/Commande/class/commande.class.php';
require_once $dolibarr_main_document_root . '/Modules/Fourn/class/fournisseur.commande.class.php';
...
$langs->loadLangs(["admin", "install", "bills", "suppliers"]);
...
if (!GETPOST('action', 'aZ09') || preg_match('/upgrade/i', GETPOST('action', 'aZ09'))) {
    print '<h3><img class="valignmiddle inline-block paddingright" src= "' . BASE_URI . '/theme/common/octicons/build/svg/database.svg" width="20" alt="Database"> ';
    ...
    if (!$error) {
        $parameters = ['versionfrom' => $versionfrom, 'versionto=' . $versionto];
        $object = new stdClass();
        $action = "upgrade";
    ...
    if ($resql) {
        ...
        if ($num) {
            ...
            while ($i < $num) {
                ...
                $sql .= ")";
                $sql .= " VALUES (";
                $sql .= $obj->barowid . "," . $obj->bbrowid . ", '/Modules/Compta/bank/line.php?rowid=', '(banktransfert)', 'banktransfert'";
                $sql .= ")";
...
function migrate_restore_missing_links($db, $langs, $conf)
{
    ...
    if (($db->type == 'mysql' || $db->type == 'mysqli')) {
        if (versioncompare($db->getVersionArray(), [4, 0]) < 0) {
        ...
        if ($num) {
            ...
            while ($i < $num) {
                ...
                if ($obj->task_duration > 0) {
                    ...
                    [$hour, $min] = explode('.', $obj->task_duration);
                    $hour = $hour * 60 * 60;
                    $min = ($min / 100) * 60 * 60;
                    $newtime = $hour + $min;
...
function migrate_mode_reglement($db, $langs, $conf)
{
    ...
    $elements = [
        'old_id' => [5, 8, 9, 10, 11],
        'new_id' => [50, 51, 52, 53, 54],
        'code' => ['VAD', 'TRA', 'LCR', 'FAC', 'PRO'],
        'tables' => ['commande_fournisseur', 'commande', 'facture_rec', 'facture', 'propal'],
    ];
    $count = 0;
                ...
                while ($obj = $db->fetch_object($resql)) {
                    if (!isset($filles[$obj->fk_categorie_fille])) {    // Only one record as child (a child has only on parent).
                        if ($obj->fk_categorie_mere != $obj->fk_categorie_fille) {
                            ...
                            $couples[$obj->fk_categorie_mere . '_' . $obj->fk_categorie_fille] = ['mere' => $obj->fk_categorie_mere, 'fille' => $obj->fk_categorie_fille];
                        }
                    }
                }
...
function migrate_delete_old_files($db, $langs, $conf)
{
    ...
    $filetodeletearray = [
        ...
        '/Modules/Categories/class/api_category.class.php',
        '/Modules/Categories/class/api_deprecated_category.class.php',
        '/Modules/Compta/facture/class/api_invoice.class.php',
        '/Modules/Commande/class/api_commande.class.php',
        '/Modules/Users/class/api_user.class.php',
        '/Modules/Products/class/api_product.class.php',
        '/Modules/Societes/class/api_contact.class.php',
        '/Modules/Societes/class/api_thirdparty.class.php',
        '/support/online.php',
 ...
 */
function migrate_reload_modules($db, $langs, $conf, $listofmodule = [], $force = 0)
{
...
function migrate_export_import_profiles($mode = 'export')
{
    ...
    if ($resql) {
        while ($obj = $db->fetch_object($resql)) {
            $oldfield = $obj->field;
            $newfield = str_replace([',f.facnumber', 'f.facnumber,', 'f.total,', 'f.tva,'], [',f.ref', 'f.ref,', 'f.total_ht,', 'f.total_tva,'], $oldfield);

            if ($mode == 'export') {
                $oldfilter = $obj->filter;
                $newfilter = str_replace(['f.facnumber=', 'f.total=', 'f.tva='], ['f.ref=', 'f.total_ht=', 'f.total_tva='], $oldfilter);
            } else {
                $oldfilter = '';
                $newfilter = '';
            }
