...
$langs->loadLangs(["admin", "install", "other"]);
...
if ($ok && GETPOST('standard', 'alpha')) {
    ...
    $listofmodulesextra = [
        'societe' => 'societe', 'adherent' => 'adherent', 'product' => 'product',
    ...
    foreach ($listofmodulesextra as $tablename => $elementtype) {
        ...
        if ($resql) {
            ...
            while ($obj = $db->fetch_object($resql)) {
                ...
                if (in_array($fieldname, ['rowid', 'tms', 'fk_object', 'import_key'])) {
                    continue;
                }
                $arrayoffieldsfound[$fieldname] = ['type' => $fieldtype];
            }
    ...
    if ($resql) {
        ...
        while ($i < $num) {
            ...
            */
            ...
            if (!empty($name)) {
                $filetotest = $dolibarr_main_data_root . '/Modules/Societes/logos/' . $name . $ext;
                $filetotestsmall = $dolibarr_main_data_root . '/Modules/Societes/logos/thumbs/' . $name . '_small' . $ext;
                ...
                if ($exists) {
                    $filetarget = $dolibarr_main_data_root . '/Modules/Societes/' . $obj->rowid . '/logos/' . $name . $ext;
                    $filetargetsmall = $dolibarr_main_data_root . '/Modules/Societes/' . $obj->rowid . '/logos/thumbs/' . $name . '_small' . $ext;
                    $existt = dol_is_file($filetarget);
                    if (!$existt) {
                        if (GETPOST('restore_thirdparties_logos', 'alpha') == 'confirmed') {
                            dol_mkdir($dolibarr_main_data_root . '/Modules/Societes/' . $obj->rowid . '/logos');
                        }
                    ...
                    if (!$existtt) {
                        if (GETPOST('restore_thirdparties_logos', 'alpha') == 'confirmed') {
                            dol_mkdir($dolibarr_main_data_root . '/Modules/Societes/' . $obj->rowid . '/logos/thumbs');
                        }
...
if ($ok && GETPOST('clean_orphelin_dir', 'alpha')) {
    $listmodulepart = ['company', 'invoice', 'invoice_supplier', 'propal', 'order', 'order_supplier', 'contract', 'tax'];
    foreach ($listmodulepart as $modulepart) {
        ...
        $filearray = dol_dir_list($upload_dir, "files", 1, '', ['^SPECIMEN\.pdf$', '^\.', '(\.meta|_preview.*\.png)$', '^temp$', '^payments$', '^CVS$', '^thumbs$'], '', SORT_DESC, 1, true);
        ...
        if ($modulepart == 'invoice') {
            ...
            include_once DOL_DOCUMENT_ROOT . '/Modules/Fourn/class/fournisseur.facture.class.php';
            $object_instance = new FactureFournisseur($db);
        } elseif ($modulepart == 'propal') {
            include_once DOL_DOCUMENT_ROOT . '/Modules/Comm/propal/class/propal.class.php';
            ...
            include_once DOL_DOCUMENT_ROOT . '/Modules/Compta/sociales/class/chargesociales.class.php';
