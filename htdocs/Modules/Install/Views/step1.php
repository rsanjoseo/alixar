...
if (!$error) {
    ...
    if ($result) {
        ...
        if (!$error && (!empty($db_create_database) || !empty($db_create_user))) {
            ...
            */
            $db = getConnetion($db_type, $db_host, $userroot, $passroot, $db_port, $db_name, $db_prefix, $langs->getDefaultLang());
        ...
        if (!$error && (empty($db_create_database) && empty($db_create_user))) {
            ...
            $db = getConnetion($db_type, $db_host, $db_user, $db_pass, $db_port, $db_name, $db_prefix, $langs->getDefaultLang());
...
if (!$error && $db->connected && $action == "set") {
    ...
    // Show title of step
    print '<h3><img class="valignmiddle inline-block paddingright" src= "' . BASE_URI . '/theme/common/octicons/build/svg/gear.svg" width="20" alt="Configuration"> ' . $langs->trans("ConfigurationFile") . '</h3>';
    ...
    if (!$error) {
        ...
        if (isset($db_create_user) && ($db_create_user == "1" || $db_create_user == "on")) {
            ...
            $db = getConnetion($conf->db->type, $conf->db->host, $userroot, $passroot, $databasefortest, $conf->db->port);
            ...
            if (!$error) {
                if ($db->connected) {
                    ...
                    print $dolibarr_main_db_user;
                    print '</td>';
                    print '<td><img src= "' . BASE_URI . '/theme/eldy/img/error.png" alt="Error"></td>';
                    print '</tr>';
        ...
        if (!$error && (isset($db_create_database) && ($db_create_database == "1" || $db_create_database == "on"))) {
            ...
            $newdb = getConnetion($conf->db->type, $conf->db->host, $userroot, $passroot, '', $conf->db->port);
        ...
        if (!$error) {
            ...
            if ($db->connected) {
                ...
                print "</td><td>";
                print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok">';
                print "</td></tr>";

                // server access ok, basic access ok
                if (true || $db->database_selected) {   // If It has been possible to connect, it will always have a selected database
                    ...
                    print $dolibarr_main_db_name;
                    print '</td><td>';
                    print '<img src= "' . BASE_URI . '/theme/eldy/img/error.png" alt="Error">';
                    print "</td></tr>";
