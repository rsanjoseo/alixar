...
print '<h3><img class="valignmiddle inline-block paddingright" src= "' . BASE_URI . '/theme/common/octicons/build/svg/gear.svg" width="20" alt="Database"> ';
...
if (!empty($useragent)) {
    ...
    if ($browsername == 'ie' && $browserversion < 7) {
        print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("WarningBrowserTooOld") . "<br>\n";
    }
}

// Check PHP version
$arrayphpminversionerror = [5, 5, 0];
$arrayphpminversionwarning = [5, 6, 0];
if (versioncompare(versionphparray(), $arrayphpminversionerror) < 0) {        // Minimum to use (error if lower)
    print '<img src= "' . BASE_URI . '/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPVersionTooLow", versiontostring($arrayphpminversionerror));
    ...
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPVersionTooLow", versiontostring($arrayphpminversionwarning));
    $checksok = 0; // 0=error, 1=warning
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPVersion") . " " . versiontostring(versionphparray());
}
if (empty($force_install_nophpinfo)) {
    print ' (<a href="Dol_Phpinfo.php" target="_blank">' . $langs->trans("MoreInformation") . '</a>)';
}
print "<br>\n";

// Check PHP support for $_GET and $_POST
if (!isset($_GET["testget"]) && !isset($_POST["testpost"])) {    // We must keep $_GET and $_POST here
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Warning" class="valignmiddle"> ' . $langs->trans("PHPSupportPOSTGETKo");
    print ' (<a href="' . dol_escape_htmltag($_SERVER["PHP_SELF"]) . '?testget=ok">' . $langs->trans("Recheck") . '</a>)';
    print "<br>\n";
    $checksok = 0;
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupportPOSTGETOk") . "<br>\n";
}

// Check if session_id is enabled
if (!function_exists("session_id")) {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupportSessions") . "<br>\n";
    $checksok = 0;
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupportSessions") . "<br>\n";
}

// Check if GD is supported (we need GD for image conversion)
if (!function_exists("imagecreate")) {
    $langs->load("errors");
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupportGD") . "<br>\n";
    // $checksok = 0;		// If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupport", "GD") . "<br>\n";
}

// Check if Curl is supported
if (!function_exists("curl_init")) {
    $langs->load("errors");
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupportCurl") . "<br>\n";
    // $checksok = 0;		// If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupport", "Curl") . "<br>\n";
}

// Check if PHP calendar extension is available
if (!function_exists("easter_date")) {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupportCalendar") . "<br>\n";
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupport", "Calendar") . "<br>\n";
}

// Check if UTF8 is supported
if (!function_exists("utf8_encode")) {
    $langs->load("errors");
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupportUTF8") . "<br>\n";
    // $checksok = 0;		// If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupport", "UTF8") . "<br>\n";
}
...
if (empty($_SERVER["SERVER_ADMIN"]) || $_SERVER["SERVER_ADMIN"] != 'doliwamp@localhost') {
    if (!function_exists("locale_get_primary_language") || !function_exists("locale_get_region")) {
        $langs->load("errors");
        print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupportIntl") . "<br>\n";
        // $checksok = 0;		// If ko, just warning. So check must still be 1 (otherwise no way to install)
    } else {
        print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupport", "Intl") . "<br>\n";
    }
}

if (!class_exists('ZipArchive')) {
    $langs->load("errors");
    print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ErrorPHPDoesNotSupport", "ZIP") . "<br>\n";
    // $checksok = 0;		// If ko, just warning. So check must still be 1 (otherwise no way to install)
} else {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPSupport", "ZIP") . "<br>\n";
}
...
if ($memmaxorig != '') {
    ...
    if ($memmax >= $memrequired || $memmax == -1) {
        print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ' . $langs->trans("PHPMemoryOK", $memmaxorig, $memrequiredorig) . "<br>\n";
    } else {
        print '<img src= "' . BASE_URI . '/theme/eldy/img/warning.png" alt="Warning" class="valignmiddle"> ' . $langs->trans("PHPMemoryTooLow", $memmaxorig, $memrequiredorig) . "<br>\n";
    }
}
...
if (!file_exists($conffile)) {
    print '<img src= "' . BASE_URI . '/theme/eldy/img/error.png" alt="Error" class="valignmiddle"> ' . $langs->trans("ConfFileDoesNotExistsAndCouldNotBeCreated", $conffiletoshow);
    ...
    if (dol_is_dir($conffile)) {
        print '<img src= "' . BASE_URI . '/theme/eldy/img/error.png" alt="Warning"> ' . $langs->trans("ConfFileMustBeAFileNotADir", $conffiletoshow);

        $allowinstall = 0;
    } elseif (!is_writable($conffile)) {
        // File exists but cannot be modified
        if ($confexists) {
            print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("ConfFileExists", $conffiletoshow);
        } else {
            print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("ConfFileCouldBeCreated", $conffiletoshow);
        }
        print "<br>";
        print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Warning"> ' . $langs->trans("ConfFileIsNotWritable", $conffiletoshow);
        print "<br>\n";
        ...
        print "<br>";
        print '<img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok"> ' . $langs->trans("ConfFileIsWritable", $conffiletoshow);
        print "<br>\n";
    ...
    if ($checksok) {
        ...
        if (empty($dolibarr_main_db_host)) {    // This means install process was not run
            ...
            // <img src= "' . BASE_URI . '/theme/eldy/img/tick.png" alt="Ok" class="valignmiddle"> ';
        }

        $choice .= '</td>';
        $choice .= '<td class="center">';
        if ($allowinstall) {
            $url = BASE_URI . '/Modules/Install/Controllers/Dol_Fileconf.php?selectlang=' . $setuplang;
            $choice .= '<a class="button" href="' . $url . '">' . $langs->trans("Start") . '</a>';
        } else {
