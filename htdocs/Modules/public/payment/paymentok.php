...
if ($ispaymentok) {
    ...
    if (array_key_exists('MEM', $tmptag) && $tmptag['MEM'] > 0) {
        ...
        include_once DOL_DOCUMENT_ROOT . '/Modules/Adherents/class/adherent_type.class.php';
        include_once DOL_DOCUMENT_ROOT . '/Modules/Adherents/class/subscription.class.php';
        ...
        include_once DOL_DOCUMENT_ROOT . '/Modules/Don/class/don.class.php';
        $don = new Don($db);
        ...
        if ($result) {
            ...
            if (!empty($FinalPaymentAmt) && $paymentTypeId > 0) {
                ...
                include_once DOL_DOCUMENT_ROOT . '/Modules/Don/class/paymentdonation.class.php';
                $paiement = new PaymentDonation($db);

                if ($currencyCodeType == $conf->currency) {
                    $paiement->amounts = [$object->id => $FinalPaymentAmt]; // Array with all payments dispatching with donation
                } else {
                ...
                if (!$error) {
                    ...
                    if ($paiement_id < 0) {
                        ...
                        if ($totalpayed >= $don->getRemainToPay()) {
                            $don->setPaid($don->id);
                        }
                    }
                }
...
if ($ispaymentok) {
    ...
    if ($sendemail) {
        ...
        if (array_key_exists('MEM', $tmptag)) {
            $url = $urlwithroot . "/Modules/Adherents/subscription.php?rowid=" . $tmptag['MEM'];
            ...
            $url = $urlwithroot . "/Modules/Compta/facture/card.php?id=" . $tmptag['INV'];
