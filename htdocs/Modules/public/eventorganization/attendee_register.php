...
/**
 *    \file       htdocs/public/Modules/Eventorganization/attendee_register.php
 *    \ingroup    project
...
if (empty($reshook) && $action == 'add' && (!empty($conference->id) && $conference->status != 2 || !empty($project->id) && $project->status == Project::STATUS_VALIDATED)) {
    ...
    if (!$error) {
        ...
        if (!empty($confattendee->date_subscription) && !empty($confattendee->amount)) {
            ...
            $redirection = $dolibarr_main_url_root . '/public/Modules/Eventorganization/subscriptionok.php?id=' . ((int) $id) . '&securekey=' . urlencode($securekeyurl);
