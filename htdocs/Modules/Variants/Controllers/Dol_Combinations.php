...
if (!$object->isProduct() && !$object->isService()) {
    header('Location: ' . dol_buildpath('/Modules/Products/card.php?id=' . $object->id, 2));
    exit();
}
if ($action == 'add') {
...
if (!empty($id) || !empty($ref)) {
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Products/list.php?type=' . $object->type . '">' . $langs->trans("BackToList") . '</a>';
            ...
            if (is_array($productCombination2ValuePairs1) && count($productCombination2ValuePairs1)) {
                ...
                <tr>
                    <td class="titlefieldcreate tdtop"><label
                                for="features"><?php echo $langs->trans('Combination') ?></label></td>
                    <td class="tdtop">
