...
if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled)) {
    ...
    require_once DOL_DOCUMENT_ROOT . '/Modules/Fourn/class/fournisseur.commande.dispatch.class.php';
}
    ...
    if ($origin) {
        ...
        if ($objectsrc->fetch($origin_id)) {    // This include the fetch_lines
            ...
            if ($origin == 'supplierorder' && ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled))) {
                print $langs->trans("RefOrder") . '</td><td colspan="3"><a href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?id=' . $objectsrc->id . '">' . img_object($langs->trans("ShowOrder"), 'order') . ' ' . $objectsrc->ref;
            }
            if ($origin == 'propal' && !empty($conf->propal->enabled)) {
                print $langs->trans("RefProposal") . '</td><td colspan="3"><a href="' . DOL_URL_ROOT . '/Modules/Comm/card.php?id=' . $objectsrc->id . '">' . img_object($langs->trans("ShowProposal"), 'propal') . ' ' . $objectsrc->ref;
            }
            print '</a></td>';
            print "</tr>\n";
            ...
            foreach ($_POST as $key => $value) {
                ...
                if (preg_match('/^product_([0-9]+)_([0-9]+)$/i', $key, $reg)) {
                    ...
                    $dispatchLines[$numAsked] = ['prod' => GETPOST($prod, 'int'), 'qty' => price2num(GETPOST($qty), 'MS'), 'ent' => GETPOST($ent, 'int'), 'pu' => price2num(GETPOST($pu), 'MU'), 'comment' => GETPOST('comment'), 'fk_commandefourndet' => GETPOST($fk_commandefourndet, 'int')];
                }
            ...
            while ($indiceAsked <= $numAsked) {
                ...
                if (is_array($extralabelslines) && count($extralabelslines) > 0) {
                    ...
                    print $line->showOptionals($extrafields, 'edit', ['style' => 'class="oddeven"', 'colspan' => $colspan], $indiceAsked);
                }
    ...
    if ($object->id > 0) {
        ...
        if (!empty($conf->projet->enabled)) {
            ...
            if (0) {    // Do not change on reception
                ...
                if (!empty($objectsrc->fk_project)) {
                    ...
                    $morehtmlref .= '<a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?id=' . $objectsrc->fk_project . '" title="' . $langs->trans('ShowProject') . '">';
        ...
        for ($i = 0; $i < $num_prod; $i++) {
            ...
            if (is_array($extralabelslines) && count($extralabelslines) > 0) {
                ...
                if ($action == 'editline' && $lines[$i]->id == $line_id) {
                    print $line->showOptionals($extrafields, 'edit', ['colspan' => $colspan], $indiceAsked);
                } else {
                    print $line->showOptionals($extrafields, 'view', ['colspan' => $colspan], $indiceAsked);
                }
            }
        }
