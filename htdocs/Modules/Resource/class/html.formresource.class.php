...
class FormResource
{
     ...
     */
    public function select_resource_list($selected = '', $htmlname = 'fk_resource', $filter = '', $showempty = 0, $showtype = 0, $forcecombo = 0, $event = [], $filterkey = '', $outputmode = 0, $limit = 20, $morecss = '', $multiple = false)
    {
        // phpcs:enable
        ...
        if ($resourcestat) {
            ...
            if ($num) {
                while ($i < $num) {
                    ...
                    array_push($outarray, ['key' => $resourcestat->lines[$i]->id, 'value' => $resourcestat->lines[$i]->label, 'label' => $resourcestat->lines[$i]->label]);
