...
$langs->loadLangs(["companies", "products", "admin", "mails", "other", "errors"]);
...
if ($action == 'edit') {
    ...
    // ID
    if (!empty($conf->use_javascript_ajax) || (isset($conf->global->MAIN_MAIL_SENDMODE) && in_array($conf->global->MAIN_MAIL_SENDMODE, ['smtps', 'swiftmailer']))) {
    ...
    if (!empty($conf->use_javascript_ajax) || (isset($conf->global->MAIN_MAIL_SENDMODE) && in_array($conf->global->MAIN_MAIL_SENDMODE, ['swiftmailer']))) {
        if (function_exists('openssl_open')) {
    ...
    if (empty($conf->global->MAIN_DISABLE_ALL_MAILS)) {
        ...
        // SMTPS ID
        if (isset($conf->global->MAIN_MAIL_SENDMODE) && in_array($conf->global->MAIN_MAIL_SENDMODE, ['smtps', 'swiftmailer'])) {
        ...
        if ($conf->global->MAIN_MAIL_SENDMODE == 'swiftmailer') {
            ...
            if (isset($conf->global->MAIN_MAIL_SENDMODE) && in_array($conf->global->MAIN_MAIL_SENDMODE, ['swiftmailer'])) {
                if (function_exists('openssl_open')) {
    ...
    if (!in_array($action, ['testconnect', 'test', 'testhtml'])) {
        $text = '';
    ...
    if ($action == 'testconnect') {
        ...
        $mail = new CMailFile('', '', '', '', [], [], [], '', '', 0, '', '', '', '', $trackid, $sendcontext);
