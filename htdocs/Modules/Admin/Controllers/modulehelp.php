...
$langs->loadLangs(['errors', 'admin', 'modulebuilder', 'exports']);
...
if ($mode == 'feature') {
    ...
    $sqlfiles = dol_dir_list(dol_buildpath($moduledir . '/sql/'), 'files', 0, 'llx.*\.sql', ['\.key\.sql', '\.sql\.back']);
    if (count($sqlfiles) > 0) {
    ...
    $triggers = $interfaces->getTriggersList([(($objMod->isCoreOrExternalModule() == 'external') ? '/' . $moduledir : '') . '/core/triggers']);
