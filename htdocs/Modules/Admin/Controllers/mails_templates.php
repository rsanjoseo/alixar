...
$langsArray = ["errors", "admin", "mails", "languages"];
...
$hookmanager->initHooks(['emailtemplates']);
...
$parameters = ['elementList' => $elementList];
    ...
    if (GETPOST('actionadd', 'alpha') || GETPOST('actionmodify', 'alpha')) {
        ...
        if ($ok && GETPOST('actionadd')) {
            ...
            foreach ($listfieldinsert as $f => $value) {
                ...
                } elseif (in_array($keycode, ['joinfiles', 'private', 'position'])) {
                    $sql .= (int) GETPOST($keycode, 'int');
                } else {
...
if ($action == 'view') {
    ...
    $fieldsforcontent = ['topic', 'joinfiles', 'content'];
    if (!empty($conf->global->MAIN_EMAIL_TEMPLATES_FOR_OBJECT_LINES)) {
        $fieldsforcontent = ['topic', 'joinfiles', 'content', 'content_lines'];
    }
...
if ($resql) {
    ...
    foreach ($fieldlist as $field => $value) {
        if ($value == 'label') {
            ...
            print '</td>';
        } elseif (!in_array($value, ['content', 'content_lines'])) {
    ...
    if ($num) {
        ...
        while ($i < $num) {
            ...
            if ($action == 'edit' && ($rowid == (!empty($obj->rowid) ? $obj->rowid : $obj->code))) {
                ...
                if (empty($reshook)) {
                    foreach ($fieldlist as $field => $value) {
                        if (in_array($fieldlist[$field], ['content', 'content_lines'])) {
                            continue;
                        }
                        $showfield = 1;
                        $align = "";
                        ...
                        if ($showfield) {
                            ...
                            if (in_array($value, ['code', 'label', 'topic'])) {
...
function fieldList($fieldlist, $obj = '', $tabname = '', $context = '')
{
    ...
    foreach ($fieldlist as $field => $value) {
        if ($value == 'fk_user') {
            ...
            print '</td>';
        } elseif ($context == 'add' && in_array($value, ['topic', 'joinfiles', 'content', 'content_lines'])) {
            continue;
        } elseif ($context == 'edit' && in_array($value, ['topic', 'joinfiles', 'content', 'content_lines'])) {
            continue;
        } elseif ($context == 'hide' && in_array($value, ['topic', 'joinfiles', 'content', 'content_lines'])) {
            continue;
        } else {
            $size = '';
            $class = '';
