...
$dirsyslogs = array_merge(['/core/modules/syslog/'], $conf->modules_parts['syslog']);
...
if (!empty($conf->loghandlers['mod_syslog_file']) && !empty($conf->cron->enabled)) {
    ...
    print ' (<a href="' . dol_buildpath('/Modules/Cron/list.php', 1) . '?search_label=CompressSyslogs&status=-1">' . $langs->trans('ConfigureCleaningCronjobToSetFrequencyOfSaves') . '</a>)</td></tr>';
}
