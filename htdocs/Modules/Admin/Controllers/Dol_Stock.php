...
if (preg_match('/set_([a-z0-9_\-]+)/i', $action, $reg)) {
    ...
    if (in_array($code, ['STOCK_CALCULATE_ON_BILL', 'STOCK_CALCULATE_ON_VALIDATE_ORDER', 'STOCK_CALCULATE_ON_SHIPMENT', 'STOCK_CALCULATE_ON_SHIPMENT_CLOSE'])) {
    ...
    if (in_array($code, ['STOCK_CALCULATE_ON_SUPPLIER_BILL', 'STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER', 'STOCK_CALCULATE_ON_RECEPTION', 'STOCK_CALCULATE_ON_RECEPTION_CLOSE', 'STOCK_CALCULATE_ON_SUPPLIER_DISPATCH_ORDER'])) {
...
if (!empty($conf->facture->enabled)) {
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_BILL', [], null, 0, 0, 0, 2, 1);
    } else {
...
if (!empty($conf->commande->enabled)) {
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_VALIDATE_ORDER', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
...
if (!empty($conf->expedition->enabled)) {
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_SHIPMENT', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
...
if (!empty($conf->expedition->enabled)) {
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_SHIPMENT_CLOSE', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
...
if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled)) {
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_SUPPLIER_BILL', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
...
if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled)) {
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
...
if (!empty($conf->reception->enabled)) {
    ...
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_RECEPTION', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
    ...
    if ($conf->use_javascript_ajax) {
        print ajax_constantonoff('STOCK_CALCULATE_ON_RECEPTION_CLOSE', [], null, 0, 0, 0, 2, 1);
    } else {
        $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
    ...
    if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled)) {
        if ($conf->use_javascript_ajax) {
            print ajax_constantonoff('STOCK_CALCULATE_ON_SUPPLIER_DISPATCH_ORDER', [], null, 0, 0, 0, 2, 1);
        } else {
            $arrval = ['0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
...
print '<td class="right">';
print $formproduct->selectWarehouses($conf->global->MAIN_DEFAULT_WAREHOUSE, 'default_warehouse', '', 1, 0, 0, '', 0, 0, [], 'left reposition');
