...
$langs->loadLangs(['companies', 'products', 'admin']);
...
$aCurrencies = [$conf->currency]; // Default currency always first position
...
if (empty($mysoc->country_code)) {
    ...
    print '<br><a href="' . BASE_URI . '?module=Admin&controller=company&mainmenu=home">' . $warnpicto . ' ' . $langs->trans("WarningMandatorySetupNotComplete") . '</a>';
} else {
    // Show examples
    ...
    if ($resql) {
        ...
        if ($num) {
            for ($i = 0; $i < $num; $i++) {
                ...
                $vat_rates[] = ['vat_rate' => $obj->vat_rate, 'code' => $obj->vat_code, 'localtax_rate1' => $obj->localtax_rate1, 'locltax_rate2' => $obj->localtax_rate2];
            }
        }
    } else {
        dol_print_error($db);
