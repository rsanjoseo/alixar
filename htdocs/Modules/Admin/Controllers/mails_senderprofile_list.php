...
$langs->loadLangs(["errors", "admin", "mails", "languages"]);
...
$hookmanager->initHooks(['emailsenderprofilelist']); // Note that conf->hooks_modules contains array
...
foreach ($object->fields as $key => $val) {
    ...
    if (!empty($val['visible'])) {
        $arrayfields['t.' . $key] = ['label' => $val['label'], 'checked' => (($val['visible'] < 0) ? 0 : 1), 'enabled' => ($val['enabled'] && ($val['visible'] != 3)), 'position' => $val['position']];
    }
}
// Extra fields
