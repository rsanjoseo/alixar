...
require_once DOL_DOCUMENT_ROOT . '/Modules/Fourn/class/fournisseur.commande.class.php';
...
$langs->loadLangs(['admin', 'orders', 'sendings', 'companies', 'bills', 'propal', 'receptions', 'supplier_proposal', 'deliveries', 'products', 'stocks', 'productbatch']);
...
$hookmanager->initHooks(['ordersuppliercard', 'globalcard']);
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Fourn/commande/list.php' . ($socid > 0 ? '?socid=' . ((int) $socid) : '');
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'reopen') {    // no test on permission here, permission to use will depends on status
        if (in_array($object->statut, [1, 2, 3, 4, 5, 6, 7, 9])) {
            if ($object->statut == 1) {
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes' && $usercandelete) {
        ...
        if ($result > 0) {
            header("Location: " . DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?restore_lastsearch_values=1');
            exit;
        } else {
            setEventMessages($object->error, $object->errors, 'errors');
        }
    ...
    if ($action == 'webservice' && GETPOST('mode', 'alpha') == "send" && !GETPOST('cancel', 'alpha')) {
        ...
        if (empty($conf->syncsupplierwebservices->enabled)) {
            ...
            foreach ($object->lines as $line) {
                $ws_parameters = ['authentication' => $ws_authentication, 'id' => '', 'ref' => $line->ref_supplier];
            ...
            $ws_parameters = ['authentication' => $ws_authentication, 'order' => $order];
...
if ($action == 'create') {
    ...
    if ($societe->id > 0) {
        ...
        print ' <a href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=create&client=0&fournisseur=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create') . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddThirdParty") . '"></span></a>';
    }
    print '</td>';
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print ' &nbsp; <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $societe->id . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $societe->id) . '"><span class="fa fa-plus-circle valignmiddle" title="' . $langs->trans("AddProject") . '"></span></a>';
        print '</td></tr>';
    }
    ...
    if ($action == 'clone') {
        ...
        $formquestion = [
            ['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company(GETPOST('socid', 'int'), 'socid', '(s.fournisseur=1)')],
        ];
    ...
    if ($action == 'approve' || $action == 'approve2') {
        ...
        if (!empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER) && $qualified_for_stock_change) {
            ...
            $formquestion = [
                ...
                ['type' => 'other', 'name' => 'idwarehouse', 'label' => $langs->trans("SelectWarehouseForStockIncrease"), 'value' => $formproduct->selectWarehouses(GETPOST('idwarehouse', 'int'), 'idwarehouse', '', 1, 0, 0, '', 0, $forcecombo)],
            ];
        }
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if (!empty($conf->global->MAIN_CAN_EDIT_SUPPLIER_ON_SUPPLIER_ORDER) && !empty($usercancreate) && $action == 'edit_thirdparty') {
        ...
        $morehtmlref .= $form->select_company($object->thirdparty->id, 'new_socid', 's.fournisseur=1', '', 0, 0, [], 0, 'minwidth300');
    ...
    if (empty($conf->global->MAIN_CAN_EDIT_SUPPLIER_ON_SUPPLIER_ORDER) || $action != 'edit_thirdparty') {
        ...
        if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
            $morehtmlref .= ' (<a href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?socid=' . $object->thirdparty->id . '&search_company=' . urlencode($object->thirdparty->name) . '">' . $langs->trans("OtherOrders") . '</a>)';
        }
    }
    ...
    if (!empty($conf->incoterm->enabled)) {
        ...
        if ($usercancreate) {
            print '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?id=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
        } else {
            print '&nbsp;';
        }
        print '</td></tr></table>';
    ...
    if ($user->socid == 0 && $action != 'editline' && $action != 'delete') {
        ...
        if (empty($reshook)) {
			...
			{
				print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddAction") . '</a></div>';
			}*/
            ...
            // Send
            if (empty($user->socid)) {
                if (in_array($object->statut, [CommandeFournisseur::STATUS_ACCEPTED, 3, 4, 5]) || !empty($conf->global->SUPPLIER_ORDER_SENDBYEMAIL_FOR_ALL_STATUS)) {
                    if ($usercanorder) {
            ...
            // Reopen
            if (in_array($object->statut, [CommandeFournisseur::STATUS_ACCEPTED])) {
                $buttonshown = 0;
            ...
            if (in_array($object->statut, [3, 4, 5, 6, 7, 9])) {
                if ($usercanorder) {
            ...
            if (!empty($conf->stock->enabled) && (!empty($conf->global->STOCK_CALCULATE_ON_SUPPLIER_DISPATCH_ORDER) || !empty($conf->global->STOCK_CALCULATE_ON_RECEPTION) || !empty($conf->global->STOCK_CALCULATE_ON_RECEPTION_CLOSE))) {
                ...
                if (in_array($object->statut, [3, 4, 5])) {
                    if (((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled)) && $usercanreceived) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/dispatch.php?id=' . $object->id . '">' . $labelofbutton . '</a></div>';
                    } else {
            ...
            if (((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_invoice->enabled)) && ($object->statut >= 2 && $object->statut != 7 && $object->billed != 1)) {  // statut 2 means approved, 7 means canceled
                if ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer) {
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("CreateBill") . '</a>';
                }
            }
            //}
        ...
        if ($action != 'makeorder') {
            ...
            $file = $conf->fournisseur->dir_output . '/Modules/Commande/' . $objref . '/' . $objref . '.pdf';
            $relativepath = $objref . '/' . $objref . '.pdf';
            $filedir = $conf->fournisseur->dir_output . '/Modules/Commande/' . $objref;
            ...
            $linktoelem = $form->showLinkToObjectBlock($object, null, ['supplier_order', 'order_supplier']);
        ...
        if ($action == 'webservice' && GETPOST('mode', 'alpha') != "send" && !GETPOST('cancel', 'alpha')) {
            ...
            if ($mode == "init") {
                ...
                $ws_parameters = ['authentication' => $ws_authentication, 'id' => '', 'ref' => $ws_user];
                ...
                if ($user_status_code == "OK") {
                    ...
                    if (empty($ws_thirdparty)) {
                        ...
                        foreach ($object->lines as $line) {
                            ...
                            $ws_parameters = ['authentication' => $ws_authentication, 'id' => '', 'ref' => $ref_supplier];
                            $result_product = $soapclient_product->call("getProductOrService", $ws_parameters, $ws_ns, '');
                            if (!$result_product) {
