    ...
    if ($action == 'confirm_paiement' && $confirm == 'yes') {
        ...
        if (!$error) {
            ...
            if (!$error) {
                ...
                if ($invoiceid > 0) {
                    $loc = DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?facid=' . $invoiceid;
                } else {
                    $loc = DOL_URL_ROOT . '/Modules/Fourn/paiement/card.php?id=' . $paiement_id;
                }
                header('Location: ' . $loc);
                exit;
            } else {
...
if ($action == 'create' || $action == 'confirm_paiement' || $action == 'add_paiement') {
    ...
    if ($resql) {
        ...
        if ($num) {
            ...
            $parameters = ['facid' => $facid, 'ref' => $ref, 'objcanvas' => $objcanvas];
