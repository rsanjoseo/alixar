...
$langs->loadLangs(['products', 'bills', 'companies', 'projects']);
...
$hookmanager->initHooks(['supplierinvoicelist']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'f.ref' => ['label' => "Ref", 'checked' => 1],
    'f.ref_supplier' => ['label' => "RefSupplier", 'checked' => 1],
    'f.type' => ['label' => "Type", 'checked' => 0],
    'f.label' => ['label' => "Label", 'checked' => 0],
    'f.datef' => ['label' => "DateInvoice", 'checked' => 1],
    'f.date_lim_reglement' => ['label' => "DateDue", 'checked' => 1],
    'p.ref' => ['label' => "ProjectRef", 'checked' => 0],
    ...
    'f.fk_cond_reglement' => ['label' => "PaymentTerm", 'checked' => 1, 'position' => 50],
    'f.fk_mode_reglement' => ['label' => "PaymentMode", 'checked' => 1, 'position' => 52],
    'f.total_ht' => ['label' => "AmountHT", 'checked' => 1, 'position' => 105],
    'f.total_vat' => ['label' => "AmountVAT", 'checked' => 0, 'position' => 110],
    'f.total_localtax1' => ['label' => $langs->transcountry("AmountLT1", $mysoc->country_code), 'checked' => 0, 'enabled' => $mysoc->localtax1_assuj == "1", 'position' => 95],
    'f.total_localtax2' => ['label' => $langs->transcountry("AmountLT2", $mysoc->country_code), 'checked' => 0, 'enabled' => $mysoc->localtax2_assuj == "1", 'position' => 100],
    'f.total_ttc' => ['label' => "AmountTTC", 'checked' => 0, 'position' => 115],
    'u.login' => ['label' => "Author", 'checked' => 1],
    'dynamount_payed' => ['label' => "Paid", 'checked' => 0],
    'rtp' => ['label' => "Rest", 'checked' => 0],
    'f.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'f.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'f.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'f.multicurrency_total_vat' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'f.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'multicurrency_dynamount_payed' => ['label' => 'MulticurrencyAlreadyPaid', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'multicurrency_rtp' => ['label' => 'MulticurrencyRemainderToPay', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)], // Not enabled by default because slow
    'f.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'f.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 500],
...
if ($resql) {
    ...
    if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
        ...
        header("Location: " . DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?facid=' . $id);
        exit;
    }
    ...
    $url = DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?action=create';
    if (!empty($socid)) {
    ...
    if (!empty($arrayfields['f.fk_statut']['checked'])) {
        ...
        $liststatus = ['0' => $langs->trans("Draft"), '1' => $langs->trans("Unpaid"), '2' => $langs->trans("Paid")];
