...
$arrayfields = [
    ...
    'p.num_paiement' => ['label' => "Numero", 'checked' => 1, 'position' => 50, 'tooltip' => "ChequeOrTransferNumber"],
    'ba.label' => ['label' => "Account", 'checked' => 1, 'position' => 60, 'enable' => (!empty($conf->banque->enabled))],
    'p.amount' => ['label' => "Amount", 'checked' => 1, 'position' => 70],
];
$arrayfields = dol_sort_array($arrayfields, 'position');

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(['paymentsupplierlist']);
...
while ($i < min($num, $limit)) {
    ...
    if (!empty($arrayfields['ba.label']['checked'])) {
        print '<td>';
        if ($objp->bid) {
            print '<a href="' . DOL_URL_ROOT . '/Modules/Compta/bank/bankentries_list.php?account=' . $objp->bid . '">' . img_object($langs->trans("ShowAccount"), 'account') . ' ' . dol_trunc($objp->label, 24) . '</a>';
        } else {
            print '&nbsp;';
        }
        print '</td>';
