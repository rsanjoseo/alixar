...
$langs->loadLangs(['bills', 'compta', 'suppliers', 'companies', 'products', 'banks', 'admin']);
...
$hookmanager->initHooks(['invoicesuppliercard', 'globalcard']);
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Fourn/facture/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
            ...
            if (!$error) {
                ...
                if (!$error && GETPOST('origin', 'alpha') && GETPOST('originid')) {
                    ...
                    if ($id > 0) {
                        ...
                        if (GETPOST('type') == FactureFournisseur::TYPE_DEPOSIT && in_array($typeamount, ['amount', 'variable'])) {
	...
	*/
    ...
    if ($id > 0 || !empty($ref)) {
        ...
        if ($action == 'clone') {
            ...
            $formquestion = [
                ['type' => 'text', 'name' => 'newsupplierref', 'label' => $langs->trans("RefSupplier"), 'value' => $langs->trans("CopyOf") . ' ' . $object->ref_supplier],
        ...
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
        ...
        if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
            $morehtmlref .= ' (<a href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/list.php?socid=' . $object->thirdparty->id . '&search_company=' . urlencode($object->thirdparty->name) . '">' . $langs->trans("OtherBills") . '</a>)';
        }
        // Project
        if (!empty($conf->projet->enabled)) {
            $langs->load("projects");
        ...
        if (!empty($conf->incoterm->enabled)) {
            ...
            if ($usercancreate) {
                print '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?facid=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
            } else {
                print '&nbsp;';
            }
            print '</td></tr></table>';
        ...
        if ($action != 'presend') {
            ...
            if (empty($reshook)) {
                ...
                if ($object->type != FactureFournisseur::TYPE_CREDIT_NOTE && $object->statut == FactureFournisseur::STATUS_VALIDATED && $object->paye == 0) {
                    print '<a class="butAction' . ($conf->use_javascript_ajax ? ' reposition' : '') . '" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/paiement.php?facid=' . $object->id . '&amp;action=create' . ($object->fk_account > 0 ? '&amp;accountid=' . $object->fk_account : '') . '">' . $langs->trans('DoPayment') . '</a>'; // must use facid because id is for payment id not invoice
                }
                ...
                if ($object->type == FactureFournisseur::TYPE_CREDIT_NOTE || $object->type == FactureFournisseur::TYPE_DEPOSIT || $object->type == FactureFournisseur::TYPE_STANDARD) {
                    ...
                    if ($object->type == FactureFournisseur::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0) {
                        if ($resteapayer == 0) {
                            ...
                            print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/paiement.php?facid=' . $object->id . '&amp;action=create&amp;accountid=' . $object->fk_account . '">' . $langs->trans('DoPaymentBack') . '</a>';
                        }
                    }
                ...
                if ($action != 'confirm_edit') {
                    ...
                    $linktoelem = $form->showLinkToObjectBlock($object, null, ['invoice_supplier']);
