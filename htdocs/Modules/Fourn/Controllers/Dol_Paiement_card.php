...
$langs->loadLangs(['bills', 'banks', 'companies', 'suppliers']);
...
$hookmanager->initHooks(['supplierpaymentcard', 'globalcard']);
...
if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->fournisseur->facture->supprimer) {
    ...
    if ($result > 0) {
        $db->commit();
        header('Location: ' . DOL_URL_ROOT . '/Modules/Fourn/paiement/list.php');
        exit;
    } else {
        setEventMessages($object->error, $object->errors, 'errors');
        $db->rollback();
...
if ($result > 0) {
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Fourn/paiement/list.php' . (!empty($socid) ? '?socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if ($resql) {
        ...
        if ($num > 0) {
            ...
            while ($i < $num) {
                ...
                // Third party
                print '<td><a href="' . DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=' . $objp->socid . '">' . img_object($langs->trans('ShowCompany'), 'company') . ' ' . $objp->name . '</a></td>';
                // Expected to pay
