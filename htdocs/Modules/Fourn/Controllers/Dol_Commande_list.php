...
$langs->loadLangs(["orders", "sendings", 'deliveries', 'companies', 'compta', 'bills', 'projects', 'suppliers', 'products']);
...
$hookmanager->initHooks(['supplierorderlist']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'cf.ref' => ['label' => "Ref", 'checked' => 1],
    'cf.ref_supplier' => ['label' => "RefOrderSupplierShort", 'checked' => 1, 'enabled' => 1],
    'p.project_ref' => ['label' => "ProjectRef", 'checked' => 0, 'enabled' => 1],
    'u.login' => ['label' => "AuthorRequest", 'checked' => 1],
    ...
    'cf.date_commande' => ['label' => "OrderDateShort", 'checked' => 1],
    'cf.date_livraison' => ['label' => "DateDeliveryPlanned", 'checked' => 1, 'enabled' => empty($conf->global->ORDER_DISABLE_DELIVERY_DATE)],
    'cf.total_ht' => ['label' => "AmountHT", 'checked' => 1],
    'cf.total_tva' => ['label' => "AmountVAT", 'checked' => 0],
    'cf.total_ttc' => ['label' => "AmountTTC", 'checked' => 0],
    'cf.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'cf.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'cf.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'cf.multicurrency_total_tva' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'cf.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'cf.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'cf.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 500],
    'cf.fk_statut' => ['label' => "Status", 'checked' => 1, 'position' => 1000],
    'cf.billed' => ['label' => "Billed", 'checked' => 1, 'position' => 1000, 'enabled' => 1],
];
// Extra fields
                    ...
                    if ($objecttmp->statut == 0) {
                        ...
                        if ($result >= 0) {
                            ...
                            if (empty($conf->global->SUPPLIER_ORDER_NO_DIRECT_APPROVE) && $user->rights->fournisseur->commande->approuver && !(!empty($conf->global->STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER) && $objecttmp->hasProductsOrServices(1))) {
                                ...
                                setEventMessages($langs->trans("SupplierOrderValidatedAndApproved"), [$objecttmp->ref]);
                            } else {
                                setEventMessages($langs->trans("SupplierOrderValidated"), [$objecttmp->ref]);
                            }
                        } else {
    ...
    if ($massaction == 'confirm_createsupplierbills') {
        ...
        if (!$error) {
            ...
            if ($nb_bills_created == 1) {
                ...
                $texttoshow = str_replace('{s1}', '<a href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?id=' . urlencode($lastid) . '">' . $lastref . '</a>', $texttoshow);
...
if ($search_request_author) {
    $sql .= natural_search(['u.lastname', 'u.firstname', 'u.login'], $search_request_author);
}
...
if ($resql) {
    ...
    if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
        ...
        $id = $obj->rowid;
        header("Location: " . DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?id=' . $id);
        exit;
    }
    ...
    $url = DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?action=create';
    if ($socid > 0) {
        $url .= '&socid=' . ((int) $socid);
        $url .= '&backtopage=' . urlencode(DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?socid=' . ((int) $socid));
    }
