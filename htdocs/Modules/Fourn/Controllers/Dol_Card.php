...
$hookmanager->initHooks(['suppliercard', 'globalcard']);
...
if ($object->id > 0) {
    ...
    print '</td></tr></table>';
    print '</td><td>' . ($object->remise_supplier_percent ? '<a href="' . DOL_URL_ROOT . '/Modules/Comm/remise.php?id=' . $object->id . '">' . $object->remise_supplier_percent . '%</a>' : '') . '</td>';
    print '</tr>';
    ...
    // Other attributes
    $parameters = ['socid' => $object->id, 'colspan' => ' colspan="3"', 'colspanvalue' => '3'];
    ...
    if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled)) {
        ...
        $link = DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?socid=' . $object->id;
        $icon = 'bill';
        if ($link) {
            $boxstat .= '<a href="' . $link . '" class="boxstatsindicator thumbstat nobold nounderline">';
        }
    ...
    if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_invoice->enabled)) {
        ...
        $link = DOL_URL_ROOT . '/Modules/Fourn/facture/list.php?socid=' . $object->id;
        $icon = 'bill';
        if ($link) {
            $boxstat .= '<a href="' . $link . '" class="boxstatsindicator thumbstat nobold nounderline">';
        }
        ...
        $link = DOL_URL_ROOT . '/Modules/Fourn/recap-fourn.php?socid=' . $object->id;
        $icon = 'bill';
        if ($link) {
            $boxstat .= '<a href="' . $link . '" class="boxstatsindicator thumbstat nobold nounderline">';
        }
    ...
    print '<td class="right"><a class="notasortlink" href="'.DOL_URL_ROOT.'/Modules/Fourn/recap-fourn.php?socid='.$object->id.'">'.$langs->trans("ShowSupplierPreview").'</a></td></tr></table></td>';
    print '</tr>';
    print '</table>';
    print '<br>';
    */
    ...
    if (!empty($conf->product->enabled) || !empty($conf->service->enabled)) {
        ...
        print '<a class="notasortlink" href="' . DOL_URL_ROOT . '/Modules/Fourn/product/list.php?fourn_id=' . $object->id . '">' . $langs->trans("AllProductReferencesOfSupplier") . '<span class="badge marginleftonlyshort">' . $object->nbOfProductRefs() . '</span>';
        print '</a></td></tr>';
    ...
    if (!empty($user->rights->supplier_proposal->lire)) {
        $langs->loadLangs(["supplier_proposal"]);
    ...
    if ($user->rights->fournisseur->commande->lire) {
        ...
        if ($resql) {
            ...
            if ($num > 0) {
                ...
                print '<td class="right"><a class="notasortlink" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?socid=' . $object->id . '">' . $langs->trans("AllOrders") . '<span class="badge marginleftonlyshort">' . $num . '</span></td>';
                print '<td width="20px" class="right"><a href="' . DOL_URL_ROOT . '/Modules/Commande/stats/index.php?mode=supplier&socid=' . $object->id . '">' . img_picto($langs->trans("Statistics"), 'stats') . '</a></td>';
                print '</tr></table>';
                print '</td></tr>';
            }
    ...
    if ($user->rights->fournisseur->facture->lire) {
        ...
        if ($resql) {
            ...
            if ($num > 0) {
                ...
                print '<tr class="liste_titre">';
                print '<td colspan="4">';
                print '<table class="nobordernopadding" width="100%"><tr><td>' . $langs->trans('LastSuppliersBills', ($num <= $MAXLIST ? "" : $MAXLIST)) . '</td><td class="right"><a class="notasortlink" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/list.php?socid=' . $object->id . '">' . $langs->trans('AllBills') . '<span class="badge marginleftonlyshort">' . $num . '</span></td>';
                print '<td width="20px" class="right"><a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/stats/index.php?mode=supplier&socid=' . $object->id . '">' . img_picto($langs->trans("Statistics"), 'stats') . '</a></td>';
                print '</tr></table>';
                print '</td></tr>';
            }
    ...
    if (empty($reshook)) {
        ...
        if ($user->rights->fournisseur->commande->creer || $user->rights->supplier_order->creer) {
            $langs->load("orders");
            if ($object->status == 1) {
                print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?action=create&socid=' . $object->id . '">' . $langs->trans("AddSupplierOrderShort") . '</a>';
            } else {
        ...
        if ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer) {
            if (!empty($orders2invoice) && $orders2invoice > 0) {
                if ($object->status == 1) {
                    // Company is open
                    print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/list.php?socid=' . $object->id . '&search_billed=0&autoselectall=1">' . $langs->trans("CreateInvoiceForThisSupplier") . '</a></div>';
                } else {
        ...
        if ($user->rights->fournisseur->facture->creer || $user->rights->supplier_invoice->creer) {
            $langs->load("bills");
            if ($object->status == 1) {
                print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/facture/card.php?action=create&socid=' . $object->id . '">' . $langs->trans("AddBill") . '</a>';
            } else {
        ...
        if (!empty($conf->agenda->enabled) && !empty($conf->global->MAIN_REPEATTASKONEACHTAB) && $object->status == 1) {
            if ($user->rights->agenda->myactions->create) {
                print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&socid=' . $object->id . '">' . $langs->trans("AddAction") . '</a>';
            } else {
