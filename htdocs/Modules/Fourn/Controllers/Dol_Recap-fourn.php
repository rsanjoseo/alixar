    ...
    if ((!empty($conf->fournisseur->enabled) && $user->rights->fournisseur->facture->lire && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || (!empty($conf->supplier_invoice->enabled) && $user->rights->supplier_invoice->lire)) {
        ...
        if ($resql) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                // Author
                print '<td class="nowrap" width="50"><a href="' . DOL_URL_ROOT . '/Modules/Users/card.php?id=' . $objf->userid . '">' . img_object($langs->trans("ShowUser"), 'user') . ' ' . $objf->login . '</a></td>';
                ...
                if ($resqlp) {
                    ...
                    while ($j < $nump) {
                        ...
                        // Auteur
                        print '<td class="nowrap" width="50"><a href="' . DOL_URL_ROOT . '/Modules/Users/card.php?id=' . $objp->userid . '">' . img_object($langs->trans("ShowUser"), 'user') . ' ' . $objp->login . '</a></td>';
