...
$hookmanager->initHooks(['orderssuppliersindex']);
...
if ($resql) {
    ...
    print "</tr>\n";
    $listofstatus = [0, 1, 2, 3, 4, 5, 6, 9];
    foreach ($listofstatus as $status) {
        $dataseries[] = [$commandestatic->LibStatut($status, 1), (isset($vals[$status]) ? (int) $vals[$status] : 0)];
    ...
    if ($resql) {
        ...
        if ($num) {
            ...
            while ($i < $num) {
                ...
                print '<td><a href="' . DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=' . $obj->socid . '">' . img_object($langs->trans("ShowCompany"), "company") . ' ' . dol_trunc($obj->name, 24) . '</a></td></tr>';
                $i++;
            }
        }
        print "</table></div><br>";
...
if ($resql) {
    ...
    if ($num) {
        ...
        while ($i < $num) {
            ...
            print '<td><a href="' . DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=' . $obj->socid . '">' . img_object($langs->trans("ShowCompany"), "company") . ' ' . $obj->name . '</a></td>';
...
print '<table class="noborder centpercent">';
print '<tr class="liste_titre">';
print '<th colspan="3">'.$langs->trans("OrdersToProcess").' <a href="'.DOL_URL_ROOT.'/Modules/Commande/list.php?search_status=1">('.$num.')</a></th></tr>';
