...
$langs->loadLangs(["stocks", "margins", "products"]);
...
if ($idprod > 0) {
    ...
    if (is_array($productSupplierArray)) {
        foreach ($productSupplierArray as $productSupplier) {
            ...
            $prices[] = ["id" => $productSupplier->product_fourn_price_id, "price" => price2num($price, 0, '', 0), "label" => $label, "title" => $title]; // For price field, we must use price2num(), for label or title, price()
        }
    }
    ...
    if (!empty($conf->stock->enabled)) {
        ...
        $prices[] = ["id" => 'pmpprice', "price" => price2num($price), "label" => $langs->trans("PMPValueShort") . ': ' . price($price, 0, $langs, 0, 0, -1, $conf->currency), "title" => $langs->trans("PMPValueShort") . ': ' . price($price, 0, $langs, 0, 0, -1, $conf->currency)]; // For price field, we must use price2num(), for label or title, price()
    }

    // Add price for costprice (at end)
    $price = $producttmp->cost_price;
    $prices[] = ["id" => 'costprice', "price" => price2num($price), "label" => $langs->trans("CostPrice") . ': ' . price($price, 0, $langs, 0, 0, -1, $conf->currency), "title" => $langs->trans("PMPValueShort") . ': ' . price($price, 0, $langs, 0, 0, -1, $conf->currency)]; // For price field, we must use price2num(), for label or title, price()
}
