...
if ($modecompta == 'CREANCES-DETTES') {
    ...
    if (count($name)) {
        foreach ($name as $key => $value) {
            ...
            $fullname = $name[$key];
            if ($key > 0) {
                $linkname = '<a href="' . DOL_URL_ROOT . '/Modules/Products/card.php?id=' . $key . '">' . img_object($langs->trans("ShowProduct"), $type[$key] == 0 ? 'product' : 'service') . ' ' . $fullname . '</a>';
            } else {
            ...
            print '<td class="right">';
            /*if ($key > 0) {
                print '<a href="'.DOL_URL_ROOT.'/Modules/Compta/facture/list.php?productid='.$key.'">';
            } else {
                print '<a href="#">';
            }*/
