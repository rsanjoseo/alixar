...
if ($modecompta == 'CREANCES-DETTES') {
    ...
    $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?year_start=' . $year_start . '&modecompta=RECETTES-DEPENSES">', '</a>'], $calcmode);
    if (!empty($conf->accounting->enabled)) {
        $calcmode .= '<br>(' . $langs->trans("SeeReportInBookkeepingMode", '{s1}', '{s2}') . ')';
        $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?year_start=' . $year_start . '&modecompta=BOOKKEEPING">', '</a>'], $calcmode);
    }
    ...
    $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?year_start=' . $year_start . '&modecompta=CREANCES-DETTES">', '</a>'], $calcmode);
    if (!empty($conf->accounting->enabled)) {
        $calcmode .= '<br>(' . $langs->trans("SeeReportInBookkeepingMode", '{s1}', '{s2}') . ')';
        $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?year_start=' . $year_start . '&modecompta=BOOKKEEPING">', '</a>'], $calcmode);
    }
    ...
    $description .= ' (' . $langs->trans("SeePageForSetup", DOL_URL_ROOT . '/Modules/Accountancy/admin/account.php?mainmenu=accountancy&leftmenu=accountancy_admin', $langs->transnoentitiesnoconv("Accountancy") . ' / ' . $langs->transnoentitiesnoconv("Setup") . ' / ' . $langs->trans("Chartofaccounts")) . ')';
    $builddate = dol_now();
    //$exportlink=$langs->trans("NotYetAvailable");
}
