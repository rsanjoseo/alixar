...
require_once DOL_DOCUMENT_ROOT . '/Modules/Accountancy/class/accountancycategory.class.php';
...
if ($modecompta == "CREANCES-DETTES") {
    ...
    $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?date_startyear=' . $tmps['year'] . '&date_startmonth=' . $tmps['mon'] . '&date_startday=' . $tmps['mday'] . '&date_endyear=' . $tmpe['year'] . '&date_endmonth=' . $tmpe['mon'] . '&date_endday=' . $tmpe['mday'] . '&modecompta=RECETTES-DEPENSES">', '</a>'], $calcmode);
    if (!empty($conf->accounting->enabled)) {
        $calcmode .= '<br>(' . $langs->trans("SeeReportInBookkeepingMode", '{s1}', '{s2}') . ')';
        $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?date_startyear=' . $tmps['year'] . '&date_startmonth=' . $tmps['mon'] . '&date_startday=' . $tmps['mday'] . '&date_endyear=' . $tmpe['year'] . '&date_endmonth=' . $tmpe['mon'] . '&date_endday=' . $tmpe['mday'] . '&modecompta=BOOKKEEPING">', '</a>'], $calcmode);
    }
    ...
    $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?date_startyear=' . $tmps['year'] . '&date_startmonth=' . $tmps['mon'] . '&date_startday=' . $tmps['mday'] . '&date_endyear=' . $tmpe['year'] . '&date_endmonth=' . $tmpe['mon'] . '&date_endday=' . $tmpe['mday'] . '&modecompta=CREANCES-DETTES">', '</a>'], $calcmode);
    if (!empty($conf->accounting->enabled)) {
        $calcmode .= '<br>(' . $langs->trans("SeeReportInBookkeepingMode", '{s1}', '{s2}') . ')';
        $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?date_startyear=' . $tmps['year'] . '&date_startmonth=' . $tmps['mon'] . '&date_startday=' . $tmps['mday'] . '&date_endyear=' . $tmpe['year'] . '&date_endmonth=' . $tmpe['mon'] . '&date_endday=' . $tmpe['mday'] . '&modecompta=BOOKKEEPING">', '</a>'], $calcmode);
    }
    ...
    $calcmode = str_replace(['{s1}', '{s2}'], ['<a href="' . $_SERVER["PHP_SELF"] . '?date_startyear=' . $tmps['year'] . '&date_startmonth=' . $tmps['mon'] . '&date_startday=' . $tmps['mday'] . '&date_endyear=' . $tmpe['year'] . '&date_endmonth=' . $tmpe['mon'] . '&date_endday=' . $tmpe['mday'] . '&modecompta=CREANCES-DETTES">', '</a>'], $calcmode);
    $period = $form->selectDate($date_start, 'date_start', 0, 0, 0, '', 1, 0) . ' - ' . $form->selectDate($date_end, 'date_end', 0, 0, 0, '', 1, 0);
    $arraylist = ['no' => $langs->trans("No"), 'yes' => $langs->trans("AccountWithNonZeroValues"), 'all' => $langs->trans("All")];
    ...
    $description .= ' (' . $langs->trans("SeePageForSetup", DOL_URL_ROOT . '/Modules/Accountancy/admin/account.php?mainmenu=accountancy&leftmenu=accountancy_admin', $langs->transnoentitiesnoconv("Accountancy") . ' / ' . $langs->transnoentitiesnoconv("Setup") . ' / ' . $langs->transnoentitiesnoconv("Chartofaccounts")) . ')';
    $builddate = dol_now();
    //$exportlink=$langs->trans("NotYetAvailable");
}
...
report_header($name, '', $period, $periodlink, $description, $builddate, $exportlink, ['modecompta' => $modecompta], $calcmode);
...
if ($modecompta == 'BOOKKEEPING') {
    ...
    if ($result) {
        ...
        while ($i < $num) {
            ...
            print "<td>" . $langs->trans("Bills") . ' <a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/list.php?socid=' . $objp->socid . '">' . $objp->name . "</td>\n";
        ...
        if ($result) {
            ...
            if ($num) {
                while ($i < $num) {
                    ...
                    print "<td>" . $langs->trans("Donation") . " <a href=\"" . DOL_URL_ROOT . "/Modules/Don/list.php?search_company=" . $obj->name . "&search_name=" . $obj->firstname . " " . $obj->lastname . "\">" . $obj->name . " " . $obj->firstname . " " . $obj->lastname . "</a></td>\n";
    ...
    if ($result) {
        ...
        if ($num > 0) {
            while ($i < $num) {
                ...
                print '<tr class="oddeven"><td>&nbsp;</td>';
                print "<td>" . $langs->trans("Bills") . " <a href=\"" . DOL_URL_ROOT . "/Modules/Fourn/facture/list.php?socid=" . $objp->socid . "\">" . $objp->name . "</a></td>\n";
        ...
        if ($result) {
            ...
            if ($num) {
                while ($obj = $db->fetch_object($result)) {
                    ...
                    print "<td>" . $langs->trans("ExpenseReport") . " <a href=\"" . DOL_URL_ROOT . "/Modules/ExpenseReports/list.php?search_user=" . $obj->userid . "\">" . $obj->firstname . " " . $obj->lastname . "</a></td>\n";
