...
class PaymentSocialContribution extends CommonObject
{
    ...
    public function addPaymentToBank($user, $mode, $label, $accountid, $emetteur_nom, $emetteur_banque)
    {
        ...
        if (!empty($conf->banque->enabled)) {
            ...
            if ($bank_line_id > 0) {
                ...
                $url = '';
                if ($mode == 'payment_sc') {
                    $url = DOL_URL_ROOT . '/Modules/Compta/payment_sc/card.php?id=';
                }
                if ($url) {
                ...
                foreach ($this->amounts as $key => $value) {
                    if ($mode == 'payment_sc') {
                        ...
                        $result = $acc->add_url_line($bank_line_id, $socialcontrib->id, DOL_URL_ROOT . '/Modules/Compta/charges.php?id=', $socialcontrib->type_label . (($socialcontrib->lib && $socialcontrib->lib != $socialcontrib->type_label) ? ' (' . $socialcontrib->lib . ')' : ''), 'sc');
                        if ($result <= 0) {
                            dol_print_error($this->db);
                        }
    ...
    public function getNomUrl($withpicto = 0, $maxlen = 0)
    {
        ...
        if (!empty($this->id)) {
            $link = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/payment_sc/card.php?id=' . $this->id . '" title="' . dol_escape_htmltag($label, 1) . '" class="classfortooltip">';
            $linkend = '</a>';
