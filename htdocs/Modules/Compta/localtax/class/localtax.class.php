...
class Localtax extends CommonObject
{
        ...
        if ($result) {
            ...
            if ($this->id > 0) {
                ...
                if (!empty($conf->banque->enabled)) {
                    ...
                    // Mise a jour liens
                    $result = $acc->add_url_line($bank_line_id, $this->id, DOL_URL_ROOT . '/Modules/Compta/localtax/card.php?id=', "(VATPayment)", "payment_vat");
                    if ($result < 0) {
                        $this->error = $acc->error;
                        $ok = 0;
                    }
    ...
    public function getNomUrl($withpicto = 0, $option = '')
    {
        ...
        $link = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/localtax/card.php?id=' . $this->id . '" title="' . dol_escape_htmltag($label, 1) . '" class="classfortooltip">';
        $linkend = '</a>';
