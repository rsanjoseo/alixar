...
if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->tax->charges->supprimer) {
    ...
    if ($result > 0) {
        $db->commit();
        header("Location: " . DOL_URL_ROOT . "/Modules/Compta/sociales/payments.php");
        exit;
    } else {
...
$head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/payment_sc/card.php?id=' . $id;
...
$linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/sociales/payments.php">' . $langs->trans("BackToList") . '</a>';
