...
require_once DOL_DOCUMENT_ROOT . '/Modules/Compta/bank/class/bankcateg.class.php';
...
require_once DOL_DOCUMENT_ROOT . '/Modules/Compta/tva/class/paymentvat.class.php';
...
require_once DOL_DOCUMENT_ROOT . '/Modules/Compta/bank/class/paymentvarious.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Compta/prelevement/class/bonprelevement.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Don/class/don.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Don/class/paymentdonation.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/ExpenseReports/class/paymentexpensereport.class.php';
...
$arrayfields = [
    ...
    'b.label' => ['label' => $langs->trans("Description"), 'checked' => 1],
    'b.dateo' => ['label' => $langs->trans("DateOperationShort"), 'checked' => 1],
    'b.datev' => ['label' => $langs->trans("DateValueShort"), 'checked' => 1],
    'type' => ['label' => $langs->trans("Type"), 'checked' => 1],
    'b.num_chq' => ['label' => $langs->trans("Numero"), 'checked' => 1],
    'bu.label' => ['label' => $langs->trans("ThirdParty") . '/' . $langs->trans("User"), 'checked' => 1, 'position' => 500],
    'ba.ref' => ['label' => $langs->trans("BankAccount"), 'checked' => (($id > 0 || !empty($ref)) ? 0 : 1), 'position' => 1000],
    'b.debit' => ['label' => $langs->trans("Debit"), 'checked' => 1, 'position' => 600],
    'b.credit' => ['label' => $langs->trans("Credit"), 'checked' => 1, 'position' => 605],
    'balancebefore' => ['label' => $langs->trans("BalanceBefore"), 'checked' => 0, 'position' => 1000],
    'balance' => ['label' => $langs->trans("Balance"), 'checked' => 1, 'position' => 1001],
    'b.num_releve' => ['label' => $langs->trans("AccountStatement"), 'checked' => 1, 'position' => 1010],
    'b.conciliated' => ['label' => $langs->trans("Conciliated"), 'enabled' => $object->rappro, 'checked' => ($action == 'reconcile' ? 1 : 0), 'position' => 1020],
];
// Extra fields
...
if ($id > 0 || !empty($ref)) {
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/bank/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
    ...
    if ($action != 'reconcile') {
        if ($object->canBeConciliated() > 0) {
            ...
            if ($user->rights->banque->consolidate) {
                ...
                $buttonreconcile = '<a class="butAction" style="margin-bottom: 5px !important; margin-top: 5px !important" href="' . DOL_URL_ROOT . '/Modules/Compta/bank/bankentries_list.php?action=reconcile&sortfield=b.datev,b.dateo,b.rowid&amp;sortorder=asc,asc,asc&search_conciliated=0' . $newparam . '">' . $titletoconciliatemanual . '</a>';
            } else {
            ...
            if ($allowautomaticconciliation) {
                ...
                if ($user->rights->banque->consolidate) {
                    $newparam = $param;
                    $newparam = preg_replace('/search_conciliated=\d+/i', '', $newparam);
                    $buttonreconcile .= ' <a class="butAction" style="margin-bottom: 5px !important; margin-top: 5px !important" href="' . DOL_URL_ROOT . '/Modules/Compta/bank/bankentries_list.php?action=reconcile&sortfield=b.datev,b.dateo,b.rowid&amp;sortorder=asc,asc,asc&search_conciliated=0' . $newparam . '">' . $titletoconciliateauto . '</a>';
                } else {
    ...
    llxHeader('', $langs->trans("BankTransactions"), '', '', 0, 0, [], [], $param);
}
    ...
    if ($user->rights->banque->consolidate && $action == 'reconcile') {
        ...
        if ($resqlr) {
            ...
            while (($i < $numr) && ($i < $nbmax)) {
                ...
                $i++;
                $liste = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/bank/releve.php?account=' . $id . '&amp;num=' . $objr->num_releve . '">' . $objr->num_releve . '</a> &nbsp; ' . $liste;
            }
            if ($numr >= $nbmax) {
                $liste = "... &nbsp; " . $liste;
            }
    ...
    if ($action != 'addline' && $action != 'reconcile') {
        if (empty($conf->global->BANK_DISABLE_DIRECT_INPUT)) {
            if (empty($conf->global->BANK_USE_OLD_VARIOUS_PAYMENT)) {    // Default is to record miscellaneous direct entries using miscellaneous payments
                $newcardbutton = dolGetButtonTitle($langs->trans('AddBankRecord'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/card.php?action=create&accountid=' . urlencode($search_account) . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . urlencode($search_account)), '', $user->rights->banque->modifier);
    ...
    */
    ...
    while ($i < min($num, $limit)) {
        ...
        if ($objp->conciliated && $bankaccount->canBeConciliated() > 0) {  // If line not conciliated and account can be conciliated
            print '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Compta/bank/line.php?save_lastsearch_values=1&amp;rowid=' . $objp->rowid . '&amp;account=' . $objp->bankid . '&amp;page=' . $page . '">';
            print img_edit();
            print '</a>';
        } else {
