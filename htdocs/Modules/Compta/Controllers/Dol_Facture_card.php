...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Compta/facture/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Compta/facture/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
        ...
        if (($usercandelete && $isErasable > 0)
            ...
            if ($result > 0) {
                header('Location: ' . DOL_URL_ROOT . '/Modules/Compta/facture/list.php?restore_lastsearch_values=1');
                exit();
            } else {
                setEventMessages($object->error, $object->errors, 'errors');
                $action = '';
        ...
        $array_to_check = ['IDPROF1', 'IDPROF2', 'IDPROF3', 'IDPROF4', 'IDPROF5', 'IDPROF6', 'EMAIL'];
        ...
        if (in_array($object->statut, [Facture::STATUS_CLOSED, Facture::STATUS_VALIDATED])
            && $object->type == Facture::TYPE_SITUATION
            && $usercancreate
            && !$objectidnext
...
if ($action == 'create') {
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print ' <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $soc->id . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $soc->id . ($fac_rec ? '&fac_rec=' . $fac_rec : '')) . '"><span class="fa fa-plus-circle valignmiddle" title="' . $langs->trans("AddProject") . '"></span></a>';
        print '</td></tr>';
    }
    ...
    // Other attributes
    $parameters = ['objectsrc' => $objectsrc, 'colspan' => ' colspan="2"', 'cols' => '2', 'socid' => $socid];
    ...
    if ($action == 'clone') {
        ...
        $formquestion = [
            ['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company($object->socid, 'socid', '(s.client=1 OR s.client=2 OR s.client=3)', 1)],
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
        $morehtmlref .= ' (<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/list.php?socid=' . $object->thirdparty->id . '&search_societe=' . urlencode($object->thirdparty->name) . '">' . $langs->trans("OtherBills") . '</a>)';
    }
    // Project
    if (!empty($conf->projet->enabled)) {
        $langs->load("projects");
    ...
    if ($object->fk_fac_rec_source > 0) {
        ...
        if ($result > 0) {
            ...
            $s = str_replace('{s1}', '<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card-rec.php?facid=' . $tmptemplate->id . '">' . dol_escape_htmltag($tmptemplate->ref) . '</a>', $s);
            print $s;
            print '</span>';
        }
    }
    ...
    if (!empty($conf->incoterm->enabled)) {
        ...
        if ($usercancreate) {
            print '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?facid=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
        } else {
            print '&nbsp;';
        }
        print '</td></tr></table>';
        ...
        if (empty($reshook)) {
                ...
                if ($resteapayer > 0) {
                    if ($usercancreatewithdrarequest) {
                        if (!$objectidnext && $object->close_code != 'replaced') {                // Not replaced by another invoice
                            print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/prelevement.php?facid=' . $object->id . '" title="' . dol_escape_htmltag($langs->trans("MakeWithdrawRequest")) . '">' . $langs->trans("MakeWithdrawRequest") . '</a>';
                        } else {
            ...
            if ($object->type != Facture::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0 && $usercanissuepayment) {
                if ($objectidnext) {
                    ...
                    //} else {
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/paiement.php?facid=' . $object->id . '&amp;action=create&amp;accountid=' . $object->fk_account . '">' . $langs->trans('DoPayment') . '</a>';
                    //}
                }
            }
            ...
            if ($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_SITUATION) {
                ...
                if ($object->type == Facture::TYPE_CREDIT_NOTE && $object->statut == Facture::STATUS_VALIDATED && $object->paye == 0 && $usercanissuepayment) {
                    if ($resteapayer == 0) {
                        ...
                        print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/paiement.php?facid=' . $object->id . '&amp;action=create&amp;accountid=' . $object->fk_account . '">' . $langs->trans('DoPaymentBack') . '</a>';
                    }
                }
            ...
            if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $object->statut == 0 && $usercancreate) {
                if (!$objectidnext && count($object->lines) > 0) {
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card-rec.php?facid=' . $object->id . '&amp;action=create">' . $langs->trans("ChangeIntoRepeatableInvoice") . '</a>';
                }
            }
