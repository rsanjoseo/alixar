...
if (!empty($conf->tax->enabled) && !empty($user->rights->tax->charges->lire)) {
    if (!$socid) {
        ...
        if ($resql) {
            ...
            print '<table class="noborder centpercent">';
            print '<tr class="liste_titre">';
            print '<th>' . $langs->trans("ContributionsToPay") . ($num ? ' <a href="' . DOL_URL_ROOT . '/Modules/Compta/sociales/list.php?status=0"><span class="badge">' . $num . '</span></a>' : '') . '</th>';
...
if (!empty($conf->facture->enabled) && !empty($conf->commande->enabled) && $user->rights->commande->lire && empty($conf->global->WORKFLOW_DISABLE_CREATE_INVOICE_FROM_ORDER)) {
    ...
    if ($resql) {
        ...
        if ($num) {
            ...
            print '<a href="' . DOL_URL_ROOT . '/Modules/Commande/list.php?search_status=' . Commande::STATUS_CLOSED . '&amp;billed=0">';
