...
if (empty($reshook)) {
    ...
    if ($cancel) {
        if ($action != 'addlink') {
            $urltogo = $backtopage ? $backtopage : dol_buildpath('/Modules/Compta/bank/various_payment/list.php', 1);
    ...
    if ($action == 'add') {
        ...
        if (!$error) {
            ...
            if ($ret > 0) {
                $db->commit();
                $urltogo = ($backtopage ? $backtopage : DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/list.php');
                header("Location: " . $urltogo);
                exit;
            } else {
                $db->rollback();
    ...
    if ($action == 'delete') {
        ...
        if ($object->rappro == 0) {
            ...
            if ($ret > 0) {
                ...
                if ($result >= 0) {
                    $db->commit();
                    header("Location: " . DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/list.php');
                    exit;
                } else {
...
if ($id) {
    ...
    $morehtmlref .= '</div>';
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/bank/various_payment/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    // Clone
    if ($user->rights->banque->modifier) {
        print '<div class="inline-block divButAction"><a class="butAction" href="' . dol_buildpath("/Modules/Compta/bank/various_payment/card.php", 1) . '?id=' . $object->id . '&amp;action=clone">' . $langs->trans("ToClone") . "</a></div>";
    }
