...
$object = new Facture($db);
$hookmanager->initHooks(['invoicelist']);
...
$arrayfields = [
    ...
    'p.ref' => ['label' => "ProjectRef", 'checked' => 1, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1), 'position' => 40],
    'p.title' => ['label' => "ProjectLabel", 'checked' => 0, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1), 'position' => 41],
    ...
    'f.module_source' => ['label' => "POSModule", 'checked' => ($contextpage == 'poslist' ? 1 : 0), 'enabled' => ((empty($conf->cashdesk->enabled) && empty($conf->takepos->enabled) && empty($conf->global->INVOICE_SHOW_POS)) ? 0 : 1), 'position' => 90],
    'f.pos_source' => ['label' => "POSTerminal", 'checked' => ($contextpage == 'poslist' ? 1 : 0), 'enabled' => ((empty($conf->cashdesk->enabled) && empty($conf->takepos->enabled) && empty($conf->global->INVOICE_SHOW_POS)) ? 0 : 1), 'position' => 91],
    ...
    'u.login' => ['label' => "Author", 'checked' => 1, 'position' => 165],
    'sale_representative' => ['label' => "SaleRepresentativesOfThirdParty", 'checked' => 0, 'position' => 166],
    'f.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 280],
    'f.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 285],
    'f.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 290],
    'f.multicurrency_total_vat' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 291],
    'f.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 292],
    'multicurrency_dynamount_payed' => ['label' => 'MulticurrencyAlreadyPaid', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 295],
    'multicurrency_rtp' => ['label' => 'MulticurrencyRemainderToPay', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 296], // Not enabled by default because slow
    'total_pa' => ['label' => ($conf->global->MARGIN_TYPE == '1' ? 'BuyingPrice' : 'CostPrice'), 'checked' => 0, 'position' => 300, 'enabled' => (empty($conf->margin->enabled) || !$user->rights->margins->liretous ? 0 : 1)],
    'total_margin' => ['label' => 'Margin', 'checked' => 0, 'position' => 301, 'enabled' => (empty($conf->margin->enabled) || !$user->rights->margins->liretous ? 0 : 1)],
    'total_margin_rate' => ['label' => 'MarginRate', 'checked' => 0, 'position' => 302, 'enabled' => (empty($conf->margin->enabled) || !$user->rights->margins->liretous || empty($conf->global->DISPLAY_MARGIN_RATES) ? 0 : 1)],
    'total_mark_rate' => ['label' => 'MarkRate', 'checked' => 0, 'position' => 303, 'enabled' => (empty($conf->margin->enabled) || !$user->rights->margins->liretous || empty($conf->global->DISPLAY_MARK_RATES) ? 0 : 1)],
    'f.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'f.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 502],
    'f.note_public' => ['label' => 'NotePublic', 'checked' => 0, 'position' => 510, 'enabled' => (empty($conf->global->MAIN_LIST_ALLOW_PUBLIC_NOTES))],
    'f.note_private' => ['label' => 'NotePrivate', 'checked' => 0, 'position' => 511, 'enabled' => (empty($conf->global->MAIN_LIST_ALLOW_PRIVATE_NOTES))],
...
if (!$sall) {
    ...
    $parameters = ['all' => $all, 'fieldstosearchall' => $fieldstosearchall];
...
if ($resql) {
    ...
    if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
        ...
        header("Location: " . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?facid=' . $id);
        exit;
    }
    ...
    if ($contextpage != 'poslist') {
        $url = DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create';
        if (!empty($socid)) {
            $url .= '&socid=' . $socid;
        }
    ...
    if ($massaction == 'makepayment') {
        ...
        $formquestion = [
            ...
            ['type' => 'date', 'name' => 'datepaiment', 'label' => $langs->trans("Date")],
    ...
    if (!empty($arrayfields['f.fk_statut']['checked'])) {
        ...
        $liststatus = ['0' => $langs->trans("BillShortStatusDraft"), '1' => $langs->trans("BillShortStatusNotPaid"), '0,1' => $langs->trans("BillShortStatusDraft") . '+' . $langs->trans("BillShortStatusNotPaid"), '2' => $langs->trans("BillShortStatusPaid"), '1,2' => $langs->trans("BillShortStatusNotPaid") . '+' . $langs->trans("BillShortStatusPaid"), '3' => $langs->trans("BillShortStatusCanceled")];
