...
if ($action == 'delete') {
    ...
    if ($object->rappro == 0) {
        ...
        if ($ret > 0) {
            ...
            if ($result >= 0) {
                $db->commit();
                header("Location: " . DOL_URL_ROOT . '/Modules/Compta/localtax/list.php?localTaxType=' . $object->ltt);
                exit;
            } else {
...
if ($id) {
    $h = 0;
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/localtax/card.php?id=' . $object->id;
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/localtax/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
