...
if ($action == 'add_payment' || ($action == 'confirm_paiement' && $confirm == 'yes')) {
    ...
    if ($cancel) {
        $loc = DOL_URL_ROOT . '/Modules/Compta/sociales/card.php?id=' . $chid;
        header("Location: " . $loc);
        exit;
    }
...
if ($action == 'create') {
    ...
    print '<tr><td class="titlefieldcreate">' . $langs->trans("Ref") . '</td><td><a href="' . DOL_URL_ROOT . '/Modules/Compta/sociales/card.php?id=' . $chid . '">' . $chid . '</a></td></tr>';
