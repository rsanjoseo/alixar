...
if (empty($reshook)) {
    if ($action == 'add_paiement' || ($action == 'confirm_paiement' && $confirm == 'yes')) {
        ...
        foreach ($_POST as $key => $value) {
            if (substr($key, 0, 7) == 'amount_' && GETPOST($key) != '') {
                ...
                $formquestion[$i++] = ['type' => 'hidden', 'name' => $key, 'value' => $_POST[$key]];
    ...
    if ($action == 'confirm_paiement' && $confirm == 'yes') {
        ...
        if (!$error) {
            ...
            if ($invoiceid > 0) {
                $loc = DOL_URL_ROOT . '/Modules/Compta/facture/card.php?facid=' . $invoiceid;
            } else {
                $loc = DOL_URL_ROOT . '/Modules/Compta/paiement/card.php?id=' . $paiement_id;
            }
            header('Location: ' . $loc);
            exit;
        } else {
        ...
        if (!empty($conf->use_javascript_ajax)) {
            ...
            print '$(document).ready(function () {
						...
						function callForResult(imgId)
						{
							...
							$.post("' . DOL_URL_ROOT . '/Modules/Compta/ajaxpayment.php", json, function(data)
							{
								json = $.parseJSON(data);
...
if (!GETPOST('action', 'aZ09')) {
    ...
    if ($resql) {
        ...
        while ($i < min($num, $limit)) {
            ...
            print '<td><a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?facid=' . $objp->facid . '">' . $objp->ref . "</a></td>\n";
