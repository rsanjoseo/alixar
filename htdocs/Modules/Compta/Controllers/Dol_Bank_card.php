...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Compta/bank/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Compta/bank/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_delete' && GETPOST("confirm") == "yes" && $user->rights->banque->configurer) {
        ...
        if ($result > 0) {
            ...
            header("Location: " . DOL_URL_ROOT . "/Modules/Compta/bank/list.php");
            exit;
        } else {
            setEventMessages($object->error, $object->errors, 'errors');
            $action = '';
