...
if (!empty($conf->accounting->enabled)) {
    include_once DOL_DOCUMENT_ROOT . '/Modules/Accountancy/class/accountingjournal.class.php';
}
...
if ($action == 'confirm_delete' && $confirm == 'yes') {
    ...
    if (empty($totalpaye)) {
        ...
        if ($ret > 0) {
            ...
            if ($result >= 0) {
                $db->commit();
                header("Location: " . DOL_URL_ROOT . '/Modules/Compta/tva/list.php');
                exit;
            } else {
...
if ($id) {
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/tva/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
    ...
    if ($resql) {
        ...
        if ($num > 0) {
            ...
            while ($i < $num) {
                ...
                print '<a href="' . DOL_URL_ROOT . '/Modules/Compta/payment_vat/card.php?id=' . $objp->rowid . '">' . img_object($langs->trans("Payment"), "payment") . ' ' . $objp->rowid . '</a></td>';
    ...
    if ($action != 'edit') {
        ...
        if ($object->paye && $user->rights->tax->charges->creer) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=' . $object->id . '&action=reopen&token=' . newToken() . '">' . $langs->trans("ReOpen") . "</a></div>";
        }

        // Edit
        if ($object->paye == 0 && $user->rights->tax->charges->creer) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=' . $object->id . '&action=edit&token=' . newToken() . '">' . $langs->trans("Modify") . "</a></div>";
        }
        ...
        if ($object->paye == 0 && ((price2num($object->amount) < 0 && price2num($resteapayer, 'MT') < 0) || (price2num($object->amount) > 0 && price2num($resteapayer, 'MT') > 0)) && $user->rights->tax->charges->creer) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/paiement_vat.php?id=' . $object->id . '&action=create&token=' . newToken() . '">' . $langs->trans("DoPayment") . '</a></div>';
        }
        ...
        if ($object->paye == 0
            ...
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=' . $object->id . '&token=' . newToken() . '&action=paid">' . $langs->trans("ClassifyPaid") . "</a></div>";
        }

        // Clone
        if ($user->rights->tax->charges->creer) {
            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=' . $object->id . '&token=' . newToken() . '&action=clone">' . $langs->trans("ToClone") . "</a></div>";
        }
