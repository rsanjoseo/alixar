...
if (empty($reshook)) {
    ...
    if ($action == 'confirm_deleteinvoice' && $confirm == 'yes' && $user->rights->facture->supprimer) {
        ...
        header("Location: " . DOL_URL_ROOT . '/Modules/Compta/facture/invoicetemplate_list.php');
        exit;
    }
...
if ($action == 'create') {
    ...
    if ($object->fetch($id, $ref) > 0) {
        ...
        if (!empty($conf->projet->enabled) && is_object($object->thirdparty) && $object->thirdparty->id > 0) {
            ...
            print ' &nbsp; <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $object->thirdparty->id . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $object->thirdparty->id . (!empty($id) ? '&id=' . $id : '')) . '">' . $langs->trans("AddProject") . '</a>';
            print '</td></tr>';
        }
        ...
        print "<input type='text' name='frequency' value='" . GETPOST('frequency', 'int') . "' size='4' />&nbsp;" . $form->selectarray('unit_frequency', ['d' => $langs->trans('Day'), 'm' => $langs->trans('Month'), 'y' => $langs->trans('Year')], (GETPOST('unit_frequency') ? GETPOST('unit_frequency') : 'm'));
        print "</td></tr>";
    ...
    if ($object->id > 0) {
        ...
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/invoicetemplate_list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
        ...
        if ($action == 'editfrequency') {
            ...
            print '<tr><td>';
            print "<input type='text' name='frequency' value='" . $object->frequency . "' size='5' />&nbsp;" . $form->selectarray('unit_frequency', ['d' => $langs->trans('Day'), 'm' => $langs->trans('Month'), 'y' => $langs->trans('Year')], ($object->unit_frequency ? $object->unit_frequency : 'm'));
            print '</td>';
            ...
            if ($user->rights->facture->creer) {
                if (!empty($object->frequency) && $object->nb_gen_max > 0 && ($object->nb_gen_done >= $object->nb_gen_max)) {
                    ...
                    if (empty($object->frequency) || $object->date_when <= $nowlasthour) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&socid=' . $object->thirdparty->id . '&fac_rec=' . $object->id . '">' . $langs->trans("CreateBill") . '</a></div>';
                    } else {
