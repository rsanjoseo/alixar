...
class PaymentVAT extends CommonObject
{
    ...
    public function addPaymentToBank($user, $mode, $label, $accountid, $emetteur_nom, $emetteur_banque)
    {
        ...
        if (!empty($conf->banque->enabled)) {
            ...
            if ($bank_line_id > 0) {
                ...
                $url = '';
                if ($mode == 'payment_vat') {
                    $url = DOL_URL_ROOT . '/Modules/Compta/payment_vat/card.php?id=';
                }
                if ($url) {
                ...
                foreach ($this->amounts as $key => $value) {
                    if ($mode == 'payment_vat') {
                        $tva = new Tva($this->db);
                        $tva->fetch($key);
                        $result = $acc->add_url_line($bank_line_id, $tva->id, DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=', '(' . $tva->label . ')', 'vat');
                        if ($result <= 0) {
                            dol_print_error($this->db);
                        }
                    }
    ...
    public function getNomUrl($withpicto = 0, $maxlen = 0)
    {
        ...
        if (!empty($this->id)) {
            $link = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/payment_vat/card.php?id=' . $this->id . '" title="' . dol_escape_htmltag($label, 1) . '" class="classfortooltip">';
            $linkend = '</a>';
