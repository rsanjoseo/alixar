...
class Tva extends CommonObject
{
        ...
        if ($result) {
            ...
            if ($this->id > 0) {
                ...
                if (!empty($conf->banque->enabled) && !empty($this->amount)) {
                    ...
                    // Update links
                    $result = $acc->add_url_line($bank_line_id, $this->id, DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=', "(VATPayment)", "payment_vat");
                    if ($result < 0) {
                        $this->error = $acc->error;
                        $ok = 0;
                    }
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $url = DOL_URL_ROOT . '/Modules/Compta/tva/card.php?id=' . $this->id;
