...
class BonPrelevement extends CommonObject
{
    ...
    public $statut;
    public $labelStatus = [];
    public $invoice_in_error = [];        // STATUS_CREDITED and STATUS_DEBITED is same. Difference is in ->type
    public $thirdparty_in_error = [];        // STATUS_CREDITED and STATUS_DEBITED is same. Difference is in ->type
    ...
    public function EnregDestinataireSEPA($row_code_client, $row_nom, $row_address, $row_zip, $row_town, $row_country_code, $row_cb, $row_cg, $row_cc, $row_somme, $row_ref, $row_idfac, $row_iban, $row_bic, $row_datec, $row_drum, $row_rum, $type = 'direct-debit')
    {
        ...
        if ($type != 'bank-transfer') {
            ...
            $addressline1 = dol_string_unaccent(strtr($row_address, [CHR(13) => ", ", CHR(10) => ""]));
            $addressline2 = dol_string_unaccent(strtr($row_zip . (($row_zip && $row_town) ? ' ' : '' . $row_town), [CHR(13) => ", ", CHR(10) => ""]));
    ...
    public function EnregEmetteurSEPA($configuration, $ladate, $nombre, $total, $CrLf = '\n', $format = 'FRST', $type = 'direct-debit')
    {
        ...
        if ($resql) {
            ...
            if ($type != 'bank-transfer') {
                ...
                $addressline1 = dol_string_unaccent(strtr($configuration->global->MAIN_INFO_SOCIETE_ADDRESS, [CHR(13) => ", ", CHR(10) => ""]));
                $addressline2 = dol_string_unaccent(strtr($configuration->global->MAIN_INFO_SOCIETE_ZIP . (($configuration->global->MAIN_INFO_SOCIETE_ZIP || ' ' . $configuration->global->MAIN_INFO_SOCIETE_TOWN) ? ' ' : '') . $configuration->global->MAIN_INFO_SOCIETE_TOWN, [CHR(13) => ", ", CHR(10) => ""]));
                if ($addressline1) {
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $url = DOL_URL_ROOT . '/Modules/Compta/prelevement/card.php?id=' . $this->id;
        if ($this->type == 'bank-transfer') {
            $url = DOL_URL_ROOT . '/Modules/Compta/prelevement/card.php?id=' . $this->id;
        }
