...
if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->tax->charges->supprimer) {
    ...
    if ($result > 0) {
        $db->commit();
        header("Location: " . DOL_URL_ROOT . "/Modules/Compta/tva/payments.php?mode=tvaonly");
        exit;
    } else {
...
$head[$h][0] = DOL_URL_ROOT . '/Modules/Compta/payment_vat/card.php?id=' . $id;
$head[$h][1] = $langs->trans("VATPayment");
$hselected = $h;
$h++;

/*$head[$h][0] = DOL_URL_ROOT.'/Modules/Compta/payment_sc/info.php?id='.$id;
...
$linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/tva/payments.php">' . $langs->trans("BackToList") . '</a>';
