...
if ($action == 'confirm_reject_check' && $confirm == 'yes' && $user->rights->banque->cheque) {
    ...
    if ($paiement_id > 0) {
        ...
        //header("Location: ".DOL_URL_ROOT.'/Modules/Compta/paiement/card.php?id='.$paiement_id);
        //exit;
        $action = '';
    } else {
        setEventMessages($object->error, $object->errors, 'errors');
...
if ($action == 'new') {
    ...
    $hselected = $h;
    $h++;
    //  $head[$h][0] = DOL_URL_ROOT.'/Modules/Compta/paiement/cheque/info.php?id='.$object->id;
...
if ($action == 'new') {
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/paiement/cheque/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
