...
class Paiement extends CommonObject
{
    ...
    public $amounts = []; // array: invoice ID => amount for that invoice (in the main currency)>
    public $multicurrency_amounts = []; // array: invoice ID => amount for that invoice (in the invoice's currency)>
        ...
        if (!empty($conf->banque->enabled)) {
            ...
            if ($bank_line_id > 0) {
                ...
                if (!$error) {
                    $url = '';
                    if ($mode == 'payment') {
                        $url = DOL_URL_ROOT . '/Modules/Compta/paiement/card.php?id=';
                    }
                    if ($mode == 'payment_supplier') {
                        $url = DOL_URL_ROOT . '/Modules/Fourn/paiement/card.php?id=';
                    }
                    if ($url) {
                        ...
                        if ($mode == 'payment') {
                            ...
                            if (!in_array($fac->thirdparty->id, $linkaddedforthirdparty)) { // Not yet done for this thirdparty
                                $result = $acc->add_url_line(
                                    $bank_line_id,
                                    $fac->thirdparty->id,
                                    DOL_URL_ROOT . '/Modules/Comm/card.php?socid=',
                                    $fac->thirdparty->name,
                                    'company'
                                );
                                if ($result <= 0) {
                        ...
                        if ($mode == 'payment_supplier') {
                            ...
                            if (!in_array($fac->thirdparty->id, $linkaddedforthirdparty)) { // Not yet done for this thirdparty
                                $result = $acc->add_url_line(
                                    $bank_line_id,
                                    $fac->thirdparty->id,
                                    DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=',
                                    $fac->thirdparty->name,
                                    'company'
                                );
                                if ($result <= 0) {
                ...
                if (!$error && $label == '(WithdrawalPayment)') {
                    $result = $acc->add_url_line(
                        $bank_line_id,
                        $this->id_prelevement,
                        DOL_URL_ROOT . '/Modules/Compta/prelevement/card.php?id=',
                        $this->num_payment,
                        'withdraw'
                    );
                }
    ...
    public function getNomUrl($withpicto = 0, $option = '', $mode = 'withlistofinvoices', $notooltip = 0, $morecss = '')
    {
        ...
        $url = DOL_URL_ROOT . '/Modules/Compta/paiement/card.php?id=' . $this->id;
