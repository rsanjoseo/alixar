...
if ($resql) {
    ...
    $url = DOL_URL_ROOT . '/Modules/Compta/paiement/cheque/card.php?action=new';
    if (!empty($socid)) {
        $url .= '&socid=' . $socid;
    }
    ...
    if ($num > 0) {
        while ($i < min($num, $limit)) {
            ...
            print '<td>';
            if ($objp->bid) {
                print '<a href="' . DOL_URL_ROOT . '/Modules/Compta/bank/bankentries_list.php?account=' . $objp->bid . '">' . img_object($langs->trans("ShowAccount"), 'account') . ' ' . $objp->label . '</a>';
            } else {
                print '&nbsp;';
            }
            print '</td>';
