...
$langs->loadLangs(["admin", "other", "website", "errors"]);
...
if (GETPOST('refreshpage', 'alpha') && !in_array($action, ['updatecss'])) {
    $action = 'preview';
}
...
if ($action == 'addcontainer' && $usercanedit) {
    ...
    if (GETPOSTISSET('fetchexternalurl')) {    // Fetch from external url
        ...
        if (!$error) {
            $tmp = getURLContent($urltograb, 'GET', '', 1, [], ['http', 'https'], 0);
            if ($tmp['curl_error_no']) {
                ...
                foreach ($regs[0] as $key => $val) {
                    ...
                    $tmpgeturl = getURLContent($urltograbbis, 'GET', '', 1, [], ['http', 'https'], 0);
                    if ($tmpgeturl['curl_error_no']) {
                        $errorforsubresource++;
        ...
        if (!$error && GETPOST('WEBSITE_ALIASALT', 'alpha')) {
            $arrayofaliastotest = explode(',', str_replace(['<', '>'], '', GETPOST('WEBSITE_ALIASALT', 'alpha')));
        ...
        $objectpage->title = str_replace(['<', '>'], '', GETPOST('WEBSITE_TITLE', 'alphanohtml'));
        ...
        $objectpage->description = str_replace(['<', '>'], '', GETPOST('WEBSITE_DESCRIPTION', 'alphanohtml'));
        $objectpage->lang = GETPOST('WEBSITE_LANG', 'aZ09');
        $objectpage->otherlang = GETPOST('WEBSITE_OTHERLANG', 'aZ09comma');
        $objectpage->image = GETPOST('WEBSITE_IMAGE', 'alpha');
        $objectpage->keywords = str_replace(['<', '>'], '', GETPOST('WEBSITE_KEYWORDS', 'alphanohtml'));
...
if ($action == 'updatecss' && $usercanedit) {
    ...
    if (GETPOST('refreshsite') || GETPOST('refreshsite_x') || GETPOST('refreshsite.x') || GETPOST('refreshpage') || GETPOST('refreshpage_x') || GETPOST('refreshpage.x')) {
        ...
        if (!$error) {
            ...
            $htmlheadercontent .= preg_replace(['/<html>\n*/ims', '/<\/html>\n*/ims'], ['', ''], GETPOST('WEBSITE_HTML_HEADER', 'none'));
    ...
    if (!$error && $res > 0) {
        if ($action == 'updatesource' || $action == 'updatecontent') {
            ...
            if (empty($conf->global->WEBSITE_PHP_ALLOW_EXEC)) {    // If option is not on, we disallow functions to execute commands
                $forbiddenphpcommands = ["exec", "passthru", "shell_exec", "system", "proc_open", "popen", "eval", "dol_eval", "executeCLI"];
            }
            if (empty($conf->global->WEBSITE_PHP_ALLOW_WRITE)) {    // If option is not on, we disallow functions to write files
                $forbiddenphpcommands = array_merge($forbiddenphpcommands, ["fopen", "file_put_contents", "fputs", "fputscsv", "fwrite", "fpassthru", "unlink", "mkdir", "rmdir", "symlink", "touch", "umask"]);
            }
...
if (!GETPOST('hide_websitemenu')) {
    ...
    if ($action != 'file_manager') {
        ...
        if ($websitekey && $websitekey != '-1' && ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone' || $action == 'deletesite')) {
            ...
            if (!empty($conf->categorie->enabled)) {
                //print '<a href="'.DOL_URL_ROOT.'/Modules/Categories/index.php?leftmenu=website&dol_hide_leftmenu=1&nosearch=1&type=website_page&website='.$website->ref.'" class="button bordertransp"'.$disabled.' title="'.dol_escape_htmltag($langs->trans("Categories")).'"><span class="fa fa-tags"><span></a>';
                print dolButtonToOpenUrlInDialogPopup('categories', $langs->transnoentitiesnoconv("Categories"), '<span class="fa fa-tags"><span>', '/Modules/Categories/index.php?leftmenu=website&nosearch=1&type=website_page&website=' . $website->ref, $disabled);
            }
    ...
    if (in_array($action, ['editcss', 'editmenu', 'file_manager', 'replacesite', 'replacesiteconfirm'])) {
        if ($action == 'editcss') {
    ...
    if ($websitekey && $websitekey != '-1' && !in_array($action, ['editcss', 'editmenu', 'importsite', 'file_manager', 'replacesite', 'replacesiteconfirm']) && !$file_manager) {
        ...
        if ($action == 'preview' || $action == 'createfromclone' || $action == 'createpagefromclone' || $action == 'deletesite') {
            ...
            if ($action == 'deletesite') {
                ...
                $formquestion = [
                    ['type' => 'checkbox', 'name' => 'delete_also_js', 'label' => $langs->trans("DeleteAlsoJs"), 'value' => 0],
                    ['type' => 'checkbox', 'name' => 'delete_also_medias', 'label' => $langs->trans("DeleteAlsoMedias"), 'value' => 0],
            ...
            if ($action == 'createfromclone') {
                // Create an array for form
                $formquestion = [
                    ['type' => 'text', 'name' => 'siteref', 'label' => $langs->trans("WebSite"), 'value' => 'copy_of_' . $object->ref],
                ];
            ...
            if ($pageid > 0 && $atleastonepage) {        // pageid can be set without pages, if homepage of site is set and all pages were removed
                ...
                if ($action == 'createpagefromclone') {
                    ...
                    $formquestion = [
                        ['type' => 'hidden', 'name' => 'sourcepageurl', 'value' => $objectpage->pageurl],
                        ['type' => 'other', 'tdclass' => 'fieldrequired', 'name' => 'newwebsite', 'label' => $langs->trans("WebSite"), 'value' => $formwebsite->selectWebsite($object->id, 'newwebsite', 0)],
                        ['type' => 'text', 'tdclass' => 'maxwidth200 fieldrequired', 'moreattr' => 'autofocus="autofocus"', 'name' => 'newtitle', 'label' => $langs->trans("WEBSITE_TITLE"), 'value' => $langs->trans("CopyOf") . ' ' . $objectpage->title],
                        ['type' => 'text', 'tdclass' => 'maxwidth200', 'name' => 'newpageurl', 'label' => $langs->trans("WEBSITE_PAGENAME"), 'value' => ''],
                    ];
                    if (count($onlylang) > 1) {
                        $formquestion[] = ['type' => 'checkbox', 'tdclass' => 'maxwidth200', 'name' => 'is_a_translation', 'label' => $langs->trans("PageIsANewTranslation"), 'value' => 0, 'morecss' => 'margintoponly'];
                    }

                    $value = $formadmin->select_language($preselectedlanguage, 'newlang', 0, null, $textifempty, 0, 0, 'minwidth200', 1, 0, 0, $onlylang, 1);
                    $formquestion[] = ['type' => 'other', 'name' => 'newlang', 'label' => $form->textwithpicto($langs->trans("Language"), $langs->trans("DefineListOfAltLanguagesInWebsiteProperties")), 'value' => $value];
        ...
        if (!in_array($action, ['editcss', 'editmenu', 'file_manager', 'replacesite', 'replacesiteconfirm', 'createsite', 'createcontainer', 'createfromclone', 'createpagefromclone', 'deletesite'])) {
...
if ($action == 'editcss') {
    ...
    print '</td><td>';
    print $formadmin->select_language((GETPOSTISSET('WEBSITE_LANG') ? GETPOST('WEBSITE_LANG', 'aZ09comma') : ($object->lang ? $object->lang : '0')), 'WEBSITE_LANG', 0, null, 1, 0, 0, 'minwidth300', 2, 0, 0, [], 1);
    print '</td>';
    print '</tr>';
    ...
    $htmltext = str_replace(['{s1}', '{s2}'], ['<i>', '</i>'], $htmltext);
    $htmltext .= '<br>';
    $htmltext .= '<br>' . $langs->trans("CheckVirtualHostPerms", $langs->transnoentitiesnoconv("ReadPerm"), DOL_DOCUMENT_ROOT);
    $htmltext .= '<br>' . $langs->trans("CheckVirtualHostPerms", $langs->transnoentitiesnoconv("WritePerm"), '{s1}');
    $htmltext = str_replace('{s1}', DOL_DATA_ROOT . '/website<br>' . DOL_DATA_ROOT . '/medias', $htmltext);
...
if ($action == 'createsite') {
	...
	*/
    ...
    print $formadmin->select_language((GETPOSTISSET('WEBSITE_LANG') ? GETPOST('WEBSITE_LANG', 'aZ09comma') : $shortlangcode), 'WEBSITE_LANG', 0, null, 1, 0, 0, 'minwidth300', 2, 0, 0, [], 1);
    print '</td></tr>';
...
if ($action == 'editmeta' || $action == 'createcontainer') {    // Edit properties of a web site OR properties of a web page
	...
	*/
    ...
    if ($action == 'createcontainer') {
        ...
        $arraygrabimagesinto = ['root' => $langs->trans("WebsiteRootOfImages"), 'subpage' => $langs->trans("SubdirOfPage")];
    ...
    if (GETPOST('WEBSITE_TITLE', 'alpha')) {
        $pagetitle = str_replace(['<', '>'], '', GETPOST('WEBSITE_TITLE', 'alphanohtml'));
    }
    if (GETPOST('WEBSITE_PAGENAME', 'alpha')) {
        $pageurl = GETPOST('WEBSITE_PAGENAME', 'alpha');
    }
    if (GETPOST('WEBSITE_ALIASALT', 'alpha')) {
        $pagealiasalt = str_replace(['<', '>'], '', GETPOST('WEBSITE_ALIASALT', 'alphanohtml'));
    }
    if (GETPOST('WEBSITE_DESCRIPTION', 'alpha')) {
        $pagedescription = str_replace(['<', '>'], '', GETPOST('WEBSITE_DESCRIPTION', 'alphanohtml'));
    }
    if (GETPOST('WEBSITE_IMAGE', 'alpha')) {
        $pageimage = GETPOST('WEBSITE_IMAGE', 'alpha');
    }
    if (GETPOST('WEBSITE_KEYWORDS', 'alpha')) {
        $pagekeywords = str_replace(['<', '>'], '', GETPOST('WEBSITE_KEYWORDS', 'alphanohtml'));
    }
    ...
    if (empty($translatedby) && ($action == 'editmeta' || $action == 'createcontainer' || $objectpage->fk_page > 0)) {
        ...
        if ($result == 0) {
            ...
            print $formwebsite->selectContainer($website, 'pageidfortranslation', ($translationof ? $translationof : -1), 1, $action, 'minwidth300', [$objectpage->id]);
        ...
        if ($listofpages['code'] == 'OK') {
            ...
            foreach ($listofpages['list'] as $answerrecord) {
                if (get_class($answerrecord) == 'WebsitePage') {
                    ...
                    print '<span class="marginleftonly marginrightonly"></span>' . ajax_object_onoff($answerrecord, 'status', 'status', 'Enabled', 'Disabled', [], 'valignmiddle');
