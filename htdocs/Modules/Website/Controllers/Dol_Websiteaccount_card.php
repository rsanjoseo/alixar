...
$hookmanager->initHooks(['websiteaccountcard']); // Note that conf->hooks_modules contains array
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    $linkback = '';
    if ($socid) {
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Societes/website.php?socid=' . $socid . '&restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToListForThirdParty") . '</a>';
    }
    if ($fk_website) {
