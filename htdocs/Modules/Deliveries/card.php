...
$langs->loadLangs(["sendings", "bills", 'deliveries', 'orders']);
...
if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->expedition->delivery->supprimer) {
    ...
    if ($result > 0) {
        ...
        if (!empty($backtopage)) {
            ...
            header("Location: " . DOL_URL_ROOT . '/Modules/Expedition/list.php?restore_lastsearch_values=1');
        }
        exit;
    } else {
        $db->rollback();
...
} else // View
{
    if ($object->id > 0) {
        ...
        if ($object->id > 0) {
            ...
            {
                $linkback = '<a href="'.DOL_URL_ROOT.'/Modules/Expedition/list.php?restore_lastsearch_values=1">'.$langs->trans("BackToList").'</a>';
            ...
            */
            ...
            if (!$conf->expedition_bon->enabled && !empty($conf->stock->enabled)) {
                ...
                print '<td colspan="3"><a href="' . DOL_URL_ROOT . '/Modules/Products/stock/card.php?id=' . $entrepot->id . '">' . $entrepot->label . '</a></td>';
                print '</tr>';
            }
            ...
            while ($i < $num_prod) {
                ...
                if (empty($reshook)) {
                    ...
                    if ($object->lines[$i]->fk_product > 0) {
                        ...
                        $text = '<a href="' . DOL_URL_ROOT . '/Modules/Products/card.php?id=' . $object->lines[$i]->fk_product . '">';
                    ...
                    if (!empty($extrafields)) {
                        ...
                        print $object->lines[$i]->showOptionals($extrafields, $mode, ['style' => 'class="oddeven"', 'colspan' => $colspan], $i);
                    }
                }
            ...
            if ($user->socid == 0) {
                ...
                if ($user->rights->expedition->delivery->supprimer) {
                    if ($conf->expedition_bon->enabled) {
                        print '<a class="butActionDelete" href="' . $_SERVER['PHP_SELF'] . '?id=' . $object->id . '&expid=' . $object->origin_id . '&action=delete&token=' . newToken() . '&amp;backtopage=' . urlencode(DOL_URL_ROOT . '/Modules/Expedition/card.php?id=' . $object->origin_id) . '">' . $langs->trans("Delete") . '</a>';
                    } else {
