...
$langs->loadLangs(['companies', 'bills', 'interventions']);
if (!empty($conf->projet->enabled)) {
    $langs->load("projects");
}
if (!empty($conf->contrat->enabled)) {
...
$hookmanager->initHooks(['interventionlist']);
...
$arrayfields = [
    ...
    'pr.ref' => ['label' => 'Project', 'checked' => 1, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1)],
    'c.ref' => ['label' => 'Contract', 'checked' => 1, 'enabled' => (empty($conf->contrat->enabled) ? 0 : 1)],
    'f.description' => ['label' => 'Description', 'checked' => 1],
    ...
    'fd.description' => ['label' => "DescriptionOfLine", 'checked' => 1, 'enabled' => empty($conf->global->FICHINTER_DISABLE_DETAILS) ? 1 : 0],
    'fd.date' => ['label' => 'DateOfLine', 'checked' => 1, 'enabled' => empty($conf->global->FICHINTER_DISABLE_DETAILS) ? 1 : 0],
    'fd.duree' => ['label' => 'DurationOfLine', 'checked' => 1, 'enabled' => empty($conf->global->FICHINTER_DISABLE_DETAILS) ? 1 : 0],
];
// Extra fields
...
if ($search_desc) {
    if (empty($conf->global->FICHINTER_DISABLE_DETAILS) && $atleastonefieldinlines) {
        $sql .= natural_search(['f.description', 'fd.description'], $search_desc);
    } else {
        $sql .= natural_search(['f.description'], $search_desc);
    }
}
...
if ($resql) {
    ...
    $url = DOL_URL_ROOT . '/Modules/Fichinter/card.php?action=create';
    if (!empty($socid)) {
        $url .= '&socid=' . $socid;
    }
