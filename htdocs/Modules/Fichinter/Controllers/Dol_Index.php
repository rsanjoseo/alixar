...
$hookmanager->initHooks(['interventionindex']);
...
if ($resql) {
    ...
    foreach ($listofstatus as $status) {
        $dataseries[] = [$fichinterstatic->LibStatut($status, $bool, 1), (isset($vals[$status . $bool]) ? (int) $vals[$status . $bool] : 0)];
        if ($status == 3 && !$bool) {
            $bool = true;
        } else {
            $bool = false;
...
if ($resql) {
    ...
    if ($num) {
        ...
        while ($i < $num) {
            ...
            print '<td><a href="' . DOL_URL_ROOT . '/Modules/Comm/card.php?socid=' . $obj->socid . '">' . img_object($langs->trans("ShowCompany"), "company") . ' ' . $obj->name . '</a></td>';
...
if (!empty($conf->ficheinter->enabled)) {
    ...
    if ($resql) {
        ...
        print '<table class="noborder centpercent">';
        print '<tr class="liste_titre">';
        print '<th colspan="3">' . $langs->trans("FichinterToProcess") . ' <a href="' . DOL_URL_ROOT . '/Modules/Fichinter/list.php?search_status=1"><span class="badge">' . $num . '</span></a></th></tr>';
        ...
        if ($num) {
            ...
            while ($i < $num) {
                ...
                print '<td><a href="' . DOL_URL_ROOT . '/Modules/Comm/card.php?socid=' . $obj->socid . '">' . img_object($langs->trans("ShowCompany"), "company") . ' ' . dol_trunc($obj->name, 24) . '</a></td>';
