...
$langs->loadLangs(['bills', 'companies', 'interventions', 'stocks']);
...
$hookmanager->initHooks(['interventioncard', 'globalcard']);
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Fichinter/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Fichinter/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_validate' && $confirm == 'yes' && $user->rights->ficheinter->creer) {
        ...
        header('Location: ' . DOL_URL_ROOT . '/Modules/Fichinter/list.php?leftmenu=ficheinter&restore_lastsearch_values=1');
        exit;
...
if ($action == 'create') {
    ...
    if ($socid > 0) {
        ...
        if (!empty($conf->projet->enabled)) {
            ...
            if ($numprojet == 0) {
                print ' &nbsp; <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $soc->id . '&action=create"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddProject") . '"></span></a>';
            }
            print '</td></tr>';
        }
        ...
        if ($conf->contrat->enabled) {
            ...
            if ($numcontrat == 0) {
                print ' &nbsp; <a href="' . DOL_URL_ROOT . '/Modules/Contrats/card.php?socid=' . $soc->id . '&action=create"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddContract") . '"></span></a>';
            }
            print '</td></tr>';
        }
    ...
    if ($action == 'clone') {
        ...
        $formquestion = [
            ...
            // => 1),
            ['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company(GETPOST('socid', 'int'), 'socid', '', '', 0, 0, null, 0, 'minwidth200')],
        ];
    ...
    // Intervention card
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Fichinter/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                if ($action != 'editline' || GETPOST('line_id', 'int') != $objp->rowid) {
                    ...
                    print $line->showOptionals($extrafields, 'view', ['colspan' => 5]);
                }
                ...
                if ($object->statut == 0 && $action == 'editline' && $user->rights->ficheinter->creer && GETPOST('line_id', 'int') == $objp->rowid) {
                    ...
                    print $line->showOptionals($extrafields, 'edit', ['colspan' => 5]);
                }
            ...
            if ($object->statut == 0 && $user->rights->ficheinter->creer && $action <> 'editline' && empty($conf->global->FICHINTER_DISABLE_DETAILS)) {
                ...
                print $lineadd->showOptionals($extrafields, 'edit', ['colspan' => 5]);
        ...
        if ($user->socid == 0) {
            if ($action != 'editdescription' && ($action != 'presend')) {
                ...
                if ($conf->global->MAIN_FEATURES_LEVEL >= 1 && $object->statut == Fichinter::STATUS_DRAFT && $user->rights->ficheinter->creer && (count($object->lines) > 0)) {
                    ...
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fichinter/card-rec.php?id=' . $object->id . '&action=create">' . $langs->trans("ChangeIntoRepeatableIntervention") . '</a>';
                    print '</div>';
                }
                ...
                if ($conf->service->enabled && !empty($conf->propal->enabled) && $object->statut > Fichinter::STATUS_DRAFT) {
                    ...
                    if ($object->statut < Fichinter::STATUS_BILLED) {
                        if ($user->rights->propal->creer) {
                            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Comm/propal/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddProp") . '</a></div>';
                        } else {
                ...
                if (!empty($conf->facture->enabled) && $object->statut > Fichinter::STATUS_DRAFT) {
                    ...
                    if ($object->statut < Fichinter::STATUS_BILLED) {
                        if ($user->rights->facture->creer) {
                            print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddBill") . '</a></div>';
                        } else {
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['fichinter']);
