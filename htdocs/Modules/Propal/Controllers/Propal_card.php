...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Comm/propal/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Comm/propal/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_clone' && $confirm == 'yes' && $usercancreate) {
        ...
        if ($result > 0) {
            header('Location: ' . DOL_URL_ROOT . '/Modules/Comm/propal/list.php?restore_lastsearch_values=1');
            exit();
        } else {
            $langs->load("errors");
            setEventMessages($object->error, $object->errors, 'errors');
...
if ($action == 'create') {
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print ' <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $soc->id . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $soc->id) . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddProject") . '"></span></a>';
        print '</td>';
        print '</tr>';
    }
    ...
    if ($action == 'closeas') {
        ...
        $formquestion = [
            ['type' => 'select', 'name' => 'statut', 'label' => '<span class="fieldrequired">' . $langs->trans("CloseAs") . '</span>', 'values' => [$object::STATUS_SIGNED => $object->LibStatut($object::STATUS_SIGNED), $object::STATUS_NOTSIGNED => $object->LibStatut($object::STATUS_NOTSIGNED)]],
    ...
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Comm/propal/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
        $morehtmlref .= ' (<a href="' . DOL_URL_ROOT . '/Modules/Comm/propal/list.php?socid=' . $object->thirdparty->id . '&search_societe=' . urlencode($object->thirdparty->name) . '">' . $langs->trans("OtherProposals") . '</a>)';
    }
    // Project
    if (!empty($conf->projet->enabled)) {
        $langs->load("projects");
    ...
    if (!empty($conf->incoterm->enabled)) {
        ...
        if ($usercancreate) {
            print '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Comm/propal/card.php?id=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
        } else {
            print '&nbsp;';
        }
        print '</td></tr></table>';
    ...
    if ($action != 'presend') {
        ...
        if (empty($reshook)) {
            if ($action != 'editline') {
				...
				{
					print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddAction") . '</a></div>';
				}*/
                // Edit
                ...
                if (!empty($conf->commande->enabled) && $object->statut == Propal::STATUS_SIGNED) {
                    if ($usercancreateorder) {
                        print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Commande/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddOrder") . '</a>';
                    }
                }
                ...
                if ($object->statut == Propal::STATUS_SIGNED && empty($conf->global->PROPOSAL_ARE_NOT_BILLABLE)) {
                    if (!empty($conf->facture->enabled) && $usercancreateinvoice) {
                        print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("CreateBill") . '</a>';
                    }
