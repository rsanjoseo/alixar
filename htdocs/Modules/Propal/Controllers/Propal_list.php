...
$arrayfields = [
    ...
    'pr.ref' => ['label' => "ProjectRef", 'checked' => 1, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1)],
    'pr.title' => ['label' => "ProjectLabel", 'checked' => 0, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1)],
    ...
    'p.fk_shipping_method' => ['label' => "SendingMethod", 'checked' => 0, 'enabled' => !empty($conf->expedition->enabled)],
    ...
    'p.total_ht_invoiced' => ['label' => "AmountInvoicedHT", 'checked' => 0, 'enabled' => !empty($conf->global->PROPOSAL_SHOW_INVOICED_AMOUNT)],
    'p.total_invoiced' => ['label' => "AmountInvoicedTTC", 'checked' => 0, 'enabled' => !empty($conf->global->PROPOSAL_SHOW_INVOICED_AMOUNT)],
    'p.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'p.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'p.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'p.multicurrency_total_tva' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'p.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1)],
    'p.multicurrency_total_ht_invoiced' => ['label' => 'MulticurrencyAmountInvoicedHT', 'checked' => 0, 'enabled' => !empty($conf->multicurrency->enabled) && !empty($conf->global->PROPOSAL_SHOW_INVOICED_AMOUNT)],
    'p.multicurrency_total_invoiced' => ['label' => 'MulticurrencyAmountInvoicedTTC', 'checked' => 0, 'enabled' => !empty($conf->multicurrency->enabled) && !empty($conf->global->PROPOSAL_SHOW_INVOICED_AMOUNT)],
    ...
    'p.note_public' => ['label' => 'NotePublic', 'checked' => 0, 'position' => 510, 'enabled' => (empty($conf->global->MAIN_LIST_ALLOW_PUBLIC_NOTES))],
    'p.note_private' => ['label' => 'NotePrivate', 'checked' => 0, 'position' => 511, 'enabled' => (empty($conf->global->MAIN_LIST_ALLOW_PRIVATE_NOTES))],
...
if ($resql) {
    ...
    if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
        ...
        header("Location: " . DOL_URL_ROOT . '/Modules/Comm/propal/card.php?id=' . $id);
        exit;
    }
    ...
    if ($search_date_start) {
        $param .= '&search_date_startday=' . urlencode(dol_print_date($search_date_start, '%d')) . '&search_date_startmonth=' . urlencode(dol_print_date($search_date_start, '%m')) . '&search_date_startyear=' . urlencode(dol_print_date($search_date_start, '%Y'));
    }
    if ($search_date_end) {
        $param .= '&search_date_endday=' . urlencode(dol_print_date($search_date_end, '%d')) . '&search_date_endmonth=' . urlencode(dol_print_date($search_date_end, '%m')) . '&search_date_endyear=' . urlencode(dol_print_date($search_date_end, '%Y'));
    }
    if ($search_dateend_start) {
        $param .= '&search_dateend_startday=' . urlencode(dol_print_date($search_dateend_start, '%d')) . '&search_dateend_startmonth=' . urlencode(dol_print_date($search_dateend_start, '%m')) . '&search_dateend_startyear=' . urlencode(dol_print_date($search_dateend_start, '%Y'));
    }
    if ($search_dateend_end) {
        $param .= '&search_dateend_endday=' . urlencode(dol_print_date($search_dateend_end, '%d')) . '&search_dateend_endmonth=' . urlencode(dol_print_date($search_dateend_end, '%m')) . '&search_dateend_endyear=' . urlencode(dol_print_date($search_dateend_end, '%Y'));
    }
    if ($search_datedelivery_start) {
    ...
    $url = DOL_URL_ROOT . '/Modules/Comm/propal/card.php?action=create';
    if (!empty($socid)) {
        $url .= '&socid=' . $socid;
    }
