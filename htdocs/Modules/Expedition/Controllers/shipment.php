...
if ($id > 0 || !empty($ref)) {
    ...
    if ($object->fetch($id, $ref) > 0) {
        ...
        if (!empty($conf->incoterm->enabled)) {
            ...
            if ($user->rights->commande->creer) {
                print '<a class="editfielda" href="' . $_SERVER['PHP_SELF'] . '/Modules/Expedition/shipment.php?id=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
            } else {
                print '&nbsp;';
            }
            print '</td></tr></table>';
                    ...
                    if ($objp->fk_product > 0 && !empty($conf->global->PRODUIT_SOUSPRODUITS)) {
                        ...
                        if (count($prods_arbo) > 0) {
                            foreach ($prods_arbo as $key => $value) {
                                ...
                                print '<tr class="oddeven"><td>&nbsp; &nbsp; &nbsp; -> <a href="' . DOL_URL_ROOT . "/Modules/Products/card.php?id=" . $value['id'] . '">' . $value['fullpath'] . '</a> (' . $value['nb'] . ')</td>';
        ...
        if (empty($user->socid)) {
            ...
            if (empty($conf->stock->enabled) && ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED)) {
                if ($user->rights->expedition->creer) {
                    print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Expedition/card.php?action=create&amp;origin=commande&amp;object_id=' . $id . '">' . $langs->trans("CreateShipment") . '</a>';
        ...
        if (!empty($conf->stock->enabled) && ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED)) {
            if ($user->rights->expedition->creer) {
                ...
                print '<form method="GET" action="' . DOL_URL_ROOT . '/Modules/Expedition/card.php">';
                ...
                if (!empty($conf->stock->enabled)) {
                    ...
                    if (count($formproduct->cache_warehouses) <= 0) {
                        print ' &nbsp; ' . $langs->trans("WarehouseSourceNotDefined") . ' <a href="' . DOL_URL_ROOT . '/Modules/Products/stock/card.php?action=create">' . $langs->trans("AddOne") . '</a>';
                    }
                    //print '</td>';
                }
                //print '<td class="center">';
