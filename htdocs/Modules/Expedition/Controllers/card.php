...
if (empty($reshook)) {
    ...
    if ($action == 'add' && $user->rights->expedition->creer) {
        ...
        if ($result > 0) {
            header("Location: " . DOL_URL_ROOT . '/Modules/Expedition/index.php');
            exit;
        } else {
            setEventMessages($object->error, $object->errors, 'errors');
        }
...
if ($action == 'create') {
    ...
    if ($origin) {
        ...
        if ($object->fetch($origin_id)) {    // This include the fetch_lines
            ...
            if (!empty($conf->projet->enabled)) {
                ...
                print ' <a class="paddingleft" href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $soc->id . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $soc->id) . '"><span class="fa fa-plus-circle valignmiddle"></span></a>';
                print '</td>';
                print '</tr>';
            }
                                ...
                                if (count($prods_arbo) > 0) {
                                    foreach ($prods_arbo as $key => $value) {
                                        ...
                                        print "<tr class=\"oddeven\"><td>&nbsp; &nbsp; &nbsp; ->
											<a href=\"" . DOL_URL_ROOT . "/Modules/Products/card.php?id=" . $value['id'] . "\">" . $value['fullpath'] . "
    ...
    if ($object->id > 0) {
        ...
        // Shipment card
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Expedition/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
        ...
        if (!empty($conf->incoterm->enabled)) {
            ...
            if ($user->rights->expedition->creer) {
                print '<a class="editfielda" href="' . DOL_URL_ROOT . '/Modules/Expedition/card.php?id=' . $object->id . '&action=editincoterm&token=' . newToken() . '">' . img_edit() . '</a>';
            } else {
                print '&nbsp;';
            }
            print '</td></tr></table>';
        ...
        for ($i = 0; $i < $num_prod; $i++) {
            $parameters = ['i' => $i, 'line' => $lines[$i], 'line_id' => $line_id, 'num' => $num_prod, 'alreadysent' => $alreadysent, 'editColspan' => $editColspan, 'outputlangs' => $outputlangs];
            $reshook = $hookmanager->executeHooks('printObjectLine', $parameters, $object, $action);
            if ($reshook < 0) {
                setEventMessages($hookmanager->error, $hookmanager->errors, 'errors');
            }
            ...
            if (empty($reshook)) {
                ...
                if (!empty($extrafields)) {
                    ...
                    if ($action == 'editline' && $line->id == $line_id) {
                        print $lines[$i]->showOptionals($extrafields, 'edit', ['colspan' => $colspan], $indiceAsked, '', 0, 'card');
                    } else {
                        print $lines[$i]->showOptionals($extrafields, 'view', ['colspan' => $colspan], $indiceAsked, '', 0, 'card');
                    }
                }
            }
        }
