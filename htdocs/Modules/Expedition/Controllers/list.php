...
$arrayfields = [
    ...
    'e.shipping_method_id' => ['label' => $langs->trans('SendingMethod'), 'checked' => 1, 'position' => 10],
    ...
    'l.ref' => ['label' => $langs->trans("DeliveryRef"), 'checked' => 1, 'enabled' => (empty($conf->delivery_note->enabled) ? 0 : 1)],
    'l.date_delivery' => ['label' => $langs->trans("DateReceived"), 'checked' => 1, 'enabled' => (empty($conf->delivery_note->enabled) ? 0 : 1)],
    'e.billed' => ['label' => $langs->trans("Billed"), 'checked' => 1, 'position' => 1000, 'enabled' => (!empty($conf->global->WORKFLOW_BILL_ON_SHIPMENT))],
];
...
// $url = DOL_URL_ROOT.'/Modules/Expedition/card.php?action=create';
// if (!empty($socid)) $url .= '&socid='.$socid;
// $newcardbutton = dolGetButtonTitle($langs->trans('NewSending'), '', 'fa fa-plus-circle', $url, '', $user->rights->expedition->creer);
$newcardbutton = dolGetButtonTitle($langs->trans('NewSending'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Expedition/card.php?action=create2', '', $user->rights->expedition->creer);
...
if (!empty($arrayfields['e.fk_statut']['checked'])) {
    ...
    print $form->selectarray('search_status', ['0' => $langs->trans('StatusSendingDraftShort'), '1' => $langs->trans('StatusSendingValidatedShort'), '2' => $langs->trans('StatusSendingProcessedShort')], $search_status, 1);
    print '</td>';
}
// Status billed
