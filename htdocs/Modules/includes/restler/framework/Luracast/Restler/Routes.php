...
class Routes
{
        ...
        foreach ($methods as $method) {
            ...
            foreach ($params as $param) {
                ...
                if ($type == 'array' && $contentType && $qualified = Scope::resolve($contentType, $scope)) {
                    [$p['type'], $children, $modelName] = static::getTypeAndModel(
                        new ReflectionClass($qualified), $scope,
                        $className . Text::title($methodUrl), $p
                    );
                }
                if ($type instanceof ReflectionClass) {
                    [$type, $children, $modelName] = static::getTypeAndModel($type, $scope,
                        $className . Text::title($methodUrl), $p);
     ...
     * @throws RestException
     */
    protected static function getTypeAndModel(ReflectionClass $class, array $scope, $prefix = '', array $rules = [])
    {
    ...
    public static function findAll(array $excludedPaths = [], array $excludedHttpMethods = [], $version = 1)
    {
        $map = [];
                ...
                foreach ($routes as $httpMethod => $route) {
                    ...
                    if (!isset($filter[$hash])) {
                        ...
                        $map[$route['metadata']['resourcePath']][]
                            = ['access' => static::verifyAccess($route), 'route' => $route, 'hash' => $hash];
                        $filter[$hash] = true;
                    }
                }
            }
