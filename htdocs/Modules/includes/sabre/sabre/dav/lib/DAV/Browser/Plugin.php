    ...
    function generateDirectoryIndex($path)
    {
        ...
        if ($node instanceof DAV\ICollection) {
            ...
            foreach ($subNodes as $subPath => $subProps) {
                ...
                [, $displayPath] = URLUtil::splitPath($subPath);
...
HTML;
        ...
        if ($path) {
            [$parentUri] = URLUtil::splitPath($path);
            ...
            switch ($postVars['sabreAction']) {
                ...
                case 'mkcol' :
                    if (isset($postVars['name']) && trim($postVars['name'])) {
                        ...
                        [, $folderName] = URLUtil::splitPath(trim($postVars['name']));
                ...
                case 'put' :
                    ...
                    [, $newName] = URLUtil::splitPath(trim($file['name']));
                    if (isset($postVars['name']) && trim($postVars['name'])) {
                        $newName = trim($postVars['name']);
                    }

                    // Making sure we only have a 'basename' component
                    [, $newName] = URLUtil::splitPath($newName);
