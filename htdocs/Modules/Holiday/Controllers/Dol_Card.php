...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Holiday/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Holiday/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_send') {
        ...
        if ($object->statut == Holiday::STATUS_DRAFT && $cancreate) {
            ...
            if ($verif > 0) {
                ...
                $message .= "- " . $langs->transnoentitiesnoconv("Link") . " : " . $dolibarr_main_url_root . "/Modules/Holiday/card.php?id=" . $object->id . "\n\n";
                $message .= "\n";
    ...
    if ($error) {
        ...
        if (($id > 0) || $ref) {
            ...
            if ($canread) {
                ...
                $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Holiday/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
