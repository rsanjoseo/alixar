...
$langs->loadLangs(['products', 'stocks', 'productbatch']);
...
if ($sall) {
    $sql .= natural_search(['p.ref', 'p.label', 'p.description', 'p.note'], $sall);
}
...
if ($resql) {
    ...
    while ($i < min($num, $limit)) {
        ...
        print '<a href="' . DOL_URL_ROOT . '/Modules/Products/stock/movement_list.php?idproduct=' . $product_static->id . '&search_warehouse=' . $objp->fk_entrepot . '&search_batch=' . ($objp->batch != 'Undefined' ? $objp->batch : 'Undefined') . '">' . $langs->trans("Movements") . '</a>';
        print '</td>';
