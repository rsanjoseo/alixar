...
require_once DOL_DOCUMENT_ROOT . '/Modules/Products/dynamic_price/class/price_expression.class.php';
...
$langs->loadLangs(['products', 'suppliers', 'bills', 'margins', 'stocks']);
...
$hookmanager->initHooks(['pricesuppliercard', 'globalcard']);
...
$parameters = ['socid' => $socid, 'id_prod' => $id];
        ...
        if (!$error) {
            ...
            if (!$error) {
                ...
                if (!empty($conf->multicurrency->enabled)) {
                    ...
                    $ret = $object->update_buyprice($quantity, $newprice, $user, GETPOST("price_base_type"), $supplier, GETPOST("oselDispo"), $ref_fourn, $tva_tx, GETPOST("charges"), $remise_percent, 0, $npr, $delivery_time_days, $supplier_reputation, [], '', 0, 'HT', 1, '', $supplier_description, $barcode, $fk_barcode_type);
                }
                if ($ret < 0) {
                    $error++;
                    setEventMessages($object->error, $object->errors, 'errors');
...
if ($id > 0 || $ref) {
    if ($result) {
        ...
        if ($action != 'edit' && $action != 're-edit') {
            ...
            $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Products/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
            ...
            if (($action == 'add_price' || $action == 'update_price') && $usercancreate) {
                ...
                if ($rowid) {
                    ...
                    $events = [];
                    $events[] = ['method' => 'getVatRates', 'url' => dol_buildpath('/core/ajax/vatrates.php', 1), 'htmlname' => 'tva_tx', 'params' => []];
                    print $form->select_company(GETPOST("id_fourn", 'alpha'), 'id_fourn', 'fournisseur=1', 'SelectThirdParty', 0, 0, $events);

                    $parameters = ['filtre' => "fournisseur=1", 'html_name' => 'id_fourn', 'selected' => GETPOST("id_fourn"), 'showempty' => 1, 'prod_id' => $object->id];
                    $reshook = $hookmanager->executeHooks('formCreateThirdpartyOptions', $parameters, $object, $action);
                    if (empty($reshook)) {
                        if (empty($form->result)) {
                            print ' - <a href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=create&type=f&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $object->id . '&action=' . $action) . '">' . $langs->trans("CreateDolibarrThirdPartySupplier") . '</a>';
                        }
                    }
                }
                print '</td></tr>';
                ...
                if (!empty($conf->dynamicprices->enabled)) { //Only show price mode and expression selector if module is enabled
                    ...
                    $price_expression_list = [0 => $langs->trans("PriceNumeric")]; //Put the numeric mode as first option
                    ...
                    print '<script type="text/javascript">
						...
						function on_click() {
							window.location = "' . DOL_URL_ROOT . '/Modules/Products/dynamic_price/editor.php?id=' . $id . '&tab=fournisseurs&eid=" + $("#eid").val();
						}
						function on_change() {
							if ($("#eid").val() == 0) {
...
END;
                ...
                if (is_object($hookmanager)) {
                    $parameters = ['id_fourn' => $id_fourn, 'prod_id' => $object->id];
            ...
            if ($action != 'add_price' && $action != 'update_price') {
                ...
                if (empty($reshook)) {
                    if ($usercancreate) {
                        print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Products/fournisseurs.php?id=' . $object->id . '&amp;action=add_price">';
            ...
            if ($user->rights->fournisseur->lire) { // Duplicate ? this check is already in the head of this file
                ...
                $arrayfields = [
                    'pfp.datec' => ['label' => $langs->trans("AppliedPricesFrom"), 'checked' => 1, 'position' => 1],
                    's.nom' => ['label' => $langs->trans("Suppliers"), 'checked' => 1, 'position' => 2],
                    'pfp.fk_availability' => ['label' => $langs->trans("Availability"), 'enabled' => !empty($conf->global->FOURN_PRODUCT_AVAILABILITY), 'checked' => 0, 'position' => 4],
                    'pfp.quantity' => ['label' => $langs->trans("QtyMin"), 'checked' => 1, 'position' => 5],
                    'pfp.unitprice' => ['label' => $langs->trans("UnitPriceHT"), 'checked' => 1, 'position' => 9],
                    'pfp.multicurrency_unitprice' => ['label' => $langs->trans("UnitPriceHTCurrency"), 'enabled' => $conf->multicurrency->enabled, 'checked' => 0, 'position' => 10],
                    'pfp.delivery_time_days' => ['label' => $langs->trans("NbDaysToDelivery"), 'checked' => 1, 'position' => 13],
                    'pfp.supplier_reputation' => ['label' => $langs->trans("ReputationForThisProduct"), 'checked' => 1, 'position' => 14],
                    'pfp.barcode' => ['label' => $langs->trans("BarcodeValue"), 'enabled' => $conf->barcode->enabled, 'checked' => 0, 'position' => 15],
                    'pfp.fk_barcode_type' => ['label' => $langs->trans("BarcodeType"), 'enabled' => $conf->barcode->enabled, 'checked' => 0, 'position' => 16],
                    'pfp.packaging' => ['label' => $langs->trans("PackagingForThisProduct"), 'enabled' => !empty($conf->global->PRODUCT_USE_SUPPLIER_PACKAGING), 'checked' => 0, 'position' => 17],
                    'pfp.tms' => ['label' => $langs->trans("DateModification"), 'enabled' => $conf->barcode->enabled, 'checked' => 1, 'position' => 18],
                ];
                ...
                if (!empty($extralabels)) {
                    foreach ($extralabels as $key => $value) {
                        ...
                        if (!empty($extrafields->attributes["product_fournisseur_price"]['list'][$key]) && $extrafields->attributes["product_fournisseur_price"]['list'][$key] != 3) {
                            ...
                            $arrayfields['ef.' . $key] = ['label' => $extratitle, 'checked' => 0, 'position' => (end($arrayfields)['position'] + 1), 'langfile' => $extrafields->attributes["product_fournisseur_price"]['langfile'][$key], 'help' => $extrafields->attributes["product_fournisseur_price"]['help'][$key]];
                        }
                    }
                }
                ...
                if (is_array($product_fourn_list)) {
                    foreach ($product_fourn_list as $productfourn) {
                        ...
                        if (is_object($hookmanager)) {
                            $parameters = ['id_pfp' => $productfourn->product_fourn_price_id, 'id_fourn' => $id_fourn, 'prod_id' => $object->id];
