...
$langs->loadLangs(['bills', 'products', 'stocks']);
...
$hookmanager->initHooks(['productcompositioncard', 'globalcard']);
...
if ($action == 'search') {
    ...
    if ($key != "") {
        // For natural search
        $params = ['p.ref', 'p.label', 'p.description', 'p.note'];
        // multilang
        if (!empty($conf->global->MAIN_MULTILANGS)) {
            $params[] = 'pl.label';
...
if ($id > 0 || !empty($ref)) {
    ...
    if ($user->rights->produit->lire || $user->rights->service->lire) {
        ...
        if ((empty($action) || $action == 'view' || $action == 'edit' || $action == 'search' || $action == 're-edit') && ($user->rights->produit->creer || $user->rights->service->creer)) {
            ...
            print '<form action="' . DOL_URL_ROOT . '/Modules/Products/composition/card.php?id=' . $id . '" method="POST">';
