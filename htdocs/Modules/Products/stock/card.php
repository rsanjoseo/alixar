...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Products/stock/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Products/stock/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->stock->supprimer) {
        ...
        if ($result > 0) {
            ...
            header("Location: " . DOL_URL_ROOT . '/Modules/Products/stock/list.php?restore_lastsearch_values=1');
            exit;
        } else {
            setEventMessages($object->error, $object->errors, 'errors');
            $action = '';
...
if ($action == 'create') {
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print ' <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $socid . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $socid . (!empty($fac_rec) ? '&fac_rec=' . $fac_rec : '')) . '"><span class="fa fa-plus-circle valignmiddle" title="' . $langs->trans("AddProject") . '"></span></a>';
        print '</td></tr>';
    }
    ...
    print $formcompany->select_ziptown((!empty($object->zip) ? $object->zip : ''), 'zipcode', ['town', 'selectcountry_id', 'state_id'], 6);
    print '</td></tr>';
    print '<tr><td>' . $langs->trans('Town') . '</td><td>';
    print $formcompany->select_ziptown((!empty($object->town) ? $object->town : ''), 'town', ['zipcode', 'selectcountry_id', 'state_id']);
    print '</td></tr>';
    ...
    if ($id > 0 || $ref) {
        ...
        if ($action <> 'edit' && $action <> 're-edit') {
            ...
            // Warehouse card
            $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Products/stock/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
            ...
            if (!empty($user->rights->stock->mouvement->lire)) {
                ...
                if ($lastmovementdate) {
                    ...
                    print '<a href="' . DOL_URL_ROOT . '/Modules/Products/stock/movement_list.php?id=' . $object->id . '">' . $langs->trans("FullList") . '</a>';
                } else {
            ...
            // Hook fields
            $parameters = ['sortfield' => $sortfield, 'sortorder' => $sortorder];
            ...
            if ($resql) {
                ...
                while ($i < $num) {
                    ...
                    if ($user->rights->stock->mouvement->creer) {
                        print '<td class="center"><a href="' . DOL_URL_ROOT . '/Modules/Products/stock/product.php?dwid=' . $object->id . '&id=' . $objp->rowid . '&action=transfert&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $id) . '">';
                    ...
                    if ($user->rights->stock->creer) {
                        print '<td class="center"><a href="' . DOL_URL_ROOT . '/Modules/Products/stock/product.php?dwid=' . $object->id . '&id=' . $objp->rowid . '&action=correction&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $id) . '">';
