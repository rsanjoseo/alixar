...
$langs->loadLangs(['products', 'stocks', 'orders', 'productbatch']);

//init Hook
$hookmanager->initHooks(['massstockmove']);
...
if ($action == 'addline' && !empty($user->rights->stock->mouvement->creer)) {
    ...
    if (!$error) {
        ...
        $listofdata[$id] = ['id' => $id, 'id_product' => $id_product, 'qty' => $qty, 'id_sw' => $id_sw, 'id_tw' => $id_tw, 'batch' => $batch];
...
if ($action == 'createmovements' && !empty($user->rights->stock->mouvement->creer)) {
    ...
    if (!$error) {
        ...
        header("Location: " . DOL_URL_ROOT . '/Modules/Products/stock/index.php'); // Redirect to avoid pb when using back
        exit;
    } else {
        $db->rollback();
        ...
        if ($nblinesrecord < 1) {
            ...
            if (!$error) {
                foreach ($data as $key => $value) {
                    ...
                    $listofdata[$key] = ['id' => $key, 'id_sw' => $tmp_id_sw, 'id_tw' => $tmp_id_tw, 'id_product' => $tmp_id_product, 'qty' => $tmp_qty, 'batch' => $tmp_batch];
                }
            }
        }
    }
...
// From warehouse
print '<td>';
print img_picto($langs->trans("WarehouseSource"), 'stock', 'class="paddingright"') . $formproduct->selectWarehouses($id_sw, 'id_sw', 'warehouseopen,warehouseinternal', 1, 0, 0, '', 0, 0, [], 'minwidth200imp maxwidth200');
print '</td>';
// To warehouse
print '<td>';
print img_picto($langs->trans("WarehouseTarget"), 'stock', 'class="paddingright"') . $formproduct->selectWarehouses($id_tw, 'id_tw', 'warehouseopen,warehouseinternal', 1, 0, 0, '', 0, 0, [], 'minwidth200imp maxwidth200');
print '</td>';
// Product
print '<td>';
$filtertype = 0;
...
print $form->select_produits($id_product, 'productid', $filtertype, $limit, 0, -1, 2, '', 1, [], 0, '1', 0, 'minwidth200imp maxwidth300', 1, '', null, 1);
print '</td>';
// Batch number
if ($conf->productbatch->enabled) {
    print '<td>';
