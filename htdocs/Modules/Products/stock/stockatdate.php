...
$hookmanager->initHooks(['stockatdate']);
...
$head[0][0] = DOL_URL_ROOT . '/Modules/Products/stock/stockatdate.php';
$head[0][1] = $langs->trans("StockAtDateInPast");
$head[0][2] = 'stockatdatepast';

$head[1][0] = DOL_URL_ROOT . '/Modules/Products/stock/stockatdate.php?mode=future';
...
while ($i < ($limit ? min($num, $limit) : $num)) {
    ...
    if (!empty($conf->global->STOCK_SUPPORTS_SERVICES) || $objp->fk_product_type == 0) {
        ...
        if ($mode == 'future') {
            ...
            print '<td class="right">';
            if ($nbofmovement > 0) {
                print '<a href="' . DOL_URL_ROOT . '/Modules/Products/stock/movement_list.php?idproduct=' . $objp->rowid . ($fk_warehouse > 0 ? '&search_warehouse=' . $fk_warehouse : '') . '">' . $langs->trans("Movements") . '</a>';
