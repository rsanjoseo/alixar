...
class MouvementStock extends CommonObject
{
    ...
    public $fields = [
        'rowid' => ['type' => 'integer', 'label' => 'TechnicalID', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 10, 'showoncombobox' => 1],
        'tms' => ['type' => 'timestamp', 'label' => 'DateModification', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 15],
        'datem' => ['type' => 'datetime', 'label' => 'Datem', 'enabled' => 1, 'visible' => -1, 'position' => 20],
        'fk_product' => ['type' => 'integer:Product:product/class/product.class.php:1', 'label' => 'Product', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 25],
        'fk_entrepot' => ['type' => 'integer:Entrepot:product/stock/class/entrepot.class.php', 'label' => 'Warehouse', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 30],
        'value' => ['type' => 'double', 'label' => 'Value', 'enabled' => 1, 'visible' => -1, 'position' => 35],
        'price' => ['type' => 'double(24,8)', 'label' => 'Price', 'enabled' => 1, 'visible' => -1, 'position' => 40],
        'type_mouvement' => ['type' => 'smallint(6)', 'label' => 'Type mouvement', 'enabled' => 1, 'visible' => -1, 'position' => 45],
        'fk_user_author' => ['type' => 'integer:User:user/class/user.class.php', 'label' => 'Fk user author', 'enabled' => 1, 'visible' => -1, 'position' => 50],
        'label' => ['type' => 'varchar(255)', 'label' => 'Label', 'enabled' => 1, 'visible' => -1, 'position' => 55],
        'fk_origin' => ['type' => 'integer', 'label' => 'Fk origin', 'enabled' => 1, 'visible' => -1, 'position' => 60],
        'origintype' => ['type' => 'varchar(32)', 'label' => 'Origintype', 'enabled' => 1, 'visible' => -1, 'position' => 65],
        'model_pdf' => ['type' => 'varchar(255)', 'label' => 'Model pdf', 'enabled' => 1, 'visible' => 0, 'position' => 70],
        'fk_projet' => ['type' => 'integer:Project:projet/class/project.class.php:1:fk_statut=1', 'label' => 'Project', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 75],
        'inventorycode' => ['type' => 'varchar(128)', 'label' => 'InventoryCode', 'enabled' => 1, 'visible' => -1, 'position' => 80],
        'batch' => ['type' => 'varchar(30)', 'label' => 'Batch', 'enabled' => 1, 'visible' => -1, 'position' => 85],
        'eatby' => ['type' => 'date', 'label' => 'Eatby', 'enabled' => 1, 'visible' => -1, 'position' => 90],
        'sellby' => ['type' => 'date', 'label' => 'Sellby', 'enabled' => 1, 'visible' => -1, 'position' => 95],
        'fk_project' => ['type' => 'integer:Project:projet/class/project.class.php:1:fk_statut=1', 'label' => 'Fk project', 'enabled' => 1, 'visible' => -1, 'position' => 100],
    ];
    ...
    public function _create($user, $fk_product, $entrepot_id, $qty, $type, $price = 0, $label = '', $inventorycode = '', $datem = '', $eatby = '', $sellby = '', $batch = '', $skip_batch = false, $id_product_batch = 0, $disablestockchangeforsubproduct = 0)
    {
        ...
        require_once DOL_DOCUMENT_ROOT . '/Modules/Products/stock/class/productlot.class.php';
        ...
        $hookmanager->initHooks(['mouvementstock']);
        ...
        if ($movestock && $qty < 0 && empty($conf->global->STOCK_ALLOW_NEGATIVE_TRANSFER)) {
            if (!empty($conf->productbatch->enabled) && $product->hasbatch() && !$skip_batch) {
                ...
                if (!$foundforbatch || $qtyisnotenough) {
                    $langs->load("stocks");
                    include_once DOL_DOCUMENT_ROOT . '/Modules/Products/stock/class/entrepot.class.php';
        ...
        if ($movestock) {    // Change stock for current product, change for subproduct is done after
            ...
            if (!$error && !empty($conf->productbatch->enabled) && $product->hasbatch() && !$skip_batch) {
                if ($id_product_batch > 0) {
                    ...
                    $param_batch = ['fk_product_stock' => $fk_product_stock, 'batchnumber' => $batch];
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $maxlen = 24, $morecss = '')
    {
        ...
        $link = '<a href="' . DOL_URL_ROOT . '/Modules/Products/stock/movement_list.php?id=' . $this->warehouse_id . '&msid=' . $this->id . '"';
