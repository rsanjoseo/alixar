...
$hookmanager->initHooks(['stocklist']);
...
$arrayfields = [
    'stockqty' => ['type' => 'float', 'label' => 'PhysicalStock', 'enabled' => 1, 'visible' => -2, 'checked' => 0, 'position' => 170],
    'estimatedvalue' => ['type' => 'float', 'label' => 'EstimatedStockValue', 'enabled' => 1, 'visible' => 1, 'checked' => 1, 'position' => 171],
    'estimatedstockvaluesell' => ['type' => 'float', 'label' => 'EstimatedStockValueSell', 'enabled' => 1, 'checked' => 1, 'visible' => 2, 'position' => 172],
];
foreach ($object->fields as $key => $val) {
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: " . DOL_URL_ROOT . '/Modules/Products/stock/card.php?id=' . $id);
    exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('MenuNewWarehouse'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Products/stock/card.php?action=create&backtopage=' . urlencode($_SERVER['PHP_SELF']), '', $user->rights->stock->creer);
...
while ($i < min($num, $limit)) {
    ...
    foreach ($warehouse->fields as $key => $val) {
        ...
        if (in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && $key != 'status' && empty($val['arrayofkeyval'])) {
            $cssforfield .= ($cssforfield ? ' ' : '') . 'right';
        }
        if (in_array($key, ['fk_soc', 'fk_user', 'fk_warehouse'])) {
