...
$langs->loadLangs(['products', 'stocks', 'orders']);
...
$hookmanager->initHooks(['movementlist']);
...
$arrayfields = [
    'm.rowid' => ['label' => $langs->trans("Ref"), 'checked' => 1],
    'm.datem' => ['label' => $langs->trans("Date"), 'checked' => 1],
    'p.ref' => ['label' => $langs->trans("ProductRef"), 'checked' => 1, 'css' => 'maxwidth100'],
    'p.label' => ['label' => $langs->trans("ProductLabel"), 'checked' => 1],
    'm.batch' => ['label' => $langs->trans("BatchNumberShort"), 'checked' => 1, 'enabled' => (!empty($conf->productbatch->enabled))],
    'pl.eatby' => ['label' => $langs->trans("EatByDate"), 'checked' => 0, 'position' => 10, 'enabled' => (!empty($conf->productbatch->enabled))],
    'pl.sellby' => ['label' => $langs->trans("SellByDate"), 'checked' => 0, 'position' => 10, 'enabled' => (!empty($conf->productbatch->enabled))],
    'e.ref' => ['label' => $langs->trans("Warehouse"), 'checked' => 1, 'enabled' => (!$id > 0)], // If we are on specific warehouse, we hide it
    'm.fk_user_author' => ['label' => $langs->trans("Author"), 'checked' => 0],
    'm.inventorycode' => ['label' => $langs->trans("InventoryCodeShort"), 'checked' => 1],
    'm.label' => ['label' => $langs->trans("MovementLabel"), 'checked' => 1],
    'm.type_mouvement' => ['label' => $langs->trans("TypeMovement"), 'checked' => 1],
    'origin' => ['label' => $langs->trans("Origin"), 'checked' => 1],
    'm.value' => ['label' => $langs->trans("Qty"), 'checked' => 1],
    'm.price' => ['label' => $langs->trans("UnitPurchaseValue"), 'checked' => 0],
...
if ($resql) {
    ...
    if ($action == "correction") {
        include DOL_DOCUMENT_ROOT . '/Modules/Products/stock/tpl/stockcorrection.tpl.php';
        print '<br>';
    }
     ...
     */
    if ($action == "transfert") {
        include DOL_DOCUMENT_ROOT . '/Modules/Products/stock/tpl/stocktransfer.tpl.php';
        print '<br>';
    }
    ...
    while ($i < ($limit ? min($num, $limit) : $num)) {
        ...
        if (!empty($arrayfields['m.inventorycode']['checked'])) {
            // Inventory code
            print '<td><a href="'
                . DOL_URL_ROOT . '/Modules/Products/stock/movement_card.php?id=' . urlencode($objp->entrepot_id)
