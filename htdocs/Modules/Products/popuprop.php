...
$langs->loadLangs(['commande', 'propal', 'bills', 'other']);
...
$head[$h][0] = DOL_URL_ROOT . '/Modules/Products/stats/card.php?id=all';
$head[$h][1] = $langs->trans("Chart");
$head[$h][2] = 'chart';
$h++;

$head[$h][0] = DOL_URL_ROOT . '/Modules/Products/popuprop.php';
...
if (!empty($mode) && $mode != '-1') {
    ...
    if ($resql) {
        ...
        while ($i < $num) {
            ...
            $infoprod[$objp->rowid] = ['type' => $objp->type, 'ref' => $objp->ref, 'label' => $objp->label];
...
if ($mode && $mode != '-1') {
    foreach ($infoprod as $prodid => $vals) {
        ...
        print "<tr>";
        print '<td><a href="' . DOL_URL_ROOT . '/Modules/Products/stats/card.php?id=' . $prodid . '">';
        if ($vals['type'] == 1) {
