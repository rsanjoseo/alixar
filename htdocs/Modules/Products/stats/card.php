...
$langs->loadLangs(['companies', 'products', 'stocks', 'bills', 'other']);
...
$hookmanager->initHooks(['productstatscard', 'globalcard']);
...
if (empty($id) && empty($ref)) {
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Products/stats/card.php' . ($type != '' ? '?type=' . $type : '');
    ...
    $head[$h][0] = DOL_URL_ROOT . '/Modules/Products/popuprop.php' . ($type != '' ? '?type=' . $type : '');
...
if ($result || empty($id)) {
    ...
    if ($conf->contrat->enabled) {
        $graphfiles['contracts'] = [
            'modulepart' => 'productstats_contracts',
