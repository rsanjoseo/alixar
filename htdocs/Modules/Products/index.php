...
$langs->loadLangs(['products', 'stocks']);

// Initialize technical object to manage hooks. Note that conf->hooks_modules contains array of hooks
$hookmanager->initHooks(['productindex']);
...
if ((!empty($conf->product->enabled) || !empty($conf->service->enabled)) && ($user->rights->produit->lire || $user->rights->service->lire)) {
    ...
    if ($conf->use_javascript_ajax) {
        ...
        if (!empty($conf->product->enabled)) {
            $dataseries[] = [$langs->transnoentitiesnoconv("ProductsOnSale"), round($SommeA)];
            $dataseries[] = [$langs->transnoentitiesnoconv("ProductsOnPurchase"), round($SommeB)];
            $dataseries[] = [$langs->transnoentitiesnoconv("ProductsNotOnSell"), round($SommeC)];
        }
        if (!empty($conf->service->enabled)) {
            $dataseries[] = [$langs->transnoentitiesnoconv("ServicesOnSale"), round($SommeD)];
            $dataseries[] = [$langs->transnoentitiesnoconv("ServicesOnPurchase"), round($SommeE)];
            $dataseries[] = [$langs->transnoentitiesnoconv("ServicesNotOnSell"), round($SommeF)];
        }
...
if ((!empty($conf->product->enabled) || !empty($conf->service->enabled)) && ($user->rights->produit->lire || $user->rights->service->lire)) {
    ...
    if ($result) {
        ...
        if ($num > 0) {
            ...
            print '<th class="right" colspan="3"><a href="' . DOL_URL_ROOT . '/Modules/Products/list.php?sortfield=p.tms&sortorder=DESC">' . $langs->trans("FullList") . '</td>';
            print '</tr>';
...
$parameters = ['type' => $type, 'user' => $user];
