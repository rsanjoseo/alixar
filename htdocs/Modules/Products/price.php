...
$langs->loadLangs(['products', 'bills', 'companies', 'other']);
...
$hookmanager->initHooks(['productpricecard', 'globalcard']);
        ...
        if (!$error && (!empty($conf->global->PRODUIT_MULTIPRICES) || !empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES))) {
            ...
            for ($i = 1; $i <= $conf->global->PRODUIT_MULTIPRICES_LIMIT; $i++) {
                ...
                $pricestoupdate[$i] = [
                    ...
                    'localtaxes_array' => ['0' => $localtax1_type, '1' => $localtax1, '2' => $localtax2_type, '3' => $localtax2]  // default_vat_code should be used in priority in a future
                ];
...
if ($action == 'edit_price' && $object->getRights()->creer) {
    ...
    if (empty($conf->global->PRODUIT_MULTIPRICES) && empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES)) {
        ...
        if (!empty($conf->dynamicprices->enabled)) {
            ...
            $price_expression_list = [0 => $langs->trans("Numeric") . ' <span class="opacitymedium">(' . $langs->trans("NoDynamicPrice") . ')</span>']; //Put the numeric mode as first option
...
if ((empty($conf->global->PRODUIT_CUSTOMER_PRICES) || $action == 'showlog_default_price') && !in_array($action, ['edit_price', 'edit_vat'])) {
...
if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
    ...
    if ($action == 'add_customer_price') {
        ...
        print '<td>';
        print img_picto('', 'company') . $form->select_company('', 'socid', 's.client IN (1,2,3)', 'SelectThirdParty', 0, 0, [], 0, 'minwidth300');
        print '</td>';
        print '</tr>';
        ...
        $filter = ['t.fk_product' => $object->id, 't.fk_soc' => GETPOST('socid', 'int')];
