...
$langs->loadLangs(['products', 'stocks', 'suppliers', 'companies', 'margins']);
...
$hookmanager->initHooks(['productservicelist']);
...
$arrayfields = [
    'p.rowid' => ['type' => 'integer', 'label' => 'TechnicalID', 'enabled' => 1, 'visible' => -2, 'noteditable' => 1, 'notnull' => 1, 'index' => 1, 'position' => 1, 'comment' => 'Id', 'css' => 'left'],
    'p.ref' => ['label' => "Ref", 'checked' => 1, 'position' => 10],
    //'pfp.ref_fourn'=>array('label'=>$langs->trans("RefSupplier"), 'checked'=>1, 'enabled'=>(! empty($conf->barcode->enabled))),
    'p.label' => ['label' => "Label", 'checked' => 1, 'position' => 10],
    'p.fk_product_type' => ['label' => "Type", 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && !empty($conf->service->enabled)), 'position' => 11],
    'p.barcode' => ['label' => "Gencod", 'checked' => 1, 'enabled' => (!empty($conf->barcode->enabled)), 'position' => 12],
    'p.duration' => ['label' => "Duration", 'checked' => ($contextpage != 'productlist'), 'enabled' => (!empty($conf->service->enabled) && (string) $type == '1'), 'position' => 13],
    'p.finished' => ['label' => "Nature", 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && $type != '1'), 'position' => 19],
    'p.weight' => ['label' => 'Weight', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && $type != '1'), 'position' => 20],
    'p.weight_units' => ['label' => 'WeightUnits', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && $type != '1'), 'position' => 21],
    'p.length' => ['label' => 'Length', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SIZE) && $type != '1'), 'position' => 22],
    'p.length_units' => ['label' => 'LengthUnits', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SIZE) && $type != '1'), 'position' => 23],
    'p.width' => ['label' => 'Width', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SIZE) && $type != '1'), 'position' => 24],
    'p.width_units' => ['label' => 'WidthUnits', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SIZE) && $type != '1'), 'position' => 25],
    'p.height' => ['label' => 'Height', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SIZE) && $type != '1'), 'position' => 26],
    'p.height_units' => ['label' => 'HeightUnits', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SIZE) && $type != '1'), 'position' => 27],
    'p.surface' => ['label' => 'Surface', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SURFACE) && $type != '1'), 'position' => 28],
    'p.surface_units' => ['label' => 'SurfaceUnits', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_SURFACE) && $type != '1'), 'position' => 29],
    'p.volume' => ['label' => 'Volume', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_VOLUME) && $type != '1'), 'position' => 30],
    'p.volume_units' => ['label' => 'VolumeUnits', 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && empty($conf->global->PRODUCT_DISABLE_VOLUME) && $type != '1'), 'position' => 31],
    'cu.label' => ['label' => "DefaultUnitToShow", 'checked' => 0, 'enabled' => (!empty($conf->product->enabled) && !empty($conf->global->PRODUCT_USE_UNITS)), 'position' => 32],
    'p.sellprice' => ['label' => "SellingPrice", 'checked' => 1, 'enabled' => empty($conf->global->PRODUIT_MULTIPRICES), 'position' => 40],
    'p.tva_tx' => ['label' => "VATRate", 'checked' => 0, 'enabled' => empty($conf->global->PRODUIT_MULTIPRICES), 'position' => 41],
    'p.minbuyprice' => ['label' => "BuyingPriceMinShort", 'checked' => 1, 'enabled' => (!empty($user->rights->fournisseur->lire)), 'position' => 42],
    'p.numbuyprice' => ['label' => "BuyingPriceNumShort", 'checked' => 0, 'enabled' => (!empty($user->rights->fournisseur->lire)), 'position' => 43],
    'p.pmp' => ['label' => "PMPValueShort", 'checked' => 0, 'enabled' => (!empty($user->rights->fournisseur->lire)), 'position' => 44],
    'p.cost_price' => ['label' => "CostPrice", 'checked' => 0, 'enabled' => (!empty($user->rights->fournisseur->lire)), 'position' => 45],
    'p.seuil_stock_alerte' => ['label' => "StockLimit", 'checked' => 0, 'enabled' => (!empty($conf->stock->enabled) && $user->rights->stock->lire && $contextpage != 'service'), 'position' => 50],
    'p.desiredstock' => ['label' => "DesiredStock", 'checked' => 1, 'enabled' => (!empty($conf->stock->enabled) && $user->rights->stock->lire && $contextpage != 'service'), 'position' => 51],
    'p.stock' => ['label' => "PhysicalStock", 'checked' => 1, 'enabled' => (!empty($conf->stock->enabled) && $user->rights->stock->lire && $contextpage != 'service'), 'position' => 52],
    'stock_virtual' => ['label' => "VirtualStock", 'checked' => 1, 'enabled' => (!empty($conf->stock->enabled) && $user->rights->stock->lire && $contextpage != 'service' && $virtualdiffersfromphysical), 'position' => 53],
    'p.tobatch' => ['label' => "ManageLotSerial", 'checked' => 0, 'enabled' => (!empty($conf->productbatch->enabled)), 'position' => 60],
    'p.fk_country' => ['label' => "Country", 'checked' => 0, 'position' => 100],
    'p.fk_state' => ['label' => "State", 'checked' => 0, 'position' => 101],
    $alias_product_perentity . '.accountancy_code_sell' => ['label' => "ProductAccountancySellCode", 'checked' => 0, 'position' => 400],
    $alias_product_perentity . '.accountancy_code_sell_intra' => ['label' => "ProductAccountancySellIntraCode", 'checked' => 0, 'enabled' => $isInEEC, 'position' => 401],
    $alias_product_perentity . '.accountancy_code_sell_export' => ['label' => "ProductAccountancySellExportCode", 'checked' => 0, 'position' => 402],
    $alias_product_perentity . '.accountancy_code_buy' => ['label' => "ProductAccountancyBuyCode", 'checked' => 0, 'position' => 403],
    $alias_product_perentity . '.accountancy_code_buy_intra' => ['label' => "ProductAccountancyBuyIntraCode", 'checked' => 0, 'enabled' => $isInEEC, 'position' => 404],
    $alias_product_perentity . '.accountancy_code_buy_export' => ['label' => "ProductAccountancyBuyExportCode", 'checked' => 0, 'position' => 405],
    'p.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'p.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 500],
    'p.tosell' => ['label' => $langs->transnoentitiesnoconv("Status") . ' (' . $langs->transnoentitiesnoconv("Sell") . ')', 'checked' => 1, 'position' => 1000],
    'p.tobuy' => ['label' => $langs->transnoentitiesnoconv("Status") . ' (' . $langs->transnoentitiesnoconv("Buy") . ')', 'checked' => 1, 'position' => 1000],
];
...
if ($conf->global->PRODUIT_MULTIPRICES) {
    for ($i = 1; $i <= $conf->global->PRODUIT_MULTIPRICES_LIMIT; $i++) {
        ...
        $arrayfields['p.sellprice' . $i] = ['label' => $labelp, 'checked' => ($i == 1 ? 1 : 0), 'enabled' => $conf->global->PRODUIT_MULTIPRICES, 'position' => floatval('40.' . sprintf('%03s', $i))];
        $arraypricelevel[$i] = [$i];
    }
}
...
if ($resql) {
    ...
    if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
        ...
        $id = $obj->rowid;
        header("Location: " . DOL_URL_ROOT . '/Modules/Products/card.php?id=' . $id);
        exit;
    }
    ...
    if ($type === "") {
        $newcardbutton .= dolGetButtonTitle($langs->trans('NewProduct'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Products/card.php?action=create&type=0', '', $perm, $params);
    ...
    $newcardbutton .= dolGetButtonTitle($langs->trans($label), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Products/card.php?action=create&type=' . $type, '', $perm, $params);
    ...
    if (!empty($arrayfields['p.tosell']['checked'])) {
        print '<td class="liste_titre center">';
        print $form->selectarray('search_tosell', ['0' => $langs->trans('ProductStatusNotOnSellShort'), '1' => $langs->trans('ProductStatusOnSellShort')], $search_tosell, 1);
        print '</td >';
    }
    if (!empty($arrayfields['p.tobuy']['checked'])) {
        print '<td class="liste_titre center">';
        print $form->selectarray('search_tobuy', ['0' => $langs->trans('ProductStatusNotOnBuyShort'), '1' => $langs->trans('ProductStatusOnBuyShort')], $search_tobuy, 1);
        print '</td>';
    }
