...
require_once DOL_DOCUMENT_ROOT . '/Modules/Products/inventory/lib/inventory.lib.php';

// Load translation files required by the page
$langs->loadLangs(["stocks", "other"]);
...
$hookmanager->initHooks(['inventorycard', 'globalcard']); // Note that conf->hooks_modules contains array
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = dol_buildpath('/Modules/Products/inventory/card.php', 1) . '?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    // Object card
    // ------------------------------------------------------------
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Products/inventory/list.php' . (!empty($socid) ? '?socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
    ...
    */
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['inventory']);
        ...
        $morehtmlright = '<a href="' . dol_buildpath('/Modules/Products/inventory/inventory_info.php', 1) . '?id=' . $object->id . '">';
