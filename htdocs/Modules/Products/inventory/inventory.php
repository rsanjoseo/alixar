...
include_once DOL_DOCUMENT_ROOT . '/Modules/Products/class/html.formproduct.class.php';
include_once DOL_DOCUMENT_ROOT . '/Modules/Products/class/product.class.php';
include_once DOL_DOCUMENT_ROOT . '/Modules/Products/inventory/class/inventory.class.php';
include_once DOL_DOCUMENT_ROOT . '/Modules/Products/inventory/lib/inventory.lib.php';
include_once DOL_DOCUMENT_ROOT . '/Modules/Products/stock/class/mouvementstock.class.php';

// Load translation files required by the page
$langs->loadLangs(["stocks", "other", "productbatch"]);
...
$hookmanager->initHooks(['inventorycard']); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
    ...
    $backurlforlist = DOL_URL_ROOT . '/Modules/Products/inventory/list.php';
    $backtopage = DOL_URL_ROOT . '/Modules/Products/inventory/inventory.php?id=' . $object->id;
...
if ($object->id > 0) {
    ...
    // Object card
    // ------------------------------------------------------------
    $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Products/inventory/list.php">' . $langs->trans("BackToList") . '</a>';
    ...
    */
    ...
    if ($object->status == $object::STATUS_VALIDATED) {
        print '<tr>';
        print '<td>';
        print $formproduct->selectWarehouses((GETPOSTISSET('fk_warehouse') ? GETPOST('fk_warehouse', 'int') : $object->fk_warehouse), 'fk_warehouse', 'warehouseopen', 1, 0, 0, '', 0, 0, [], 'maxwidth300');
        print '</td>';
        print '<td>';
    ...
    if ($resql) {
        ...
        while ($i < $num) {
            ...
            if ($object->status == $object::STATUS_VALIDATED) {
                ...
                print '<a class="reposition" href="' . DOL_URL_ROOT . '/Modules/Products/inventory/inventory.php?id=' . $object->id . '&lineid=' . $obj->rowid . '&action=deleteline&token=' . newToken() . '">' . img_delete() . '</a>';
                print '</td>';
