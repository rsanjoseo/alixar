...
$hookmanager->initHooks(['productcard', 'globalcard']);
...
$parameters = ['id' => $id, 'ref' => $ref, 'objcanvas' => $objcanvas];
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Products/list.php?type=' . $type;
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Products/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes' && $usercandelete) {
        ...
        if ($result > 0) {
            header('Location: ' . DOL_URL_ROOT . '/Modules/Products/list.php?type=' . $object->type . '&delprod=' . urlencode($object->ref));
            exit;
        } else {
            setEventMessages($langs->trans($object->error), null, 'errors');
            $reload = 0;
    ...
    if ($object->id > 0 && $action == 'addin') {
        ...
        if ($thirpdartyid > 0) {
            ...
            if ($conf->global->PRODUIT_MULTIPRICES && $soc->price_level) {
                ...
                $filter = ['t.fk_product' => $object->id, 't.fk_soc' => $soc->id];
            ...
            if (GETPOST('propalid') > 0) {
                ...
                if ($result > 0) {
                    header("Location: " . DOL_URL_ROOT . "/Modules/Comm/propal/card.php?id=" . $propal->id);
                    return;
                }
                ...
                if ($result > 0) {
                    header("Location: " . DOL_URL_ROOT . "/Modules/Commande/card.php?id=" . urlencode($commande->id));
                    exit;
                }
            } elseif (GETPOST('factureid') > 0) {
                // Define cost price for margin calculation
                ...
                if ($result > 0) {
                    header("Location: " . DOL_URL_ROOT . "/Modules/Compta/facture/card.php?facid=" . $facture->id);
                    exit;
                }
            }
        } else {
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($action == 'create' && $usercancreate) {
        ...
        $statutarray = ['1' => $langs->trans("OnSell"), '0' => $langs->trans("NotOnSell")];
        ...
        $statutarray = ['1' => $langs->trans("ProductStatusOnBuy"), '0' => $langs->trans("ProductStatusNotOnBuy")];
        ...
        if (!empty($conf->productbatch->enabled)) {
            ...
            $statutarray = ['0' => $langs->trans("ProductStatusNotOnBatch"), '1' => $langs->trans("ProductStatusOnBatch"), '2' => $langs->trans("ProductStatusOnSerial")];
        ...
        if ($type != 1 && !empty($conf->stock->enabled)) {
            ...
            print $formproduct->selectWarehouses(GETPOST('fk_default_warehouse', 'int'), 'fk_default_warehouse', 'warehouseopen', 1, 0, 0, '', 0, 0, [], 'minwidth300 widthcentpercentminusxx maxwidth500');
            print ' <a href="' . DOL_URL_ROOT . '/Modules/Products/stock/card.php?action=create&token=' . newToken() . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id . '&action=edit&token=' . newToken()) . '">';
        ...
        // Other attributes
        $parameters = ['colspan' => ' colspan="2"', 'cols' => 2];
        ...
        if ($action == 'edit' && $usercancreate) {
            ...
            if ($object->isProduct() && !empty($conf->stock->enabled)) {
                ...
                print ' <a href="' . DOL_URL_ROOT . '/Modules/Products/stock/card.php?action=create&amp;backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?action=create&type=' . GETPOST('type', 'int')) . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddWarehouse") . '"></span></a>';
                print '</td></tr>';
                /*
            ...
            $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Products/list.php?restore_lastsearch_values=1&type=' . $object->type . '">' . $langs->trans("BackToList") . '</a>';
            ...
            if ($object->isService()) {
                ...
                if ($object->duration_value > 1) {
                    $dur = ["i" => $langs->trans("Minute"), "h" => $langs->trans("Hours"), "d" => $langs->trans("Days"), "w" => $langs->trans("Weeks"), "m" => $langs->trans("Months"), "y" => $langs->trans("Years")];
...
if (($action == 'clone' && (empty($conf->use_javascript_ajax) || !empty($conf->dol_use_jmobile)))        // Output when action = clone if jmobile or no js
    ...
    $formquestionclone = [
        ...
        ['type' => 'text', 'name' => 'clone_ref', 'label' => $langs->trans("NewRefForClone"), 'value' => empty($tmpcode) ? $langs->trans("CopyOf") . ' ' . $object->ref : $tmpcode, 'morecss' => 'width150'],
        ['type' => 'checkbox', 'name' => 'clone_content', 'label' => $langs->trans("CloneContentProduct"), 'value' => 1],
        ['type' => 'checkbox', 'name' => 'clone_categories', 'label' => $langs->trans("CloneCategoriesProduct"), 'value' => 1],
    ];
    if (!empty($conf->global->PRODUIT_MULTIPRICES)) {
        $formquestionclone[] = ['type' => 'checkbox', 'name' => 'clone_prices', 'label' => $langs->trans("ClonePricesProduct") . ' (' . $langs->trans("CustomerPrices") . ')', 'value' => 0];
    }
    if (!empty($conf->global->PRODUIT_SOUSPRODUITS)) {
        $formquestionclone[] = ['type' => 'checkbox', 'name' => 'clone_composition', 'label' => $langs->trans('CloneCompositionProduct'), 'value' => 1];
    }
...
$parameters = ['formConfirm' => $formconfirm, 'object' => $object];
...
if ($action != 'create' && $action != 'edit' && $action != 'delete') {
    ...
    $morehtmlright = '<a href="' . DOL_URL_ROOT . '/Modules/Products/agenda.php?id=' . $object->id . '">';
