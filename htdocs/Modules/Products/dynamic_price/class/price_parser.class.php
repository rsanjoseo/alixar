...
require_once DOL_DOCUMENT_ROOT . '/Modules/Products/dynamic_price/class/price_global_variable.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Products/dynamic_price/class/price_global_variable_updater.class.php';
...
class PriceParser
{
    ...
    public function translatedError()
    {
        ...
        */
        ...
        if (in_array($code, [9, 14, 19, 20])) { //Errors which have 0 arg
            return $langs->trans("ErrorPriceExpression" . $code);
        } elseif (in_array($code, [1, 2, 3, 4, 5, 8, 10, 11, 17, 21, 22])) { //Errors which have 1 arg
            return $langs->trans("ErrorPriceExpression" . $code, $info);
        } elseif (in_array($code, [6, 23])) { //Errors which have 2 args
            return $langs->trans("ErrorPriceExpression" . $code, $info[0], $info[1]);
        } elseif (in_array($code, [7, 12, 13, 15, 16, 18])) { //Internal errors
    ...
    public function parseExpression($product, $expression, $values)
    {
        ...
        if ($result = $hookmanager->executeHooks('doDynamiPrice', [
        ...
        if (strpos($expression, $this->special_chr) !== false) {
            ...
            $this->error_parser = [23, [$variable, $expression]];
            return -6;
        }
