...
$langs->loadLangs(['products', 'languages']);
...
$hookmanager->initHooks(['producttranslationcard', 'globalcard']);
...
if (empty($reshook)) {
    if ($action == '') {
        if ($user->rights->produit->creer || $user->rights->service->creer) {
            print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Products/traduction.php?action=add&token=' . newToken() . '&id=' . $object->id . '">' . $langs->trans("Add") . '</a>';
            if ($cnt_trans > 0) {
                print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Products/traduction.php?action=edit&token=' . newToken() . '&id=' . $object->id . '">' . $langs->trans("Update") . '</a>';
            }
        }
    }
}
