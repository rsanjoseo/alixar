...
if ($conf->use_javascript_ajax) {
    ...
    foreach ($AdherentType as $key => $adhtype) {
        $dataval['draft'][] = [$i, isset($MembersToValidate[$key]) ? $MembersToValidate[$key] : 0];
        $dataval['notuptodate'][] = [$i, isset($MembersValidated[$key]) ? $MembersValidated[$key] - (isset($MembersUpToDate[$key]) ? $MembersUpToDate[$key] : 0) : 0];
        $dataval['uptodate'][] = [$i, isset($MembersUpToDate[$key]) ? $MembersUpToDate[$key] : 0];
        $dataval['excluded'][] = [$i, isset($MembersExcluded[$key]) ? $MembersExcluded[$key] : 0];
        $dataval['resiliated'][] = [$i, isset($MembersResiliated[$key]) ? $MembersResiliated[$key] : 0];
    ...
    $dataseries = [];
    $dataseries[] = [$langs->transnoentitiesnoconv("OutOfDate"), round($SumValidated)];
    $dataseries[] = [$langs->transnoentitiesnoconv("UpToDate"), round($SumUpToDate)];
    $dataseries[] = [$langs->transnoentitiesnoconv("MembersStatusExcluded"), round($SumExcluded)];
    $dataseries[] = [$langs->transnoentitiesnoconv("MembersStatusResiliated"), round($SumResiliated)];
    $dataseries[] = [$langs->transnoentitiesnoconv("MembersStatusToValid"), round($SumToValidate)];
    ...
    $dolgraph->SetDataColor([$badgeStatus1, $badgeStatus4, '-' . $badgeStatus8, $badgeStatus6, '-' . $badgeStatus0]);
