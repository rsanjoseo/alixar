...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Adherents/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Adherents/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
        ...
        if ($result >= 0 && !count($object->errors)) {
            ...
            if ($object->email && GETPOST("send_mail")) {
                ...
                if (empty($labeltouse) || (int) $labeltouse === -1) {
                    ...
                    $result = $object->send_an_email($texttosend, $subjecttosend, [], [], [], "", "", 0, -1, '', $moreinheader);
                    if ($result < 0) {
                        $error++;
                        setEventMessages($object->error, $object->errors, 'errors');
                    }
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($action == 'create') {
        ...
        print $formcompany->select_ziptown((GETPOSTISSET('zipcode') ? GETPOST('zipcode', 'alphanohtml') : $object->zip), 'zipcode', ['town', 'selectcountry_id', 'state_id'], 6);
        print ' ';
        print $formcompany->select_ziptown((GETPOSTISSET('town') ? GETPOST('town', 'alphanohtml') : $object->town), 'town', ['zipcode', 'selectcountry_id', 'state_id']);
        print '</td></tr>';
    ...
    if ($id > 0 && $action != 'edit') {
        ...
        if ($action == 'valid') {
            ...
            $formquestion = [];
            if ($object->email) {
                $formquestion[] = ['type' => 'checkbox', 'name' => 'send_mail', 'label' => $label, 'value' => ($conf->global->ADHERENT_DEFAULT_SENDINFOBYMAIL ? true : false)];
            }
        ...
        if ($action == 'resiliate') {
            ...
            $formquestion = [];
            if ($object->email) {
                $formquestion[] = ['type' => 'checkbox', 'name' => 'send_mail', 'label' => $label, 'value' => (!empty($conf->global->ADHERENT_DEFAULT_SENDINFOBYMAIL) ? 'true' : 'false')];
            }
            if ($backtopage) {
        ...
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Adherents/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
            ...
            if ($action == 'editthirdparty') {
                ...
                if ($object->socid) {
                    ...
                    if (!empty($tmparray['refs'])) {
                        print ' - ' . img_picto($langs->trans("Invoices"), 'bill', 'class="paddingright"') . '<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/list.php?socid=' . $object->socid . '">' . $langs->trans("Invoices") . ' (' . count($tmparray['refs']) . ')';
        ...
        print '<a href="' . DOL_URL_ROOT . '/Modules/Adherents/vcard.php?id=' . $object->id . '">';
        ...
        if ($action != 'presend') {
            ...
            $morehtmlright = '<a href="' . DOL_URL_ROOT . '/Modules/Adherents/agenda.php?id=' . $object->id . '">';
