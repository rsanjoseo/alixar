...
if ($conf->facture->enabled) {
    ...
    if (!empty($conf->banque->enabled)) {
        print '<td>';
        print $form->selectarray('ADHERENT_VAT_FOR_SUBSCRIPTIONS', ['0' => $langs->trans("NoVatOnSubscription"), 'defaultforfoundationcountry' => $langs->trans("Default")], (empty($conf->global->ADHERENT_VAT_FOR_SUBSCRIPTIONS) ? '0' : $conf->global->ADHERENT_VAT_FOR_SUBSCRIPTIONS), 0);
        print '</td>';
    } else {
 ...
 * Edit info of model document
 */
$constantes = ['ADHERENT_ETIQUETTE_TYPE', 'ADHERENT_ETIQUETTE_TEXT'];
