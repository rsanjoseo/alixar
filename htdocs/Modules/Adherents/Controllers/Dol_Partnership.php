...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action != 'presend') {
        ...
        if (empty($reshook)) {
            // Show
            if ($permissiontoadd) {
                print dolGetButtonAction($langs->trans('AddPartnership'), '', 'default', DOL_URL_ROOT . '/partnership/partnership_card.php?action=create&fk_member=' . $object->id . '&backtopage=' . urlencode(DOL_URL_ROOT . '/Modules/Adherents/partnership.php?id=' . $object->id), '', $permissiontoadd);
            }
        }
        print '</div>' . "\n";
    }
