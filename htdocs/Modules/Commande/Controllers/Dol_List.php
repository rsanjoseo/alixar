...
$hookmanager->initHooks(['orderlist']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'c.ref' => ['label' => "Ref", 'checked' => 1, 'position' => 5],
    'c.ref_client' => ['label' => "RefCustomerOrder", 'checked' => -1, 'position' => 10],
    'p.ref' => ['label' => "ProjectRef", 'checked' => -1, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1), 'position' => 20],
    'p.title' => ['label' => "ProjectLabel", 'checked' => 0, 'enabled' => (empty($conf->projet->enabled) ? 0 : 1), 'position' => 25],
    's.nom' => ['label' => "ThirdParty", 'checked' => 1, 'position' => 30],
    's.name_alias' => ['label' => "AliasNameShort", 'checked' => -1, 'position' => 31],
    's.town' => ['label' => "Town", 'checked' => -1, 'position' => 35],
    's.zip' => ['label' => "Zip", 'checked' => -1, 'position' => 40],
    'state.nom' => ['label' => "StateShort", 'checked' => 0, 'position' => 45],
    'country.code_iso' => ['label' => "Country", 'checked' => 0, 'position' => 50],
    'typent.code' => ['label' => "ThirdPartyType", 'checked' => $checkedtypetiers, 'position' => 55],
    'c.date_commande' => ['label' => "OrderDateShort", 'checked' => 1, 'position' => 60],
    'c.date_delivery' => ['label' => "DateDeliveryPlanned", 'checked' => 1, 'enabled' => empty($conf->global->ORDER_DISABLE_DELIVERY_DATE), 'position' => 65],
    'c.fk_shipping_method' => ['label' => "SendingMethod", 'checked' => -1, 'position' => 66, 'enabled' => !empty($conf->expedition->enabled)],
    'c.fk_cond_reglement' => ['label' => "PaymentConditionsShort", 'checked' => -1, 'position' => 67],
    'c.fk_mode_reglement' => ['label' => "PaymentMode", 'checked' => -1, 'position' => 68],
    'c.fk_input_reason' => ['label' => "Channel", 'checked' => -1, 'position' => 69],
    'c.total_ht' => ['label' => "AmountHT", 'checked' => 1, 'position' => 75],
    'c.total_vat' => ['label' => "AmountVAT", 'checked' => 0, 'position' => 80],
    'c.total_ttc' => ['label' => "AmountTTC", 'checked' => 0, 'position' => 85],
    'c.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 90],
    'c.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 95],
    'c.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 100],
    'c.multicurrency_total_vat' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 105],
    'c.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (empty($conf->multicurrency->enabled) ? 0 : 1), 'position' => 110],
    'u.login' => ['label' => "Author", 'checked' => 1, 'position' => 115],
    'sale_representative' => ['label' => "SaleRepresentativesOfThirdParty", 'checked' => 0, 'position' => 116],
    'c.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 120],
    'c.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 125],
    'c.date_cloture' => ['label' => "DateClosing", 'checked' => 0, 'position' => 130],
    'c.note_public' => ['label' => 'NotePublic', 'checked' => 0, 'enabled' => (empty($conf->global->MAIN_LIST_ALLOW_PUBLIC_NOTES)), 'position' => 135],
    'c.note_private' => ['label' => 'NotePrivate', 'checked' => 0, 'enabled' => (empty($conf->global->MAIN_LIST_ALLOW_PRIVATE_NOTES)), 'position' => 140],
    'shippable' => ['label' => "Shippable", 'checked' => 1, 'enabled' => (!empty($conf->expedition->enabled)), 'position' => 990],
    'c.facture' => ['label' => "Billed", 'checked' => 1, 'enabled' => (empty($conf->global->WORKFLOW_BILL_ON_SHIPMENT)), 'position' => 995],
    'c.fk_statut' => ['label' => "Status", 'checked' => 1, 'position' => 1000],
];
// Extra fields
...
if ($resql) {
    ...
    if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
        ...
        $id = $obj->rowid;
        header("Location: " . DOL_URL_ROOT . '/Modules/Commande/card.php?id=' . $id);
        exit;
    }
    ...
    if (in_array($massaction, ['presend', 'predelete', 'createbills'])) {
        $arrayofmassactions = [];
    }
    $massactionbutton = $form->selectMassAction('', $arrayofmassactions);

    $url = DOL_URL_ROOT . '/Modules/Commande/card.php?action=create';
    if (!empty($socid)) {
        $url .= '&socid=' . $socid;
    }
            ...
            if (!empty($conf->facture->enabled) && !empty($conf->global->ORDER_BILLING_ALL_CUSTOMER)) {
                if ($user->rights->facture->creer) {
                    if (($obj->fk_statut > 0 && $obj->fk_statut < 3) || ($obj->fk_statut == 3 && $obj->billed == 0)) {
                        print '&nbsp;<a href="' . DOL_URL_ROOT . '/Modules/Commande/list.php?socid=' . $companystatic->id . '&search_billed=0&autoselectall=1">';
