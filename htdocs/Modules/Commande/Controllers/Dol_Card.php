...
$langs->loadLangs(['orders', 'sendings', 'companies', 'bills', 'propal', 'deliveries', 'products', 'other']);
...
$hookmanager->initHooks(['ordercard', 'globalcard']);
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Commande/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Commande/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'import_lines_from_object'
        ...
        if (!empty($importLines) && is_array($importLines) && !empty($fromElement) && ctype_alpha($fromElement) && !empty($fromElementid)) {
            if ($fromElement == 'commande') {
                ...
                dol_include_once('/Modules/Comm/' . $fromElement . '/class/' . $fromElement . '.class.php');
...
if ($action == 'create' && $usercancreate) {
    ...
    if ($socid > 0) {
        ...
        print ' <a href="' . DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=create&client=3&fournisseur=0&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create') . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddThirdParty") . '"></span></a>';
        print '</td>';
    }
    print '</tr>' . "\n";
    ...
    if (!empty($conf->projet->enabled)) {
        ...
        print ' <a href="' . DOL_URL_ROOT . '/Modules/Projects/card.php?socid=' . $soc->id . '&action=create&status=1&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?action=create&socid=' . $soc->id) . '"><span class="fa fa-plus-circle valignmiddle" title="' . $langs->trans("AddProject") . '"></span></a>';
        print '</td>';
        print '</tr>';
    }
    ...
    // Other attributes
    $parameters = ['objectsrc' => $objectsrc, 'socid' => $socid];
    // Note that $action and $object may be modified by hook
    ...
    if ($object->id > 0) {
        ...
        if ($action == 'validate') {
            ...
            if (!empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER) && $qualified_for_stock_change) {
                ...
                $formquestion = [
                    ...
                    ['type' => 'other', 'name' => 'idwarehouse', 'label' => $langs->trans("SelectWarehouseForStockDecrease"), 'value' => $formproduct->selectWarehouses(GETPOST('idwarehouse', 'int') ? GETPOST('idwarehouse', 'int') : 'ifone', 'idwarehouse', '', 1, 0, 0, '', 0, $forcecombo)],
                ];
            }
        ...
        if ($action == 'modif') {
            ...
            if (!empty($conf->stock->enabled) && !empty($conf->global->STOCK_CALCULATE_ON_VALIDATE_ORDER) && $qualified_for_stock_change) {
                ...
                $formquestion = [
                    ...
                    // array('type' => 'checkbox', 'name' => 'clone_content', 'label' => $langs->trans("CloneMainAttributes"), 'value' => 1),
                    // array('type' => 'checkbox', 'name' => 'update_prices', 'label' => $langs->trans("PuttingPricesUpToDate"), 'value' => 1),
                    ['type' => 'other', 'name' => 'idwarehouse', 'label' => $langs->trans("SelectWarehouseForStockIncrease"), 'value' => $formproduct->selectWarehouses(GETPOST('idwarehouse') ? GETPOST('idwarehouse') : 'ifone', 'idwarehouse', '', 1, 0, 0, '', 0, $forcecombo)],
                ];
            }
		...
		*/
        ...
        if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
            $morehtmlref .= ' (<a href="' . DOL_URL_ROOT . '/Modules/Commande/list.php?socid=' . $object->thirdparty->id . '&search_societe=' . urlencode($object->thirdparty->name) . '">' . $langs->trans("OtherOrders") . '</a>)';
        }
        // Project
        if (!empty($conf->projet->enabled)) {
            $langs->load("projects");
        ...
        $addrelativediscount = '<a href="' . DOL_URL_ROOT . '/Modules/Comm/remise.php?id=' . $soc->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?facid=' . $object->id . '">' . $langs->trans("EditRelativeDiscounts") . '</a>';
        $addabsolutediscount = '<a href="' . DOL_URL_ROOT . '/Modules/Comm/remx.php?id=' . $soc->id . '&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?facid=' . $object->id . '">' . $langs->trans("EditGlobalDiscounts") . '</a>';
        $addcreditnote = '<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&socid=' . $soc->id . '&type=2&backtopage=' . urlencode($_SERVER["PHP_SELF"]) . '?facid=' . $object->id . '">' . $langs->trans("AddCreditNote") . '</a>';
		...
		*/
        ...
        if ($action != 'presend' && $action != 'editline') {
            ...
            if (empty($reshook)) {
                ...
                /*if ($conf->agenda->enabled && ! empty($conf->global->MAIN_ADD_EVENT_ON_ELEMENT_CARD))
				{
					...
					// page.
					print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Comm/action/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddAction") . '</a>';
				}*/
                ...
                if (!empty($conf->global->WORKFLOW_CAN_CREATE_PURCHASE_ORDER_FROM_SALE_ORDER)) {
                    if (((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled)) && $object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED && $object->getNbOfServicesLines() > 0) {
                        if ($usercancreatepurchaseorder) {
                            print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fourn/commande/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans("AddPurchaseOrder") . '</a>';
                        }
                    }
                }
                ...
                if ($conf->ficheinter->enabled) {
                    ...
                    if ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED && $object->getNbOfServicesLines() > 0) {
                        if ($user->rights->ficheinter->creer) {
                            print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Fichinter/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans('AddIntervention') . '</a>';
                        } else {
                ...
                if ($conf->contrat->enabled && ($object->statut == Commande::STATUS_VALIDATED || $object->statut == Commande::STATUS_SHIPMENTONPROCESS || $object->statut == Commande::STATUS_CLOSED)) {
                    ...
                    if ($user->rights->contrat->creer) {
                        print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Contrats/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans('AddContract') . '</a>';
                    }
                }
                    ...
                    if ($object->statut > Commande::STATUS_DRAFT && $object->statut < Commande::STATUS_CLOSED && ($object->getNbOfProductsLines() > 0 || !empty($conf->global->STOCK_SUPPORTS_SERVICES))) {
                        if (($conf->expedition_bon->enabled && $user->rights->expedition->creer) || ($conf->delivery_note->enabled && $user->rights->expedition->delivery->creer)) {
                            if ($user->rights->expedition->creer) {
                                print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Expedition/shipment.php?id=' . $object->id . '">' . $langs->trans('CreateShipment') . '</a>';
                            } else {
                ...
                if ($object->statut > Commande::STATUS_DRAFT && !$object->billed && $object->total_ttc >= 0) {
                    if (!empty($conf->facture->enabled) && $user->rights->facture->creer && empty($conf->global->WORKFLOW_DISABLE_CREATE_INVOICE_FROM_ORDER)) {
                        print '<a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&token=' . newToken() . '&origin=' . $object->element . '&originid=' . $object->id . '&socid=' . $object->socid . '">' . $langs->trans("CreateBill") . '</a>';
                    }
        ...
        if ($action != 'presend') {
            ...
            $linktoelem = $form->showLinkToObjectBlock($object, null, ['order']);
