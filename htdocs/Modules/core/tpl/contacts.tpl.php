    ...
    if (empty($hideaddcontactforuser)) {
        ...
        <form class="tagtr impair nohover" action="<?php echo $_SERVER["PHP_SELF"] . '?id=' . $object->id; ?>"
            ...
            <div class="tagtd maxwidthonsmartphone">
                ...
                ?></div>
            <div class="tagtd">&nbsp;</div>
            <div class="tagtd center"><input type="submit" class="button" value="<?php echo $langs->trans("Add"); ?>">
            </div>
        </form>
        ...
        <form class="tagtr pair nohover" action="<?php echo $_SERVER["PHP_SELF"] . '?id=' . $object->id; ?>"
            ...
            <div class="tagtd maxwidthonsmartphone noborderbottom">
                ...
                if (!empty($object->socid) && $object->socid > 1 && $user->rights->societe->creer) {
                    $newcardbutton .= '<a href="' . DOL_URL_ROOT . '/Modules/Contacts/card.php?socid=' . $selectedCompany . '&action=create&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?id=' . $object->id) . '" title="' . $langs->trans('NewContact') . '"><span class="fa fa-plus-circle valignmiddle paddingleft"></span></a>';
                }
                print $newcardbutton;
                ?>
            </div>
...
$arrayfields = [
    'rowid' => ['label' => $langs->trans("Id"), 'checked' => 1],
    'nature' => ['label' => $langs->trans("NatureOfContact"), 'checked' => 1],
    'thirdparty' => ['label' => $langs->trans("ThirdParty"), 'checked' => 1],
    'contact' => ['label' => $langs->trans("Users") . '/' . $langs->trans("Contacts"), 'checked' => 1],
    'type' => ['label' => $langs->trans("ContactType"), 'checked' => 1],
    'status' => ['label' => $langs->trans("Status"), 'checked' => 1],
    'link' => ['label' => $langs->trans("Link"), 'checked' => 1],
];
