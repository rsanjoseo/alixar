...
if (in_array($object->element, ['propal', 'supplier_proposal', 'facture', 'facturerec', 'invoice', 'commande', 'order', 'order_supplier', 'invoice_supplier'])) {
...
<tr class="oddeven tredited">
    ...
    <td>
        ...
        <?php if ($line->fk_product > 0) { ?>
            ...
            if (empty($canchangeproduct)) {
                ...
                ?>
                <a href="<?php echo DOL_URL_ROOT . '/Modules/Products/card.php?id=' . $line->fk_product; ?>">
                    <?php
        ...
        if (is_object($hookmanager)) {
            ...
            $parameters = ['line' => $line, 'fk_parent_line' => $fk_parent_line, 'var' => $var, 'dateSelector' => $dateSelector, 'seller' => $seller, 'buyer' => $buyer];
            $reshook = $hookmanager->executeHooks('formEditProductOptions', $parameters, $this, $action);
        }

        $situationinvoicelinewithparent = 0;
        if ($line->fk_prev_id != null && in_array($object->element, ['facture', 'facturedet'])) {
        ...
        if (!empty($extrafields)) {
            $temps = $line->showOptionals($extrafields, 'edit', ['class' => 'tredited'], '', '', 1, 'line');
            if (!empty($temps)) {
    ...
    if ($object->element == 'supplier_proposal' || $object->element == 'order_supplier' || $object->element == 'invoice_supplier') {    // We must have same test in printObjectLines
        $coldisplay++;
        ?>
        <td class="right"><input id="fourn_ref" name="fourn_ref" class="flat minwidth50 maxwidth150"
                                 value="<?php echo($line->ref_supplier ? $line->ref_supplier : $line->ref_fourn); ?>">
        </td>
        <?php
    }
        ...
        if (!empty($user->rights->margins->creer)) {
            ...
            <td class="margininfos right">
                ...
                <input class="flat maxwidth75 right" type="text" id="buying_price" name="buying_price"
                       class="hideobject" value="<?php echo price($line->pa_ht, 0, '', 0); ?>">
            </td>
        <?php }
    ...
    <td class="center valignmiddle" colspan="<?php echo $colspan; ?>"><?php $coldisplay += $colspan; ?>
        <input type="submit" class="button buttongen marginbottomonly button-save" id="savelinebutton marginbottomonly"
               name="save" value="<?php echo $langs->trans("Save"); ?>"><br>
        <input type="submit" class="button buttongen marginbottomonly button-cancel" id="cancellinebutton" name="cancel"
...
<script>
    ...
    jQuery(document).ready(function () {
        ...
        <?php if (in_array($this->table_element_line, ['propaldet', 'commandedet', 'facturedet'])) { ?>
