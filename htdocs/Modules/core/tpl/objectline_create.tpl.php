...
$usemargins = 0;
if (!empty($conf->margin->enabled) && !empty($object->element) && in_array($object->element, ['facture', 'facturerec', 'propal', 'commande'])) {
    $usemargins = 1;
}
...
if (in_array($object->element, ['propal', 'commande', 'order', 'facture', 'facturerec', 'invoice', 'supplier_proposal', 'order_supplier', 'invoice_supplier'])) {
    ...
    <tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
        ...
        <td class="nobottom linecoldescription minwidth500imp">
            ...
            if (!empty($conf->product->enabled) || !empty($conf->service->enabled)) {
                ...
                if (empty($senderissupplier)) {
                    ...
                    if (!empty($conf->global->ENTREPOT_EXTRA_STATUS)) {
                        ...
                        $form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, [], $buyer->id, '1', 0, 'maxwidth500', 0, 'warehouseopen,warehouseinternal', GETPOST('combinations', 'array'));
                    } else {
                        $form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, [], $buyer->id, '1', 0, 'maxwidth500', 0, '', GETPOST('combinations', 'array'));
                    }
                ...
                if ($senderissupplier != 2) {
                    $ajaxoptions = [
                        'update' => ['qty' => 'qty', 'remise_percent' => 'discount', 'idprod' => 'idprod'], // html id tags that will be edited with each ajax json response key
                    ...
                    $ajaxoptions = [
                        'update' => ['remise_percent' => 'discount', 'price_ht' => 'price_ht']            // html id tags that will be edited with each ajax json response key
                    ];
            ...
            if (is_object($hookmanager) && empty($senderissupplier)) {
                $parameters = ['fk_parent_line' => GETPOST('fk_parent_line', 'int')];
            ...
            if (is_object($hookmanager) && !empty($senderissupplier)) {
                $parameters = ['htmlname' => 'addproduct'];
            ...
            $coldisplay++;
            ?>
        <td class="nobottom linecolresupplier"><input id="fourn_ref" name="fourn_ref"
                                                      class="flat minwidth50 maxwidth125"
    ...
    ?>
        ...
        if (!empty($inputalsopricewithtax)) {
            ...
            <td class="nobottom linecoluttc right">
                <input type="text" size="5" name="price_ttc" id="price_ttc" class="flat"
        ...
        $coldisplay++;
        ?>
        <td class="nobottom nowrap linecoldiscount right"><input type="text" size="1" name="remise_percent"
                                                                 id="remise_percent" class="flat right"
                                                                 value="<?php echo(GETPOSTISSET("remise_percent") ? GETPOST("remise_percent", 'alpha', 2) : $remise_percent); ?>"><span
                    class="hideonsmartphone">%</span></td>
        <?php
        if (isset($this->situation_cycle_ref) && $this->situation_cycle_ref) {
            $coldisplay++;
...
if (in_array($this->table_element_line, ['propaldet', 'commandedet', 'facturedet'])) { ?>
...
if (!empty($usemargins) && $user->rights->margins->creer) {
    ...
    options += '
    <option value="inputprice" price="'+defaultprice+'"><?php echo $langs->trans("InputPrice"); ?></option>';
