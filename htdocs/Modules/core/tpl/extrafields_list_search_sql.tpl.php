...
if (!empty($extrafieldsobjectkey) && !empty($search_array_options) && is_array($search_array_options)) {    // $extrafieldsobject is the $object->table_element like 'societe', 'socpeople', ...
    ...
    foreach ($search_array_options as $key => $val) {
        ...
        if ($crit != '' && in_array($typ, ['date', 'datetime', 'timestamp'])) {
            if (is_numeric($crit)) {
                if ($typ == 'date') {
        ...
        } elseif ($crit != '' && (!in_array($typ, ['select', 'sellist']) || $crit != '0') && (!in_array($typ, ['link']) || $crit != '-1')) {
            $mode_search = 0;
            if (in_array($typ, ['int', 'double', 'real', 'price'])) {
                $mode_search = 1; // Search on a numeric
            }
            if (in_array($typ, ['sellist', 'link']) && $crit != '0' && $crit != '-1') {
                $mode_search = 2; // Search on a foreign key int
            }
            if (in_array($typ, ['sellist']) && !is_numeric($crit)) {
                $mode_search = 0;// Search on a foreign key string
            }
            if (in_array($typ, ['chkbxlst', 'checkbox'])) {
