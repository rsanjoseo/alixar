...
if ($action == 'add') {
    if (GETPOST("button") != $langs->trans("Cancel")) {
        ...
        if (!$error) {
            if (in_array(GETPOST('attrname', 'aZ09'), ['and', 'keyword', 'table', 'index', 'int', 'integer', 'float', 'double', 'real', 'position'])) {
                $error++;
                $langs->load("errors");
        ...
        if (!$error) {
            ...
            if (GETPOSTISSET("attrname") && preg_match("/^[a-z0-9-_]+$/", GETPOST('attrname', 'aZ09')) && !is_numeric(GETPOST('attrname', 'aZ09'))) {
                ...
                if ($type == 'sellist' || $type == 'chkbxlst') {
                    foreach ($parameters_array as $param_ligne) {
                        $params['options'] = [$parameters => null];
                    }
                } else {
...
if ($action == 'update') {
    if (GETPOST("button") != $langs->trans("Cancel")) {
        ...
        // Check reserved keyword with more than 3 characters
        if (!$error) {
            if (in_array(GETPOST('attrname', 'aZ09'), ['and', 'keyword', 'table', 'index', 'integer', 'float', 'double', 'position']) && empty($conf->global->MAIN_DISABLE_EXTRAFIELDS_CHECK_FOR_UPDATE)) {
                $error++;
                $langs->load("errors");
                $mesg[] = $langs->trans("ErrorReservedKeyword", GETPOST('attrname', 'aZ09'));
                $action = 'edit';
