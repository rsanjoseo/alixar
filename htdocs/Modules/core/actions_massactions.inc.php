        ...
        foreach ($listofobjectthirdparties as $thirdpartyid) {
            ...
            foreach ($listofobjectref[$thirdpartyid] as $objectid => $objectobj) {
                ...
                if ($_POST['addmaindocfile']) {
                    ...
                    if (dol_is_file($file)) {
                        // Create form object
                        $attachedfilesThirdpartyObj[$thirdpartyid][$objectid] = [
                            'paths' => [$file],
                            'names' => [$filename],
                            'mimes' => [$mime],
                        ];
            ...
            if (count($listofqualifiedobj) > 0) {
                ...
                foreach ($looparray as $objectid => $objecttmp) {        // $objecttmp is a real object or an empty object if we choose to send one email per thirdparty instead of one per object
                    ...
                    $attachedfiles = ['paths' => [], 'names' => [], 'mimes' => []];
                    if ($oneemailperrecipient) {
...
if ($massaction == 'confirm_createbills') {   // Create bills from orders.
    ...
    if (!$error) {
        ...
        if ($nb_bills_created == 1) {
            ...
            $texttoshow = str_replace('{s1}', '<a href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?id=' . urlencode($lastid) . '">' . $lastref . '</a>', $texttoshow);
...
if (!$error && $massaction == "builddoc" && $permissiontoread && !GETPOST('button_search')) {
    ...
    if (!empty($conf->global->USE_PDFTK_FOR_PDF_CONCAT)) {
        ...
        // Save merged file
        if (in_array($objecttmp->element, ['facture', 'facture_fournisseur']) && $search_status == Facture::STATUS_VALIDATED) {
            if ($option == 'late') {
        ...
        $format = [$page_largeur, $page_hauteur];
...
if (!$error && ($massaction == 'delete' || ($action == 'delete' && $confirm == 'yes')) && $permissiontodelete) {
    ...
    foreach ($toselect as $toselectid) {
        ...
        if ($result > 0) {
            ...
            if (in_array($objecttmp->element, ['societe', 'member'])) {
...
if (!$error && ($massaction == 'enable' || ($action == 'enable' && $confirm == 'yes')) && $permissiontoadd) {
    ...
    foreach ($toselect as $toselectid) {
        $result = $objecttmp->fetch($toselectid);
        if ($result > 0) {
            if (in_array($objecttmp->element, ['societe'])) {
            ...
            if ($objecttmp->statut == Holiday::STATUS_VALIDATED && $user->id == $objecttmp->fk_validator) {
                ...
                if (!$error) {
                    ...
                    if (!$emailTo) {
                        ...
                        $message .= "- " . $langs->transnoentitiesnoconv("Link") . " : " . $dolibarr_main_url_root . "/Modules/Holiday/card.php?id=" . $objecttmp->id . "\n\n";
                        $message .= "\n";
