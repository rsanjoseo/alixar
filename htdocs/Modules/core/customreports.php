...
$langs->loadLangs(["companies", "other", "exports", "sendings"]);

$extrafields = new ExtraFields($db);

$hookmanager->initHooks(['customreport']); // Note that conf->hooks_modules contains array
...
$arrayoftype = [
    'thirdparty' => ['label' => 'ThirdParties', 'ObjectClassName' => 'Societe', 'enabled' => $conf->societe->enabled, 'ClassPath' => "/Modules/Societes/class/societe.class.php"],
    'contact' => ['label' => 'Contacts', 'ObjectClassName' => 'Contact', 'enabled' => $conf->societe->enabled, 'ClassPath' => "/Modules/Contacts/class/contact.class.php"],
    'proposal' => ['label' => 'Proposals', 'ObjectClassName' => 'Propal', 'enabled' => $conf->propal->enabled, 'ClassPath' => "/Modules/Comm/propal/class/propal.class.php"],
    'order' => ['label' => 'Orders', 'ObjectClassName' => 'Commande', 'enabled' => $conf->commande->enabled, 'ClassPath' => "/Modules/Commande/class/commande.class.php"],
    'invoice' => ['label' => 'Invoices', 'ObjectClassName' => 'Facture', 'enabled' => $conf->facture->enabled, 'ClassPath' => "/Modules/Compta/facture/class/facture.class.php"],
    'invoice_template' => ['label' => 'PredefinedInvoices', 'ObjectClassName' => 'FactureRec', 'enabled' => $conf->facture->enabled, 'ClassPath' => "/Modules/Compta/class/facturerec.class.php", 'langs' => 'bills'],
    'contract' => ['label' => 'Contracts', 'ObjectClassName' => 'Contrat', 'enabled' => $conf->contrat->enabled, 'ClassPath' => "/Modules/Contracts/class/contrat.class.php", 'langs' => 'contract'],
    'bom' => ['label' => 'BOM', 'ObjectClassName' => 'Bom', 'enabled' => $conf->bom->enabled],
    'mo' => ['label' => 'MO', 'ObjectClassName' => 'Mo', 'enabled' => $conf->mrp->enabled, 'ClassPath' => "/mrp/class/mo.class.php"],
    'ticket' => ['label' => 'Ticket', 'ObjectClassName' => 'Ticket', 'enabled' => $conf->ticket->enabled],
    'member' => ['label' => 'Adherent', 'ObjectClassName' => 'Adherent', 'enabled' => $conf->adherent->enabled, 'ClassPath' => "/Modules/Adherents/class/adherent.class.php", 'langs' => 'members'],
    'cotisation' => ['label' => 'Subscriptions', 'ObjectClassName' => 'Subscription', 'enabled' => $conf->adherent->enabled, 'ClassPath' => "/Modules/Adherents/class/subscription.class.php", 'langs' => 'members'],
];

// Complete $arrayoftype by external modules
$parameters = ['objecttype' => $objecttype, 'tabfamily' => $tabfamily];
...
print $form->searchComponent([$object->element => $object->fields], $search_component_params);
print '</div>';
...
if ($mode == 'grid') {
    ...
    foreach ($object->fields as $key => $val) {
        if (empty($val['measure']) && (!isset($val['enabled']) || dol_eval($val['enabled'], 1))) {
            if (in_array($key, ['id', 'rowid', 'entity', 'last_main_doc', 'extraparams'])) {
                continue;
            }
            ...
            if (in_array($val['type'], ['timestamp', 'date', 'datetime'])) {
                $arrayofyaxis['t.' . $key . '-year'] = ['label' => $langs->trans($val['label']) . ' (' . $YYYY . ')', 'position' => $val['position']];
                $arrayofyaxis['t.' . $key . '-month'] = ['label' => $langs->trans($val['label']) . ' (' . $YYYY . '-' . $MM . ')', 'position' => $val['position']];
                $arrayofyaxis['t.' . $key . '-day'] = ['label' => $langs->trans($val['label']) . ' (' . $YYYY . '-' . $MM . '-' . $DD . ')', 'position' => $val['position']];
            } else {
                $arrayofyaxis['t.' . $key] = ['label' => $val['label'], 'position' => (int) $val['position']];
            }
        }
        ...
        if ($object->isextrafieldmanaged) {
            foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
                if (!empty($extrafields->attributes[$object->table_element]['totalizable'][$key]) && (!isset($extrafields->attributes[$object->table_element]['enabled'][$key]) || dol_eval($extrafields->attributes[$object->table_element]['enabled'][$key], 1))) {
                    $arrayofyaxis['te.' . $key] = ['label' => $extrafields->attributes[$object->table_element]['label'][$key], 'position' => (int) $extrafields->attributes[$object->table_element]['pos'][$key]];
                }
            }
        }
    }
...
if ($mode == 'graph') {
    print '<div class="divadvancedsearchfield">';
    $arrayofgraphs = ['bars' => 'Bars', 'lines' => 'Lines']; // also 'pies'
...
if ($sql) {
    ...
    while ($obj = $db->fetch_object($resql)) {
        ...
        if ($useagroupby) {
            ...
            $labeltouse = (($xlabel || $xlabel == '0') ? dol_trunc($xlabel, 20, 'middle') : ($xlabel === '' ? $langs->trans("Empty") : $langs->trans("NotDefined")));
            $xarrayforallseries = ['label' => $labeltouse];
            foreach ($search_measures as $key => $val) {
                $fieldfory = 'y_' . $key;
