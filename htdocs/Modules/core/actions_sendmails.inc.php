...
if (($action == 'send' || $action == 'relance') && !$_POST['addfile'] && !$_POST['removAll'] && !$_POST['removedfile'] && !$_POST['cancel'] && !$_POST['modelselected']) {
    ...
    if (is_object($object)) {
        ...
        if (method_exists($object, "fetch_thirdparty") && !in_array($object->element, ['member', 'user', 'expensereport', 'societe', 'contact'])) {
    ...
    if ($result > 0) {
        ...
        if (count($receiver) > 0) {
            ...
            foreach ($receiver as $key => $val) {
                if ($val == 'thirdparty') { // Key selected means current third party ('thirdparty' may be used for current member or current user too)
                    $tmparray[] = dol_string_nospecial($thirdparty->getFullName($langs), ' ', [","]) . ' <' . $thirdparty->email . '>';
                } elseif ($val == 'contact') { // Key selected means current contact
                    $tmparray[] = dol_string_nospecial($contact->getFullName($langs), ' ', [","]) . ' <' . $contact->email . '>';
                    $sendtoid[] = $contact->id;
        ...
        if (count($receivercc) > 0) {
            foreach ($receivercc as $key => $val) {
                if ($val == 'thirdparty') {    // Key selected means current thirdparty (may be usd for current member or current user too)
                    // Recipient was provided from combo list
                    $tmparray[] = dol_string_nospecial($thirdparty->name, ' ', [","]) . ' <' . $thirdparty->email . '>';
                } elseif ($val == 'contact') {    // Key selected means current contact
                    // Recipient was provided from combo list
                    $tmparray[] = dol_string_nospecial($contact->name, ' ', [","]) . ' <' . $contact->email . '>';
        ...
        if (dol_strlen($sendto)) {
            ...
            if ($fromtype === 'robot') {
                $from = dol_string_nospecial($conf->global->MAIN_MAIL_EMAIL_FROM, ' ', [","]) . ' <' . $conf->global->MAIN_MAIL_EMAIL_FROM . '>';
            } elseif ($fromtype === 'user') {
                $from = dol_string_nospecial($user->getFullName($langs), ' ', [","]) . ' <' . $user->email . '>';
            } elseif ($fromtype === 'company') {
                $from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', [","]) . ' <' . $conf->global->MAIN_INFO_SOCIETE_MAIL . '>';
                ...
                if ($obj) {
                    $from = dol_string_nospecial($obj->label, ' ', [","]) . ' <' . $obj->email . '>';
                }
            } else {
                $from = dol_string_nospecial($_POST['fromname'], ' ', [","]) . ' <' . $_POST['frommail'] . '>';
            }

            $replyto = dol_string_nospecial($_POST['replytoname'], ' ', [","]) . ' <' . $_POST['replytomail'] . '>';
