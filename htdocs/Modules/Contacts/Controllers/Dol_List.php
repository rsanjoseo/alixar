...
$langs->loadLangs(["companies", "suppliers", "categories"]);
...
$hookmanager->initHooks(['contactlist']);
...
$arrayfields['country.code_iso'] = ['label' => "Country", 'position' => 22, 'checked' => 0];
if (empty($conf->global->SOCIETE_DISABLE_CONTACTS)) {
    $arrayfields['s.nom'] = ['label' => "ThirdParty", 'position' => 25, 'checked' => 1];
}
...
if (strlen($search_phone)) {
    $sql .= natural_search(['p.phone', 'p.phone_perso', 'p.phone_mobile'], $search_phone);
}
if (strlen($search_cti)) {
    $sql .= natural_search(['p.phone', 'p.phone_perso', 'p.phone_mobile'], $search_cti);
}
if (strlen($search_firstlast_only)) {
    $sql .= natural_search(['p.lastname', 'p.firstname'], $search_firstlast_only);
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && ($sall != '' || $search_cti != '')) {
    ...
    $id = $obj->rowid;
    header("Location: " . DOL_URL_ROOT . '/Modules/Contacts/card.php?id=' . $id);
    exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('NewContactAddress'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Contacts/card.php?action=create', '', $user->rights->societe->contact->creer);
...
if (!empty($arrayfields['p.no_email']['checked'])) {
    print '<td class="liste_titre center">';
    print $form->selectarray('search_no_email', ['-1' => '', '0' => $langs->trans('No'), '1' => $langs->trans('Yes')], $search_no_email);
    print '</td>';
}
if (!empty($conf->socialnetworks->enabled)) {
    foreach ($socialnetworks as $key => $value) {
...
if (!empty($arrayfields['p.statut']['checked'])) {
    print '<td class="liste_titre center">';
    print $form->selectarray('search_status', ['-1' => '', '0' => $langs->trans('ActivityCeased'), '1' => $langs->trans('InActivity')], $search_status);
    print '</td>';
}
