...
$langs->loadLangs(['companies', 'users', 'other', 'commercial']);
...
$hookmanager->initHooks(['contactcard', 'globalcard']);
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Contacts/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Contacts/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'add' && !empty($permissiontoadd)) {
        ...
        if (empty($error)) {
            ...
            if ($id <= 0) {
                $error++;
                $errors = array_merge($errors, ($object->error ? [$object->error] : $object->errors));
                $action = 'create';
            }
        }
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes' && $user->rights->societe->contact->supprimer) {
        ...
        if ($result > 0) {
            if ($backtopage) {
                ...
                header("Location: " . DOL_URL_ROOT . '/Modules/Contacts/list.php');
                exit;
            }
        } else {
            setEventMessages($object->error, $object->errors, 'errors');
    ...
    if ($action == 'update' && empty($cancel) && !empty($permissiontoadd)) {
        if (!GETPOST("lastname", 'alpha')) {
            $error++;
            $errors = [$langs->trans("ErrorFieldRequired", $langs->transnoentities("Name") . ' / ' . $langs->transnoentities("Label"))];
            $action = 'edit';
        }
        ...
        if (!$error) {
            ...
            // Photo save
            $dir = $conf->societe->multidir_output[$object->entity] . "/Modules/Contacts/" . $object->id . "/photos";
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($user->rights->societe->contact->creer) {
        if ($action == 'create') {
            ...
            print $formcompany->select_ziptown((GETPOST("zipcode", 'alpha') ? GETPOST("zipcode", 'alpha') : $object->zip), 'zipcode', ['town', 'selectcountry_id', 'state_id'], 6) . '&nbsp;';
            print $formcompany->select_ziptown((GETPOST("town", 'alpha') ? GETPOST("town", 'alpha') : $object->town), 'town', ['zipcode', 'selectcountry_id', 'state_id']);
            print '</td></tr>';
            ...
            $selectarray = ['0' => $langs->trans("ContactPublic"), '1' => $langs->trans("ContactPrivate")];
            ...
            if (!empty($socid)) {
                ...
                print $form->multiselectarray('roles', $contactType, [], 0, 0, 'minwidth500');
                print '</td></tr>';
            }

            // Other attributes
            $parameters = ['socid' => $socid, 'objsoc' => $objsoc, 'colspan' => ' colspan="3"', 'cols' => 3];
    ...
    if (!empty($id) && $action != 'edit' && $action != 'create') {
        ...
        if (!empty($conf->mailing->enabled)) {
            ...
            print '<td><a href="' . DOL_URL_ROOT . '/Modules/Comm/mailing/list.php?filteremail=' . urlencode($object->email) . '">' . $object->getNbOfEMailings() . '</a></td></tr>';
        }
        ...
        print '<a href="' . DOL_URL_ROOT . '/Modules/Contacts/vcard.php?id=' . $object->id . '">';
        ...
        if ($action != 'presend') {
            ...
            $morehtmlright = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', DOL_URL_ROOT . '/Modules/Contacts/agenda.php?id=' . $object->id);
        ...
        $diroutput = $conf->societe->dir_output . '/Modules/Contacts/';
