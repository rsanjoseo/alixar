...
if ($action == 'update' && !GETPOST("cancel") && $user->rights->societe->contact->creer) {
    ...
    if ($result > 0) {
        ...
        // Logo/Photo save
        $dir = $conf->societe->dir_output . '/Modules/Contacts/' . get_exdir($object->id, 0, 0, 1, $object, 'contact') . '/photos';
        ...
        if ($file_OK) {
            ...
            if (GETPOST('deletephoto')) {
                $fileimg = $conf->societe->dir_output . '/Modules/Contacts/' . get_exdir($object->id, 0, 0, 1, $object, 'contact') . '/photos/' . $object->photo;
                $dirthumbs = $conf->societe->dir_output . '/Modules/Contacts/' . get_exdir($object->id, 0, 0, 1, $object, 'contact') . '/photos/thumbs';
