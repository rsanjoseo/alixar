...
$langs->loadLangs(["companies", "partnership", "other"]);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action != 'presend') {
        ...
        if (empty($reshook)) {
            // Show
            if ($permissiontoadd) {
                print dolGetButtonAction($langs->trans('AddPartnership'), '', 'default', DOL_URL_ROOT . '/partnership/partnership_card.php?action=create&fk_soc=' . $object->id . '&backtopage=' . urlencode(DOL_URL_ROOT . '/Modules/Societes/partnership.php?id=' . $object->id), '', $permissiontoadd);
            }
        }
        print '</div>' . "\n";
    }
