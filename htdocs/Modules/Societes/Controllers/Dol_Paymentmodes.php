...
require_once DOL_DOCUMENT_ROOT . '/Modules/Societes/class/companypaymentmode.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Societes/class/societeaccount.class.php';
require_once DOL_DOCUMENT_ROOT . '/Modules/Compta/prelevement/class/bonprelevement.class.php';
require_once DOL_DOCUMENT_ROOT . '/stripe/class/stripe.class.php';

$langs->loadLangs(["companies", "commercial", "banks", "bills", 'paypal', 'stripe', 'withdrawals']);
...
$hookmanager->initHooks(['thirdpartybancard', 'globalcard']);
...
if (empty($reshook)) {
    ...
    if ($action == 'setasbankdefault' && GETPOST('ribid', 'int') > 0) {
        ...
        if ($res) {
            $url = DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . $object->id;
            header('Location: ' . $url);
            exit;
        } else {
...
if ($socid && $action != 'edit' && $action != 'create' && $action != 'editcard' && $action != 'createcard') {
    ...
    if (!(empty($conf->stripe->enabled)) && $object->client) {
                    ...
                    if (empty($conf->global->STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION)) {
                        ...
                        try {
                            if (empty($stripeacc)) {                // If the Stripe connect account not set, we use common API usage
                                $paymentmethodobjsA = \Stripe\PaymentMethod::all(["customer" => $customerstripe->id, "type" => "card"]);
                                $paymentmethodobjsB = \Stripe\PaymentMethod::all(["customer" => $customerstripe->id, "type" => "sepa_debit"]);
                            } else {
                                $paymentmethodobjsA = \Stripe\PaymentMethod::all(["customer" => $customerstripe->id, "type" => "card"], ["stripe_account" => $stripeacc]);
                                $paymentmethodobjsB = \Stripe\PaymentMethod::all(["customer" => $customerstripe->id, "type" => "sepa_debit"], ["stripe_account" => $stripeacc]);
                            }
        ...
        // Hook fields
        $parameters = ['arrayfields' => [], 'param' => '', 'sortfield' => '', 'sortorder' => '', 'linetype' => 'stripetitle'];
                    ...
                    while ($i < $num_rows) {
                        ...
                        if ($obj) {
                            ...
                            if (empty($companypaymentmodetemp->default_rib)) {
                                print '<a href="' . DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . $object->id . '&id=' . $companypaymentmodetemp->id . '&action=setlocalassourcedefault&token=' . newToken() . '">';
                            ...
                            print '</td>';
                            // Fields from hook
                            $parameters = ['arrayfields' => [], 'obj' => $obj, 'linetype' => 'stripecard'];
                            ...
                            if ($user->rights->societe->creer) {
                                ...
                                print '<a class="editfielda marginleftonly marginrightonly" href="' . DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . $object->id . '&id=' . $companypaymentmodetemp->id . '&action=editcard&token=' . newToken() . '">';
                                print img_picto($langs->trans("Modify"), 'edit');
                                print '</a>';
                                print '&nbsp;';
                                print '<a class="marginleftonly marginrightonly" href="' . DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . $object->id . '&id=' . $companypaymentmodetemp->id . '&action=deletecard&token=' . newToken() . '">'; // source='.$companypaymentmodetemp->stripe_card_ref.'&
        ...
        if (is_array($listofsources) && count($listofsources)) {
            foreach ($listofsources as $src) {
                ...
                if ((empty($customerstripe->invoice_settings) && $customerstripe->default_source != $src->id) ||
                    ...
                    print '<a href="' . DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . $object->id . '&source=' . $src->id . '&action=setassourcedefault&token=' . newToken() . '">';
                    print img_picto($langs->trans("Default"), 'off');
                    print '</a>';
                } else {
                    print img_picto($langs->trans("Default"), 'on');
                ...
                print '</td>';
                // Fields from hook
                $parameters = ['arrayfields' => [], 'stripesource' => $src, 'linetype' => 'stripecardremoteonly'];
                $reshook = $hookmanager->executeHooks('printFieldListValue', $parameters, $object); // Note that $action and $object may have been modified by hook
                print $hookmanager->resPrint;
                // Action column
                print '<td class="right nowraponall">';
                if ($user->rights->societe->creer) {
                    print '<a href="' . DOL_URL_ROOT . '/Modules/Societes/paymentmodes.php?socid=' . $object->id . '&source=' . $src->id . '&action=deletecard&token=' . newToken() . '">';
                    print img_picto($langs->trans("Delete"), 'delete');
                    print '</a>';
                }
                print '</td>';
    ...
    if (!empty($conf->stripe->enabled) && !empty($conf->stripeconnect->enabled) && !empty($stripesupplieracc)) {
        ...
        $balance = \Stripe\Balance::retrieve(["stripe_account" => $stripesupplieracc]);
        print '<table class="liste centpercent">' . "\n";
        print '<tr class="liste_titre">';
...
if ($socid && $action == 'edit' && $user->rights->societe->creer) {
    ...
    if ($conf->prelevement->enabled) {
        ...
        $tblArraychoice = ["FRST" => $langs->trans("FRST"), "RECUR" => $langs->trans("RECUR")];
