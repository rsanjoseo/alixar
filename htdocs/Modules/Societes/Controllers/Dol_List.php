...
$langs->loadLangs(["companies", "commercial", "customers", "suppliers", "bills", "compta", "categories", "cashdesk"]);
...
$hookmanager->initHooks(['thirdpartylist']);
...
$checkedcustomercode = (in_array($contextpage, ['thirdpartylist', 'customerlist', 'prospectlist', 'poslist']) ? 1 : 0);
$checkedsuppliercode = (in_array($contextpage, ['supplierlist']) ? 1 : 0);
$checkedcustomeraccountcode = (in_array($contextpage, ['customerlist']) ? 1 : 0);
$checkedsupplieraccountcode = (in_array($contextpage, ['supplierlist']) ? 1 : 0);
$checkedtypetiers = 1;
$checkedprofid1 = 0;
$checkedprofid2 = 0;
$checkedprofid3 = 0;
...
$checkprospectlevel = (in_array($contextpage, ['prospectlist']) ? 1 : 0);
$checkstcomm = (in_array($contextpage, ['prospectlist']) ? 1 : 0);
$arrayfields = [
    's.rowid' => ['label' => "TechnicalID", 'position' => 1, 'checked' => (!empty($conf->global->MAIN_SHOW_TECHNICAL_ID)), 'enabled' => (!empty($conf->global->MAIN_SHOW_TECHNICAL_ID))],
    's.nom' => ['label' => "ThirdPartyName", 'position' => 2, 'checked' => 1],
    's.name_alias' => ['label' => "AliasNameShort", 'position' => 3, 'checked' => 1],
    's.barcode' => ['label' => "Gencod", 'position' => 5, 'checked' => 1, 'enabled' => (!empty($conf->barcode->enabled))],
    's.code_client' => ['label' => "CustomerCodeShort", 'position' => 10, 'checked' => $checkedcustomercode],
    's.code_fournisseur' => ['label' => "SupplierCodeShort", 'position' => 11, 'checked' => $checkedsuppliercode, 'enabled' => ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled))],
    's.code_compta' => ['label' => "CustomerAccountancyCodeShort", 'position' => 13, 'checked' => $checkedcustomeraccountcode],
    's.code_compta_fournisseur' => ['label' => "SupplierAccountancyCodeShort", 'position' => 14, 'checked' => $checkedsupplieraccountcode, 'enabled' => ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled))],
    's.address' => ['label' => "Address", 'position' => 19, 'checked' => 0],
    's.zip' => ['label' => "Zip", 'position' => 20, 'checked' => 1],
    's.town' => ['label' => "Town", 'position' => 21, 'checked' => 0],
    'state.nom' => ['label' => "State", 'position' => 22, 'checked' => 0],
    'region.nom' => ['label' => "Region", 'position' => 23, 'checked' => 0],
    'country.code_iso' => ['label' => "Country", 'position' => 24, 'checked' => 0],
    's.email' => ['label' => "Email", 'position' => 25, 'checked' => 0],
    's.url' => ['label' => "Url", 'position' => 26, 'checked' => 0],
    's.phone' => ['label' => "Phone", 'position' => 27, 'checked' => 1],
    's.fax' => ['label' => "Fax", 'position' => 28, 'checked' => 0],
    'typent.code' => ['label' => "ThirdPartyType", 'position' => 29, 'checked' => $checkedtypetiers],
    'staff.code' => ['label' => "Staff", 'position' => 31, 'checked' => 0],
    's.siren' => ['label' => "ProfId1Short", 'position' => 40, 'checked' => $checkedprofid1],
    's.siret' => ['label' => "ProfId2Short", 'position' => 41, 'checked' => $checkedprofid2],
    's.ape' => ['label' => "ProfId3Short", 'position' => 42, 'checked' => $checkedprofid3],
    's.idprof4' => ['label' => "ProfId4Short", 'position' => 43, 'checked' => $checkedprofid4],
    's.idprof5' => ['label' => "ProfId5Short", 'position' => 44, 'checked' => $checkedprofid5],
    's.idprof6' => ['label' => "ProfId6Short", 'position' => 45, 'checked' => $checkedprofid6],
    's.tva_intra' => ['label' => "VATIntraShort", 'position' => 50, 'checked' => 0],
    'customerorsupplier' => ['label' => 'NatureOfThirdParty', 'position' => 61, 'checked' => 1],
    's.fk_prospectlevel' => ['label' => "ProspectLevelShort", 'position' => 62, 'checked' => $checkprospectlevel],
    's.fk_stcomm' => ['label' => "StatusProsp", 'position' => 63, 'checked' => $checkstcomm],
    's2.nom' => ['label' => 'ParentCompany', 'position' => 64, 'checked' => 0],
    's.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    's.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 500],
    's.status' => ['label' => "Status", 'checked' => 1, 'position' => 1000],
    's.import_key' => ['label' => "ImportId", 'checked' => 0, 'position' => 1100],
];
if (!empty($conf->global->PRODUIT_MULTIPRICES) || !empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES)) {
    $arrayfields['s.price_level'] = ['label' => "PriceLevel", 'position' => 30, 'checked' => 0];
}
...
if ($search_type > 0 && in_array($search_type, ['1,3', '1,2,3', '2,3'])) {
    $sql .= " AND s.client IN (" . $db->sanitize($search_type) . ")";
}
if ($search_type > 0 && in_array($search_type, ['4'])) {
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && ($search_all != '' || $search_cti != '') && $action != 'list') {
    ...
    if (!empty($conf->global->SOCIETE_ON_SEARCH_AND_LIST_GO_ON_CUSTOMER_OR_SUPPLIER_CARD)) {
        if ($obj->client > 0) {
            header("Location: " . DOL_URL_ROOT . '/Modules/Comm/card.php?socid=' . $id);
            exit;
        }
        if ($obj->fournisseur > 0) {
            header("Location: " . DOL_URL_ROOT . '/Modules/Fourn/card.php?socid=' . $id);
            exit;
        }
    }

    header("Location: " . DOL_URL_ROOT . '/index.php?module=societes&controller=card&socid=' . $id);
    exit;
}
...
if (GETPOST('nomassaction', 'int') || in_array($massaction, ['presend', 'predelete', 'preaffecttag', 'preenable', 'preclose'])) {
    $arrayofmassactions = [];
}
...
if ($contextpage != 'poslist') {
    $url = DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=create' . $typefilter;
    if (!empty($socid)) {
        $url .= '&socid=' . $socid;
    }
    ...
    $url = DOL_URL_ROOT . '/index.php?module=societes&controller=card&action=create&type=t&contextpage=poslist&optioncss=print&backtopage=' . urlencode($_SERVER["PHP_SELF"] . '?type=t&contextpage=poslist&nomassaction=1&optioncss=print&place=' . $place);
    $label = 'MenuNewCustomer';
...
if (!empty($arrayfields['s.status']['checked'])) {
    ...
    print $form->selectarray('search_status', ['0' => $langs->trans('ActivityCeased'), '1' => $langs->trans('InActivity')], $search_status, 1, 0, 0, '', 0, 0, 0, '', '', 1);
    print '</td>';
}
