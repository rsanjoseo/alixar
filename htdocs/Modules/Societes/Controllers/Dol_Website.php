...
$langs->loadLangs(["companies", "website"]);
...
$hookmanager->initHooks(['websitethirdpartylist']); // Note that conf->hooks_modules contains array
...
foreach ($objectwebsiteaccount->fields as $key => $val) {
    ...
    if (!empty($val['visible'])) {
        $arrayfields['t.' . $key] = ['label' => $val['label'], 'checked' => (($val['visible'] < 0) ? 0 : 1), 'enabled' => $val['enabled']];
    }
}
