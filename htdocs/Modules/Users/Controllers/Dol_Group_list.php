...
if (!empty($search_group)) {
    natural_search(["g.nom", "g.note"], $search_group);
}
if ($sall) {
    $sql .= natural_search(["g.nom", "g.note"], $sall);
}
...
if ($resql) {
    ...
    $newcardbutton = '';
    if ($caneditperms) {
        $newcardbutton .= dolGetButtonTitle($langs->trans('NewGroup'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Users/group/card.php?action=create&leftmenu=');
    }
    ...
    while ($i < $num) {
        ...
        print '<a href="' . DOL_URL_ROOT . '/Modules/Users/group/perms.php?id=' . $obj->rowid . '">' . $obj->nbpermissions . '</a>';
        print '</td>';
