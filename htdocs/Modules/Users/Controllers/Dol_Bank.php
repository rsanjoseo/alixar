...
require_once DOL_DOCUMENT_ROOT . '/Modules/Users/class/userbankaccount.class.php';
...
$langs->loadLangs(['companies', 'commercial', 'banks', 'bills', 'trips', 'holiday', 'salaries']);
...
if ($action != 'edit' && $action != 'create') {        // If not bank account yet, $account may be empty
    ...
    if (!empty($conf->holiday->enabled) &&
        ...
        if ($resql) {
            ...
            print '<tr class="liste_titre">';
            print '<td colspan="4"><table class="nobordernopadding centpercent"><tr><td>' . $langs->trans("LastHolidays", ($num <= $MAXLIST ? "" : $MAXLIST)) . '</td><td class="right"><a class="notasortlink" href="' . DOL_URL_ROOT . '/Modules/Holiday/list.php?id=' . $object->id . '">' . $langs->trans("AllHolidays") . '<span class="badge marginleftonlyshort">' . $num . '</span></a></td>';
            print '</tr></table></td>';
            print '</tr>';
    ...
    if (!empty($conf->expensereport->enabled) &&
        ...
        if ($resql) {
            ...
            print '<tr class="liste_titre">';
            print '<td colspan="4"><table class="nobordernopadding centpercent"><tr><td>' . $langs->trans("LastExpenseReports", ($num <= $MAXLIST ? "" : $MAXLIST)) . '</td><td class="right"><a class="notasortlink" href="' . DOL_URL_ROOT . '/Modules/ExpenseReports/list.php?id=' . $object->id . '">' . $langs->trans("AllExpenseReports") . '<span class="badge marginleftonlyshort">' . $num . '</span></a></td>';
            print '</tr></table></td>';
            print '</tr>';
