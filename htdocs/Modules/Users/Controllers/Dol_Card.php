...
$langs->loadLangs(['users', 'companies', 'ldap', 'admin', 'hrm', 'stocks', 'other']);
...
$parameters = ['id' => $id, 'socid' => $socid, 'group' => $group, 'caneditgroup' => $caneditgroup];
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Users/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Users/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_delete' && $confirm == "yes" && $candisableuser) {
        if ($id != $user->id) {
            ...
            if ($result < 0) {
                ...
                header("Location: " . DOL_URL_ROOT . "/Modules/Users/list.php?restore_lastsearch_values=1");
                exit;
            }
        }
    }
...
if ($action == 'create' || $action == 'adduserldap') {
    ...
    print '<td>';
    print img_picto('', 'user') . $form->select_dolusers($object->fk_user, 'fk_user', 1, [$object->id], 0, '', 0, $conf->entity, 0, 0, '', 0, '', 'maxwidth300');
    print '</td>';
    print "</tr>\n";
    ...
    if (!empty($conf->expensereport->enabled)) {
        ...
        print '</td>';
        print '<td>';
        print img_picto('', 'user') . $form->select_dolusers($object->fk_user_expense_validator, 'fk_user_expense_validator', 1, [$object->id], 0, '', 0, $conf->entity, 0, 0, '', 0, '', 'maxwidth300');
        print '</td>';
        print "</tr>\n";
    }
    ...
    if (!empty($conf->holiday->enabled)) {
        ...
        print '</td>';
        print '<td>';
        print img_picto('', 'user') . $form->select_dolusers($object->fk_user_holiday_validator, 'fk_user_holiday_validator', 1, [$object->id], 0, '', 0, $conf->entity, 0, 0, '', 0, '', 'maxwidth300');
        print '</td>';
        print "</tr>\n";
    }
    ...
    $parameters = ['valuetoshow' => $valuetoshow, 'password' => $password];
    ...
    print $formcompany->select_ziptown($object->zip, 'zipcode', ['town', 'selectcountry_id', 'state_id'], 6);
    print '</td></tr>';
    ...
    if ($id > 0) {
        ...
        if ($action != 'edit') {
            ...
            print '<td>';
            print '<a href="' . DOL_URL_ROOT . '/Modules/Users/vcard.php?id=' . $object->id . '">';
            ...
            // Other form for user password
            $parameters = ['valuetoshow' => $valuetoshow];
            $reshook = $hookmanager->executeHooks('printUserPasswordField', $parameters, $object, $action); // Note that $action and $object may have been modified by hook
            if ($reshook > 0) {
                $valuetoshow = $hookmanager->resPrint; // to replace
            } else {
            ...
            if ($action != 'presend' && $action != 'send') {
                ...
                if ($canreadgroup) {
                    ...
                    $parameters = ['caneditgroup' => $caneditgroup, 'groupslist' => $groupslist, 'exclude' => $exclude];
        ...
        if ($action == 'edit' && ($canedituser || $caneditfield || $caneditpassword || ($user->id == $object->id))) {
            ...
            print '<td>';
            if ($caneditfield) {
                print img_picto('', 'user') . $form->select_dolusers($object->fk_user, 'fk_user', 1, [$object->id], 0, '', 0, $object->entity, 0, 0, '', 0, '', 'widthcentpercentminusx maxwidth300');
            } else {
            ...
            if (!empty($conf->expensereport->enabled)) {
                ...
                print '<td>';
                if ($caneditfield) {
                    print img_picto('', 'user') . $form->select_dolusers($object->fk_user_expense_validator, 'fk_user_expense_validator', 1, [$object->id], 0, '', 0, $object->entity, 0, 0, '', 0, '', 'widthcentpercentminusx maxwidth300');
                } else {
            ...
            if (!empty($conf->holiday->enabled)) {
                ...
                print '<td>';
                if ($caneditfield) {
                    print img_picto('', 'user') . $form->select_dolusers($object->fk_user_holiday_validator, 'fk_user_holiday_validator', 1, [$object->id], 0, '', 0, $object->entity, 0, 0, '', 0, '', 'widthcentpercentminusx maxwidth300');
                } else {
            ...
            // Other form for user password
            $parameters = ['valuetoshow' => $valuetoshow, 'caneditpassword' => $caneditpassword];
            $reshook = $hookmanager->executeHooks('printUserPasswordField', $parameters, $object, $action); // Note that $action and $object may have been modified by hook
            if ($reshook > 0) {
                $valuetoshow = $hookmanager->resPrint; // to replace
            } else {
            ...
            if (!empty($conf->societe->enabled)) {
                ...
                if ($object->socid > 0) {
                    ...
                    if ($object->contact_id) {
                        $contact = new Contact($db);
                        $contact->fetch($object->contact_id);
                        print ' / <a href="' . DOL_URL_ROOT . '/Modules/Contacts/card.php?id=' . $object->contact_id . '">' . img_object($langs->trans("ShowContact"), 'contact') . ' ' . dol_trunc($contact->getFullName($langs), 32) . '</a>';
                    }
                } else {
            ...
            if (!empty($conf->stock->enabled) && !empty($conf->global->MAIN_DEFAULT_WAREHOUSE_USER)) {
                ...
                print $formproduct->selectWarehouses($object->fk_warehouse, 'fk_warehouse', 'warehouseopen', 1);
                print ' <a href="' . DOL_URL_ROOT . '/Modules/Products/stock/card.php?action=create&token=' . newToken() . '&backtopage=' . urlencode($_SERVER['PHP_SELF'] . '?id=' . $object->id . '&action=edit&token=' . newToken()) . '"><span class="fa fa-plus-circle valignmiddle paddingleft" title="' . $langs->trans("AddWarehouse") . '"></span></a>';
                print '</td></tr>';
            }
