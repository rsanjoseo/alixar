...
$langs->loadLangs(['users', 'companies']);
...
if (!is_array($user_arbo) && $user_arbo < 0) {
    ...
    $newcardbutton = '';
    $newcardbutton .= dolGetButtonTitle($langs->trans('NewUser'), '', 'fa fa-plus-circle', DOL_URL_ROOT . '/Modules/Users/card.php?action=create' . ($mode == 'employee' ? '&employee=1' : '') . '&leftmenu=', '', $canadduser);

    $morehtmlright = '';

    $morehtmlright .= dolGetButtonTitle($langs->trans("List"), '', 'fa fa-list paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Users/list.php' . (($search_statut != '' && $search_statut >= 0) ? '?search_statut=' . $search_statut : ''));
    $param = ['morecss' => 'marginleftonly btnTitleSelected'];
    $morehtmlright .= dolGetButtonTitle($langs->trans("HierarchicView"), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Users/hierarchy.php' . (($search_statut != '' && $search_statut >= 0) ? '?search_statut=' . $search_statut : ''), '', 1, $param);
    ...
    print $form->selectarray('search_statut', ['-1' => '', '1' => $langs->trans('Enabled')], $search_statut);
    print '</td>';
