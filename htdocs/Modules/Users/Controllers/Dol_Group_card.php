...
$hookmanager->initHooks(['groupcard', 'globalcard']);
...
$parameters = ['id' => $id, 'userid' => $userid, 'caneditperms' => $caneditperms];
...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Users/group/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Users/group/card.php?id=' . ($id > 0 ? $id : '__ID__');
            }
        }
    }
    ...
    if ($action == 'confirm_delete' && $confirm == "yes") {
        if ($caneditperms) {
            $object->fetch($id);
            $object->delete($user);
            header("Location: " . DOL_URL_ROOT . "/Modules/Users/group/list.php?restore_lastsearch_values=1");
            exit;
        } else {
            $langs->load("errors");
...
if ($action == 'create') {
    ...
    if ($id) {
        ...
        if ($action != 'edit') {
            ...
            $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Users/group/list.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
            ...
            $parameters = ['caneditperms' => $caneditperms, 'exclude' => $exclude];
