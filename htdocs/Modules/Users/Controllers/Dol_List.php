...
$langs->loadLangs(['users', 'companies', 'hrm', 'salaries']);
...
$hookmanager->initHooks(['userlist']);
...
$arrayfields = [
    'u.login' => ['label' => "Login", 'checked' => 1, 'position' => 10],
    'u.lastname' => ['label' => "Lastname", 'checked' => 1, 'position' => 15],
    'u.firstname' => ['label' => "Firstname", 'checked' => 1, 'position' => 20],
    'u.entity' => ['label' => "Entity", 'checked' => 1, 'position' => 50, 'enabled' => (!empty($conf->multicompany->enabled) && empty($conf->global->MULTICOMPANY_TRANSVERSE_MODE))],
    'u.gender' => ['label' => "Gender", 'checked' => 0, 'position' => 22],
    'u.employee' => ['label' => "Employee", 'checked' => ($mode == 'employee' ? 1 : 0), 'position' => 25],
    'u.fk_user' => ['label' => "HierarchicalResponsible", 'checked' => 1, 'position' => 27],
    'u.accountancy_code' => ['label' => "AccountancyCode", 'checked' => 0, 'position' => 30],
    'u.office_phone' => ['label' => "PhonePro", 'checked' => 1, 'position' => 31],
    'u.user_mobile' => ['label' => "PhoneMobile", 'checked' => 1, 'position' => 32],
    'u.email' => ['label' => "EMail", 'checked' => 1, 'position' => 35],
    'u.api_key' => ['label' => "ApiKey", 'checked' => 0, 'position' => 40, "enabled" => (!empty($conf->api->enabled) && $user->admin)],
    'u.fk_soc' => ['label' => "Company", 'checked' => ($contextpage == 'employeelist' ? 0 : 1), 'position' => 45],
    'u.salary' => ['label' => "Salary", 'checked' => 1, 'position' => 80, 'enabled' => (!empty($conf->salaries->enabled) && !empty($user->rights->salaries->readall))],
    'u.datelastlogin' => ['label' => "LastConnexion", 'checked' => 1, 'position' => 100],
    'u.datepreviouslogin' => ['label' => "PreviousConnexion", 'checked' => 0, 'position' => 110],
    'u.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'u.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 500],
    'u.statut' => ['label' => "Status", 'checked' => 1, 'position' => 1000],
];
// Extra fields
...
if ($search_warehouse > 0) {
    $sql .= natural_search(['u.fk_warehouse'], $search_warehouse);
}
if ($search_login != '') {
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: " . DOL_URL_ROOT . '/Modules/Users/card.php?id=' . $id);
    exit;
}
...
$url = DOL_URL_ROOT . '/Modules/Users/card.php?action=create' . ($mode == 'employee' ? '&employee=1' : '') . '&leftmenu=';
if (!empty($socid)) {
...
$moreparam = ['morecss' => 'btnTitleSelected'];
$morehtmlright = dolGetButtonTitle($langs->trans("List"), '', 'fa fa-list paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Users/list.php' . (($search_statut != '' && $search_statut >= 0) ? '?search_statut=' . $search_statut : ''), '', 1, $moreparam);
$moreparam = ['morecss' => 'marginleftonly'];
$morehtmlright .= dolGetButtonTitle($langs->trans("HierarchicView"), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT . '/Modules/Users/hierarchy.php' . (($search_statut != '' && $search_statut >= 0) ? '?search_statut=' . $search_statut : ''), '', 1, $moreparam);
...
if (!empty($arrayfields['u.fk_user']['checked'])) {
    print '<td class="liste_titre">';
    print $form->select_dolusers($search_supervisor, 'search_supervisor', 1, [], 0, '', 0, 0, 0, 0, '', 0, '', 'maxwidth200');
    print '</td>';
}
...
if (!empty($arrayfields['u.statut']['checked'])) {
    ...
    print $form->selectarray('search_statut', ['-1' => '', '0' => $langs->trans('Disabled'), '1' => $langs->trans('Enabled')], $search_statut);
    print '</td>';
}
// Action column
