...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall && !$page) {
    ...
    $id = $obj->rowid;
    header("Location: " . DOL_URL_ROOT . '/Modules/Contrats/card.php?id=' . $id);
    exit;
}
...
$url = DOL_URL_ROOT . '/Modules/Contrats/card.php?action=create';
if (!empty($socid)) {
    $url .= '&socid=' . $socid;
}
...
while ($i < min($num, $limit)) {
    ...
    if (!empty($arrayfields['c.ref']['checked'])) {
        ...
        if (!empty($obj->note_private) || !empty($obj->note_public)) {
            ...
            print '<a href="' . DOL_URL_ROOT . '/Modules/Contrats/note.php?id=' . $obj->rowid . '&save_lastsearch_values=1">' . img_picto($langs->trans("ViewPrivateNote"), 'note') . '</a>';
            print '</span>';
        }
