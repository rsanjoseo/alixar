...
if (empty($reshook)) {
    $backurlforlist = DOL_URL_ROOT . '/Modules/Contrats/list.php';
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = DOL_URL_ROOT . '/Modules/Contrats/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
                ...
                if ($id > 0) {
                    ...
                    if ($result > 0) {
                        ...
                        for ($i = 0; $i < $num; $i++) {
                            ...
                            if ($product_type == 1 || (!empty($conf->global->CONTRACT_SUPPORT_PRODUCTS) && in_array($product_type, [0, 1]))) {    // TODO Exclude also deee
                                // service prédéfini
...
if ($action == 'create') {
    ...
    if ($object->id > 0) {
        ...
        if ($action == 'delete') {
            ...
            // Clone confirmation
            $formquestion = [['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company(GETPOST('socid', 'int'), 'socid', '(s.client=1 OR s.client=2 OR s.client=3)')]];
        ...
        $linkback = '<a href="' . DOL_URL_ROOT . '/Modules/Contrats/list.php?restore_lastsearch_values=1' . (!empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
        ...
        if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
            $morehtmlref .= ' (<a href="' . DOL_URL_ROOT . '/Modules/Contrats/list.php?socid=' . $object->thirdparty->id . '&search_name=' . urlencode($object->thirdparty->name) . '">' . $langs->trans("OtherContracts") . '</a>)';
        }
        // Project
        if (!empty($conf->projet->enabled)) {
            $langs->load("projects");
        ...
        $usemargins = 0;
        if (!empty($conf->margin->enabled) && !empty($object->element) && in_array($object->element, ['facture', 'propal', 'commande'])) {
            $usemargins = 1;
        }
            ...
            if (empty($reshook)) {
                ...
                if (!empty($conf->commande->enabled) && $object->statut > 0 && $object->nbofservicesclosed < $nbofservices) {
                    ...
                    if ($user->rights->commande->creer) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Commande/card.php?action=create&token=' . newToken() . '&origin=' . $object->element . '&originid=' . $object->id . '&socid=' . $object->thirdparty->id . '">' . $langs->trans("CreateOrder") . '</a></div>';
                    } else {
                ...
                if (!empty($conf->facture->enabled) && $object->statut > 0) {
                    ...
                    if ($user->rights->facture->creer) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . DOL_URL_ROOT . '/Modules/Compta/facture/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->thirdparty->id . '">' . $langs->trans("CreateBill") . '</a></div>';
                    } else {
        ...
        if ($action != 'presend') {
            ...
            $morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-list-alt imgforviewmode', DOL_URL_ROOT . '/Modules/Contrats/agenda.php?id=' . $object->id);
